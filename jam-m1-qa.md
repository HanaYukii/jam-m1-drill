...
線上互動版：<https://hanayukii.github.io/jam-m1-drill/> · 匯出於 2026-08-28
...
<sub>3.7 Sequences — ●●○ · 概念 · §3.7 (⟦T⟧, ⟦T⟧_n, ⟦T⟧_{:n}, ⟦T⟧_{n:}, ⟦T⟧_N); applied in eq. 8.1 and eq. 6.22</sub>

**標準答案**　⟦H⟧_{:O} is the set of hash sequences of length at most O, ⟦H⟧_Q of length exactly Q and ⟦H⟧_4 of length exactly 4; the outer ⟦·⟧_C means exactly one inner sequence per core

GP §3.7：⟦T⟧ 是元素取自 T、長度任意的序列集合（定義一個 ℕ → T 的 partial mapping）；⟦T⟧_n 是「containing exactly n elements」（complete mapping ℕ_n → T）；「sets of sequences of at most n elements and at least n elements may be denoted ⟦T⟧_{:n} and ⟦T⟧_{n:} respectively」；⟦T⟧_N 則是長度落在集合 N 內。套進題目：α ∈ ⟦⟦H⟧_{:O}⟧_C 是每個 core（C = 341）一條至多 O = 8 個 authorizer hash 的 pool，φ ∈ ⟦⟦H⟧_Q⟧_C 每 core 恰好 Q = 80，eq. 6.22 的 η ∈ ⟦H⟧_4 恰好 4 個 entropy 值。你們的 Go 型別完全對應：`AuthPool.Validate` 只檢查 len ≤ AuthPoolMaxSize(8)，`AuthQueue.Validate` 要求 len == AuthQueueSize(80)，兩者外層都要求 len == CoresCount（internal/types/types.go §8.1 區段）。

**逐項辨析**

1. ✅ ⟦H⟧_{:O} is the set of hash sequences of length at most O, ⟦H⟧_Q of length exactly Q and ⟦H⟧_4 of length exactly 4; the outer ⟦·⟧_C means exactly one inner sequence per core  
   冒號在前是上限、無冒號是恰好：pool 可未滿甚至為空，queue 每 core 恰好 Q = 80。
2. ❌ ⟦H⟧_{:O} is the set of hash sequences of length at least O (a pool must be kept full), while ⟦H⟧_Q and ⟦H⟧_4 are sequences of length at most Q and at most 4 respectively  
   把 _{:n} 與 _{n:} 的方向弄反了；「至少 n 個」要寫成 ⟦T⟧_{n:}。
3. ❌ ⟦H⟧_{:O} denotes the first O elements of a hash sequence (a slice), so α is an O-element prefix of each core's queue, and ⟦H⟧_Q denotes the single element at index Q  
   把型別標記當成切片語法；取前 n 個元素要寫 s_{…n} 或 →s^n。
4. ❌ All three denote sequences of exactly that many hashes; the colon in ⟦H⟧_{:O} only marks that the pool is indexed cyclically (H_T mod O) rather than from zero  
   cyclic index 是 ⟲ 上標（s[i]^⟲ ≡ s[i mod |s|]）的事，與型別下標無關。

> **陷阱**　冒號在前（_{:n}）= 上限、冒號在後（_{n:}）= 下限、沒冒號 = 恰好；pool 可空、queue 固定 80。

<sub>`ch03-sequence-set-subscripts`</sub>

---

### 3-2　Three state-transition rules use the arrow operators of GP §3.7.2: β_H′ ≡ ←(β_H† ⧺ e)^H (eq. 7.8), α′[c] ≡ ←(F(c) ⧺ φ′[c][H_T]^⟲)^O (eq. 8.2) and γ′_A ≡ →(n ∪ γ_A sorted ascending by ticket id)^E (eq. 6.35). Which reading is correct?

<sub>3.7.2 Editing — ●●○ · 概念 · §3.7.2 (⌢, ⧺, →s^n, ←s^n) and §3.7 (⟲); applied in eq. 7.8, eq. 8.2 and eq. 6.35</sub>

**標準答案**　→^n keeps the first n elements and ←^n the last n: β_H′ and α′[c] retain the H = 8 and O = 8 most recently appended entries, while γ′_A retains the E = 600 smallest ticket ids

GP §3.7.2：「We denote the sequence made up of the first n elements of sequence s to be →s^n ≡ [s_0, s_1, …, s_{n−1}], and only the final elements as ←s^n」。同節定義 ⌢ 為序列串接、⧺ 為元素接尾（x ⧺ i ≡ x ⌢ [i]）；§3.7 的 s[i]^⟲ ≡ s[i % |s|] 是模數索引，所以 φ′[c][H_T]^⟲ = φ′[c][H_T mod Q]（Q = 80）。三條規則的共同結構都是「先接、再截」：eq. 7.8 與 eq. 8.2 用 ←^n 保留最後 H = 8 / O = 8 筆（丟最舊），eq. 6.35 用 →^E 取排序後最前面 E = 600 個，GP 原文即「the accumulator becomes the lowest items of the sorted union」。你們的 code 三處都對得上：`alpha[c] = alpha[c][len-8:]`（authorization.go）、ticket accumulator 升冪排序後 `[:EpochLength]`（extrinsic_tickets.go）、`AddItem2BetaHPrime` 滿 8 筆時 `copy(historyPrime, historyDagger[1:])` 丟最舊。

**逐項辨析**

1. ❌ ←^n keeps the first n elements and →^n the last n: β_H′ and α′[c] therefore drop the newest entry once they are full, while γ′_A keeps the E largest ticket ids  
   箭頭意義對調了：這樣會保留最舊、丟掉最新，且排序後取 →^E 拿到的是最小者不是最大者。
2. ✅ →^n keeps the first n elements and ←^n the last n: β_H′ and α′[c] retain the H = 8 and O = 8 most recently appended entries, while γ′_A retains the E = 600 smallest ticket ids  
   → 留頭、← 留尾：β_H′ 與 α′[c] 保留最後 8 筆，γ′_A 取排序後最前 600 個最小 ticket id。
3. ❌ Both arrows keep n elements counted from the front; the direction only records whether the sequence was extended with ⧺ (append at the right) or with ⌢ (prepend at the left)  
   忽略了 ← 的定義（「only the final elements」）；⌢ 與 ⧺ 都接在右側，沒有 prepend 語意。
4. ❌ The arrows are cyclic rotations by n positions, the same operation as the ⟲ superscript, so β_H′ and α′[c] behave as ring buffers whose oldest slot is overwritten in place  
   把截斷與 ⟲ 模數索引混為一談；β_H、α 是會伸縮的序列，不是原地覆寫的 ring buffer。

> **陷阱**　箭頭指向哪邊就留哪邊：→ 留頭、← 留尾。§3.4 另有 % 為 mod、「5 ÷ 3 = 1 R 2」為商餘寫法（§6.1 的 e R m = τ/E 即 e = ⌊τ/E⌋、m = τ mod E）。

<sub>`ch03-first-last-n-arrows`</sub>

---

### 3-3　GP §3.7.3 defines bits(B) for an octet sequence B by the example bits([160, 0]) = [1, 0, 1, 0, 0, …], and M_σ (eq. D.5) keys the state trie by bits(k) of the 31-octet state key. The team's trie splits entries at each depth with the code below. Is it consistent with the GP?

<sub>3.7.3 Boolean values — ●●● · 程式碼 · §3.7.3 bits(); eq. D.5–D.6 (M_σ, M); App. C.1.4 (bit-sequence encoding); eq. A.15 (PVM ℬ_n)</sub>
...
```
<sub>internal/utilities/merklization/merklization.go (partitionByBit)</sub>

**標準答案**　Yes: 160 = 0b10100000, so bits() emits each octet's most significant bit first; masking key[depth/8] with 1 << (7 − depth mod 8) walks the path in exactly that order

GP §3.7.3：bits(B) 是「the sequence of bits, ordered with the most significant first, which represent the octet sequence B」，160 = 0b10100000 → [1,0,1,0,0,0,0,0]。eq. D.5：M_σ(σ) ≡ M({bits(k) ↦ (k, v) | (k ↦ v) ∈ T(σ)})，eq. D.6 依 key 的第一個 bit 分左右子樹（k_0 = 0 → 左、1 → 右）再遞迴，所以深度 d 對應 key[d/8] 的第 (7 − d mod 8) 位——`partitionByBit` 正確。真正的陷阱是 GP 裡並存三個方向不同的 bit 函數：App. C.1.4 的 E(b ∈ 𝕓)「pack the bits into octets in order of least significant to most」（b_i 放在 2^i），因此 bits() **不是** E(𝕓) 的反函數（E(bits([160])) = [5] ≠ [160]）；PVM 的 ℬ_n（eq. A.15）同樣是 LSB-first 且作用在自然數上。你們的 assurance bitfield 解碼 `(bytes[i/8] >> (i%8)) & 1`（types.go `MakeBitfieldFromByteSlice`）是 LSB-first、對應 codec，與 trie 的 MSB-first 各自都對，混用才錯。

**逐項辨析**

1. ❌ No: bits() is least-significant-bit first, matching the codec's bit-sequence encoding E(b ∈ 𝕓) which packs b_i into 2^i, so the mask must be 1 << (depth mod 8)  
   LSB-first 的是 App. C.1.4 的 E(b ∈ 𝕓) 與 assurance bitfield；§3.7.3 的 bits() 是 MSB-first。
2. ❌ No: bits() is MSB-first, but M_σ keys the trie by bits(H(k)) — the Blake2b hash of the state key — so the partition must be taken over H(k) rather than the raw key  
   那是 Ethereum MPT 的做法（keccak(key) 當 path）；eq. D.5 直接用 31-octet state key 的 bits(k)。
3. ❌ No: bits() is MSB-first, but the first bit of every node is the branch/leaf discriminator, so the key path must skip bit 0 of key[0] and the mask should start at 1 << (6 − depth mod 8)  
   discriminator 在 node encoding（eq. D.3）裡、不在 key path 裡，key 的每個 bit 都要走。
4. ✅ Yes: 160 = 0b10100000, so bits() emits each octet's most significant bit first; masking key[depth/8] with 1 << (7 − depth mod 8) walks the path in exactly that order  
   160 = 0b10100000 對上 [1,0,1,0,0,…]，1 << (7 − depth mod 8) 正是 MSB-first 的走法。

> **陷阱**　bits() = MSB-first（trie path）；E(𝕓)、PVM ℬ_n = LSB-first（bitfield、暫存器）。

<sub>`ch03-bits-msb-first-trie`</sub>

---

### 3-4　GP §3.8.2 writes a Bandersnatch VRF signature as Ṽ_k^m⟨x⟩ ⊂ B_96 with VRF output Y(·) ∈ H, and eq. 6.16 requires H_S ∈ Ṽ_{H_A}^{E_U(H)}⟨X_T ⌢ η′_3 ⧺ i_e⟩. Which statement is correct?

<sub>3.8.2 Signing Schemes — ●●● · 概念 · §3.8.2; applied in eq. 6.16 (seal), eq. 6.18 (H_V), eq. 6.30 (ticket proof) and eq. 6.32 (ticket id = Y)</sub>

**標準答案**　k = H_A is the signer's public key, the superscript E_U(H) is the signed message and the angle-bracketed term is the context; Y(H_S) is determined by the key and the context but not by the message

GP §3.8.2：Bandersnatch 簽名 Ṽ_k^m⟨x⟩ 的下標 k 是公鑰、上標 m 是訊息、⟨⟩ 內的 x 是 context（LaTeX 巨集 \bssignature{k}{x}{m} 排版成 Ṽ_k^m⟨x⟩）；「both define a VRF output, a high entropy hash influenced by x but not by m」。具名式與匿名式的差別則是「the member is identified in the former [Ṽ] and is anonymous in the latter [V̊]」——ring proof V̊_r^m⟨x⟩ ⊂ B_784 以 ring root r ∈ B̊ ⊂ B_144（由 O(keys) 算出）為下標。所以 eq. 6.16 的 seal 以未簽名 header E_U(H) 為訊息、X_T ⌢ η′_3 ⧺ i_e 為 context，Y(H_S) 只由金鑰與 context 決定——這正是 i_y = Y(H_S)（ticket id 在上一個 epoch 就能驗）以及 eq. 6.18 H_V ∈ Ṽ_{H_A}^{[]}⟨X_E ⌢ Y(H_S)⟩ 能當 bias-resistant entropy 的原因：作者改動 header 內容不會改變 VRF output。你們的 `createSignatureContext`（extrinsic_tickets.go）組的就是 ⟨⟩ 裡的 context（$jam_ticket_seal ⌢ η′_2 ⧺ attempt），訊息為空。

**逐項辨析**

1. ❌ The angle brackets carry the message and the superscript the context, so Y(H_S) depends on the header serialization E_U(H); this is what lets the seal's VRF output commit to the block content  
   ⟨⟩ 裝的是 context、上標才是訊息；若反過來，Y 會隨 header 內容改變而失去 bias-resistance。
2. ✅ k = H_A is the signer's public key, the superscript E_U(H) is the signed message and the angle-bracketed term is the context; Y(H_S) is determined by the key and the context but not by the message  
   下標鑰、上標訊息、⟨⟩ context，且 VRF output「influenced by x but not by m」。
3. ❌ Ṽ is the anonymous Ring-VRF form: H_A is a ring root over the validator keys and the signer cannot be identified, whereas the identifying form V̊ ⊂ B_784 is the one tickets use  
   具名與匿名說反了：Ṽ 具名、V̊ 才匿名；H_A 是單一公鑰，不是 B̊ 裡的 ring root。
4. ❌ Y(H_S) is a 96-octet value; the 32-octet ticket identifier i_y is obtained as H(Y(H_S)), and Ed25519 signatures V̄_k⟨m⟩ likewise define a VRF output  
   Y(·) ∈ H 本身就是 32 octet，eq. 6.16 直接令 i_y = Y(H_S)；Ed25519 也沒有 VRF output。

> **陷阱**　Ṽ_k^m⟨x⟩：下標鑰、上標訊息、⟨⟩ context；Y 看 context 不看訊息。Ṽ 具名 96 B、V̊ 匿名 784 B、V̄ Ed25519 64 B。

<sub>`ch03-vrf-signature-notation`</sub>

---

### 3-5　Per GP §3.8.1, which statement about the hash functions and the subscripted codec functions is correct?

<sub>3.8.1 Hashing — ●●○ · 概念 · §3.8.1 (H, H_K, H_0, E_l and E^{-1}_l assertions); Keccak applied in eq. 7.7</sub>

**標準答案**　H is Blake2b-256 and H_K is Keccak-256, both into H ≡ B_32; a non-blob argument is implicitly passed through E; E_4(x) asserts x ∈ N_2^32 and yields B_4, and E^{-1}_8(y) asserts y ∈ B_8 and yields N_2^64
...
1. ❌ H is Keccak-256 as in the Yellow Paper and H_K is Blake2b-256; the subscript on E counts the octets of the input, so E_4 accepts only arguments already in B_4 and E^{-1}_8 only naturals below 2^64  
   H 與 H_K 對調了；下標指的也是 octet 序列那一端的長度，不是輸入端。
2. ❌ H is Blake2b-256 and H_K is Keccak-256; hashing a tuple is undefined unless it is written explicitly as H(E(…)), and E_4 is the general variable-length natural encoding capped at four octets  
   GP 明說輸入會隱含通過 E，tuple 可直接放進 H；E_4 是固定長度 little-endian，不是 compact 編碼的上限。
3. ✅ H is Blake2b-256 and H_K is Keccak-256, both into H ≡ B_32; a non-blob argument is implicitly passed through E; E_4(x) asserts x ∈ N_2^32 and yields B_4, and E^{-1}_8(y) asserts y ∈ B_8 and yields N_2^64  
   下標永遠釘住 octet 端：E_4(x) 斷言 x ∈ N_2^32 且結果 ∈ B_4，E^{-1}_8 反向亦然。
4. ❌ H is Blake2b-512 truncated to 32 octets and H_K is Keccak-256; H_0 denotes H([]), the hash of the empty blob, which is also how M_σ identifies an empty sub-trie  
   H 是 Blake2b-256 而非截斷的 512；H_0 是 [0]_32，不是 H([])。

> **陷阱**　H = Blake2b、H_K = Keccak、H_0 = 32 個 0；E_l 的 l 指 octet 長度那一端；tuple 進 H 隱含先 E。

<sub>`ch03-hash-functions-and-codec-subscripts`</sub>

---

### 3-6　Let d = {1 ↦ a, 2 ↦ b} and e = {2 ↦ c, 3 ↦ a} be dictionaries in ⟨N → B⟩. Applying GP §3.5 (eq. 3.7–3.11), which statement is correct?

<sub>3.5 Dictionaries — ●●○ · 概念 · eq. 3.7–3.11; 𝒰 in eq. 3.2; ∅ / A? / ∇ in §3.3</sub>

**標準答案**　d ∪ e = {1 ↦ a, 2 ↦ c, 3 ↦ a} because the right operand wins on a collision; V(d ∪ e) = {a, c}; and writing d[3] in a rule asserts that key 3 exists — a block relying on it without handling ∅ is invalid

逐步計算：eq. 3.11 d ∪ e ≡ (d ∖ K(e)) ∪ e，「priorities the right-side operand in the case of a key-collision」：K(e) = {2, 3}（eq. 3.9），d ∖ K(e) = {1 ↦ a}（eq. 3.8，減去的是鍵集合 s ⊆ K），再聯集 e 得 {1 ↦ a, 2 ↦ c, 3 ↦ a}；eq. 3.10 的 V(·) 是**集合**，「should different keys with equal values appear in the dictionary, the set will only contain one such value」，故 V(d ∪ e) = {a, c}。查詢的陷阱藏在 eq. 3.7 之後那句：形式上 d[k] ≡ v 若存在、否則 ∅，但 GP 緊接著說「when using a subscript, it is an implicit assertion that the key exists in the dictionary. Should the key not exist, the result is undefined and any block which relies on it must be considered invalid」——要容許缺鍵就必須明寫處理，GP 自己的模式是 𝒰(d[k], default)（eq. 3.2 substitute-if-nothing）或先用 s ∈ K(d) 守門（accumulation 的 providable 函數 Y）。另外 §3.3：∅ 表示「validly left without a specific value」（|∅| = 0，A? ≡ A ∪ {∅}），∇ 才表示錯誤／無效。

**逐項辨析**

1. ❌ d ∪ e = {1 ↦ a, 2 ↦ b, 3 ↦ a} because the left operand wins on a collision; V(d ∪ e) = [a, b, a] keeps duplicates in insertion order; and d[3] = ∅ is an ordinary, valid lookup result in every context  
   eq. 3.11 是右邊優先，V(·) 回傳的集合也會去重；d[k] 更是隱含斷言 key 存在。
2. ❌ d ∪ e is undefined because key 2 collides — dictionary union is only defined when K(d) ⫰ K(e), which is why the GP always writes (d ∖ K(e)) ∪ e out explicitly instead of d ∪ e  
   union 正是為了處理衝突而定義的，(d ∖ K(e)) ∪ e 就是它的定義本身，不是替代寫法。
3. ❌ d ∖ {2} removes the pairs whose value is 2, so d ∖ {2} = d; K(e) = {c, a}; and d ∪ e = {1 ↦ a, 2 ↦ c, 3 ↦ a} because the right operand wins on a collision  
   ∖ 減的是鍵集合（eq. 3.8）、K(·) 取的也是鍵（eq. 3.9），所以 K(e) = {2, 3} 而非值。
4. ✅ d ∪ e = {1 ↦ a, 2 ↦ c, 3 ↦ a} because the right operand wins on a collision; V(d ∪ e) = {a, c}; and writing d[3] in a rule asserts that key 3 exists — a block relying on it without handling ∅ is invalid  
   右邊優先、V 去重，且 GP 明說下標是「an implicit assertion that the key exists」。

> **陷阱**　∪ 右邊贏；∖ 減鍵；K 鍵集、V 值集（去重）；缺鍵要嘛 𝒰(…) 給預設、要嘛 block 無效。

<sub>`ch03-dictionary-semantics`</sub>

---

### 3-7　Almost every type in the Gray Paper is built from the vocabulary fixed in §3.4 and §3.7.4: N, N_n, Z, N_L, B, B_x and B_$. Which statement reads all of them the way the GP defines them?

<sub>3.4 Numbers / 3.7.4 Octets and Blobs — ●○○ · 概念 · §3.4; §3.7.4; §3.8.1</sub>

**標準答案**　The subscripted natural set is the naturals strictly below the subscript, so N_n has n members; N_L is the GP's shorthand for the set of *lengths* of octet sequences and is equivalent to N_{2^32}; B_x is the set of blobs of length exactly x and B_$ the subset which are ASCII-encoded strings; and an octet, though bijective with a natural under 256, is not the same entity for serialization.

§3.4：「N denotes the set of naturals including zero whereas N_n implies a restriction on that set to values less than n. Formally, N = {0, 1, …} and N_n = {x | x ∈ N, x < n}」——嚴格小於，所以 N_n 有 n 個元素。同節：「we denote N_L as the set of lengths of octet sequences and is equivalent to N_{2^32}」——N_L 是**長度的集合**（一個自然數集），不是 blob 的集合。§3.7.4：「B denotes the set of octet strings (“blobs”) of arbitrary length… B_x denotes the set of such sequences of length x. B_$ denotes the subset of B which are ascii-encoded strings」。同節最後那句是面試最愛考的：「while an octet has an implicit and obvious bijective relationship with natural numbers less than 256…we do not treat them as exactly equivalent entities. In particular for the purpose of serialization, an octet is always serialized to itself, whereas a natural number may be serialized as a sequence of potentially several octets, depending on its magnitude and the encoding variant」——這是 App. C 裡 blob 直接落地、而自然數走 E 或 compact 變體的根據。另外 §3.8.1 定義 hash 集合 H ≡ B_32。

**逐項辨析**

1. ❌ The subscripted natural set is the naturals strictly below the subscript, so N_n has n members; N_L is the set of lengths of octet sequences and is equivalent to N_{2^32}; B_x is the set of blobs of length exactly x but B_$ is the subset whose leading octet is a sentinel marker; and an octet just is a natural under 256, so the two serialize identically.  
   B_$ 是 ASCII 字串子集、與 sentinel 標記無關；octet 與自然數在序列化上明確不等價。
2. ❌ The subscripted natural set includes the subscript itself, so N_n has n + 1 members; N_L is equivalent to N_{2^64} because octet-sequence lengths are carried as 64-bit values; B_x is the set of blobs of length exactly x and B_$ the subset which are ASCII-encoded strings; and an octet, though bijective with a natural under 256, is not the same entity under serialization.  
   N_n 是嚴格小於（恰 n 個元素），且 N_L 等價於 N_{2^32} 而不是 N_{2^64}。
3. ✅ The subscripted natural set is the naturals strictly below the subscript, so N_n has n members; N_L is the GP's shorthand for the set of *lengths* of octet sequences and is equivalent to N_{2^32}; B_x is the set of blobs of length exactly x and B_$ the subset which are ASCII-encoded strings; and an octet, though bijective with a natural under 256, is not the same entity for serialization.  
   四項全對上 §3.4 與 §3.7.4：嚴格小於、長度集合、恰好 x 個 octet、octet 與自然數序列化不同。
4. ❌ The subscripted natural set is the naturals strictly below the subscript, so N_n has n members; N_L is the set of octet sequences shorter than 2^32 rather than the set of their lengths; B_x is the set of blobs of at most x octets and B_$ the subset which are ASCII-encoded strings; and an octet serializes exactly as the general natural-number encoding does.  
   N_L 是**長度**的集合而非 blob 集合；B_x 是恰好 x，且 octet 一律序列化為自身。

> **陷阱**　N_L 是「長度」的集合（= N_{2^32}），不是 blob 的集合；B_x 是「恰好 x」不是「至多 x」。

<sub>`c3-ch03-bounded-numeric-blob-types`</sub>

---

### 3-8　§3.3 introduces ∅, ∇ and the optional constructor A?, and eq. 3.2 defines the substitute-if-nothing function 𝒰. §13.2 then writes a service's accumulation entry as 𝒰(S[s], (0, 0, 0)), where S is a dictionary from service index to a triple. Which reading is correct?

<sub>3.3 Sets / 3.2 Functions and Operators — ●○○ · 概念 · §3.3; eq. 3.2 (substitute-if-nothing); applied at eq. 13.13</sub>

**標準答案**　∅ marks a term validly left without a specific value and is defined to have cardinality zero; the optional constructor gives A? ≡ A ∪ {∅}; ∇ marks an unexpected failure or an invalid value, and the GP prefers it to ⊥ so it is not confused with Boolean false; and 𝒰 yields its first argument that is not ∅ — so the entry is the service's own triple when the dictionary holds one and the all-zero triple otherwise.

§3.3：「We commonly use ∅ to indicate that some term is validly left without a specific value. Its cardinality is defined as zero. We define the operation A? such that A? ≡ A ∪ {∅}」；「The term ∇ is utilized to indicate the unexpected failure of an operation or that a value is invalid or unexpected. (We try to avoid the use of the more conventional ⊥ here to avoid confusion with Boolean false…)」——∅ 與 ∇ 語意完全不同：前者是「合法地沒有值」，後者是「錯了」。eq. 3.2：𝒰(a_0, … a_n) ≡ a_x : (a_x ≠ ∅ ∨ x = n), ⋀_{i=0}^{x−1} a_i = ∅，GP 自己給的例子是 𝒰(∅, 1, ∅, 2) = 1 與 𝒰(∅, ∅) = ∅——取**第一個非 ∅** 的引數，且可以有兩個以上引數。至於 dictionary 查不到 key 時的結果：eq. 3.7 明寫 d[k] ≡ v if ∃k : (k ↦ v) ∈ d，otherwise ∅（不是 ∇）；這正是 𝒰(S[s], (0, 0, 0)) 能運作的原因——沒被 accumulate 的 service 查出 ∅，於是落到後備的全零三元組；若 dictionary miss 真的給 ∇，eq. 13.13 這個包裝就救不回來，因為 ∇ 代表的是「出錯了」而不是「合法地沒有值」。

**逐項辨析**

1. ✅ ∅ marks a term validly left without a specific value and is defined to have cardinality zero; the optional constructor gives A? ≡ A ∪ {∅}; ∇ marks an unexpected failure or an invalid value, and the GP prefers it to ⊥ so it is not confused with Boolean false; and 𝒰 yields its first argument that is not ∅ — so the entry is the service's own triple when the dictionary holds one and the all-zero triple otherwise.  
   §3.3 與 eq. 3.2 逐句對上：∅ 的 cardinality 為零、A? ≡ A ∪ {∅}、∇ 表錯誤、𝒰 取第一個非 ∅。
2. ❌ ∅ is the empty set itself, so it has cardinality zero only because it happens to hold no members; the optional constructor A? is a codec directive meaning the component may be omitted from the encoding rather than a set operation; ∇ is the GP's chosen spelling of Boolean false, which is why the more conventional ⊥ is avoided; and 𝒰 yields the last of its arguments that is not ∅ — so the entry is the all-zero triple whenever the dictionary holds anything at all.  
   A? 是集合運算不是 codec 指示；GP 不用 ⊥ 正是怕與 Boolean false 混淆，∇ 不是 false。
3. ❌ ∅ marks a term validly left without a specific value and the optional constructor gives A? ≡ A ∪ {∅}; ∇ is a synonym for ∅ reserved for the case where the absence was anticipated, so the two are interchangeable inhabitants of A?; and 𝒰 yields the first of its arguments that *is* ∅, falling back to its last argument when none of them is — so the entry is ∅ for every service that was accumulated and the all-zero triple only for the untouched ones.  
   ∇ 與 ∅ 語意相反：前者是「出錯了」，後者才是「合法地沒有值」；𝒰 取的也是非 ∅ 的那個。
4. ❌ ∅ has cardinality one, being a distinct inhabitant that A? adds to A; ∇ marks an unexpected failure or an invalid value and the GP prefers it to ⊥ so it is not confused with Boolean false; 𝒰 is defined for exactly two arguments, a candidate and a default; and a dictionary subscript on a missing key yields ∇ rather than ∅ — which is precisely why eq. 13.13 needs its fallback.  
   ∅ 的 cardinality 依 §3.3 定義為零；eq. 3.7 的 dictionary miss 給 ∅，且 𝒰 不限兩個引數。

> **陷阱**　∅ = 合法的「沒有值」；∇ = 錯誤。dictionary miss 給 ∅，不是 ∇；𝒰 取第一個非 ∅。

<sub>`c3-ch03-optional-none-error`</sub>

---

### 3-9　Let s = [10, 20, 30, 40, 50]. Applying the ellipsis conventions of GP §3.4 for integer sets and §3.7 for sequence slicing, which line is correct throughout?
...
### 3-10　Eq. 13.1–13.2 declare π ≡ (π_V, π_L, π_C, π_S) with (π_V, π_L) ∈ ⟦(b ∈ N, t ∈ N, p ∈ N, d ∈ N, g ∈ N, a ∈ N)⟧², and §13.1 then works through π_V†, π_V‡ and π′_V. An interviewer asks you to read that notation aloud. Which reading is right?
...
1. ❌ A type declaration and a concrete value are both written with ∈, the tricolon being reserved for the key/value pairs of a dictionary, so the six counters are positional only and must be addressed by index rather than by name; squaring the sequence-set says each of the two records is itself a sequence, one six-field entry per validator; the prime denotes the Grandpa-finalized value of a state component, so π′ is only well defined once finality has caught up with the chain; and dagger and double-dagger mark the same component as it stood on a fork that was later discarded.  
   型別宣告用 ∈、具體值用 tricolon，欄位一律以名字下標；prime 也與 finality 無關。
2. ❌ A type declaration introduces each component with ∈ while a concrete value binds its fields with the tricolon, and either way a field is read back by subscripting its name; squaring the sequence-set means the pair holds exactly two records in total, one per epoch, so each record carries a single validator's six counters; the prime marks the value that will be current at the start of the next epoch; and dagger and double-dagger are two further components of σ, each with its own state-trie key.  
   上標 2 是把整個序列集合取平方：π_V 與 π_L 各自是序列、每個 validator 一筆；dagger 也沒有 trie key。
3. ❌ ∈ and the tricolon are interchangeable spellings of the same binding, so a field may equally be read back by name or by position; squaring the sequence-set says each of the two records is itself a sequence, one six-field entry per validator; §3 defines the prime as 'the value after the extrinsic has been applied'; and the dagger marks a value that has already been through the epoch rollover while the double-dagger marks one that has not.  
   ∈ 與 tricolon 不可互換、沒有位置索引；prime 不在 §3 定義，dagger 與 double-dagger 的先後也說反了。
4. ✅ A type declaration introduces each component with ∈ while a concrete value binds its fields with the tricolon, and either way a field is read back by subscripting its name; squaring the sequence-set says each of the two records is itself a sequence, one six-field entry per validator; the undecorated symbol is the prior state and the primed one the posterior — a convention the GP fixes not in §3 but alongside σ′ ≡ Υ(σ, B) — and dagger and double-dagger mark intermediate values inside a single transition that appear in neither σ nor σ′.  
   prime 由 eq. 4.1 旁的文字定下、dagger 出自 §4 的 dependency graph，兩者都不在 §3。

> **陷阱**　「prime 在 GP 哪一節定義？」——不在 §3，而是 §4 eq. 4.1；dagger/double-dagger 也是 §4 交代的中間值。

<sub>`c3-ch03-prime-dagger-record`</sub>
...
### 4-1　GP 0.8.0's dependency graph names four dagger-superscripted intermediate states: β_H†, ρ†, ρ‡ and δ‡. Which description of what each one has just absorbed is correct?
...
1. ❌ β_H† = β_H after this block's guarantees E_G are appended to the newest entry; ρ† = ρ after the assurances E_A; ρ‡ = ρ† after the guarantees E_G; δ‡ = δ after the preimages E_P are integrated, before accumulation runs  
   eq. 4.6 的 β_H† 只依賴 (H, β_H)、補的是 parent 的 state root；E_P 也在 accumulation 之後才併入。
2. ✅ β_H† = β_H with the parent's posterior state root H_R written into its newest entry; ρ† = ρ after the disputes E_D; ρ‡ = ρ† after the assurances E_A; δ‡ = δ after accumulating R*, before E_P is folded in  
   四個中間態分別出自 eq. 4.6、4.12、4.13 與 4.16，順序與依賴列完全對得上。
3. ❌ β_H† = β_H after the accumulation-output log θ′ has been committed to the belt; ρ† = ρ after the guarantees E_G; ρ‡ = ρ† after the disputes E_D; δ‡ = δ after E_P is integrated and before deferred transfers are applied  
   belt β_B 由 θ′ 更新、與 dagger 無關；ρ 的兩步也把 E_D 與 E_G 對調了。
4. ❌ β_H† = β_H with H_R written in; ρ† = ρ after E_A; ρ‡ = ρ† after E_D; δ‡ = δ after E_P — the order of disputes and assurances is immaterial because both only remove entries from ρ  
   E_D 與 E_A 不可交換：assurance 只能對 ρ†[c] ≠ ∅ 的 core 設 bit（eq. 11.15）。

> **陷阱**　口訣：β_H† 補 root；ρ† 判、ρ‡ 保、ρ′ 收；δ‡ 算完帳、δ′ 才收 preimage。

<sub>`ch04-dagger-intermediate-states`</sub>

---

### 4-2　Reading the 0.8.0 dependency graph (eq. 4.5–4.20) literally, which statement about where the five extrinsic components enter the transition is TRUE?

<sub>4.2.1 State Transition Dependency Graph — ●●● · 概念 · eq. 4.11–4.20</sub>

**標準答案**　The guarantees extrinsic E_G is an input to ρ′, β_H′, α′ and π′, but not to the accumulation step, whose only extrinsic-derived input is R* (derived from E_A and ρ†)

逐條讀 eq. 4.5–4.20：E_T → γ′ (4.7)、π′ (4.20)；E_D → ψ′ (4.11)、ρ† (4.12)；E_A → ρ‡ (4.13)、R* (4.15)、π′；E_G → ρ′ (4.14)、β_H′ (4.17)、α′ (4.19)、π′；E_P → δ′ (4.18)、π′。結構上的關鍵是 accumulation（4.16）與 extrinsic 之間隔了一層：report 必須先 **available**（E_A 超過 2/3 assurance，eq. 11.17）才會進入 R*，所以同一塊 E_G 裡的 report 最快也要下一塊才被 accumulate；而 preimage 的併入更排在 accumulation 之後（GP：「accumulation of work happen before the preimage lookup extrinsic is folded into state」）。你們 STF 的順序（code-map §2：ψ′ → ρ† → ρ‡ → accumulation → E_P 併入 → α′ → π′ 最後）與此一致。

**逐項辨析**

1. ❌ The preimages extrinsic E_P is an input to the accumulation step (eq. 4.16), so a service's accumulate code can read a preimage that was provided in the same block  
   eq. 4.18 是 δ′ ≺ (E_P, δ‡, τ′)：preimage 在 accumulation 之後才併入，accumulate 讀不到同塊的。
2. ❌ The assurances extrinsic E_A is a direct input to ρ′, i.e. ρ′ ≺ (E_G, E_A, ρ†, κ, τ′), because guarantees and assurances are processed together in one pass over the cores  
   E_A 先產生 ρ‡（4.13），再由 E_G 覆蓋成 ρ′（4.14），並不是同一個 pass 的直接輸入。
3. ✅ The guarantees extrinsic E_G is an input to ρ′, β_H′, α′ and π′, but not to the accumulation step, whose only extrinsic-derived input is R* (derived from E_A and ρ†)  
   eq. 4.16 的輸入 (R*, ω, ξ, δ, χ, ι, φ, τ, τ′) 裡沒有任何 extrinsic，只有已 available 的 R*。
4. ❌ The disputes extrinsic E_D is a direct input to π′, because the validator statistics record which validators were reported as offenders in the block  
   π′ ≺ (E_G, E_P, E_A, E_T, τ, κ′, π, H, S) 不含 E_D；offender 記在 ψ′_O 與 header 的 H_O。

> **陷阱**　accumulation 的唯一「外部」輸入是 R*；E_P 最後、E_D 不進 π′。

<sub>`ch04-extrinsic-dependency-inputs`</sub>

---

### 4-3　The Overview states that JAM should be able to do 'upwards of 300 times' the computation in-core as a single machine running the VM at full speed. What justifies this figure, and what keeps such unreplicated computation safe?

<sub>4.8.1 In-core Consensus — ●●○ · 設計理由 · §4.9.1</sub>

**標準答案**　A given computation is executed by only a subset of validators, so throughput scales with network size rather than one machine; the guarantee/assure/audit (and if needed judge) game secures it, and stateless in-core execution can be reproduced by any node synced to the finalized chain

GP §4.9.1：「In this model… only a subset of the network is responsible for actually executing any given computation and assuring the availability of any input data it relies upon… we are able to scale the amount of computation done in consensus commensurate with the size of the network, and not with the computational power of any single machine… upwards of 300 times」。GP 沒有推導 300 這個數字；直觀上它與 core 數同量級——1023 validators / 每 core 3 guarantors = C = 341 cores，扣掉 audit、availability 等額外負擔即「300 多倍」（此為推論，非 GP 原文）。安全性來自「a crypto-economic game of three stages called guaranteeing, assuring, auditing and, potentially, judging」：guarantee 讓無效計算付出經濟代價、assure 確保輸入資料一段期間內可取得、audit 確保會有可期待為誠實的一方檢查正確性（judging 只在有爭議時發生）。並且「All execution done in-core must be reproducible by any node synchronized to the portion of the chain which has been finalized… designed to be as stateless as possible. The requirements… include only the refinement code of the service, the code of the authorizer and any preimage lookups it carried out」，lookup-anchor 必須落在 finalized chain 且夠新。

**逐項辨析**

1. ✅ A given computation is executed by only a subset of validators, so throughput scales with network size rather than one machine; the guarantee/assure/audit (and if needed judge) game secures it, and stateless in-core execution can be reproduced by any node synced to the finalized chain  
   GP 給的理由正是「only a subset of the network」執行，算力隨網路規模而非單機擴展。
2. ❌ Every validator still executes every in-core computation, but the PVM's recompiler makes RISC-V code roughly 300 times faster than EVM bytecode; safety therefore follows from full replication, exactly as in the on-chain model, and no auditing stage is required  
   那是 on-chain（everybody does everything）的描述；300x 也不是 PVM 對 EVM 的速度比。
3. ❌ In-core results are accepted on the guarantors' signatures alone and are never re-executed by anyone else; the economic stake bonded by the guarantors (slashed on a later dispute) is what makes the 300x figure safe without any further verification  
   漏掉 auditing——「確保會有可期待為誠實的一方檢查」正是 ELVES 存在的理由。
4. ❌ The 300x comes from each of the 341 cores using the whole 6-second slot in parallel; in-core code may read arbitrary on-chain state as of the lookup-anchor block, and that anchoring is what makes its results reproducible by other nodes  
   違反 stateless 設計：refine 只能透過 historical_lookup 讀 preimage，不能讀任意 on-chain state。

> **陷阱**　三段式（guarantee → assure → audit，必要時 judge）；in-core = stateless、可由 finalized chain 重現；300x ≈ core 數量級。

<sub>`ch04-in-core-300x-rationale`</sub>

---

### 4-4　GP §4.6 distinguishes the 'best block' from what the Grandpa finality gadget reports. Which statement matches the GP?

<sub>4.6 Best block — ●●○ · 概念 · §4.6; §4.3 (head, finalized); §19 (best chain)</sub>

**標準答案**　The best block is the head of the best chain (§19), used when latency matters more than certainty — choosing the parent for a block one is about to author, or reporting the latest state to a downstream application — at the risk that it never becomes canonical

GP §4.6：「The simplest and least risky means… would be to inspect the Grandpa finality mechanism… However… Grandpa will typically return a block some small period older than the most recently authored block. (Existing deployments suggest around 1-2 blocks in the past…) There are often circumstances when we may wish to have less latency at the risk of the returned block not ultimately forming a part of the future canonical chain. E.g. we may be in a position of being able to author a block, and we need to decide what its parent should be. Alternatively, we may care to speculate about the most recent state… In these cases, we define the best block as the head of the best chain, itself defined in section 19.」§19 的 best block B♭ 必須：以 finalized block 為祖先、未 finalized 的部分沒有 equivocation（同一 timeslot 兩個有效 block）、已 audited；候選中選「使用 slot-sealer ticket（而非 fallback key）封印的祖先數最多」者，Grandpa 投票投的就是它（連同 posterior state root）。§4.3 另把 head 定義為「the block with the most ancestors」——finalized、best、head 是三個不同的概念。

**逐項辨析**

1. ❌ The best block is the latest Grandpa-finalized block; JAM nodes never author on top of an unfinalized block, which is precisely why Grandpa lags only 1–2 blocks behind the most recent head and why no speculative state is ever exposed to applications  
   與 GP 相反：那樣每塊都要等 finality，而 Grandpa 本來就落後最新 head 約 1–2 塊。
2. ❌ The best block is simply the head — the valid block with the most ancestors — with ties between competing heads broken by the smaller header hash; Grandpa merely confirms that head some blocks later and plays no part in choosing a parent for authoring  
   §19 的 best chain 不是最長鏈，選的是 ticket 封印祖先數最多者，GP 也沒有 header hash tie-break。
3. ✅ The best block is the head of the best chain (§19), used when latency matters more than certainty — choosing the parent for a block one is about to author, or reporting the latest state to a downstream application — at the risk that it never becomes canonical  
   GP 明講 best block 就是 §19 best chain 的 head，用於出塊選 parent 與查最新狀態這類低延遲場合。
4. ❌ The best block is the most recent block whose timeslot is not in the future; Grandpa is consulted only afterwards, to prune whichever fork lost, so no separate best-chain rule is needed either for authoring or for serving state to applications  
   「timeslot 不在未來」只是 block 暫時有效性的條件（§4.4、§5），不是 best block 的定義。

> **陷阱**　finalized（Grandpa，落後 1–2 塊、最安全）≠ best（§19 best chain 的 head，出塊/查詢用）≠ head（祖先最多）。

<sub>`ch04-best-block-vs-finalized`</sub>

---

### 4-5　Per GP 0.8.0, the extrinsic E of a JAM block is a tuple of exactly five components. Which of the following is NOT one of them?

<sub>4.1 The Block — ●○○ · 概念 · eq. 4.2–4.3 (block / extrinsic)</sub>

**標準答案**　E_X — user transactions signed by external accounts

GP eq. 4.3：E ≡ (E_T, E_D, E_P, E_A, E_G)，即 tickets、disputes、preimages、assurances、guarantees（reports）。JAM 是 transactionless 的：沒有任何「使用者簽名的交易」進入區塊，所有 extrinsic 都是由 validator 產生的；外部資料只能透過 work-package（in-core 的 refine）以及 preimage 進入系統（§4.7：「there is no such concept of a transactor」）。

**逐項辨析**

1. ❌ E_T — tickets for the Safrole slot-sealer contest  
   E_T 是 eq. 4.3 五元組的第一項：Safrole 的 ticket extrinsic。
2. ✅ E_X — user transactions signed by external accounts  
   「使用者簽名的交易」是 Ethereum 模型的概念，eq. 4.3 的五元組裡沒有這一項。
3. ❌ E_A — availability assurances by validators  
   E_A 是 validator 對 availability 的 assurance，確實列在 eq. 4.3 之中。
4. ❌ E_D — disputes (verdicts, culprits, faults)  
   E_D 是 disputes（verdicts、culprits、faults），eq. 4.3 的第二項。

> **陷阱**　面試常問：JAM 為什麼沒有 transaction？答：授權（authorization）與 blockspace 購買（coretime）被拆開，外部輸入經 refine 進入。

<sub>`ch04-stf-extrinsic`</sub>

---

### 4-6　GP eq. 4.4 partitions the state σ into 17 components. Which pairing of symbol → meaning is WRONG?

<sub>4.2 The State — ●●○ · 概念 · eq. 4.4 (state composition)</sub>

**標準答案**　ω → the authorizer queue from which each core's pool is refilled

σ ≡ (α, β, θ, γ, δ, η, ι, κ, λ, ρ, τ, φ, χ, ψ, π, ω, ξ)，共 17 個分量。最容易混的是 authorization 與 accumulation 兩組佇列：α 是 authorizer pool、φ 是 authorizer queue（pool 的補給來源）、ω 是 ready queue、ξ 是防重複與 dependency 判斷用的已 accumulate 集合、θ 是本區塊的 accumulation output log（(service, hash) pairs），會被 β 的 belt 吸收。

**逐項辨析**

1. ❌ ρ → each core's current availability assignment (a guaranteed but not-yet-available work-report)  
   ρ 確實是每個 core 的 availability assignment：已 guarantee 但尚未 available 的 work-report。
2. ❌ ξ → work-packages recently accumulated (one epoch of history)  
   ξ 確實是最近一個 epoch 內已 accumulate 的 work-package hash，配對無誤。
3. ✅ ω → the authorizer queue from which each core's pool is refilled  
   ω 是 ready queue（已 available 但 dependency 未滿足的 report）；authorizer queue 是 φ。
4. ❌ θ → the accumulation output log of the most recent block  
   θ 確實是本區塊的 accumulation output log，配對無誤。

> **陷阱**　ω (vartheta) vs φ (phi) 容易混：ω = ready/queued reports，φ = auth queue。

<sub>`ch04-state-components`</sub>

---

### 4-7　In the state-transition dependency graph, the posterior authorizer pool α′ is defined as α′ ≺ (H, E_G, φ′, α). What does this imply for the order of computation inside a block import?

<sub>4.2.1 State Transition Dependency Graph — ●●○ · 概念 · eq. 4.5–4.20</sub>
...
1. ❌ α′ can be computed before any extrinsic is validated, since eq. 4.5–4.20 makes it depend only on the header's timeslot H_T and the prior pool α  
   eq. 4.5–4.20 的依賴列是 α′ ≺ (H, E_G, φ′, α)，共四個輸入，不只 header 與 prior pool。
2. ✅ α′ must be computed after accumulation, because φ′ (the posterior authorizer queue) is only known once accumulate has run (the `assign` host call may change it)  
   φ′ 只有在 accumulation 跑完後才定義，§8.2 明講這一步必須排在 accumulation 之後。
3. ❌ α′ must be computed before the guarantees extrinsic is validated, since eq. 11.32 checks each report's authorizer against the posterior pool α′[w_c]  
   prior 與 posterior 反了：eq. 11.32 檢查的是 w_a ∈ α[w_c]，用的是 prior pool。
4. ❌ α′ is independent of the extrinsic and may run in parallel with Safrole, since φ changes only through the `designate` privilege held by the delegator χ_V  
   認錯了 host call 與權限：改 φ[c] 的是 core 的 assigner χ_A 用 assign，designate 是 χ_V 設 ι。

> **陷阱**　guarantee 驗證看 prior α；pool 更新用 posterior φ′。

<sub>`ch04-dependency-graph-alpha`</sub>

---

### 4-8　When does the JAM Common Era begin, and why was that particular time of day chosen?

<sub>4.4 Time — ●○○ · 概念 · §4.4 (JAM Common Era)</sub>
...
1. ❌ 00:00 UTC on 1 January 2025 — a midnight start makes timeslot 0 the first slot of a calendar day, so every 14,400-slot boundary is a date change worldwide  
   從 UTC 午夜起算的 24 小時倍數，UTC−5 還停在前一天、UTC+9 已進隔天，正是 midday 要避免的。
2. ✅ 12:00 UTC on 1 January 2025 — midday ensures every major timezone is on the same calendar date at any exact 24-hour multiple from the epoch start  
   GP 選 midday 的理由就是讓所有主要時區在 24 小時整數倍時落在同一個日期。
3. ❌ 12:00 UTC on 1 January 2024 — the day the first Gray Paper was published, so the slot index counts from the specification's own birthday  
   年份錯了（是 2025），而且 GP 從未把 common era 綁在自身的出版日上。
4. ❌ 00:00 UTC on 1 January 1970 — the Unix epoch itself, so a JAM timeslot index is simply the current Unix time divided by P = 6 seconds  
   若 era 就是 Unix epoch，腳註那個 1,735,732,800 秒的偏移就會是 0。

> **陷阱**　數字題：1,735,732,800；slot = 6 秒；epoch = 600 slots = 1 小時。

<sub>`ch04-common-era`</sub>

---

### 4-9　Which statement correctly contrasts JAM's in-core and on-chain consensus models?

<sub>4.8 The Core Model and Services — ●○○ · 概念 · §4.9.1–4.9.2</sub>

**標準答案**　In-core computation (refine) is executed by a subset of validators and secured by the guarantee/assure/audit/judge game; on-chain computation (accumulate) is executed by all validators

GP §4.9.1：in-core model 下「only a subset of the network is responsible for actually executing any given computation」，靠 guaranteeing → assuring → auditing →（可能的）judging 這組 crypto-economic game 確保正確性，可達單機約 300 倍的算力；on-chain（accumulate）則是 GP 稱為「everybody does everything」的模型。§4.9.2 補上兩者的性格：refine 是「a sort of high-performance stateless processor, able to accept arbitrary input data」，accumulate 則「more stateful, providing access to certain on-chain functionality including the possibility of transferring balance」，而 GP 對兩者差異的總結是「the primary difference between them one of scalability versus synchroneity」。

**逐項辨析**

1. ❌ In-core computation (refine) is executed by every validator, while on-chain computation (accumulate) is executed only by the block author and replayed by others on a dispute  
   兩邊都顛倒了；accumulate 是 STF 的一部分，每個匯入區塊的節點都必須重算才對得上 H_R。
2. ✅ In-core computation (refine) is executed by a subset of validators and secured by the guarantee/assure/audit/judge game; on-chain computation (accumulate) is executed by all validators  
   §4.9.1 的 in-core 只由子集執行、以 guarantee/assure/audit/judge 擔保；on-chain 才是全體都算。
3. ❌ In-core computation (refine) is stateful and may transfer balances between services; on-chain computation (accumulate) is stateless and accepts arbitrarily large inputs  
   說反了：refine 是 stateless、吃任意大的輸入，accumulate 才 stateful、能 transfer balance。
4. ❌ Both are executed by all validators and differ only in pricing: in-core gas is charged per octet of the work-package, while on-chain gas is charged per PVM instruction  
   差異的本質是「誰執行」與可擴展性，不是計價；沒有按 work-package octet 計 gas 這種規則。

> **陷阱**　refine = stateless / in-core / 大輸入；accumulate = stateful / on-chain / 小輸出。

<sub>`ch04-in-core-vs-on-chain`</sub>

---

### 4-10　Which of the following is TRUE about the numeric domains used by JAM?

<sub>4.6–4.7 — ●○○ · 概念 · eq. 4.21 (balance), eq. 4.28 (timeslot)</sub>

**標準答案**　Balances are N_2^64 (u64) with a standard denomination of 10^9 tokens; timeslots are N_2^32, giving the protocol a lifespan into the year 2840

GP eq. 4.21：N_B ≡ N_2^64，並假設 10^9 為標準面額，因此最多約 18×10^9 tokens；用 u64 正是為了讓餘額能塞進固定寬度的序列化。eq. 4.28：N_T ≡ N_2^32，6 秒一 slot，壽命到 2840 年 8 月中。GP 在同處把 Polkadot 的 10^10、Kusama 的 10^12 與 Ethereum 的 10^18 明文列為「different to」JAM 的對照組，這三個數字正是干擾項的來源。

**逐項辨析**

1. ✅ Balances are N_2^64 (u64) with a standard denomination of 10^9 tokens; timeslots are N_2^32, giving the protocol a lifespan into the year 2840  
   eq. 4.21 的 N_B ≡ N_2^64 配 10^9 面額，eq. 4.28 的 N_T ≡ N_2^32 才推得出 2840 年這個壽命。
2. ❌ Balances are N_2^128 to match Polkadot's 10^10 denomination; timeslots are N_2^64, so the slot index can never wrap and the protocol has no dated end of life  
   10^10 是 Polkadot 的面額；balance 是 u64 不是 u128，timeslot 放大成 u64 也就失去 2840 年這個結論。
3. ❌ Balances are N_2^64 with Ethereum's 10^18 denomination; timeslots are N_2^32, which caps total issuance at roughly 18 whole tokens once the denomination applies  
   10^18 是 Ethereum 的值；套上去 18×10^9 tokens 只剩 18 顆，正說明 GP 為何選 10^9。
4. ❌ Balances and timeslots are both N_2^64, with Kusama's 10^12 denomination, putting the protocol's end of life far beyond the year 2840  
   三處都錯：timeslot 是 u32、面額是 10^9，而 10^12 是 Kusama 的。

> **陷阱**　u64 balance、u32 timeslot、u32 service index。

<sub>`ch04-balance-timeslot-ranges`</sub>

---

### 4-11　How does JAM replace Ethereum's gas-purchase model for buying blockspace?

<sub>4.8.2 On Services and Accounts — ●●○ · 設計理由 · §4.9.2 last paragraphs</sub>

**標準答案**　Coretime is pre-purchased (Agile-Coretime style) and assigned to an authorization agent; procurement is out of scope of the GP and expected to be handled by a system parachain

GP §4.9.2：「In place of Ethereum's gas model for purchasing and measuring blockspace, JAM has the concept of coretime, which is prepurchased and assigned to an authorization agent… Its procurement is out of scope in the present work and is expected to be managed by a system parachain operating within a parachains service itself blessed with a number of cores」。授權代理（authorizer）讓外部行為者不必像 Ethereum 交易那樣自我識別即可提供輸入；同節斷言「In JAM, these are separated and there is no such concept of a 'transactor'」。這正是 §8 authorization system 的動機：把「買 coretime」與「提交具體工作」解耦。

**逐項辨析**

1. ❌ Blockspace is metered in gas and debited from the accumulating service's balance a_b as its work-report accumulates; a service whose balance runs dry has its reports dropped  
   gas 在 JAM 只是 PVM 的計量與上限，從頭到尾沒有餘額被扣；a_b 管的是 threshold a_t 與 transfer。
2. ✅ Coretime is pre-purchased (Agile-Coretime style) and assigned to an authorization agent; procurement is out of scope of the GP and expected to be handled by a system parachain  
   §4.9.2 明講 coretime 是預先購買並指派給 authorization agent，採購本身不在 GP 範圍內。
3. ❌ Validators auction each six-second slot to the highest-bidding work-package builder, and the authorization system of §8 exists to settle that auction and pay out the winning bid  
   §8 只維護 authorizer pool α 與 queue φ，沒有出價、沒有拍賣，也不付錢給出塊者。
4. ❌ Work-package builders sign a transaction that debits their own account on inclusion, exactly as in Ethereum — the one place where JAM keeps a signature-identified transactor  
   直接違反「there is no such concept of a 'transactor'」——區塊裡沒有使用者簽名的交易。

> **陷阱**　面試「design rationale」常問：為什麼要 authorization system？答：同時支援 Ethereum-style 與 Polkadot-style 的互動模式。

<sub>`ch04-coretime-vs-gas`</sub>

---

### 4-12　Which description of the PVM as summarized in the Overview is correct?

<sub>4.5 The Virtual Machine and Gas — ●○○ · 概念 · §4.7 The Virtual Machine and Gas, eq. 4.22–4.27</sub>
...
1. ❌ A stack machine derived from WebAssembly with 32-bit words, 16 registers and a linear memory that grows in 64 KiB pages with no inaccessible pages  
   GP 明寫 PVM 是 RISC register machine；eq. 4.24 的 access 陣列正是為了讓頁面能是 ∅，page fault 才存在。
2. ✅ A RISC-V (RV64EM) based register machine with 13 64-bit registers, little-endian, and a pageable 32-bit address space in 4096-octet pages  
   RV64EM、13 個 64-bit 暫存器、little-endian、2^32 octet 的 pageable RAM 與 Z_P = 4096 全部對上 §4.5。
3. ❌ A RISC-V (RV32IM) based register machine with 16 registers of 32 bits, big-endian, and a flat 64-bit address space that is not paged at all  
   四項全反：暫存器是 64-bit、13 個、little-endian，位址空間是 32-bit 且分頁。
4. ❌ An EVM-compatible machine that keeps the Yellow Paper's cryptographic precompiles and environment opcodes in its instruction set, with 256-bit words  
   與 GP 的簡化理由正好相反：密碼學與環境互動指令一律拿掉，能力改走 host call。

> **陷阱**　13 registers / 64-bit / 4 KiB pages / 32-bit addressing。

<sub>`ch04-pvm-summary`</sub>

---

### 4-13　JAM states three goals about forks: (1) two heads should rarely form, (2) when they do they should be resolved quickly, (3) it should be possible to identify a recent block that will remain in history in perpetuity. Which mechanism delivers which goal?

<sub>4.3 Which History? — ●○○ · 設計理由 · §4.3</sub>
...
1. ✅ Safrole delivers (1), Grandpa delivers (3), and both contribute to (2)  
   正是 §4.3 的分工：出塊的 Safrole 給 (1)、finality 的 Grandpa 給 (3)，(2) 由兩者共同貢獻。
2. ❌ Grandpa delivers (1) and (2); Safrole delivers (3)  
   把兩者對調了：Grandpa 管的是 finalization，不是「兩個 head 很少形成」。
3. ❌ Safrole delivers all three; Grandpa is only used for BEEFY bridging  
   Grandpa 在 GP 裡就是 finality gadget 本身，不是只為 BEEFY bridging 服務的附件。
4. ❌ ELVES auditing delivers (1); Safrole delivers (2); Grandpa delivers (3)  
   ELVES 是 in-core 計算正確性的 auditing 機制，與這三個 fork 目標無直接關係。

<sub>`ch04-forks-safrole-grandpa`</sub>
...
<sub>5 The Header (serialization, Appendix C.2) — ●●○ · 程式碼 · eq. 5.1 & §C.2 (E(H), E_U(H))</sub>
...
```
<sub>internal/utilities/block_serialization.go (HeaderUSerialization) + field order of Header.Encode in internal/types/encode.go</sub>
...
1. ❌ E_U(H) omits both Bandersnatch signatures — the entropy source H_V as well as the seal H_S — so a conformant implementation must cut 192 octets from E(H), and the seal's message contains no VRF material at all, which is what stops the two signatures depending on one another  
   E_U 只排除 seal；連 H_V 也拿掉的話，eq. 6.16/6.17 的 seal 就簽不到本塊的 entropy 來源。
2. ✅ E(H) = E(E_U(H), H_S): the 96-octet seal is the last, fixed-size field, so cutting 96 bytes off E(H) yields exactly the seal's message; inside E_U the order is H_P, H_R, H_X, E_4(H_T), H_E (0/1 discriminator), H_W (0/1 discriminator), E_2(H_I), H_V, var(H_O)  
   H_S 固定 96 octets 且永遠排在最後，去尾 96 bytes 恰好就是 seal 所簽的 message。
3. ❌ E_U(H) is E(H) with the seal replaced by 96 zero octets, keeping the header length constant so that H_P can be computed before sealing; simple truncation therefore yields a message 96 octets too short and an implementation must re-encode with a zeroed seal instead  
   E_U 是直接省略 seal 而不是填零：C.2 寫的是 E(H) = E(E_U(H), H_S)。
4. ❌ The serialized field order follows eq. 5.1 literally (…, H_W, var(H_O), E_2(H_I), H_V, H_S), so truncating the last 96 octets is only correct when H_O is empty, because the length-prefixed offenders list shifts every field that follows it, the seal included  
   H_O 再長也只是把 seal 的起點往後推，「尾端 96 bytes 就是 seal」與 H_O 長度無關。

> **陷阱**　序列化順序 p r x t e w i v o | s（H_O 在 H_V 之後、seal 最後），eq. 5.1 的 tuple 順序是 p r x t e w o i v s；E_U = E(H) 去尾 96 bytes。

<sub>`ch05-unsigned-header-serialization`</sub>
...
> **陷阱**　H_X 是「hash of five hashes」，不是 Merkle root；只有 p 與 g 以 hash 代替內容，E_T/E_A/E_D 是整包 encode 後 hash。

<sub>`ch05-extrinsic-hash-inclusion-proof`</sub>

---

### 5-3　A node receives a block at wall-clock time T whose header has H_T · P > T, while P(H)_t < H_T holds (its parent is known and older). How does GP §5 classify this block?

<sub>5 The Header — ●●○ · 概念 · eq. 5.8 & §5 text</sub>

**標準答案**　It fails eq. 5.8 for now, but the GP notes such a block 'may become valid as T advances' — unlike a block with H_T ≤ P(H)_t, which can never become valid, it is only temporarily invalid and may be re-evaluated later

eq. 5.8：P(H)_t < H_T ∧ H_T·P ≤ T（P = 6 秒，T 為自 JAM Common Era 起的 wall-clock 秒數）。§5：「A block may only be regarded as valid once the time-slot index H_T is in the past」，而且「Blocks considered invalid by this rule may become valid as T advances」——節點可以暫存未來區塊、等 T 追上再 import。對照你們的 code：STF 裡只檢查 τ < τ′（safrole.go 的 BadSlot：「timeslot value must be strictly monotonic」），wall-clock 條件放在 header_controller.ValidateTimeSlot（出塊路徑，用 time.Now() 與 JamCommonEra 2025-01-01 12:00 UTC 相減）；STF test vectors 與 fuzzer trace 都不含 wall-clock，所以這條規則只在 live 節點的 gossip/import 路徑上有意義。

**逐項辨析**

1. ❌ It is permanently invalid, and since authoring ahead of the clock is an offence the author's Ed25519 key must reach ψ_O as a culprit in the next block's disputes extrinsic, so the node discards the header rather than keeping it  
   ψ_O 只由 E_D 的 culprits/faults 增長，那些爭議全部圍繞 work-report 的正確性，與出塊時間無關。
2. ❌ It is valid as long as the drift stays below one slot period P = 6 s, which the GP grants as an explicit clock-skew tolerance so that honest authors at the very start of a slot are not rejected by peers whose clocks run marginally slow  
   GP 沒有任何 clock-skew 容忍條款，eq. 5.8 就是 H_T·P ≤ T 這個硬比較。
3. ❌ It is valid: only the ordering rule P(H)_t < H_T belongs to consensus, while the wall-clock comparison is authoring guidance — which is exactly why the STF test vectors carry no notion of T and an importing node must accept the block  
   eq. 5.8 的兩個 conjunct 都是有效性條件；vectors 不含 T 只因離線 trace 表達不了 wall-clock。
4. ✅ It fails eq. 5.8 for now, but the GP notes such a block 'may become valid as T advances' — unlike a block with H_T ≤ P(H)_t, which can never become valid, it is only temporarily invalid and may be re-evaluated later  
   T 會前進，所以未來區塊只是暫時無效；而 H_T ≤ P(H)_t 永遠不可能變真。

> **陷阱**　「暫時無效」只適用於未來 slot；跳過的 slot（H_T > P(H)_t + 1）是合法的，不要跟未來 slot 混淆。

<sub>`ch05-future-slot-temporarily-invalid`</sub>
...
**標準答案**　H_B — the BEEFY root committing to this block's own accumulation outputs

eq. 5.1：H ≡ (H_P parent hash, H_R prior state root, H_X extrinsic hash, H_T timeslot, H_E epoch marker, H_W winning-tickets marker, H_O offenders marker, H_I author index, H_V VRF signature, H_S seal)，共十個欄位——一個 parent hash、兩個 root/hash、時間、三個 marker、作者索引，以及兩個 Bandersnatch 簽章。BEEFY / accumulation-output root 屬於 β 的 accumulation-output belt（β_B，見 §7）與 §18 Beefy Distribution 的鏈下流程，從來不是 header 的一部分。

**逐項辨析**

1. ❌ H_R — the prior state root, which is the parent block's posterior state root  
   H_R 是 eq. 5.1 的第二個欄位，§5 定義它就是 parent 的 posterior state root。
2. ❌ H_W — the winning-tickets marker, carrying the next epoch's E = 600 tickets  
   H_W 型別為 ([ticket]_E)?（eq. 5.11），E = 600，裝的正是下一 epoch 的 slot-sealer tickets。
3. ✅ H_B — the BEEFY root committing to this block's own accumulation outputs  
   accumulation output 走 β 的 accumulation-output belt，簽的是 last(β)_b 的 super-peak，header 無此欄。
4. ❌ H_V — the entropy-yielding VRF signature that feeds the accumulator η_0  
   H_V 的輸出 Y(H_V) 正是 entropy accumulator 的輸入：η′_0 ≡ H(η_0 ⌢ Y(H_V))。

> **陷阱**　記憶法：p r x t | e w o | i v s。

<sub>`ch05-header-fields`</sub>
...
**標準答案**　To facilitate pipelining of block computation, in particular of Merklization: the author need not finish Merklizing the new state before publishing the block

GP §5：「This is a departure from both Polkadot and the Yellow Paper's Ethereum… We do this to facilitate the pipelining of block computation and in particular of Merklization.」實務上 block author 可以先發布區塊，把耗時的 state Merklization 延後到下一個 slot 才需要（下一個區塊的 H_R 才會用到）。後果：β 裡最新一筆的 state root 先填零，由下一個區塊的 β† 用 H_R 補正（eq. 7.5）。

**逐項辨析**

1. ❌ To reduce header size: the extrinsic hash H_X already commits to every input that can change state, so committing a second 32-octet root would be redundant  
   prior 與 posterior root 都是 32 octet，換一個並不省空間；H_X 承諾的是輸入而不是結果狀態。
2. ✅ To facilitate pipelining of block computation, in particular of Merklization: the author need not finish Merklizing the new state before publishing the block  
   GP 給的理由就是 pipelining：作者不必先算完新狀態的 Merklization 才發布區塊。
3. ❌ Because Grandpa votes carry the block header alone, which makes the header's prior state root the only place a finalized state root is ever committed  
   §19 明寫 Grandpa 投的是 best block header「together with its posterior state root」。
4. ❌ Because the posterior state root is undefined until the block's work-reports have been audited, and auditing only completes several tranches later  
   σ′ 由 (σ, B) 完全決定、區塊內就算得出；auditing 是鏈下流程，只影響 finalization 與選鏈。

> **陷阱**　設計理念題。延伸：Grandpa vote 會帶 posterior state root（§19）以彌補 header 只有 prior root。

<sub>`ch05-prior-state-root`</sub>
...
GP eq. 5.6–5.7：p = E(var[(E_4(s), H(d)) for (s,d) in E_P])，g = E(var[(H(w), E_4(t), var(a)) for (w,t,a) in E_G])。GP 為此給的理由（§5）：「taking care to allow for the possibility of reports and preimages to individually have their inclusion proven」——只放 hash，就能用 Merkle proof 證明某個 preimage/report 在區塊裡，而不必附上整個 blob。你們的 code（block_serialization / header hash）在 0.7.2 對 guarantees 已是用 report hash，0.8.0 再把 preimages 改為 (service, blake(data))。
...
**標準答案**　P(H)_t < H_T ∧ H_T · P ≤ T — strictly greater than the parent's slot, and no later than the current wall-clock time T

GP eq. 5.8：P(H)_t < H_T ∧ H_T·P ≤ T，其中 P = 6 秒、T 為 wall-clock 秒數（自 Common Era 起算）。§5 原文：「A block may only be regarded as valid once the time-slot index H_T is in the past. It is always strictly greater than that of its parent.」且「Blocks considered invalid by this rule may become valid as T advances」——太新的區塊只是暫時無效，不是永久拒絕。你們 STF 裡 `BadSlot` 對應的是 τ ≥ τ′ 的情況。

**逐項辨析**

1. ✅ P(H)_t < H_T ∧ H_T · P ≤ T — strictly greater than the parent's slot, and no later than the current wall-clock time T  
   eq. 5.8 正是這兩條：對 parent 嚴格大於，且 H_T·P ≤ T（P = 6 秒的 wall-clock）。
2. ❌ P(H)_t ≤ H_T ∧ H_T · P ≤ T — greater than or equal to the parent's slot, and no later than the current wall-clock time T  
   放寬成「大於或等於 parent」會允許父子同 slot，GP 明寫 strictly greater。
3. ❌ P(H)_t < H_T ∧ H_T · P ≥ T — strictly greater than the parent's slot, and at or beyond the current wall-clock time T  
   wall-clock 條件方向反了：H_T·P ≥ T 等於只接受當下或未來的區塊。
4. ❌ H_T = P(H)_t + 1 ∧ H_T · P ≤ T — exactly one slot after the parent, and no later than the current wall-clock time T  
   要求恰好 parent + 1 就禁止跳 slot，但沒人出塊時 slot 本來就會被跳過。

> **陷阱**　跳 slot 是合法的（例如沒人出塊），這也是為什麼 Safrole 要處理 e′ > e+1（整個 epoch 被跳過）的情況。

<sub>`ch05-timeslot-validity`</sub>
...
**標準答案**　Into the posterior active set κ′; H_A ≡ κ′[H_I]_b, and H_A is NOT serialized — it is merely an equivalence

GP eq. 5.10：H_I ∈ N_{|κ′|}，H_A ≡ κ′[H_I]_b，「this is merely an equivalence, and is not serialized as part of the header」——驗證者是自己用 κ′ 算出作者的 Bandersnatch key。用 posterior κ′ 是因為 epoch 交界的第一個區塊已由新的 active set 出塊（key rotation 在該區塊內完成）。你們的 sealing.go 驗 seal/VRF 時傳入的也是 posterior state（code-map 3.2.6 註：參數名叫 priorState 但 STF 傳的是 posterior）。

**逐項辨析**

1. ❌ Into the prior active set κ; H_A = κ[H_I]_b, and H_A is serialized as an eleventh header field beside H_I  
   prior/posterior 反了，而且 eq. 5.1 的 header 只有十個欄位，H_A 從不上鏈。
2. ✅ Into the posterior active set κ′; H_A ≡ κ′[H_I]_b, and H_A is NOT serialized — it is merely an equivalence  
   eq. 5.10 就是 H_I ∈ N_{|κ′|}、H_A ≡ κ′[H_I]_b，並明說它不被序列化。
3. ❌ Into the pending set γ_k; H_A = γ_k[H_I]_b, which is why H_I may exceed |κ′| in an epoch's first block  
   違反 eq. 5.10 的型別 H_I ∈ N_{|κ′|}——索引域永遠是 κ′，不可能超出。
4. ❌ Into the staging set ι; H_A = ι[H_I]_b, since ι holds the keys the delegator designates for the very next epoch  
   ι 慢一輪：eq. 6.14 要先 ι → γ_k，再過一個 epoch 才成為 κ，對應的是下下個 epoch。

> **陷阱**　epoch 第一個區塊：作者屬於 κ′ = 舊的 γ_k。

<sub>`ch05-author-index`</sub>
...
GP eq. 11.38：∀x ∈ contexts: ∃h, h′ ∈ A: h_t = x_l（lookup-anchor time）∧ H(h) = x_l（lookup-anchor hash）∧ h′_p = H(h) ∧ h′_r = x_{l,s}（posterior state root of lookup anchor，0.8.0 新增）。§11.4：「this is one of the few conditions which cannot be checked purely with on-chain state and must be checked by virtue of retaining the series of the last L headers」。這也是 fuzzer 的「Ancestry」feature：M1 必須支援。

**逐項辨析**

1. ✅ Verifying that a guarantee's lookup-anchor block (hash, timeslot and posterior state root) really occurs in the chain, which state σ on its own cannot attest  
   lookup anchor 最舊可到 L = 14,400 slot 之前，而 recent history β 只留 H = 8 塊，非靠 A 不可。
2. ❌ Verifying that each new block's parent hash H_P is the Blake2b of the parent header, which needs every header back to the last finalized block  
   eq. 5.2（H_P ≡ H(E(P(H)))）只用到直接的 parent header 一份，不必往回保留任何東西。
3. ❌ Verifying that each availability assurance's anchor matches the parent hash H_P, which needs a search of the retained headers to locate that anchor  
   eq. 11.12 的 a_a = H_P 是一次等值比對，不需要在保留的 header 序列裡搜尋。
4. ❌ Recomputing the fallback slot-sealer sequence F(η′_2, κ′) after a skipped epoch, which needs the entropy VRF H_V of every header in the skipped epoch  
   F(η′_2, κ′)（eq. 6.27）的輸入只有 posterior entropy 與 κ′ 兩個 state 分量，不讀歷史 header。

> **陷阱**　L = 14,400 slots = 24h；recent history H = 8 blocks 是給 anchor 用的，lookup anchor 用 A。

<sub>`ch05-ancestors-lookup-anchor`</sub>
...
**標準答案**　H_E ∈ (H, H, [(bandersnatch, ed25519)]_V)? ; H_W ∈ ([ticket]_E)? ; H_O ∈ [ed25519 key] — the offenders marker is a plain sequence that may be empty but is never None

GP eq. 5.11：H_E ∈ (H, H, [(bandersnatch, ed25519)]_V)?（兩個 entropy hash + 下一 epoch 每個 validator 的 Bandersnatch 與 Ed25519 key，0.6.4 起加入 Ed25519），H_W ∈ ([ticket]_E)?（E = 600 張 tickets），H_O ∈ [ed25519 key]。時機也不同：H_E 只在 e′ > e 時非空，H_W 只在同一 epoch 內首次跨越 Y 且 accumulator 飽和時非空，而 H_O 與 epoch 邊界無關，由 §10 disputes 帶進來的 culprits/faults 決定。fallback slot-sealer 序列 F(η′_2, κ′) 則是鏈上重算（eq. 6.27），不佔任何 header 欄位。

**逐項辨析**

1. ✅ H_E ∈ (H, H, [(bandersnatch, ed25519)]_V)? ; H_W ∈ ([ticket]_E)? ; H_O ∈ [ed25519 key] — the offenders marker is a plain sequence that may be empty but is never None  
   eq. 5.11 的 H_O 沒有 ? 包裝：它可以是空序列，但永遠不是 None。
2. ❌ H_E ∈ (H, H, [(bandersnatch, ed25519)]_V)? ; H_W ∈ ([ticket]_E)? ; H_O ∈ [ed25519 key]? — all three are optional and all three are None outside an epoch's first block  
   型別上 H_O 不帶 ?；語意上只要該塊帶了 culprits 或 faults，epoch 中間任何一塊都可能非空。
3. ❌ H_E ∈ ([336-octet validator key]_V)? ; H_W ∈ ([bandersnatch key]_E)? ; H_O ∈ [ed25519 key] — the epoch marker carries whole keys and the winners marker the fallback sealers  
   H_E 只放 (bandersnatch, ed25519) 這對子集合；fallback key 由鏈上重算，從不進 H_W。
4. ❌ H_E ∈ (H, H, [(bandersnatch, ed25519)]_V)? ; H_W ∈ ([ticket]_E)? ; H_O ∈ [N_V]? — the offenders marker is optional and holds validator indices rather than keys  
   H_O 裝的是 Ed25519 公鑰而不是 validator index，而且它並非 optional。

> **陷阱**　H_E 放的是 key 的子集合（bs + ed），不是完整 336-octet key；H_O 放 key 不是 index。

<sub>`ch05-markers-types`</sub>
...
return nil
```
<sub>internal/safrole/extrinsic_tickets.go (CreateNewTicketAccumulator, tail)</sub>

**標準答案**　Eq. 6.36 (n ⊆ γ′_A) makes the block INVALID, but this code silently drops the 3 tickets in the [:E] truncation and accepts it — a block that the reference implementation rejects
...
1. ❌ The block is valid per eq. 6.35: γ′_A simply keeps the lowest E ids, so the 3 tickets are not retained and nothing further is required of the block; this code is fully conformant and needs no extra check  
   漏掉 6.36：γ′_A 的確等於 γ_A，但「沒擠進前 E 名的 ticket」讓整個區塊必須被拒絕。
2. ✅ Eq. 6.36 (n ⊆ γ′_A) makes the block INVALID, but this code silently drops the 3 tickets in the [:E] truncation and accepts it — a block that the reference implementation rejects  
   6.36 要求 n ⊆ γ′_A，而這裡 `[:maxTicketsAccumulatorSize]` 截掉後就 SetGammaA、return nil。
3. ❌ The block is invalid per eq. 6.34 because the new ids collide with γ_A, and VerifyTicketsDuplicate on the merged, sorted sequence is exactly the check that catches this case, so the code is conformant  
   6.34 講的是 id **相同**；VerifyTicketsDuplicate 只比排序後相鄰是否相等，抓不到較大的合法 id。
4. ❌ The block is invalid per eq. 6.31 because tickets are only accepted while |γ_A| < E; the code should compare len(previousTicketsAccumulator) with E before merging and return UnexpectedTicket  
   6.31 限的是 |E_T| ≤ K，與 |γ_A| 無關——accumulator 已滿仍可接受分數更好的 ticket。

> **陷阱**　口訣：進不了前 E 名的 ticket 不是被丟掉，而是讓整個區塊作廢（6.36）。

<sub>`ch06-code-useless-ticket-gap`</sub>

---

### 6-2　This is VerifyTicketsAttempt on the team's 0.8.0 branch (PR #1025); on main (0.7.2) it compared Attempt against the constant TicketsPerValidator. Which statement about it is correct?

<sub>6.7 The Extrinsic and Tickets — ●●○ · 程式碼 · eq. 6.30 (n = ⌈2E/|γ′_P|⌉) — internal/safrole/extrinsic_tickets.go VerifyTicketsAttempt (branch 1012-update-to-v080, PR #1025) · ⚠ 0.7.2→0.8.0</sub>
...
```
<sub>internal/safrole/extrinsic_tickets.go (VerifyTicketsAttempt, branch 1012-update-to-v080)</sub>

**標準答案**　It implements n = ⌈2E/|γ′_P|⌉ with the posterior pending set (the team's GammaK), i.e. the ring the tickets are proven against; (2E + |γ′_P| − 1) / |γ′_P| is the integer ceiling and the bound is exclusive because N_n = {0, …, n−1}

eq. 6.30：E_T ∈ [(r ∈ N_n, p ∈ F̄^{X_T ⌢ η′_2 ++ r}_{γ′_Z}([]))]，n = ⌈2E/|γ′_P|⌉——分母是 **posterior pending set** γ′_P（你們命名為 GammaK），因為 ticket 是對 γ′_Z（由 γ′_P 的 Bandersnatch key 建 ring）做 ring proof，是「下個 epoch 的 validator」在投票，與本 epoch 出塊的 κ′ 無關。§3.4：N_n = {x ∈ N, x < n}。算例：tiny E = 12、|γ′_P| = 6 → (24+5)/6 = 4；full E = 600、1023 → (1200+1022)/1023 = 2。0.7.2 main 用常數 TicketsPerValidator（tiny 3 / full 2），tiny 下 entry index 3 在 0.8.0 變成合法（PR #1025 因此把舊 vector publish-tickets-no-mark-1「bad ticket attempt number」標成 IsV080IncompatibleVector 跳過）；同一個 PR 也把 VerifyEpochTail 的上限從 ValidatorsCount 改成 MaxTicketsPerBlock（K，eq. 6.31）。GP 的理由：「To ensure the accumulator can be saturated, when there are fewer validators, each validator is permitted more tickets」（每個 slot 期望約 2 張 ticket）。

**逐項辨析**

1. ❌ It has an off-by-one: eq. 6.30 puts the entry index in N_n, which by §3.4 includes n itself, so Attempt == n must be accepted and the comparison should be > n rather than >= n; in tiny mode (E = 12, |γ′_P| = 6) this wrongly rejects entry index 4  
   誤讀 §3.4：N_n = {x | x < n} 是嚴格小於，tiny 下合法的 entry index 是 0…3 而非 0…4。
2. ❌ It uses the wrong set: n must be derived from |κ′|, the active set, because tickets are submitted by the validators authoring blocks right now; taking GetGammaK makes the bound change one epoch too early on any chain whose set size varies  
   分母必須與 ticket 所證明的 ring 是同一個集合——ring 由 γ′_P 的 Bandersnatch key 建成。
3. ❌ It is only a refactor: because offenders are zeroed in place rather than removed, |γ′_P| always equals V, so 0.7.2's constant TicketsPerValidator (3 tiny / 2 full) already produced exactly the same bound and no test vector changes behaviour  
   與 PR #1025 的事實相反：tiny 的 bound 從 3 變成 4，舊 vector 才被標成 IsV080IncompatibleVector。
4. ✅ It implements n = ⌈2E/|γ′_P|⌉ with the posterior pending set (the team's GammaK), i.e. the ring the tickets are proven against; (2E + |γ′_P| − 1) / |γ′_P| is the integer ceiling and the bound is exclusive because N_n = {0, …, n−1}  
   分母取 posterior 的 γ′_P，(2E + |γ′_P| − 1) / |γ′_P| 是整數 ceiling，上界 exclusive 也正確。

> **陷阱**　n 的分母是 |γ′_P| 不是 |κ′|；上界是 exclusive（r < n）；tiny 的 n 從 3 變 4。

<sub>`ch06-code-attempt-cap-v080`</sub>
...
<sub>6.4 Sealing and Entropy Accumulation — ●●● · 程式碼 · eq. 6.16–6.18, 6.28; §5 eq. 5.10 — internal/stf/sft.go RunSTF, validate_header.go ValidateHeaderVrf</sub>
...
```
<sub>internal/stf/sft.go (RunSTF) + validate_header.go (ValidateHeaderVrf)</sub>

**標準答案**　Because all three depend on this block's own rotation: H_S is checked against γ′_S[H_T mod E] with H_A = κ′[H_I]_b and context η′_3, H_V against the same H_A, and H_E against γ′_P = Φ(ι); the prior κ/η/γ_S pass mid-epoch but fail at epoch boundaries

eq. 6.16/6.17：i = γ′_S[H_T mod E]（posterior 序列），H_S 由 H_A 簽、context 用 η′_3；§5 eq. 5.10：H_A ≡ κ′[H_I]_b（posterior active set）；eq. 6.18：H_V 同樣由 H_A 驗；eq. 6.28：H_E 的 key 列表是 γ′_P = Φ(ι)，但其兩個 entropy 欄位比對的是 prior η_0、η_1（ValidateHeaderEpochMark 用 priorState.Eta[0]/[1]）。這些 posterior 值在 epoch 第一塊全由該塊自己的 KeyRotate（κ′ = γ_P）、UpdateEntropy（η′_3 = η_2）、UpdateSlotKeySequence 產生，所以 seal/entropy/epoch-mark 只能在 UpdateSafrole 之後驗；epoch 中段 κ′ = κ、η′_3 = η_3、γ′_S = γ_S，用 prior state 也會過——這正是 #784「on some cases」只在 epoch 邊界失敗的原因（trace 1758621879/00000348 被誤判為 error 7 VrfSealInvalid），PR #791 改成傳 posterior Kappa & Eta。H_W、H_O、H_X、H_R、H_I 只依賴 prior state 與 extrinsic，所以留在 ValidateNonVRFHeader（reviewer yu2C 堅持不要整個搬到 safrole 之後），其中 H_I 的 range check 必須最早做（#825）。順帶一提 sft.go 的註解「(H_E, H_W, H_O, H_I)」已過時——H_E 實際在 ValidateHeaderVrf 裡驗。

**逐項辨析**

1. ❌ Because the seal signs E_U(H), which embeds H_E and H_W, so those markers must be finalized first; the validator set itself is irrelevant, since κ′ = κ whenever the block is valid and the seal context η′_3 equals η_3 in every block  
   「κ′ = κ whenever valid」在 e′ > e 就不成立，η′_3 也只有 epoch 中段才等於 η_3。
2. ✅ Because all three depend on this block's own rotation: H_S is checked against γ′_S[H_T mod E] with H_A = κ′[H_I]_b and context η′_3, H_V against the same H_A, and H_E against γ′_P = Φ(ι); the prior κ/η/γ_S pass mid-epoch but fail at epoch boundaries  
   三者的比對對象 γ′_S、κ′、η′_3、γ′_P 全是本塊自己的 rotation 產物，只能在 UpdateSafrole 之後驗。
3. ❌ Only for performance: ring-VRF verification is slow, so it is deferred until the cheap checks have passed; verifying against the prior state would give identical results in every block because κ, η and γ_S are stable within a block  
   效能不是理由，而且 seal 是 IETF VRF、不是昂貴的 ring VRF。
4. ❌ Because H_E must be compared with the posterior entropy η′_0, η′_1, which only exist after UpdateEtaPrime0; the seal itself could safely be verified against the prior state, since H_A = κ[H_I]_b by definition  
   H_E 比對的是 **prior** η_0、η_1；且 §5 eq. 5.10 明寫 H_A ≡ κ′[H_I]_b，用的是 posterior。

> **陷阱**　口訣：seal、H_V 看 posterior（γ′_S、κ′、η′_3）；H_E 看 prior η_0/η_1 + posterior γ′_P；H_W/H_O/H_I 只看 prior + extrinsic。

<sub>`ch06-code-header-checks-posterior`</sub>

---

### 6-4　After fuzzer bug #825 (panic 'index out of range [65535] with length 6' while computing η′_0), the team added this check to ValidateNonVRFHeader, which runs BEFORE UpdateSafrole. Which assessment is right under GP 0.8.0?
...
1. ❌ The check is exactly what the GP specifies: eq. 5.10 bounds H_I by |κ|, the prior active set, which is precisely why it can be verified before key rotation without loss of generality, and it therefore covers the fuzzer's 65535 case at any validator-set size  
...
> **陷阱**　面試官可能追問：H_I 越界時你們回什麼？答：沒有官方 code，自訂 13；重點是必須在任何 κ′[H_I] 之前檢查，否則 panic。

<sub>`ch06-code-author-index-bound`</sub>

---

### 6-5　Full parameters (E = 600, Y = 500). The prior block has τ = 1195 and a saturated accumulator (|γ_A| = E); the next block has H_T = 1810 (nothing was authored in between). Expressing posterior values in terms of PRIOR ones, which description of this transition is correct?

<sub>6.3–6.7 (epoch transition across skipped epochs) — ●●● · 概念 · eq. 6.2, 6.14, 6.24, 6.25, 6.28–6.29, 6.35</sub>

**標準答案**　Rotation happens exactly once: κ′ = γ_P, λ′ = κ, γ′_P = Φ(ι), (η′_1, η′_2, η′_3) = (η_0, η_1, η_2); γ′_S = F(η_1, γ_P) despite m = 595 ≥ Y and a full γ_A; H_E is present, H_W absent; γ′_A is rebuilt from this block's E_T alone

計算：e ⌢ m = 1195/600 → e = 1、m = 595；e′ ⌢ m′ = 1810/600 → e′ = 3、m′ = 10（eq. 6.2）。eq. 6.14 與 6.24 的條件都是布林的 e′ > e，與 e′ − e 無關，所以 key 與 entropy **只 rotate 一次**：κ′ = γ_P、λ′ = κ、γ′_P = Φ(ι)、(η′_1, η′_2, η′_3) = (η_0, η_1, η_2)——state 只記得最後一塊，沒有「中間 epoch」可言。eq. 6.25 第一種情況要求 e′ = e + 1，這裡不成立，即使 m = 595 ≥ Y 且 |γ_A| = E 也走 F(η′_2, κ′) = F(η_1, γ_P)（vector skip_epochs-1：「Progress skipping epochs with a full tickets accumulator. Tickets mark is not generated. Accumulated tickets discarded. Fallback method enacted」）。eq. 6.28：e′ > e → H_E = (η_0, η_1, γ′_P 的 (k_b, k_e))；eq. 6.29：H_W 需要 e′ = e → ∅。eq. 6.35：e′ > e → γ′_A 只由本塊的 n 組成（m′ = 10 < Y，可帶最多 K 張、對新的 γ′_Z 與 context X_T ⌢ η_1 驗）。本塊 seal 走 fallback：H_A = γ′_S[10]、context X_F ⌢ η′_3 = X_F ⌢ η_2、T = 0。你們的程式：UpdateEntropy/KeyRotate 用 `ePrime > e`，UpdateSlotKeySequence 用 `ePrime == e+1`，正好對應。

**逐項辨析**

1. ❌ Rotation is applied once per elapsed epoch, i.e. twice here: κ′ = Φ(ι), λ′ = γ_P, (η′_1, η′_2, η′_3) = (η′_0, η_0, η_1); γ′_S = F(η_0, Φ(ι)); H_E is present and H_W absent  
   6.14 與 6.24 的條件是布林的 e′ > e，與 e′ − e 差幾格無關，state 只記得最後一塊。
2. ✅ Rotation happens exactly once: κ′ = γ_P, λ′ = κ, γ′_P = Φ(ι), (η′_1, η′_2, η′_3) = (η_0, η_1, η_2); γ′_S = F(η_1, γ_P) despite m = 595 ≥ Y and a full γ_A; H_E is present, H_W absent; γ′_A is rebuilt from this block's E_T alone  
   一次 rotation 加 fallback：e′ = e + 2 讓 6.25 第一種情況失效，H_W 需要 e′ = e 也不會出現。
3. ❌ γ′_S = Z(γ_A) because the prior block sat in the tail (m = 595 ≥ Y) with a saturated accumulator; keys and entropy rotate once; H_W = Z(γ_A) is emitted as well since this block closes the contest  
   6.25 的第一種情況要求 e′ = e + 1（精確相等）；本題 e′ = e + 2，累積的 ticket 一律作廢。
4. ❌ The block is rejected with BadSlot: eq. 6.25 has no case for e′ = e + 2, so a chain may skip individual slots but never an entire epoch; the next valid H_T is therefore some slot of epoch 2, and only from there can epoch 3 be reached  
   BadSlot 只在 τ′ ≤ τ 時觸發；跳過任意多個 slot 甚至整個 epoch 都是合法的。

> **陷阱**　跳 N 個 epoch ≠ rotate N 次；6.25 要的是 e′ = e + 1（精確），6.14/6.24 要的是 e′ > e（任意）。

<sub>`ch06-skip-epochs-transition`</sub>

---

### 6-6　Tiny parameters (E = 12). During epoch e the delegator service set the staging set ι to 9 validators while γ_P and κ still hold 6 — a legal 0.8.0 configuration (eq. 6.8). Consider the first block of epoch e + 1. Which statement is correct?
...
1. ❌ n = ⌈24/6⌉ = 4, because n is computed from the active set κ′ that seals this epoch; γ′_Z stays a 6-key ring and H_E lists 6 pairs until the 9 validators actually become active in epoch e + 2  
   分母是 γ′_P 不是 κ′；γ′_Z 與 H_E 在這一塊就已換成 9 人（eq. 6.14 的 z 對 γ′_P 取）。
2. ❌ n = ⌊24/9⌋ = 2, and the seal must now come from one of the 9 incoming validators (H_I < 9), because γ′_Z commits to their keys from this block on and H_E lists their 9 pairs  
...
<sub>`ch06-variable-set-boundary`</sub>

---

### 6-7　On an epoch change, one validator in ι has its Ed25519 key in ψ′_O, so Φ replaces its entire 336-octet entry with zeros. How does this affect γ′_Z and ticket verification in the new epoch?

<sub>6.3 Key Rotation / Appendix G — ●●● · 概念 · eq. 6.14–6.15, G.3 (ring root), App. G padding-point note</sub>

**標準答案**　The ring keeps |γ′_P| members: a zero key decodes to no Bandersnatch point, so the padding point is substituted at that position in z = O([k_b | k ∈ γ′_P]); the offender cannot produce a valid ring proof, everyone else's tickets verify against this γ′_Z all epoch

eq. 6.15：Φ 把 k_e ∈ ψ′_O 的整組 key 換成 [0, 0, …]；eq. 6.14：z = O([k_b | k ∈ γ′_P]) 仍對 **全部** |γ′_P| 個位置取 commitment。App. G 明文：「in the case a key has no corresponding Bandersnatch point when constructing the ring, then the Bandersnatch padding point … should be substituted」。w3f vector enact-epoch-change-with-padding-1 就是測這個：一把 key 因在 posterior offenders 而歸零、另一把本來就 undecodable，「Both the invalid keys are replaced with the padding point during ring commitment computation」。offender 沒有 padding point 的私鑰，做不出合法 ring proof，整個 epoch 都無法投 ticket。注意 Φ 只作用在進入 γ′_P 的 ι——已在 κ′ 裡的 offender 本 epoch 的 seal 仍照常驗（§11 eq. 11.22 的 guarantor 分配才另外套 Φ(κ′)）；而歸零的 entry 下個 epoch 進到 κ 後，fallback F 若抽到它，那個 slot 就沒人能出塊。工程面：γ′_Z ∈ B_144（G.3）是 key 序列的純函數，只在 e′ > e 時重算（KeyRotate 否則直接沿用 prior γ_Z）；你們的 GetVerifier 以 Blake2b(所有 Bandersnatch key 串接) 當快取鍵（#1041），取代原本按 epoch 快取、fork-restore 就得重建的做法（#1040）。

**逐項辨析**

1. ❌ The offender's slot is removed from the ring, so γ′_Z commits to |γ′_P| − 1 keys and everyone's ring position shifts by one; this is why the accumulator has to be re-sorted at the epoch boundary and why H_E lists the surviving keys only  
   ring 的大小與每個人的位置都不變：eq. 6.14 的 z 仍對全部 |γ′_P| 個位置取 commitment。
2. ✅ The ring keeps |γ′_P| members: a zero key decodes to no Bandersnatch point, so the padding point is substituted at that position in z = O([k_b | k ∈ γ′_P]); the offender cannot produce a valid ring proof, everyone else's tickets verify against this γ′_Z all epoch  
   App. G 明文：無對應 Bandersnatch point 時該位置改用 padding point，ring 大小維持不變。
3. ❌ The all-zero key is a valid Bandersnatch point (the identity), so γ′_Z is computed normally and the offender can keep submitting tickets until the following epoch removes it; only its Ed25519-signed artefacts are refused meanwhile  
   全零 32 bytes 不是合法點：Twisted Edwards 的 identity 是 (0, 1)，壓縮編碼不是全零。
4. ❌ O(…) fails on an undecodable key, so a block that would place a nulled key into γ′_P is invalid; offenders therefore have to be removed from ι by the delegator service (designate) before the boundary, shrinking the set by 3 each time  
   vector enact-epoch-change-with-padding-1 正是把兩把無效 key 換成 padding point，而非拒絕區塊。

> **陷阱**　零 key ≠ 移除：ring 成員數不變、位置不變，只是那格換成 padding point。

<sub>`ch06-ring-root-nulled-keys`</sub>

---

### 6-8　A ticket proof is a ring-VRF proof with context X_T ⌢ η′_2 ++ r over the EMPTY message, while the seal H_S is a plain Bandersnatch signature with context X_T ⌢ η′_3 ++ i_e over E_U(H), the whole unsigned header. Why can eq. 6.16 nevertheless demand i_y = Y(H_S), i.e. that the seal's VRF output equal the ticket id?

<sub>6.4 Sealing and Entropy Accumulation — ●●● · 設計理由 · eq. 6.16, 6.24, 6.30, 6.32; §3.8.2 Signing Schemes; App. G eq. G.2/G.5</sub>

**標準答案**　Because a VRF output depends only on the secret key and the context ('influenced by x but not by m'), and by seal time the submission entropy has rotated from η′_2 into η′_3 so the contexts coincide: the same key reproduces the ticket id under any message — anonymous in the ring proof, attributable in the seal

§3.8.2：Bandersnatch signature 與 RingVRF proof「both define a VRF output, a high entropy hash influenced by x but not by m」，且「the member is identified in the former and is anonymous in the latter」；App. G（G.2/G.5）：Y(·) 取 output 的前 32 bytes。eq. 6.32：ticket id y = Y(p)；eq. 6.30 的 context 是 X_T ⌢ η′_2 ++ r（提交當時的 η′_2）；eq. 6.24 在下個 epoch 邊界把它 rotate 成 η′_3，所以 eq. 6.16 seal 的 context X_T ⌢ η′_3 ++ i_e 與當初提票的 context 逐 byte 相同。同一把 secret key 加上相同 context ⇒ 相同 VRF output，無論訊息是 [] 還是 E_U(H)，因此驗證者比對 i_y = Y(H_S) 就能確定「出塊者就是持票人」——提票時 ring proof 只證明「γ′_P 某成員」，出塊時 IETF 簽章（H_A 已知）揭露身分；這就是 Safrole「事前匿名、事後可歸屬」的核心。§6.7 也說 ticket id「is used both as a score in the contest and as input for the block's entropy source VRF signature」（即 H_V 的 context X_E ⌢ Y(H_S)）。你們 ValidateByTickets 先比 vrf.VRFIetfOutput(header.Seal) == ticket.ID，再 IETFVerify(context, E_U(H))。

**逐項辨析**

1. ✅ Because a VRF output depends only on the secret key and the context ('influenced by x but not by m'), and by seal time the submission entropy has rotated from η′_2 into η′_3 so the contexts coincide: the same key reproduces the ticket id under any message — anonymous in the ring proof, attributable in the seal  
   VRF output 只看 (key, context)；6.24 的 rotate 讓提票時的 η′_2 到封章時正好變成 η′_3。
2. ❌ Because the ticket id is defined as the Blake2b hash of the proof bytes, and eq. 6.16 requires the seal to embed a copy of the original 784-byte ring proof so that every verifier can recompute that hash from the header alone; the E_U(H) message merely binds the embedded copy to this particular block  
   6.32 取的是 VRF **輸出**而非 proof bytes 的 hash；seal 是 96-byte 簽章，不含 784-byte ring proof。
3. ❌ Because both the ticket proof and the seal are in fact signed over the empty message — E_U(H) is only ever the message of H_V, whose output feeds η′_0 — so the two VRF outputs agree trivially; the identity check i_y = Y(H_S) is accordingly made against H_V rather than against the seal  
   seal 的訊息就是 E_U(H)（這才把它綁到 header）；i_y 比對的對象是 H_S 而不是 H_V。
4. ❌ Because η′_2 = η′_3 whenever the tickets were submitted in the same epoch as the block being sealed, so the author simply re-signs an identical context; the message is ignored by the verifier for seals though not for ring proofs, which is why tickets carry [] and seals carry E_U(H)  
   η′_2 與 η′_3 差一個 epoch；訊息也仍然參與簽章驗證，只是不影響 VRF output。

> **陷阱**　VRF output 只看 (key, context)，不看 message——這是 ticket 能匿名提交、之後又能對上 seal 的原因。

<sub>`ch06-vrf-output-message-independence`</sub>

---

### 6-9　Eq. 6.25 uses the accumulator as the next slot-sealer sequence only when e′ = e + 1 ∧ m ≥ Y ∧ |γ_A| = E. What does the conjunct m ≥ Y — a property of the PRIOR block — actually guarantee, and why does the GP tie the ticket regime to it?

<sub>6.5 The Slot-Sealer Sequence — ●●● · 設計理由 · eq. 6.25, 6.29, 6.31, 6.35; §6.6–6.7 prose</sub>

**標準答案**　That some block of the ending epoch was authored in the tail, which — γ_A being frozen once m′ ≥ Y — is exactly the condition under which H_W = Z(γ_A) was published in that epoch, so a header-only node is never asked to verify ticket seals for a sequence it never saw

eq. 6.31：m′ ≥ Y 時 |E_T| = 0，§6.7 末尾更指出此時 γ′_A = γ_A——進入 tail 後 accumulator 凍結。eq. 6.29：同一 epoch 內第一個跨過 Y 的區塊（m < Y ≤ m′）若 |γ_A| = E 就發佈 H_W = Z(γ_A)。把兩者合起來：「前一塊在 tail（m ≥ Y）且 |γ_A| = E」⟺「本 epoch 已有某塊發佈了 H_W，且內容恰是 Z(γ_A)」（tail 內任何一塊都是或晚於那個第一塊，而 γ_A 從那之後不再變動）。§6.6 說 marker 的目的就是讓「nodes which do not synchronize the entire state」能「using only the chain of headers」追蹤 sealer/validator 的變化，§6.7 也說 Y 之後「the following epoch's slot-sealer sequence becomes fixed」。若沒有任何區塊落在 tail（前一塊 m < Y 直接跳到下個 epoch），H_W 從未出現，header-only 節點無從得知 ticket 序列，所以 GP 寧可走 fallback（vector skip_epoch_tail-1：「Tickets mark has no chance to be generated. Accumulated tickets discarded. Fallback method enacted」）。你們的 #284 就是最初只判 ePrime == e+1，會在沒有 H_W 的情況下啟用 ticket（也會在跳過 epoch 時用錯 accumulator）。

**逐項辨析**

1. ❌ That at least Y tickets were submitted during the epoch, so the contest was competitive enough for the accumulator to be trusted over the fallback keys; with fewer than Y entries the outside-in ordering Z would leave slots unassigned  
   Y = 500 是 slot phase，與「提交了幾張 ticket」無關；飽和由 |γ_A| = E 這個 conjunct 負責。
2. ❌ That no ticket submitted inside the tail is counted: tickets arriving at m ≥ Y would otherwise have been proven against the wrong ring root and entropy, since γ′_Z and η′_2 already refer to the epoch after next by then  
   tail 內本來就不可能有 ticket（6.31 要求 |E_T| = 0），沒有「算錯 ring root」的餘地。
3. ✅ That some block of the ending epoch was authored in the tail, which — γ_A being frozen once m′ ≥ Y — is exactly the condition under which H_W = Z(γ_A) was published in that epoch, so a header-only node is never asked to verify ticket seals for a sequence it never saw  
   m ≥ Y 等價於「本 epoch 已有某塊發佈過 H_W」，header-only 節點才不會驗到沒見過的序列。
4. ❌ That the prior block's author held a ticket (T = 1), since a fallback-sealed block is not allowed to close a contest on behalf of the ticket holders; this is also what the best-chain rule of §19 counts when it prefers ticketed ancestors  
   T 旗標不是 6.25 的輸入，它只用在 §19 best-chain 對 ticket-sealed 祖先的偏好上。

> **陷阱**　m ≥ Y 不是「比賽夠久」，而是「H_W 一定已經在 header 鏈上」——三個 conjunct 各管一件事：e′ = e+1（是下一個 epoch）、m ≥ Y（H_W 已發佈）、|γ_A| = E（飽和）。

<sub>`ch06-why-m-ge-Y`</sub>

---

### 6-10　Each validator key K bundles a Bandersnatch, an Ed25519 and a BLS public key. Which mapping of key → protocol use is correct, and why has the epoch marker H_E carried the Ed25519 keys alongside the Bandersnatch keys since GP 0.6.4?
...
1. ❌ Bandersnatch: seals, H_V and BEEFY; Ed25519: ticket ring proofs, guarantees, assurances and audit announcements; BLS: judgments, culprits, faults and offender identity (ψ_O, H_O, Φ). H_E carries Ed25519 keys because the ring root γ′_Z is a commitment to the Ed25519 keys of γ′_P, which header-only nodes must be able to rebuild for themselves  
   ticket 是 Bandersnatch ring proof、BEEFY 用 BLS；ψ_O 是 Ed25519 key 集合，γ′_Z 也是對 k_b 取的。
2. ✅ Bandersnatch: ticket ring proofs, H_S, H_V, audit-selection VRFs; Ed25519: guarantees, assurances, judgments/culprits/faults, audit announcements, offender identity (ψ_O, H_O, Φ); BLS: BEEFY only. H_E carries Ed25519 keys so a header-only node can attribute every Ed25519-signed artefact and every H_O entry to a validator  
   三把 key 的分工與 H_E 的動機都對：header-only 節點要能把 Ed25519 簽章與 H_O 對應到 validator。
3. ❌ Bandersnatch: ticket ring proofs, seals and H_V; Ed25519: everything else, including BEEFY signatures and audit announcements; BLS is reserved metadata with no protocol use yet, exactly like k_m. H_E carries Ed25519 keys so that GRANDPA voters can be weighted by stake using nothing but the header chain  
   BEEFY（18.1）用 κ′[v]_bls，BLS 並非未用的保留欄位；GP 也沒有「以 stake 加權 GRANDPA」。
4. ❌ Bandersnatch: ticket ring proofs, seals, H_V and audit-selection VRFs; Ed25519: guarantees and assurances only; BLS: judgments, culprits, faults and BEEFY. H_E carries Ed25519 keys because Φ needs them to null the offenders inside κ′ at the epoch boundary, and κ′ is not otherwise reconstructible from headers  
   judgments/culprits/faults 都直接帶 Ed25519 key 與簽章；Φ 作用在 ι → γ′_P、讀的是 state 而非 header。

> **陷阱**　口訣：Bandersnatch = Safrole（票、seal、entropy、audit 抽籤）；Ed25519 = 其他所有簽章與 offender 身分；BLS = 只有 BEEFY。

<sub>`ch06-three-keys-epoch-marker`</sub>

---

### 6-11　The Safrole state γ ≡ (γ_P, γ_Z, γ_S, γ_A). Which description is correct?

<sub>6.2 Safrole Basic State — ●○○ · 概念 · eq. 6.3–6.6</sub>

**標準答案**　γ_P pending validator keys for the next epoch; γ_Z the Bandersnatch ring root over γ_P; γ_S the current epoch's slot-sealer sequence (E tickets or E keys); γ_A the ticket accumulator (≤ E tickets) for the next epoch

GP eq. 6.3–6.5：γ_P（pending set，每個 epoch 開頭從 ι 重置，決定下一個 epoch 的 ring root）、γ_Z ∈ ring root（由 γ_P 的 Bandersnatch key k_b 組成，eq. 6.14 的 z = O([k_b | k ∈ γ′_P])）、γ_S ∈ [C]_E ∪ [H_B]_E（本 epoch 的 slot-sealer 序列：600 張 ticket 或 fallback 的 600 把 Bandersnatch key）、γ_A ∈ [C]_{:E}（下一個 epoch 用的 ticket accumulator，最多 E 筆）。Safrole 只透過 ι、κ、τ、η 與協定其他部分互動。最容易混的是 γ_S 與 γ_A 的時間方向：γ_S 是**本** epoch 已定案、每個 slot 對應一筆的 sealer 序列，γ_A 則是本 epoch 期間持續累積、供**下**個 epoch 用的 ticket 池。

**逐項辨析**

1. ✅ γ_P pending validator keys for the next epoch; γ_Z the Bandersnatch ring root over γ_P; γ_S the current epoch's slot-sealer sequence (E tickets or E keys); γ_A the ticket accumulator (≤ E tickets) for the next epoch  
   四項全對上 eq. 6.3–6.5：pending set、對 γ_P 取的 ring root、本 epoch sealer、下個 epoch 的票池。
2. ❌ γ_P the previous epoch's validator keys (the λ set); γ_Z the Bandersnatch ring root over the active set κ; γ_S the current epoch's slot-sealer sequence (E tickets or E keys); γ_A the ticket accumulator (≤ E tickets) for the next epoch  
   γ_P 指向未來不是過去（λ 是 γ 之外的獨立狀態），且 eq. 6.14 的 ring root 是對 γ′_P 取的。
3. ❌ γ_P pending validator keys for the next epoch; γ_Z an Ed25519 ring root over γ_P; γ_S the ticket accumulator (≤ E tickets) for the next epoch; γ_A the current epoch's slot-sealer sequence (E tickets or E keys)  
   ring root 只由 Bandersnatch key k_b 構成；γ_S 與 γ_A 兩者也被對調了。
4. ❌ γ_P the ticket accumulator (≤ E tickets) for the next epoch; γ_Z the Bandersnatch ring root over γ_P; γ_S the current epoch's slot-sealer sequence (E tickets or E keys); γ_A the pending validator keys, reset to ι at the start of each epoch  
   違反 6.5/6.6 的型別：accumulator 是 ticket 序列（≤ E 筆），pending set 是 validator key 序列。

> **陷阱**　γ_S 有兩種型態（tickets 或 keys），serialization 時用 0/1 discriminator（見 D.1 C(4)）。

<sub>`ch06-gamma-components`</sub>

---

### 6-12　On an epoch change (e′ > e), how are the validator key sets rotated per eq. 6.14?

<sub>6.3 Key Rotation — ●●○ · 概念 · eq. 6.14–6.15</sub>

**標準答案**　(γ′_P, κ′, λ′, γ′_Z) = (Φ(ι), γ_P, κ, z) where z is the ring root over the Bandersnatch keys of γ′_P and Φ nulls the entire key tuple of any validator whose Ed25519 key is in ψ′_O

eq. 6.14：e′ > e 時 (γ′_P, κ′, λ′, γ′_Z) = (Φ(ι), γ_P, κ, z)，其中 z = O([k_b | k ∈ γ′_P])（ring root），否則四者不變。eq. 6.15：Φ(k) 把 k_e ∈ ψ′_O（posterior offenders，因為 disputes 已先處理）的 validator 整組換成全零 key。所以 staging set ι 要等**兩個** epoch 邊界才會成為 active（ι → γ_P → κ）。你們的 KeyRotate() 對應此式；ReplaceOffenderKeys 用的是 posterior ψ_O。

**逐項辨析**

1. ✅ (γ′_P, κ′, λ′, γ′_Z) = (Φ(ι), γ_P, κ, z) where z is the ring root over the Bandersnatch keys of γ′_P and Φ nulls the entire key tuple of any validator whose Ed25519 key is in ψ′_O  
   eq. 6.14 與 6.15 完全對上：來源是 staging set ι，ring root 對 γ′_P 取，Φ 用 posterior ψ′_O。
2. ❌ (γ′_P, κ′, λ′, γ′_Z) = (ι, γ_P, κ, z) where z is the ring root over γ′_P's Bandersnatch keys and no offender filtering is applied — offenders are skipped later by the guarantor assignment  
   eq. 6.15 的 Φ 在輪換當下就把 offender 整把 key 歸零；§11 的 P(|κ′|, η′_2, τ′) 沒有排除機制。
3. ❌ (γ′_P, κ′, λ′, γ′_Z) = (Φ(ι), ι, κ, z) where z is the ring root over the Bandersnatch keys of κ′ and Φ nulls the keys of offenders in the prior ψ_O rather than the posterior set  
   錯兩處：ring root 必須對 γ′_P（下個 epoch 的提票 ring）取，且 disputes 先跑，用 posterior ψ′_O。
4. ❌ (γ′_P, κ′, λ′, γ′_Z) = (Φ(γ_P), κ, λ, γ_Z) where the ring root is only recomputed when ι changes and Φ nulls the keys of offenders in ψ′_O — the sequences advance one step every block  
   輪換只在 e′ > e 發生，且新 pending set 的來源是 ι 而非 γ_P 自身，否則 ι 永遠進不了 κ。

> **陷阱**　常考：ι 什麼時候變成 κ？答：下下個 epoch（先進 γ_P，再進 κ）。

<sub>`ch06-key-rotation`</sub>
...
### 6-14　A validator key K is a 336-octet sequence. What is the layout?

<sub>6.3 Key Rotation — ●○○ · 概念 · eq. 6.9–6.13</sub>

**標準答案**　Bandersnatch 32 | Ed25519 32 | BLS 144 | metadata 128

eq. 6.9–6.13：k_b = k[0..32)（Bandersnatch）、k_e = k[32..64)（Ed25519）、k_l = k[64..208)（BLS，144 octets）、k_m = k[208..336)（metadata，128 octets，opaque，用來放例如網路位址等實務識別資訊）。32+32+144+128 = 336。

**逐項辨析**

1. ✅ Bandersnatch 32 | Ed25519 32 | BLS 144 | metadata 128  
   eq. 6.9–6.13 的順序與長度：32 + 32 + 144 + 128 = 336。
2. ❌ Ed25519 32 | Bandersnatch 32 | BLS 96 | metadata 176  
   前兩把的順序對調了，且 BLS 是 144 octets 不是 96（metadata 固定 128）。
3. ❌ Bandersnatch 32 | BLS 144 | Ed25519 32 | metadata 128  
   Ed25519 必須緊接 Bandersnatch 落在 offset 32，BLS 要到 offset 64 才開始。
4. ❌ Bandersnatch 33 | Ed25519 32 | BLS 143 | metadata 128  
   Bandersnatch 公鑰是 32 octets、BLS 是 144；總長雖仍為 336，每個切點卻都偏了一格。

> **陷阱**　BLS 144 octets（BLS12-381 上的 key 組合）。metadata 不參與任何密碼學運算。

<sub>`ch06-validator-key-layout`</sub>

---

### 6-15　How is the entropy accumulator η updated in each block, and on an epoch change?

<sub>6.4 Sealing and Entropy Accumulation — ●●○ · 概念 · eq. 6.22–6.24</sub>

**標準答案**　η′_0 = H(η_0 ⌢ Y(H_V)) every block, folding in the entropy VRF's output; on e′ > e additionally (η′_1, η′_2, η′_3) = (η_0, η_1, η_2), otherwise (η_1, η_2, η_3) unchanged

eq. 6.23：η′_0 ≡ H(η_0 ⌢ Y(H_V))——用 **prior** η_0 與 H_V 的 VRF **輸出** Y(·)（App. G 定義為 output 的前 32 bytes）做 Blake2b。eq. 6.24：e′ > e 時 (η′_1, η′_2, η′_3) = (η_0, η_1, η_2)，注意推入的是 prior η_0，否則三者不變。H_V 的簽章 context 是 X_E ⌢ Y(H_S)（eq. 6.18），也就是 seal 的 VRF 輸出，因此 entropy 是「先固定訊息、再產生」的 bias-resistant 來源；Y(H_S) 本身只拿來當 H_V 的 context 並與 ticket id 比對，不會直接進 η。你們 UpdateEtaPrime0() / UpdateEntropy() 分別對應 6.23 / 6.24。

**逐項辨析**

1. ✅ η′_0 = H(η_0 ⌢ Y(H_V)) every block, folding in the entropy VRF's output; on e′ > e additionally (η′_1, η′_2, η′_3) = (η_0, η_1, η_2), otherwise (η_1, η_2, η_3) unchanged  
   eq. 6.23 取的是 Y(H_V) 這個 VRF 輸出，eq. 6.24 的 rotate 只在 e′ > e 且推入 prior η_0。
2. ❌ η′_0 = H(η_0 ⌢ H_V) every block, hashing the whole VRF signature rather than its 32-byte output; on e′ > e additionally (η′_1, η′_2, η′_3) = (η_0, η_1, η_2), otherwise unchanged  
   6.23 餵進 hash 的是 VRF **輸出** Y(H_V)；簽章 bytes 本身不是那個 unbiasable 隨機值。
3. ❌ η′_0 = H(η_0 ⌢ Y(H_S)) every block, folding in the seal's VRF output; on e′ > e additionally (η′_1, η′_2, η′_3) = (η′_0, η_1, η_2), pushing the posterior accumulator into the history  
   累積進 η_0 的是 H_V 的輸出；rotate 推入 posterior η′_0 更會把本塊 entropy 混進 η′_1。
4. ❌ η′_0 = H(η_0 ⌢ Y(H_V)) every block; the history rotates every block as well, so (η′_1, η′_2, η′_3) = (η_0, η_1, η_2) holds unconditionally and η_3 is always three blocks old  
   違反 6.24 的 case 條件：每塊都推的話，η_2 就不再是上上個 epoch 結束時的累積值。

> **陷阱**　H_V 的輸出餵 η′_0；H_S 的輸出（Y(H_S)）只作為 H_V 的訊息與 ticket id 比對。

<sub>`ch06-entropy-update`</sub>

---

### 6-16　Each of η′_2 and η′_3 has specific uses in GP 0.8.0. Which assignment is correct?

<sub>6.4 Sealing and Entropy Accumulation — ●●● · 概念 · eq. 6.16–6.18, 6.25, 6.30, 11.22</sub>

**標準答案**　η′_2: ticket ring-proof context (X_T ⌢ η′_2 ++ r), fallback key sequence F(η′_2, κ′), guarantor assignment shuffle; η′_3: verifying the seal signature context (X_T ⌢ η′_3 ++ i_e or X_F ⌢ η′_3)

tickets 在 epoch N 提交時用 η′_2（eq. 6.30 的 ring proof context X_T ⌢ η′_2 ++ r）；到 epoch N+1 驗 seal 時，那個值已經被 rotate 成 η′_3（eq. 6.16：X_T ⌢ η′_3 ++ i_e；6.17：X_F ⌢ η′_3），這就是 §6.4 說的「The oldest is used to regenerate this randomness when verifying the seal」。fallback F(η′_2, κ′)（6.25）與 guarantor 分配 P(|κ′|, η′_2, τ′)（11.22）也都用 η′_2；§11.3 解釋不用 η_1 是為了避免 epoch 末尾狀態不確定造成 fork magnification。epoch marker H_E 帶的則是 prior (η_0, η_1)，rotate 後即 η′_1、η′_2。

**逐項辨析**

1. ✅ η′_2: ticket ring-proof context (X_T ⌢ η′_2 ++ r), fallback key sequence F(η′_2, κ′), guarantor assignment shuffle; η′_3: verifying the seal signature context (X_T ⌢ η′_3 ++ i_e or X_F ⌢ η′_3)  
   提票、fallback 與 guarantor 分配都用 η′_2，驗 seal 用晚一個 epoch 的 η′_3，正是 §6.4 的說法。
2. ❌ η′_2: verifying the seal signature context (X_T ⌢ η′_2 ++ i_e or X_F ⌢ η′_2); η′_3: ticket ring-proof context (X_T ⌢ η′_3 ++ r), fallback key sequence F(η′_3, κ′), guarantor assignment shuffle  
   方向剛好相反：提票在前用較新的那格，驗 seal 在後才用已 rotate 的最舊那格。
3. ❌ η′_1: ticket ring-proof context (X_T ⌢ η′_1 ++ r) and fallback key sequence F(η′_1, κ′); η′_2: verifying the seal signature context (X_T ⌢ η′_2 ++ i_e); η′_3: the guarantor assignment shuffle P(|κ′|, η′_3, τ′)  
   三個用途整組往前挪一格；eq. 11.22 明寫 G ≡ (P(|κ′|, η′_2, τ′), Φ(κ′))，用的是 η′_2。
4. ❌ η′_0: ticket ring-proof context (X_T ⌢ η′_0 ++ r) and the guarantor assignment shuffle; η′_1: fallback key sequence F(η′_1, κ′) and seal verification; η′_2 and η′_3 are kept solely to populate the epoch marker H_E  
   η′_0 每塊都被 6.23 改寫，拿它當 ring proof context 會使同 epoch 內每塊 context 都不同。

> **陷阱**　口訣：提 ticket 用 η2，驗 seal 用 η3（同一個值晚一個 epoch）。

<sub>`ch06-which-eta-where`</sub>

---

### 6-17　When γ′_S is a sequence of tickets, the seal H_S must satisfy three conditions (eq. 6.16), with i = γ′_S[H_T mod E]. Which set is exactly right?

<sub>6.4 Sealing and Entropy Accumulation — ●●● · 概念 · eq. 6.16 (ticket seal)</sub>

**標準答案**　i_y = Y(H_S); H_S is a Bandersnatch signature by H_A over context X_T ⌢ η′_3 ++ i_e with message E_U(H) (the unsigned header); and the block is marked T = 1 (ticketed)

eq. 6.16：γ′_S ∈ [C] ⇒ { i_y = Y(H_S)（seal 的 VRF 輸出必須等於該 slot ticket 的 id，證明出塊者就是持票人）, H_S ∈ F^{X_T ⌢ η′_3 ++ i_e}_{H_A}(E_U(H)), T = 1 }。seal 是一般（IETF）Bandersnatch VRF 簽章，不是 ring VRF——ring 只在提交 ticket 時用；訊息則是省略 seal 欄位的 E_U(H)，否則簽章會簽進自己。你們的 sealing.go ValidateByBandersnatchs 就是檢查 Y(H_S) == ticket id。

**逐項辨析**

1. ✅ i_y = Y(H_S); H_S is a Bandersnatch signature by H_A over context X_T ⌢ η′_3 ++ i_e with message E_U(H) (the unsigned header); and the block is marked T = 1 (ticketed)  
   三個條件與 eq. 6.16 一字不差：Y(H_S) 對上該 slot 的 ticket id、η′_3 context、T = 1。
2. ❌ i_y = H(H_S), the Blake2b hash of the seal bytes; H_S is a ring-VRF proof against γ′_Z over context X_T ⌢ η′_3 ++ i_e with message E(H) (the full header); and the block is marked T = 1 (ticketed)  
   6.32 的 ticket id 是 VRF 輸出而非簽章 hash；訊息取完整 E(H) 更是循環定義（header 含 H_S）。
3. ❌ i_y = Y(H_S); H_S is a Bandersnatch signature by H_A over context X_T ⌢ η′_2 ++ i_e with message E_U(H) (the unsigned header); and the block is marked T = 1 (ticketed)  
   η′_2 是提交 ticket 時的 context；驗 seal 一律用已 rotate 的 η′_3。
4. ❌ i = H_A (the sealer entry is the author's own Bandersnatch key); H_S is a Bandersnatch signature by H_A over context X_F ⌢ η′_3 with message E_U(H) (the unsigned header); and the block is marked T = 0  
   那是 eq. 6.17 的 fallback 情況：i = H_A、X_F context、T = 0。

> **陷阱**　E_U(H) = 不含 H_S 的 header 序列化；H_V 則是 context X_E ⌢ Y(H_S)、訊息為空 []。

<sub>`ch06-seal-ticket-condition`</sub>

---

### 6-18　The posterior slot-sealer sequence γ′_S has three cases. A block arrives with e′ = e + 1, the previous block was at slot phase m = 480 (< Y = 500) and γ_A holds 600 tickets. What is γ′_S?
...
eq. 6.25：γ′_S ≡ Z(γ_A) 當 e′ = e+1 ∧ m ≥ Y ∧ |γ_A| = E；γ_S 當 e′ = e；否則 F(η′_2, κ′)。三個條件缺一不可，而 m 是 **prior** block 的 slot phase（來自 τ），這個門檻是要確認「比賽在上一塊時就已結束」。同理若跳過整個 epoch（e′ ≥ e+2）也走 fallback。fallback 一律以 posterior 的 η′_2 與 κ′ 為種子。你們 UpdateSlotKeySequence() 的三段 if 完全對應。
...
### 6-19　With a tiny epoch of E = 6 and a sorted accumulator γ_A = [a, b, c, d, e, f] (ascending ticket ids), what is Z(γ_A)?

<sub>6.5 The Slot-Sealer Sequence — ●○○ · 計算 · eq. 6.26 (Z)</sub>
...
### 6-20　The fallback key sequence F(r, k) selects, for each slot index i ∈ N_E, the Bandersnatch key k[…]_b of which validator?

<sub>6.5 The Slot-Sealer Sequence — ●●○ · 概念 · eq. 6.27 (F)</sub>

**標準答案**　k[ decode_4( H(r ⌢ E_4(i))[0..4] ) mod |k| ] — Blake2b of the entropy concatenated with the 4-byte LE slot index, first 4 bytes decoded, modulo the validator count

eq. 6.27：F(r, k) = [ k[decode_4(H(r ⌢ E_4(i))_{…4})]_b | i ∈ N_E ] 並對 |k| 取循環索引（cyclic ⟲）。H 是 Blake2b（GP §3.8 的預設 hash），E_4(i) 是 4-byte little-endian，序列記法 s_{…4} 表示取前 4 個 octet。F 是逐 slot 獨立抽樣，同一個 validator 可能被抽中多次、也可能整個 epoch 都沒被抽到——這正是 fallback 想保留的不可預測性。你們 FallbackKeySequence() 用 Blake2bHashPartial(…, 4) 再 %ValidatorsCount（註解裡殘留的「Keccak256」字樣是 stale comment——面試時別被自己的註解誤導）。

**逐項辨析**

1. ✅ k[ decode_4( H(r ⌢ E_4(i))[0..4] ) mod |k| ] — Blake2b of the entropy concatenated with the 4-byte LE slot index, first 4 bytes decoded, modulo the validator count  
   eq. 6.27 的 F(r, k) = [k[decode_4(H(r ⌢ E_4(i))_{…4})]_b | i ∈ N_E] 逐項對上。
2. ❌ k[ i mod |k| ] — a plain round-robin over the key sequence, with the entropy r used only to choose the sequence's starting offset once per epoch rather than per slot  
   F 是逐 slot 獨立抽樣；round-robin 會讓誰在哪個 slot 出塊完全可預測。
3. ❌ k[ decode_4( H_K(r ⌢ E_4(i))[0..4] ) mod |k| ] — Keccak-256 of the entropy concatenated with the 4-byte LE slot index, first 4 bytes decoded, modulo the validator count  
   H_K（Keccak）在 GP 只用於 accumulation-output belt 與 BEEFY；§3.8 的 H 是 Blake2b-256。
4. ❌ k[ decode_8( H(r ⌢ E_8(i))[0..8] ) mod |k| ] — Blake2b of the entropy concatenated with the 8-byte LE slot index, first 8 bytes decoded, modulo the validator count  
   E_4(i) 是 4-byte little-endian，且取的是 hash 的**前 4** octets（s_{…4} 即前 4 個元素）。

> **陷阱**　fallback 下 seal 用 X_F context，且 H_A 必須等於 γ′_S[H_T mod E]（eq. 6.17：i = H_A）。

<sub>`ch06-fallback-F`</sub>

---

### 6-21　What exactly does the epoch marker H_E contain in the first block of a new epoch (e′ > e)?

<sub>6.6 The Markers — ●●○ · 概念 · eq. 6.28</sub>

**標準答案**　(η_0, η_1, [(k_b, k_e) | k ∈ γ′_P]) — the prior η_0 and η_1 plus the Bandersnatch and Ed25519 keys of the pending validators γ′_P who take over in the next epoch

eq. 6.28：H_E ≡ (η_0, η_1, [(k_b, k_e) | k ∈ γ′_P]) 當 e′ > e，否則 ∅。η_0 與 η_1 是 **prior** 值（rotate 後就是 η′_1、η′_2，也就是下個 epoch 提 ticket 與 fallback 會用到的 entropy），加上 γ′_P（下一個 epoch 的 pending set，已經 Φ 過濾）的 Bandersnatch + Ed25519 key。目的（§6.6）：讓不同步完整 state 的節點（light client）只靠 header 鏈就能追蹤 validator 變化。你們 ValidateHeaderEpochMark（InvalidEpochMark = code 9）檢查此式。

**逐項辨析**

1. ✅ (η_0, η_1, [(k_b, k_e) | k ∈ γ′_P]) — the prior η_0 and η_1 plus the Bandersnatch and Ed25519 keys of the pending validators γ′_P who take over in the next epoch  
   eq. 6.28 一字不差：prior (η_0, η_1) 加上已 Φ 過濾的 γ′_P 的 Bandersnatch + Ed25519 key。
2. ❌ (η′_0, η′_1, [(k_b, k_e) | k ∈ κ′]) — the posterior η′_0 and η′_1 plus the Bandersnatch and Ed25519 keys of the validators that have just become active in this epoch  
   η′_0 已摻進本塊的 VRF 輸出；key 來源也錯，marker 預告的是下個 epoch 的 γ′_P 而非 κ′。
3. ❌ (η_2, η_3, [(k_b, k_e) | k ∈ ι]) — the two oldest historical entropies plus the Bandersnatch and Ed25519 keys of the staging set exactly as the designate host call left it  
   (η_2, η_3) 又更舊兩格；未經 Φ 過濾的 ι 會讓 light client 收到尚未歸零的 offender key。
4. ❌ (η_0, η_1, Z(γ_A)) — the prior η_0 and η_1 plus the outside-in ordered ticket identifiers, one per timeslot, that will seal each slot of the coming epoch  
   Z(γ_A) 是 winning-tickets marker H_W（6.29）的內容，與 H_E 是 header 裡互斥的兩個欄位。

> **陷阱**　H_E 放 γ′_P 不是 κ′；entropy 是 prior η_0/η_1。

<sub>`ch06-epoch-marker`</sub>

---

### 6-22　Under which exact condition is the winning-tickets marker H_W non-empty?

<sub>6.6 The Markers — ●●○ · 概念 · eq. 6.29</sub>

**標準答案**　e′ = e ∧ m < Y ≤ m′ ∧ |γ_A| = E — the first block of the same epoch whose slot phase crosses the tail start Y, with a saturated accumulator; then H_W = Z(γ_A)

eq. 6.29：H_W ≡ Z(γ_A) 當 e′ = e ∧ m < Y ≤ m′ ∧ |γ_A| = E，否則 ∅。即「同一 epoch 內，前一塊 phase 在 Y 之前、本塊 phase 在 Y 之後（含）」的那唯一一個區塊，且 accumulator 已飽和，就把下個 epoch 的最終 ticket 序列公告在 header；6.25 則在下個 epoch 的第一塊把同一份序列定案為 γ′_S。注意若 epoch 尾端沒有任何區塊跨越 Y（例如 m = 480 直接跳到下個 epoch），H_W 永遠不會出現。

**逐項辨析**

1. ✅ e′ = e ∧ m < Y ≤ m′ ∧ |γ_A| = E — the first block of the same epoch whose slot phase crosses the tail start Y, with a saturated accumulator; then H_W = Z(γ_A)  
   eq. 6.29 的三個合取項完全對上：同一 epoch 內跨越 Y 的那一塊，且 accumulator 已飽和。
2. ❌ e′ > e ∧ m ≥ Y ∧ |γ_A| = E — the first block of a new epoch, once the previous epoch's tail has been reached and the accumulator is saturated; then H_W = Z(γ_A)  
   那組條件描述的其實是 slot-sealer 序列的第一個 case；進入新 epoch 才公告已經太遲。
3. ❌ e′ = e ∧ Y ≤ m < m′ ∧ |γ_A| = E — every block of the epoch's tail after ticket submission has already closed, provided the accumulator is saturated; then H_W = Z(γ_A)  
   少了 m < Y 這個下界，整段 tail 每一塊都會重複公告同一份序列。
4. ❌ e′ = e ∧ m < Y ≤ m′ ∧ |γ_A| ≥ 1 — the first block of the same epoch whose slot phase crosses the tail start Y, with any non-empty accumulator; then H_W = Z(γ_A)  
   Z 的定義域是長度 E 的序列；accumulator 未滿時下個 epoch 直接走 fallback。

> **陷阱**　H_W 與 H_E 互斥：H_W 要 e′ = e，H_E 要 e′ > e。

<sub>`ch06-winning-tickets-marker`</sub>

---

### 6-23　Per GP 0.8.0, what bounds apply to the tickets extrinsic E_T?
...
> **陷阱**　條件是 m′ < Y（本塊的 phase），K = 16 是每個區塊的上限，不是每個 epoch。

<sub>`ch06-ticket-extrinsic-limits`</sub>

---

### 6-24　Which statement about the new tickets n and the posterior accumulator γ′_A is FALSE?

<sub>6.7 The Extrinsic and Tickets — ●●○ · 概念 · eq. 6.33–6.36</sub>

**標準答案**　Tickets with higher ids are preferred, so γ′_A keeps the highest E entries

eq. 6.33：n 依 id 排序且唯一；6.34：n 的 id 與 γ_A 不相交；6.35：γ′_A = →(sort_by_id(n ∪ (∅ if e′ > e else γ_A)))^E，取的是排序後**最前面**（即最低）的 E 筆，§6.7 原文「the accumulator becomes the lowest items of the sorted union」；6.36：n ⊆ γ′_A，「It is invalid to include useless tickets in the extrinsic」。注意 §6.2 的行文說 γ_A 是「highest-scoring ticket identifiers」——GP 兩處措辭方向相反，實作一律以 6.35 的公式為準。你們 CreateNewTicketAccumulator() 排序後取 [:E]，GetPreviousTicketsAccumulator 在 e′ > e 時回傳空 accumulator。

**逐項辨析**

1. ❌ n must be sorted ascending by ticket id and contain no duplicates, and no id in n may already be in γ_A  
   eq. 6.33 要求依 id 排序且唯一、6.34 要求與 γ_A 不相交——這句是真的。
2. ❌ γ′_A is the lowest E entries of the sorted union of n and (γ_A, or ∅ if e′ > e)  
   eq. 6.35 就是這麼寫的：排序後的聯集取最低 E 筆，跨 epoch 時起點換成空序列。
3. ❌ Every submitted ticket must appear in γ′_A — a ticket that would be evicted by the accumulator's cap is useless and makes the extrinsic invalid  
   eq. 6.36 的 n ⊆ γ′_A：「It is invalid to include useless tickets in the extrinsic」——真的。
4. ✅ Tickets with higher ids are preferred, so γ′_A keeps the highest E entries  
   eq. 6.35 留下的是排序後**最低**的 E 個 ticket id，數值小者勝。

> **陷阱**　「score」= ticket id（VRF 輸出），數值小者勝。

<sub>`ch06-ticket-accumulator-rules`</sub>

---

### 6-25　A ticket proof p in E_T is a Bandersnatch ring-VRF proof. Against which ring root, with what context, and what is the ticket identifier?

<sub>6.7 The Extrinsic and Tickets — ●●○ · 概念 · eq. 6.30, 6.32</sub>

**標準答案**　Against γ′_Z (the posterior ring root), context X_T ⌢ η′_2 ++ r, empty message []; the ticket id is the VRF output Y(p)

eq. 6.30：p ∈ F̄^{X_T ⌢ η′_2 ++ r}_{γ′_Z}([])，ring proof 的驗證 root 是 γ′_Z（若本塊是 epoch 第一塊，γ′_Z 已換成新 pending set 的 root，因此 ticket 會針對**新** ring 驗證）；訊息為空序列；eq. 6.32：ticket id y = Y(p)（VRF 輸出），r 是 entry index。ring proof 讓驗證者知道「某個 γ′_P 成員」擁有此 ticket，但不知道是誰——這正是匿名性的來源。你們 createSignatureContext 就是 "jam_ticket_seal" + η′_2(32B) + attempt(1B)。

**逐項辨析**

1. ✅ Against γ′_Z (the posterior ring root), context X_T ⌢ η′_2 ++ r, empty message []; the ticket id is the VRF output Y(p)  
   eq. 6.30 與 6.32 全對上：root 取 γ′_Z、context X_T ⌢ η′_2 ++ r、訊息為空、id = Y(p)。
2. ❌ Against γ_Z (the prior ring root), context X_T ⌢ η′_3 ++ r, message E_U(H) (the unsigned header); the ticket id is the Blake2b hash H(p)  
   prior γ_Z 會在 epoch 第一塊整批誤判；綁上 E_U(H) 更失去「先提交、後開票」的性質。
3. ❌ Against the Ed25519 keys of κ′ (the active set), context X_T ⌢ η_0 ++ r, empty message []; the ticket id is the proof p itself  
   ring 由 γ′_P 的 Bandersnatch key 組成，曲線與集合都不對；proof 可重新隨機化，不能當 id。
4. ❌ Against γ′_Z (the posterior ring root), context X_E ⌢ Y(H_S), message E_U(H) (the unsigned header); the ticket id is the VRF output Y(p)  
   X_E ⌢ Y(H_S) 是 H_V 的 context（6.18）；提票的當下根本還沒有 H_S 存在。

> **陷阱**　ring VRF 用在 ticket；一般 VRF 用在 seal 與 H_V。

<sub>`ch06-ticket-proof-context`</sub>

---

### 6-26　In fallback mode (γ′_S is a sequence of Bandersnatch keys), which checks apply to the header?

<sub>6.4 Sealing — ●●○ · 概念 · eq. 6.17–6.18</sub>

**標準答案**　γ′_S[H_T mod E] must equal H_A; H_S is a Bandersnatch signature by H_A with context X_F ⌢ η′_3 over E_U(H); T = 0; and H_V is still required with context X_E ⌢ Y(H_S) over []

eq. 6.17：γ′_S ∈ [H_B] ⇒ { i = H_A, H_S ∈ F^{X_F ⌢ η′_3}_{H_A}(E_U(H)), T = 0 }。eq. 6.18 對 ticket 與 fallback 兩種模式都適用：H_V ∈ F^{X_E ⌢ Y(H_S)}_{H_A}([])，也就是 entropy 在 fallback 下仍然每塊更新。三個 context 字串：X_F = $jam_fallback_seal、X_T = $jam_ticket_seal、X_E = $jam_entropy。T = 0 表示此塊「安全性較低」，§19 best-chain 規則會偏好 ticket-sealed 祖先較多的鏈。

**逐項辨析**

1. ✅ γ′_S[H_T mod E] must equal H_A; H_S is a Bandersnatch signature by H_A with context X_F ⌢ η′_3 over E_U(H); T = 0; and H_V is still required with context X_E ⌢ Y(H_S) over []  
   eq. 6.17 的三個條件加上 6.18 的 H_V ∈ F^{X_E ⌢ Y(H_S)}_{H_A}([]) 全部對上。
2. ❌ γ′_S[H_T mod E] must equal H_A; H_S is a Bandersnatch signature by H_A with context X_F ⌢ η′_2 over E_U(H); T = 0; and H_V is still required with context X_E ⌢ Y(H_S) over E_U(H)  
   驗 seal 一律用已 rotate 的 η′_3；6.18 的 H_V 訊息是空序列 []，不是 E_U(H)。
3. ❌ γ′_S[H_T mod E] must equal the author's Ed25519 key; H_S is a ring-VRF proof against γ′_Z with context X_F ⌢ η′_3 over E_U(H); T = 1; and H_V is still required with context X_E ⌢ Y(H_S) over []  
   三處都錯：γ′_S ∈ [H_B]_E 是 Bandersnatch key 序列，ring proof 只用於提票，fallback 的 T 必為 0。
4. ❌ γ′_S[H_T mod E] must equal H_A; no seal is required in fallback mode, there being no ticket identifier to bind it to; T = 0; and H_V alone is checked, with context X_E ⌢ η′_3 over []  
   6.17 仍要求 H_S，否則無從證明作者持有該 slot 指定的 key；H_V 的 context 也是 X_E ⌢ Y(H_S)。

> **陷阱**　fallback 的 entropy 仍然每塊更新（H_V 必填）。

<sub>`ch06-seal-fallback`</sub>

---

### 6-27　This is the team's implementation of γ′_S. Which claim about it is correct?

<sub>6.5 The Slot-Sealer Sequence — ●●○ · 程式碼 · eq. 6.25 — internal/safrole/sealing.go UpdateSlotKeySequence</sub>

```go
if ePrime == e+1 && len(gammaA) == types.EpochLength && int(slotIndex) >= types.SlotSubmissionEnd {
...
```
<sub>internal/safrole/sealing.go (UpdateSlotKeySequence)</sub>

**標準答案**　`slotIndex` is m — the slot phase of the PRIOR block (τ mod E) — and `gammaA` is the prior accumulator; `etaPrime[2]` and `posteriorState.GetKappa()` are posterior values, matching F(η′_2, κ′)

eq. 6.25 的條件是 m ≥ Y，m 是 **prior** block 的 phase（τ 除以 E 的餘數），不是 m′——因為要確認「比賽已在前一塊之前結束」，這正是 fuzzer 愛打的邊界。F 的參數則是 η′_2 與 κ′（posterior）。e′ ≥ e+2（跳過整個 epoch）必須走 fallback，因為 accumulator 裡的 ticket 是為「e+1」那個 epoch 準備的，其 ring proof 針對的是當時的 γ′_Z。

**逐項辨析**

1. ✅ `slotIndex` is m — the slot phase of the PRIOR block (τ mod E) — and `gammaA` is the prior accumulator; `etaPrime[2]` and `posteriorState.GetKappa()` are posterior values, matching F(η′_2, κ′)  
   6.25 的 m 出自 τ、F 的參數是 posterior，這段程式三處判斷都與公式一致。
2. ❌ `slotIndex` should be m′ — the phase of the block being imported, H_T mod E — because eq. 6.25 asks whether the contest has closed by the current block; reading the prior block's τ mod E is an off-by-one  
   換成 m′ 會讓「跨過 Y 的那一塊」立刻開票、少等一整段 tail——6.25 明寫的是 m。
3. ❌ The fallback branch should pass the prior κ together with the prior η_2, since the incoming validators only gain authoring rights from the second block of the epoch onwards  
   epoch 第一塊完成 6.14 輪換之後，κ′（= 舊 γ_P）才是本 epoch 真正的 active set。
4. ❌ The first branch should also fire when ePrime ≥ e + 2 provided |γ_A| = E and m ≥ Y, since accumulated tickets stay valid until they are consumed; restricting it to e + 1 forces fallback needlessly  
   跳過整個 epoch 後 ring root 與 η 都已再度 rotate，那些 ticket 是為 e+1 準備的。

> **陷阱**　你們的 OuterUsedSafrole 先做 UpdateEntropy、KeyRotate，再做 UpdateSlotKeySequence——順序正是為了讓 η′_2、κ′ 就位。

<sub>`ch06-code-slot-key-sequence`</sub>

---

### 6-28　Read the team's FallbackKeySequence. Which statement is accurate about its conformance to eq. 6.27?
...
```
<sub>internal/safrole/slot_key_sequence.go</sub>
...
> **陷阱**　面試官可能直接問：你們 fallback 用哪個 hash？答 Blake2b，並指出註解錯誤。

<sub>`ch06-code-fallback-hash`</sub>

---

### 6-29　In the team's UpdateEntropy, why is `eta[0]` overwritten with the posterior η′_0 AFTER the rotation loop, and what would go wrong if the loop ran after UpdateEtaPrime0 wrote into the same array?
...
```
<sub>internal/safrole/sealing.go</sub>

**標準答案**　Because eq. 6.24 rotates the PRIOR η_0 into η′_1; if η′_0 (already hashed with this block's VRF output) were rotated instead, η′_1 would wrongly include the current block's entropy

eq. 6.24：(η′_1, η′_2, η′_3) ≡ (η_0, η_1, η_2)——右側全是 **prior** 值。η′_0 = H(η_0 ⌢ Y(H_V))（6.23）則已經包含本塊的 VRF 輸出。若先把 η′_0 寫進 eta[0] 再 rotate，η′_1 會變成 H(η_0 ⌢ Y(H_V))，導致下個 epoch 的 ticket context（用 η′_2）與 fallback 都算錯，state root 立刻 mismatch。這種「prior vs posterior」的細節正是 fuzzer 最常抓到的 bug 類型。

**逐項辨析**

1. ✅ Because eq. 6.24 rotates the PRIOR η_0 into η′_1; if η′_0 (already hashed with this block's VRF output) were rotated instead, η′_1 would wrongly include the current block's entropy  
   6.24 右側全是 prior 值；先寫 η′_0 會讓 η′_1 變成 H(η_0 ⌢ Y(H_V))，混進本塊的 entropy。
2. ❌ Because eq. 6.24 re-seeds the accumulator on an epoch change: η′_0 has to be reset after the rotation so the new epoch starts from a clean value, which is exactly what writing eta[0] last achieves  
   GP 沒有這個依據：6.23 每一塊都照 H(η_0 ⌢ Y(H_V)) 更新，epoch 邊界也不例外。
3. ❌ Because the descending loop is what stops η_0 from being copied into all three history slots; the write to eta[0] is independent of eq. 6.24 and could equally well run before the rotation  
   正好說反：倒序確實避免了覆蓋，但**寫入時機**才是這段程式的核心。
4. ❌ Because eq. 6.24 is stated over posterior values, (η′_1, η′_2, η′_3) = (η′_0, η′_1, η′_2), so η′_0 must be settled first and the assignment after the loop is a compensating fix-up for the wrong order  
...
### 7-1　This 0.7.2 code builds the entry appended to β_H at the end of a block. What must change for GP 0.8.0, and which on-chain check motivates the change?

<sub>7 Recent History — ●●○ · 版本差異 · eq. 7.2, 7.8 & eq. 11.36; §D.1 C(3) · ⚠ 0.7.2→0.8.0</sub>
...
```
<sub>internal/recent_history/recent_history_controller.go (NewItem) — GP 0.7.2 checkout</sub>

**標準答案**　Add the block's timeslot t = H_T as a fifth field (E_4(t) between s and p under state key C(3)): refinement contexts now carry the anchor's timeslot and eq. 11.36 requires it to equal the t of the matching β† entry (GP PR #526; team PR #1031 threads H_T into NewItem)

eq. 7.2（0.8.0）：β_H ∈ [(h, s, b, t ∈ N_T, p)]_{:H}——多了 timeslot；eq. 7.8 的新 entry 是 (H(H), H_0, M_R(β′_B), H_T, p)。動機：0.8.0 PR #526 在 refinement context（eq. 11.4）加入 anchor slot 與 lookup-anchor posterior state root，讓 refine 端能得知 anchor 的時間；鏈上要驗它，eq. 11.36 就變成：∃y ∈ β†：anchor hash = y_h ∧ anchor posterior state = y_s ∧ anchor accumulation log = y_b ∧ anchor time = y_t——其中 accumulation log 只比一個 32-byte super-peak，不是逐筆比對。狀態序列化（§D.1 的 C(3)）順序是 (h, b, s, E_4(t), var(p))——注意 b 在 s 之前，與 eq. 7.2 的 tuple 順序 (h, s, b, t, p) 不同；你們 BlockInfo 的欄位 HeaderHash, BeefyRoot, StateRoot, Reported 正是照序列化順序排，PR #1031 把 4-byte timeslot 插在 state_root 與 reported 之間。

**逐項辨析**

1. ✅ Add the block's timeslot t = H_T as a fifth field (E_4(t) between s and p under state key C(3)): refinement contexts now carry the anchor's timeslot and eq. 11.36 requires it to equal the t of the matching β† entry (GP PR #526; team PR #1031 threads H_T into NewItem)  
   #526 讓 refinement context 帶上 anchor slot，鏈上要驗它，β 的每一筆就必須存 t。
2. ❌ Replace the zero state root with the posterior root M_σ(σ′), because 0.8.0 headers now commit to the posterior state rather than the prior one; eq. 7.5's β† correction therefore disappears and eq. 11.36 can compare an anchor's state root against the very block that produced it  
   0.8.0 的 header 仍只帶 prior state root，state root 照舊填零、由下一塊的 β† 補正（eq. 7.5）。
3. ❌ Store the whole accumulation-output sequence θ′ in place of the super-peak b, because eq. 11.36 now compares the anchor's accumulation log entry by entry rather than against one commitment, which is also why 0.8.0 moved the belt out of C(3) and into its own state key  
   β 存的是 belt 的 super-peak 而不是整個 θ′；C(3) 至今仍同時裝 item 與 mmrencode(β_B)。
4. ❌ Add the lookup-anchor posterior state root of every report guaranteed in the block, because 0.8.0 added that field to the refinement context and β_H is the only on-chain place where the value can be recorded — the ancestor set A holds headers, not the roots that follow them  
   lookup-anchor 的 posterior root 用 ancestor set 裡下一個 header 的 H_R 驗（eq. 11.38），不經過 β。

> **陷阱**　0.8.0 β_H 一項 = 5 欄；序列化順序 h, b, s, t, p（b 在 s 前）；anchor 的四個欄位全部要對上 β† 的同一筆。

<sub>`ch07-beta-entry-timeslot-080`</sub>

---

### 7-2　Block N accumulates nothing that yields an output, so θ′ = []. What happens to the accumulation-output belt β_B and to the super-peak b written into block N's β_H entry?

<sub>7 Recent History — ●●● · 概念 · eq. 7.6–7.7 & eq. E.1, E.3</sub>

**標準答案**　One leaf is still appended: M_B([], H_K) = N([], H_K) = H_0, so β′_B = A(β_B, H_0, H_K) and b = M_R(β′_B) changes — the belt always carries exactly one leaf per block since genesis

eq. 7.6–7.7：s = [E_4(s) ⌢ E(h) | (s, h) ∈ θ′]，β′_B ≡ A(β_B, M_B(s, H_K), H_K)。θ′ 來自 eq. 12.22/12.24 的 b = {(s, b) | b = yield 結果 ≠ ∅}——只有在 accumulate 裡呼叫 yield 的 service 才會出現，所以大多數區塊的 θ′ 都是空的。此時 s = []，依 eq. E.3：M_B(v, H) 在 |v| = 1 時為 H(v_0)，否則為 N(v, H)；再依 eq. E.1：N([], H) = H_0（零 hash）。你們的 lastAccOutRoot → merkle.Mb：len(v) == 0 時走 N 回傳 zeroHash，再交給 AppendOne（OpaqueHash 長度固定 32，不會被當成空值略過），行為一致。

**逐項辨析**

1. ❌ Nothing is appended and b repeats the parent's super-peak, so the belt holds one leaf per block that produced at least one accumulation output and BEEFY verifiers simply skip empty blocks  
   eq. 7.7 沒有「θ′ 為空就跳過」的條件，belt 的 leaf 數等於創世以來的區塊數。
2. ❌ An empty peak ∅ is appended to the peak sequence without any hashing, so the peak count grows by one while b stays unchanged until the next block with a non-empty θ′  
   ∅ 只在 MMR 進位時作為空 peak 出現（eq. E.8 的 R(r, n, ∅)），不會因 θ′ 為空而被 append。
3. ✅ One leaf is still appended: M_B([], H_K) = N([], H_K) = H_0, so β′_B = A(β_B, H_0, H_K) and b = M_R(β′_B) changes — the belt always carries exactly one leaf per block since genesis  
   M_B([], H_K) = N([], H_K) = H_0：空塊照樣 append 一片 H_0 的 leaf，super-peak 每塊都變。
4. ❌ The block's header hash H(H) is appended in place of the missing output root, so that the belt stays aligned one-to-one with the β_H entries and their header hashes  
   header hash 進的是 β_H 的 h 欄位，從來不會被 append 進 belt。

> **陷阱**　belt 是「每塊一片 leaf」的 MMR；空區塊的 leaf 是 H_0 不是 ∅。因此兩個相鄰的空區塊，其 β_H 的 b 也不相同。

<sub>`ch07-belt-empty-output`</sub>

---

### 7-3　Starting from an empty belt β_B = [] at genesis, five consecutive blocks append their per-block roots r_1, …, r_5 (each r_i = M_B(s_i, H_K)). Applying the MMR append function of eq. E.8 with H = H_K, what is β_B after block 5, and what super-peak b = M_R(β_B) enters block 5's β_H entry?

<sub>7 Recent History (MMR, Appendix E.2) — ●●● · 計算 · eq. 7.7 & eq. E.8–E.10</sub>

**標準答案**　β_B = [r_5, ∅, H_K(H_K(r_1 ⌢ r_2) ⌢ H_K(r_3 ⌢ r_4))]; b = H_K('peak' ⌢ r_5 ⌢ H_K(H_K(r_1 ⌢ r_2) ⌢ H_K(r_3 ⌢ r_4)))

逐步套用 eq. E.8 的 P(r, l, n)：(1) [] + r_1：n = 0 ≥ |r| = 0 → append → [r_1]。(2) + r_2：r[0] 已佔用 → 清成 ∅，帶著 H_K(r_1 ⌢ r_2) 往 n = 1 → append → [∅, h_12]。(3) + r_3：r[0] = ∅ → 直接放入 → [r_3, h_12]。(4) + r_4：r[0] 佔用 → 進位 H_K(r_3 ⌢ r_4) 到 n = 1，r[1] 也佔用 → 再進位 H_K(h_12 ⌢ h_34) 到 n = 2 → append → [∅, ∅, h_1234]。(5) + r_5：r[0] = ∅ → [r_5, ∅, h_1234]。這對應 5 = 101₂：index i 的 peak 代表一棵 2^i 片 leaf 的樹。super-peak（eq. E.10）：先取非 ∅ 的 peaks h = [r_5, h_1234]，|h| = 2 → H_K($peak ⌢ M_R([r_5]) ⌢ h_1234) = H_K('peak' ⌢ r_5 ⌢ h_1234)。你們 mmr.go 的 P() 與 SuperPeak()（先濾掉 nil，再遞迴摺前 n−1 個）與此一致；狀態序列化時每個 peak 前有 0/1 option byte（eq. E.9，encode.go 的 Mmr 編碼）。

**逐項辨析**

1. ❌ β_B = [H_K(H_K(r_1 ⌢ r_2) ⌢ H_K(r_3 ⌢ r_4)), ∅, r_5]; b = H_K('peak' ⌢ H_K(H_K(r_1 ⌢ r_2) ⌢ H_K(r_3 ⌢ r_4)) ⌢ r_5)  
   peak 序列的 index 就是樹高，index 0 永遠是最小的那棵；這裡把大樹放到了 index 0。
2. ✅ β_B = [r_5, ∅, H_K(H_K(r_1 ⌢ r_2) ⌢ H_K(r_3 ⌢ r_4))]; b = H_K('peak' ⌢ r_5 ⌢ H_K(H_K(r_1 ⌢ r_2) ⌢ H_K(r_3 ⌢ r_4)))  
   5 = 101₂ 給出 [r_5, ∅, h_1234]；super-peak 由左往右摺，最後一個 peak 落在最外層。
3. ❌ β_B = [M_B([r_1, r_2, r_3, r_4, r_5], H_K)]; b = that single root — the range is re-balanced on every append so there is always exactly one peak  
   MMR 是 append-only、不重新平衡；那樣還會多出 $node 前綴，append 也不再是 O(log N)。
4. ❌ β_B = [r_5, ∅, H_K(H_K(r_1 ⌢ r_2) ⌢ H_K(r_3 ⌢ r_4))]; b = H_K(r_5 ⌢ H_K(H_K(r_1 ⌢ r_2) ⌢ H_K(r_3 ⌢ r_4))) — the super-peak is just one more un-prefixed carry  
   super-peak 一定帶 $peak 前綴，無前綴的 H(r[n] ⌢ l) 只出現在 MMR 內部進位。

> **陷阱**　peak 序列的 index = 樹高（2^i 片 leaf）；super-peak 由左往右摺、最後一個 peak 在最外層、前綴是 $peak。

<sub>`ch07-belt-five-appends-calc`</sub>

---

### 7-4　Every item of β_H carries a field p. In GP 0.8.0, what does p hold, and what bounds the number of entries it can have?

<sub>7 Recent History — ●○○ · 概念 · eq. 7.2 & 7.8; §7</sub>
...
1. ✅ A dictionary keyed by each work-package hash guaranteed in that block, whose value is that package's segment-root; at most C = 341 entries, because a block can report at most one work-report per core  
   §7 原文就寫 no more than the total number of cores, C = 341，且 value 是 segment-root。
2. ❌ A set holding the header hashes of the block's ancestors; at most H = 8 entries, one for each block kept in the recent-history window, which is what bounds a refinement context's anchor to eight blocks of depth  
   header hash 存在同一個 item 的 h 欄位；anchor 深度 8 來自 β_H 本身的 :H 截斷，不是 p 的內容。
3. ❌ A dictionary keyed by each guaranteed work-package hash, whose value is the index of the core the work ran on; at most O = 8 entries, matching the size of a core's authorizer pool  
   core index 從頭到尾沒有進 β（report 自帶 w_c），O = 8 是 authorizer pool 的上限。
4. ❌ A sequence of the payload hashes of every work-item in every report of the block; at most I = 16 entries, the maximum number of work-items one work-package may carry, so the bound does not scale with the number of cores  
   I = 16 是單一 work-package 的 work-item 數上限（eq. 14.2），而一個區塊可有多達 C 份 report。

> **陷阱**　p 的 value 不是隨便的雜湊，是 segment-root；eq. 11.44 要靠這個 value 驗 segment-root 與 package hash 配對。

<sub>`c3-ch07-reported-map-shape`</sub>

---

### 7-5　When a block's guarantees extrinsic E_G is validated in §11, which of the on-chain checks actually read the reported-package maps stored inside β_H?

<sub>7 Recent History — ●●○ · 概念 · eq. 7.8 & eq. 11.41–11.44</sub>

**標準答案**　Three of them: an incoming package hash must not already be a key of any recent block's map (anti-duplicate); every prerequisite and every key of a report's segment-root lookup l must be such a key or come from this block's own guarantees; and l must be a sub-dictionary of those maps, so each segment-root matches its package hash

三處都在 §11：(1) eq. 11.41 要求 ∀p ∈ 本區塊 package hashes，p ∉ ⋃_{x ∈ β_H} K(x_p) ∪ ⋃ξ ∪ q ∪ a —— 這是反重複／反過期的主線，也是 §7 開頭說保留最近 H 個區塊的理由（"to preclude the possibility of duplicate or out of date work-reports"）；(2) eq. 11.42 要求 prerequisites 與 l 的每個 key 落在本區塊 E_G 或某個 β_H item 的 p 的 keys 裡；(3) eq. 11.44 要求 w_l ⊆ 本區塊 package-hash↦segment-root ∪ ⋃_{b ∈ β_H} b_p，是 dictionary 的包含關係，所以 value（segment-root）也必須吻合。

**逐項辨析**

1. ✅ Three of them: an incoming package hash must not already be a key of any recent block's map (anti-duplicate); every prerequisite and every key of a report's segment-root lookup l must be such a key or come from this block's own guarantees; and l must be a sub-dictionary of those maps, so each segment-root matches its package hash  
   eq. 11.41 反重複、11.42 解 prerequisite 與 l 的 key、11.44 要求 w_l 是子字典，三處都讀 p。
2. ❌ Only the anti-duplicate one; prerequisites and the segment-root lookup are instead resolved against ξ (accumulation history) and the ready queue ω, whose dependency sets are what eventually clear a report for accumulation — so the maps in β_H would be redundant there and exist purely for the duplicate test  
   ξ 與 ω 只是 eq. 11.41 聯集裡的額外來源；eq. 11.42 的 prerequisite 檢查照樣讀 β_H 的 p。
3. ❌ Only the refinement-context anchor check: the anchor's header hash, state root, super-peak and timeslot are all looked up among the maps of β†, while duplicates are caught later during accumulation, where a package hash already present in ξ is simply dropped from the ready queue instead of invalidating the block  
   anchor 檢查是 eq. 11.36、比對 h/s/b/t 與 p 無關；eq. 11.41 撞到就整塊不合法，不會留到 accumulation。
4. ❌ None of them: the maps exist to build BEEFY inclusion proofs for third parties, and §11 re-derives recent package hashes by decoding the E_G of the last eight blocks out of the block store — which is why the ancestor set A must retain L = 14,400 blocks complete with their extrinsics rather than headers alone  
   p 上鏈正是為了不必翻舊 extrinsic；ancestor set A 只留 header，BEEFY 承諾的是 β_B 的 super-peak。

> **陷阱**　同一份 map 同時被用來「拒絕重複」與「接受依賴」——方向相反，別記反了。

<sub>`c3-ch07-reported-map-downstream`</sub>

---

### 7-6　Trace how a block's accumulation outputs end up inside a BEEFY signature. Which description matches GP 0.8.0?

<sub>7 Recent History — ●●○ · 概念 · eq. 7.7, 7.8; eq. E.8, E.10; §18 Beefy</sub>
...
1. ✅ Each block Keccak-Merklizes the encoded θ′ into one root and appends that root to the belt β_B; the new β_H item stores only the belt's Keccak super-peak; validators then BLS-sign the domain-separated hash of the newest item's super-peak for each block they finalize  
   eq. 7.7 append 的是 M_B(s, H_K)、eq. 7.8 的 b 存 super-peak、eq. 18.1 簽 X_B ⌢ last(β_H)_b。
2. ❌ Each block Keccak-Merklizes θ′ into one root, appends it to β_B, and validators BLS-sign the whole encoded peak sequence of β_B; the super-peak exists only so that state key C(3) has one fixed-length field, which is why eq. 18.1's signed message is E(mmrencode(β_B)) rather than a single hash  
   完整 peak 序列只以 mmrencode(β_B) 進 C(3)；eq. 18.1 簽的是壓成 32-byte 的 super-peak。
3. ❌ The block header carries the super-peak in a dedicated marker and validators sign the header hash; β_B is guarantor-side bookkeeping that never enters the serialized state, so a bridge only ever needs the header chain and never has to read a state proof  
   eq. 5.1 只有 epoch／winners／offenders 三個 marker；belt 確實進了 C(3)，bridge 仍得走狀態證明。
4. ❌ The belt is rebuilt from θ with Blake2b every block and validators sign the root of the current block's outputs alone, so one BEEFY signature attests exactly one block's accumulation results and a verifier wanting an older output re-derives its root from that block's own C(16) entry  
   MMB 是 append-only 且全程用 Keccak；C(16) 只裝本區塊的 θ，翻不到舊區塊的 root。

> **陷阱**　θ′ 是本區塊的輸出序列（另一個狀態項），β_B 是跨區塊的 belt；被 append 的是 θ′ 的 M_B root，不是 θ′ 本身。

<sub>`c3-ch07-superpeak-to-beefy`</sub>

---

### 7-7　An implementer builds β′_H by first appending this block's new item (state root = H_0) and only then applying the parent-state-root back-fill of eq. 7.5 to 'the last element of the sequence'. What actually goes wrong?

<sub>7 Recent History — ●●○ · 設計理由 · eq. 7.5, 7.8 & eq. 11.36</sub>

**標準答案**　The back-fill lands on the block's own fresh item, so every entry ends up carrying its parent's posterior root instead of its own, and the parent's entry never receives the correction it was meant for; from then on eq. 11.36's state-root comparison rejects honest refinement contexts and the C(3) preimage diverges from every other node's

eq. 7.5 的 β† ≡ β_H 只改「β_H 最後一個 item」的 s ← H_R，而 H_R 是本區塊 header 攜帶的 *parent* posterior state root（header.tex：JAM 刻意在 header 放 prior state root 以利 pipelining）。正確順序是先 β_H → β†（修好上一個區塊的 s），再 append 新 item 且 s = H_0。順序顛倒後，被修的是剛 append 的新 item，於是它拿到的是「上一個區塊」的 root，而上一個區塊的 item 再也拿不到本該補給它的 root（注意：不會有任何 item 停在 H_0，新 item 一 append 就被錯位的修正立刻蓋掉）。後果有兩層：eq. 11.36 要求 x_s = y_s（context 的 anchor posterior state root 對上 β† item 的 s），guarantor 給的是真值，這台節點比對必失敗；而 C(3) 直接把每個 item 的 s 編進狀態序列化，state root 立刻和別人不同，fuzz trace 會停在 state-root mismatch。GP 自己的註腳講清楚了為什麼 H_0 是安全的："β′ is not utilized except to define the next block's β†"。

**逐項辨析**

1. ✅ The back-fill lands on the block's own fresh item, so every entry ends up carrying its parent's posterior root instead of its own, and the parent's entry never receives the correction it was meant for; from then on eq. 11.36's state-root comparison rejects honest refinement contexts and the C(3) preimage diverges from every other node's  
   eq. 7.5 的定義域是 append 前的 β_H，順序顛倒會讓修正落在新 item 上，整條系統性 off-by-one。
2. ❌ Nothing observable: the last element is the same item either way once the block is finished, and eq. 7.5's correction is idempotent, so both orders produce the same C(3) preimage — which is precisely why the GP states β† as an exception on β_H instead of fixing an order of operations, and why implementations differ here without diverging  
   兩種順序改到的是不同的 item，「idempotent、序列化結果一樣」因此不成立。
3. ❌ Only β_B is corrupted: the new item's super-peak would be taken from an uncorrected belt, so BEEFY roots drift apart while the recent-history entries and the state root stay correct, because eq. 7.7 folds the parent's back-filled state root into the leaf it appends and eq. 11.36 never looks at b  
   β† 完全不碰 β_B（belt 的 leaf 由 θ′ 編碼而來），而 eq. 11.36 確實也比對 x_b = y_b。
4. ❌ The history window slides one block too early, leaving β_H permanently at H − 1 = 7 entries, so eq. 11.36 rejects any anchor exactly eight blocks old while everything else still matches, since the ←(…)^H truncation belongs to eq. 7.5 rather than to the append of eq. 7.8 — the symptom is a rejected guarantee, never a state-root mismatch  
   ←(…)^H 的截斷在 eq. 7.8 的 append 而非 eq. 7.5；先炸的是 state root，不是 guarantee。

> **陷阱**　β† 修的是「上一個區塊」的 s，不是自己的；先修再 append，順序寫反不會報錯，只會 state root 不對。

<sub>`c3-ch07-dagger-before-append`</sub>

---

### 7-8　This is the team's GP 0.7.2 encoder for one β_H item under state key C(3). A reviewer objects that GP 0.8.0 declares the item as ⟨h, s, b, t, p⟩ — state root ahead of the accumulation-output-log super-peak — so the encoder must have two fields swapped. Who is right?
...
### 7-9　In GP 0.8.0 the recent-history state is β ≡ (β_H, β_B). What does each recent block entry in β_H hold, and what is β_B?
...
eq. 7.2：β_H ∈ [(h ∈ H, s ∈ H, b ∈ H, t ∈ N_T, p ∈ D⟨H→H⟩)]_{:H}，H = 8。t（timeslot）是 0.8.0 新增欄位（配合 refinement context 的 anchor slot，#526）。p 是本塊 E_G 中每個 work-package hash → 其 segment root（exports root），供之後 report 的 prerequisite / segment-root lookup 驗證用。eq. 7.3：β_B ∈ [H?]（MMR 的 peaks，∅ 表示空 peak），用 Keccak。eq. 7.4：θ ∈ [(N_S, H)] 是本塊的 accumulation output（service, hash）序列。
...
### 7-10　Why does the newest β_H entry get state root s = H_0 (the zero hash) at the end of block N, and how is it corrected?

<sub>7 Recent History — ●●○ · 概念 · eq. 7.5 & 7.8</sub>

**標準答案**　Because the posterior state root of block N is not yet known when β′ is computed (the header carries the prior root); block N+1 computes β† by overwriting the last entry's s with its own H_R (eq. 7.5) before anything reads β

eq. 7.8：新 entry 為 (H(H), H_0, super-peak(β′_B), H_T, p)，§7 說明「The new state-trie root is the zero hash, which is inaccurate but safe since β′ is not utilized except to define the next block's β†」。eq. 7.5：β† ≡ β_H except β†[|β_H|−1]_s = H_R。所以每個 block 一開始（在驗 guarantee 的 anchor 之前）先用自己的 H_R 補正父塊的 state root。這是 header 帶 prior state root（pipelining）的直接後果。你們的 STF 在 τ′ 與 header check 之後、disputes 之前就算 β†。

**逐項辨析**

1. ✅ Because the posterior state root of block N is not yet known when β′ is computed (the header carries the prior root); block N+1 computes β† by overwriting the last entry's s with its own H_R (eq. 7.5) before anything reads β  
   eq. 7.5：補正者是下一個區塊，用自己 header 的 H_R 覆寫父塊那筆——§7 說 β′ 只用來定義 β†。
2. ❌ Because a state root only matters once the block is finalized: the entry keeps H_0 until Grandpa finalizes block N, at which point the finality gadget writes the real root into that entry and eq. 11.36 starts accepting anchors on it  
   STF 是確定性函數，finality 從不回寫狀態；eq. 7.5 的補正發生在下一塊，與敲定無關。
3. ❌ Because β_H entries carry no state root at all: eq. 7.2 declares an item as ⟨h, b, t, p⟩ and the zero hash is only a placeholder that C(3) skips, so eq. 11.36 matches an anchor on its header hash, super-peak and timeslot alone  
   eq. 7.2 明列 s ∈ H 為第二個具名欄位，C(3) 讓它佔滿 32 bytes，eq. 11.36 也確實比對 x_s = y_s。
4. ❌ Because the root goes into β_B instead: each block appends its own posterior state root as the belt's next leaf, so the β_H item can leave s at H_0 permanently and an anchor's state root is checked against b  
   eq. 7.6–7.7 append 進 belt 的 leaf 由 θ′ 編碼而來，與 state root 無關；x_s 與 x_b 是兩個獨立比較。

> **陷阱**　guarantee 的 anchor 檢查（eq. 11.36）比對的是 β†（補正後），不是 β。

<sub>`ch07-beta-dagger`</sub>

---

### 7-11　The accumulation-output belt β′_B = A(β_B, M_B(s, H_K), H_K) uses Keccak (H_K) rather than Blake2b. What is s and why Keccak?

<sub>7 Recent History — ●●○ · 設計理由 · eq. 7.6–7.7</sub>

**標準答案**　s = [E_4(service) ⌢ E(hash) for each (service, hash) in θ′]; Keccak is used 'to maximize compatibility with legacy systems'

eq. 7.6：s = [E_4(s) ⌢ E(h) | (s,h) ∈ θ′]；eq. 7.7：β′_B ≡ A(β_B, M_B(s, H_K), H_K)——先用 well-balanced binary Merkle（M_B）以 Keccak 算出本塊 accumulation output 的 root，再用 MMR append（附錄 E.2）接到 belt 上。§7：「Throughout, the Keccak hash function is used to maximize compatibility with legacy systems」——EVM 有便宜的 keccak256 precompile，BEEFY（§18）就是對這個 belt 的 super-peak 做 BLS 簽章給外部系統（bridge）用，bridge 合約驗 belt 幾乎不花錢。你們的 recent_history_controller / mmr 都用 Keccak。

**逐項辨析**

1. ✅ s = [E_4(service) ⌢ E(hash) for each (service, hash) in θ′]; Keccak is used 'to maximize compatibility with legacy systems'  
   eq. 7.6 的 s 正是 θ′ 的 (service, hash) 編碼；§7 給的理由就是與 legacy 系統相容。
2. ❌ s = the sequence of every work-report hash guaranteed in this block; Keccak is chosen because it costs less PVM gas than Blake2b  
   work-report hash 只出現在 header 的 H_X 與 guarantee 簽章訊息；「省 PVM gas」不是 GP 給的理由。
3. ❌ s = the encoded header of the block being appended; Keccak is required because the Bandersnatch ring VRF transcript is defined over it  
   belt 的 leaf 來源是 θ′ 而非 header；附錄 G 的 ring VRF 有自己的建構，與 belt 雜湊選擇無關。
4. ❌ s = [H(g_w) for each guarantee g ∈ E_G]; Keccak is mandated so that Grandpa's finality votes can be checked by the same bridge contracts  
   簽 belt super-peak 的是 BEEFY 的 BLS 簽章（eq. 18.1），Grandpa 完全不碰 belt。

> **陷阱**　belt 是 MMR（append-only，peaks 序列），super-peak（E.2）才是單一 32-byte commitment。

<sub>`ch07-belt-keccak`</sub>

---

### 7-12　What is the primary purpose of retaining the H = 8 most recent blocks in β_H, according to the GP?

<sub>7 Recent History — ●○○ · 概念 · §7 & eq. 11.36, 11.41–11.44</sub>

**標準答案**　To preclude duplicate or out-of-date work-reports: a guarantee's refinement-context anchor must appear in β† and its work-package hash must not already be a key of any recent block's reported-package map

§7 第一句：「This is used to preclude the possibility of duplicate or out of date work-reports from being submitted.」具體用途：(1) eq. 11.36：refinement context 的 anchor (hash, state root, belt super-peak, timeslot) 必須匹配 β† 的某一筆——所以 anchor 最多只能是 8 個區塊之前；(2) eq. 11.41：新 report 的 package hash 不得出現在任何 β 條目的 p 裡（防重複）；(3) eq. 11.42 / 11.44：prerequisites 與 segment-root lookup 必須在 extrinsic 或 β 的 p 裡找得到。

**逐項辨析**

1. ✅ To preclude duplicate or out-of-date work-reports: a guarantee's refinement-context anchor must appear in β† and its work-package hash must not already be a key of any recent block's reported-package map  
   §7 第一句就是 preclude duplicate or out of date work-reports（eq. 11.36、11.41）。
2. ❌ To let light clients follow the chain without state: β_H is the only place the last H = 8 header hashes are kept, so a client walks them backwards instead of reading H_P out of each header  
   每個 header 自帶 H_P 即可往回走，且 β 是狀態，light client 反而拿不到。
3. ❌ To seed the fallback slot-sealer sequence: when the ticket contest falls short, F draws E = 600 Bandersnatch keys and the hashes of the last 8 blocks in β_H supply the randomness for that draw  
   fallback key sequence F 的種子是 η′_2、抽的是 κ′ 的 Bandersnatch key（§6），與 β 無關。
4. ❌ To store the last 8 posterior state roots so that Grandpa voters can compare candidate chains without re-executing them, β_B holding the matching BLS signatures over those roots  
   最新一筆的 s 在本塊仍是 H_0、要等下一塊 β† 補正；β_B 是 output belt，不存任何簽章。

> **陷阱**　anchor 深度 ≤ 8 blocks；lookup anchor 深度 ≤ 14,400 slots（用 ancestors A）。

<sub>`ch07-purpose`</sub>
...
## §8 Authorization　<sub>12 題</sub>

### 8-1　Before PR #694 (bug #692) this removal deleted every occurrence of the used authorizer hash and ignored the report's core. Which statement correctly describes the GP rule the fixed code implements and why the old behaviour was wrong?

<sub>8.2 Pool and Queue — ●●○ · 程式碼 · eq. 8.3 (F) & eq. 11.25, 11.32</sub>
...
```
<sub>internal/authorization/authorization.go (updatePoolFromQueue) & internal/types/types.go (AuthPool.RemoveLeftMostPairedValue)</sub>

**標準答案**　eq. 8.3: F(c) = α[c] ⊖ {w_a} removes one left-most instance from the pool of the report's own core; a pool is a sequence that may hold the same hash several times (e.g. a queue repeating one authorizer), so deleting every copy shrank pools and broke the test vectors
...
1. ❌ eq. 8.2: the removal only makes room for the queue entry appended this block, so deleting every duplicate is harmless — α[c] is a set in which one authorizer hash can never occur twice, and the ←(…)^O truncation would have discarded any extra copies within eight blocks anyway  
   eq. 8.1 的 α ∈ [[H]_{:O}]_C 是序列不是集合；pool 未滿時根本不截斷，多刪的 hash 永久消失。
2. ❌ eq. 11.32: the removal doubles as the pool-membership check, and a missing match must reject the block — so the old code was wrong only in failing to return an error when nothing matched, which is why the fix belongs in the guarantee validator rather than in the authorization state transition  
   F(c) 不做驗證：w_a ∈ α[w_c] 早在 eq. 11.32 就對 prior α 檢查過了。
3. ❌ eq. 8.3, but the pool to edit must be found by looking the guarantors' validator indices up in the current rotation's assignment G, not by the core index stored inside the work-report — so a guarantee signed one rotation earlier edits whichever core those validators are sitting on now  
   要編輯哪個 pool 由 (g_w)_c 決定；eq. 11.28 允許 g_t 落在前一個 rotation，反推會挑到別的 pool。
4. ✅ eq. 8.3: F(c) = α[c] ⊖ {w_a} removes one left-most instance from the pool of the report's own core; a pool is a sequence that may hold the same hash several times (e.g. a queue repeating one authorizer), so deleting every copy shrank pools and broke the test vectors  
   §3 的 s ⊖ {v} 是 excepting the left-most element equal to v，只移除最舊的那一個。

> **陷阱**　⊖（seqminusl）只砍最左邊一個；pool 與 queue 都是「序列」，允許重複。

<sub>`ch08-leftmost-removal-code`</sub>

---

### 8-2　Core c, with O = 8 and Q = 80. Before block N: α[c] = [x, y, x, z, w, u, v, t] (oldest on the left) and φ[c] = [q_0, …, q_79]. Block N has H_T = 171 and its E_G holds one guarantee for core c whose report has authorizer x; φ[c] is untouched in block N. Block N+1 has H_T = 173 and no guarantee for core c, but its accumulation calls `assign` on core c, replacing the queue with [r_0, …, r_79]. What is α[c] after block N+1?

<sub>8.2 Pool and Queue — ●●● · 計算 · eq. 8.2–8.3 & eq. 4.19</sub>

**標準答案**　[x, z, w, u, v, t, q_11, r_13]

eq. 8.2：α′[c] ≡ ←(F(c) ⌢ φ′[c][H_T]↺)^O，↺ 是 modulo 下標（§3：s[i]↺ ≡ s[i mod |s|]），所以取的 index = H_T mod Q = H_T mod 80；eq. 8.3：core c 有 guarantee 時 F(c) = α[c] ⊖ {w_a}（只移除最左邊一個），否則 F(c) = α[c]。Block N：F(c) = [x, y, x, z, w, u, v, t] ⊖ {x} = [y, x, z, w, u, v, t]（7 個）；171 mod 80 = 11 → append q_11 → [y, x, z, w, u, v, t, q_11]（8 個，不需截斷）。Block N+1：沒有 guarantee，F(c) 就是這 8 個；但 α′ 依賴的是 posterior φ′（eq. 4.19：α′ ≺ (H, E_G, φ′, α)；§8.2：「this step must be computed after accumulation」），本塊的 assign 已把 φ′[c] 換成 r，173 mod 80 = 13 → append r_13 → 9 個 → ←^8 保留最後 8 個，丟掉最舊的 y。順帶一提，block N 的 guarantee 之所以合法，是因為 eq. 11.32 以 prior α[c] 檢查 x ∈ α[c]。

**逐項辨析**

1. ❌ [x, z, w, u, v, t, q_11, q_13]  
   第二塊仍從 prior φ 取 q_13：α′ 依賴的是 posterior φ′，assign 已經把佇列換成 r。
2. ❌ [y, z, w, u, v, t, q_11, r_13]  
   第一塊把兩個 x 都移除了（#692 的 bug）；⊖ 只砍最左邊一個，被擠掉的應該是 y。
3. ❌ [x, z, w, u, v, t, q_3, r_5]  
   index 要對 Q = 80 取餘（171 → 11、173 → 13），拿 O = 8 取餘才會抓到 q_3、r_5。
4. ✅ [x, z, w, u, v, t, q_11, r_13]  
   ⊖ 一個 x、append q_11、再 append r_13 溢位後 ←^8 丟掉最舊的 y，剩下的正是這八項。

> **陷阱**　順序：先 ⊖（只對有 guarantee 的 core）→ 再 append φ′[c][H_T mod 80] → 再 ←^8 留最後 8 個；每塊每個 core 都 append，沒 guarantee 的滿 pool 每塊左移一格。

<sub>`ch08-pool-two-block-calc`</sub>
...
<sub>8.2 Pool and Queue — ●●● · 概念 · eq. 8.2, eq. 4.19 & eq. 11.32</sub>

**標準答案**　Invalid: eq. 11.32 requires w_a ∈ α[w_c] on the prior pool, and x only enters α′[c] through eq. 8.2 at the end of block N — so the first block whose E_G may carry a report authorized by x is N+1

eq. 11.32：∀w ∈ w：ρ‡[w_c] = ∅ ∧ w_a ∈ α[w_c]。依賴圖 eq. 4.19：α′ ≺ (H, E_G, φ′, α)，表示 E_G 是 α′ 的輸入而不是反過來——所以 x 要到 block N 結束時才進入 α′[c]，block N+1 的 prior α 才含 x。實務含意：assigner 想讓新 authorizer 可用，最快也是下一個 slot；而且 pool 每塊都會 append 一個 queue 項目，x 進 pool 後若一直沒被使用、pool 又沒被其他 guarantee 消耗，最多再 8 個區塊就會被 ←^O 擠掉，所以 queue 的安排要與預期的 package 提交時間對齊。你們 STF 的順序（ValidateExtrinsic → UpdateReports → UpdateAccumlate → UpdateAuthorizations）也正是先用 prior α 驗 guarantee、最後才產生 α′。

**逐項辨析**

1. ❌ Valid: α′ is deliberately computed after accumulation so that guarantees in the same block can already rely on freshly assigned authorizers, which is why §8.2 orders the pool update last  
   α′ 要在 accumulation 之後才算，那是 α′ 的產生順序，不是 guarantee 驗證所看的狀態。
2. ✅ Invalid: eq. 11.32 requires w_a ∈ α[w_c] on the prior pool, and x only enters α′[c] through eq. 8.2 at the end of block N — so the first block whose E_G may carry a report authorized by x is N+1  
   eq. 11.32 比的是 prior α（無 prime、無 dagger），x 要等 block N 結束才進得了 α′[c]。
3. ❌ Valid: the authorizer check accepts any hash present in α[c] or in φ′[c], since the queue is the source of the pool and the assigner has already approved that authorizer for the core  
   guarantee 驗證不看 φ：queue 只是 pool 的補給來源，本身不是授權依據。
4. ❌ Invalid, but for a different reason: a single block may not both alter a core's queue via `assign` and accept a guarantee on that core, so the guarantee is rejected regardless of x  
   GP 沒有「同一塊不能同時 assign 與 guarantee 同一個 core」這條規則。

> **陷阱**　guarantee 看 prior α；α′ 用 posterior φ′。新 authorizer 最早在「下一個區塊」才可用。

<sub>`ch08-new-authorizer-availability-timing`</sub>

---

### 8-4　Who is permitted to change a core's authorizer queue φ[c] in GP 0.8.0, and through what mechanism?

<sub>8.2 Pool and Queue — ●○○ · 概念 · §8.2 (note under eq. 8.1); assign host call (index 16)</sub>
...
1. ✅ Only the service currently registered as that core's assigner, and only from inside its accumulate execution, by calling `assign`, which replaces all Q = 80 entries of that one core's queue in a single call and may also hand the assigner role to another service  
   §8.2 明訂 φ 只能由 privileged service 的 accumulate 呼叫 assign 整批覆寫，並可順手轉手 χ_A[c]。
2. ❌ Any service may call `assign`, but the write only takes effect on cores whose pool has fallen empty; the manager service can override any core at any time with `bless`, and both writes land at the end of the block so that a core cannot be re-assigned twice in a row  
   Ω_A 的呼叫者 ≠ χ_A[c] 就回 HUH；bless 動的是 χ 的 privileges、碰不到 φ，也與 pool 空不空無關。
3. ❌ The block author, by including an authorization extrinsic alongside E_G as the sixth member of the extrinsic; the other validators re-check it against the coretime-sales record before accepting the block, and φ′ is then simply whatever that extrinsic declares  
   extrinsic 只有 E_T、E_P、E_G、E_A、E_D 五種，coretime 的成交紀錄也不是鏈上共識條件。
4. ❌ Nobody on-chain: φ is each node's local view of purchased coretime, gossiped off-chain by the coretime chain, and only a commitment to it ever reaches consensus — carried in the very same header marker that publishes the epoch's winning tickets  
   eq. 8.1 的 φ 是不折不扣的狀態元件（序列化在 C(2)）；eq. 5.1 的 marker 只有三個。

> **陷阱**　「誰能寫 φ」與「誰能寫 χ」不同：assign 由 per-core assigner 呼叫，bless 只屬於 manager。

<sub>`c3-ch08-queue-writer`</sub>

---

### 8-5　Each block moves exactly one queue entry into every core's pool. How is that entry picked, and what becomes of the entries belonging to slots in which no block was produced?

<sub>8.2 Pool and Queue — ●●○ · 概念 · eq. 8.1–8.2</sub>

**標準答案**　By cyclic subscription on the block's own timeslot, i.e. index H_T mod Q, with no stored cursor anywhere; a slot that produces no block simply never has its entry drawn, and that index only comes round again 80 slots later

eq. 8.2：∀c ∈ N_C : α′[c] ≡ ←(F(c) ⌢ φ′[c]↺[H_T])^O。notation.tex 定義 modulo subscription s↺[i] ≡ s[i mod |s|]，而 eq. 8.1 規定 φ ∈ ⟦⟦H⟧_Q⟧_C —— 每個 core 的 queue 是「剛好 Q = 80 個」的序列（相對地 α ∈ ⟦⟦H⟧_:O⟧_C 是「至多 O = 8 個」）。索引直接來自 header 的 timeslot，不是狀態裡的游標，所以分叉、空 slot、Safrole 沒出票的 slot 對應的那格這一輪就是被跳過；實作若自己維護 cursor，只要鏈上出現空 slot 就會與參考實作分歧。queue 不會因為被抽取而縮短或清空（它不是 FIFO），唯一能改它的是 assign 的整批覆寫。

**逐項辨析**

1. ✅ By cyclic subscription on the block's own timeslot, i.e. index H_T mod Q, with no stored cursor anywhere; a slot that produces no block simply never has its entry drawn, and that index only comes round again 80 slots later  
   索引由 H_T mod Q 當場算出、鏈上沒有游標，沒出塊的那一格這輪就跳過，80 個 slot 後才同餘回來。
2. ❌ By a per-core cursor kept beside the pool that advances one step per imported block, so no queue entry is ever skipped — an empty slot merely postpones the rotation, the cursor being serialized next to α[c] under state key C(1)  
   C(1) 只映射到 E([var(x) | x ∈ α])，附錄 D 沒有地方放游標；一旦出現空 slot 就會與參考實作分歧。
3. ❌ By index H_T mod O, so only the first eight queue positions ever rotate into the pool and the remaining 72 stay unreachable until an `assign` call shifts them forward, so an authorizer parked at index 40 stays out of reach until `assign` rotates the queue  
   O = 8 只出現在最後的 ←(…)^O 截斷；80 格每一格都會在 80 個 slot 內輪到，沒有碰不到的格子。
4. ❌ By taking the oldest not-yet-used entry (FIFO) and deleting it from the queue as it enters the pool, which is why the assigner must refill a core's queue at least every 80 blocks and why φ[c] is shorter than Q between refills  
   刪掉用過的項會讓 φ[c] 短於 Q，直接違反 eq. 8.1 的固定長度型別；queue 也不需要定期補。

> **陷阱**　抽 queue 用 mod Q（80），截 pool 用 O（8）——兩個常數各司其職，別互換。

<sub>`c3-ch08-rotation-index-cyclic`</sub>

---

### 8-6　A guarantor runs the Is-Authorized logic Ψ_I before refining anything. What is that program actually able to observe?
...
1. ✅ Its two arguments only — the work-package and the core index, the latter handed to the PVM as a 2-byte encoded argument — plus whatever `fetch` can pull out of that package, and the protocol constants; no chain state at all, and besides `fetch` the only host calls are the gas counter and heap growth  
   eq. B.2 的 F 只認 gas、grow_heap、fetch 三個 host call，GP 說 Ψ_I totally stateless。
2. ❌ The work-package plus a read-only snapshot of the service accounts δ, reached through the same `read` and `lookup` host calls Accumulate uses, so an authorizer can check a balance or confirm that the coretime buyer still holds the core it is running on; the snapshot is pinned to the anchor block so that every auditor sees identical values  
   read／lookup 在 eq. B.2 的 F 裡根本叫不出來，一律扣 M_∅ 並把 ω_7 設成 WHAT。
3. ❌ The work-package, the core index and a historical lookup pinned to the lookup-anchor timeslot, exactly as Refine gets it, since the authorization code is itself resolved by that same lookup; an authorizer may therefore read any preimage its host service had solicited by that slot, such as a whitelist published by the coretime buyer  
   歷史查詢 Λ 是 Ψ_R 的專利（eq. B.5 的 Ω_H）；auth code 由 lookup anchor 解析是外部餵進來的。
4. ❌ The work-package and the core's own authorizer pool, handed in beside the core index because it is at most O × 32 = 256 octets, so that the code can confirm its own hash is still pooled and abort early rather than burn the guarantor's gas on a package eq. 11.32 will reject anyway  
   eq. B.1 只有 (P, N_C) 兩個參數、argument blob 就是 E_2(c)；能讀 pool 就不再是 stateless。

> **陷阱**　Ψ_I 無狀態 ⇒ 授權邏輯不能讀鏈上餘額；要收錢只能靠 accumulate 那一端。

<sub>`c3-ch08-psi-i-visibility`</sub>

---

### 8-7　The pool α[c] stores nothing but 32-byte authorizer hashes. So where does a guarantor obtain the executable is-authorized code, and as of which point in history is it resolved — and why there?

<sub>8.1 Authorizers and Authorizations — ●●○ · 設計理由 · eq. 14.11; §8.1</sub>

**標準答案**　From the preimage store of the separate auth-code host service named by the work-package, resolved by a historical lookup at the package's lookup-anchor timeslot, so that an auditor re-running the check much later resolves byte-identical code even if that service has since replaced or forgotten the preimage

eq. 14.11：p_a ≡ H(p_u ⌢ p_f)（authorizer = auth code hash 與 config blob 串接後的 Blake2b，0.8.0 PR #522 把 ch.8 與 ch.14 的說法對齊），而 E(var(metadata), auth code) ≡ Λ(δ[p_h], (p_c)_t, p_u)，其中 p_u 是 auth code hash、p_h 是 auth code host service。三個重點：(1) 供應 code 的是 work-package 自己宣告的 auth code host service p_h，那是 eq. 14.2 裡與 work-item 的 service 分開的獨立欄位 —— 這正是「授權」與「工作」分離的具體長相：買 coretime 的人挑 authorizer，真正做事的是別的 service；(2) 時間點是 lookup anchor 的 timeslot (p_c)_t，因為 Λ 的設計目標是「在可能被 audit 的整段期間答案不變」，GP 用 D = L + 4,800 = 19,200 slots 的 expunge period 保證這件事；(3) α[c] 只有 32 bytes 的 hash，既沒有 config 也沒有 code，光靠它取不到程式。W_A = 64,000 是「解析出來的」is-authorized code 的大小上限（eq. B.1 的 BIG 條件）；bundle 自己的上限是 W_B ≈ 13.8 MB。

**逐項辨析**

1. ✅ From the preimage store of the separate auth-code host service named by the work-package, resolved by a historical lookup at the package's lookup-anchor timeslot, so that an auditor re-running the check much later resolves byte-identical code even if that service has since replaced or forgotten the preimage  
   Λ(δ[p_h], (p_c)_t, p_u)：由 package 自己宣告的 auth code host service 於 lookup anchor 時點解析。
2. ❌ From the account of the first work-item's service, read at the anchor block's posterior state, since the anchor is what already pins the refinement context to a recent state root every validator has agreed on — eq. 11.36 has matched that root against β†, so no separate retention rule is needed to keep the code resolvable  
   anchor 最多只有 8 個區塊深，而 audit 可能兩個 epoch 之後才發生，重跑會拿到不同的 code。
3. ❌ From the work-package itself: the code blob travels in the bundle beside the token and the configuration blob, which is exactly what the 64,000-octet limit bounds, so a guarantor needs no preimage lookup at all and the hash pooled in α[c] is simply the Blake2b of that code section  
   eq. 14.2 的 package 只帶 p_h、p_u、p_f；W_A = 64,000 是解析出來的 code 上限，不是 bundle 欄位。
4. ❌ From the pool entry: the hash in α[c] is the preimage hash of the code concatenated with its configuration, so a guarantor fetches that blob by hash from any peer and runs it against the current best head, which is why an authorizer only becomes usable once someone has published its preimage  
   α[c] 存的是 H(p_u ⌢ p_f)，本身不是任何 blob 的 preimage key；綁 best head 正是 Λ 要避免的漂移。

> **陷阱**　anchor 與 lookup anchor 是兩個不同的區塊：anchor 管 state/beefy/timeslot 比對，lookup anchor 管所有 preimage 的解析時點。

<sub>`c3-ch08-authcode-lookup-anchor`</sub>

---

### 8-8　This 0.7.2 code implements the authorizer half of eq. 11.32. Which pool is it obliged to read, and what is the protocol-level consequence when the membership test fails?
...
```
<sub>internal/extrinsic/guarantee_controller.go:172-186 (ValidateWorkReports; gas checks elided)</sub>

**標準答案**　The prior pool, since the posterior one is only formed after accumulation out of the posterior queue; and the failure is an ordinary block-validity failure — the guarantee cannot be part of a valid block, so there is no report to punish anyone over and nothing is written to the disputes state

eq. 11.32 寫的是 ∀r ∈ I : ρ‡[r_c] = ∅ ∧ r_a ∈ α[r_c] —— α 沒有 prime。順序上也只能如此：eq. 8.2 的 α′ 依賴 φ′，而 φ′ 是 accumulation 的產出，§8.2 明說 "practically speaking, this step must be computed after accumulation"，所以 guarantee 驗證的當下 α′ 根本還不存在。程式碼用 GetPriorStates().GetAlpha() 是對的，也因此本區塊 accumulation 才剛 assign 進 queue 的新 authorizer，最快要下一個區塊才可能被引用。至於後果：§11 的這些條件是「區塊有效性條件」，不滿足就整個區塊不合法，import 時直接拒（團隊實作回 CoreUnauthorized）。這是刻意的分工：culprit／fault 只處理需要離線資料（重跑 refine、拉 audit DA）才能判定的爭議，而 authorizer 在不在 pool 裡是任何節點光看鏈上狀態就能算出來的條件。順帶一提 ρ‡ 是 assurances 之後、guarantees 之前的中間態，這正是 core-engaged 檢查要用它而不是 ρ 的原因。

**逐項辨析**

1. ✅ The prior pool, since the posterior one is only formed after accumulation out of the posterior queue; and the failure is an ordinary block-validity failure — the guarantee cannot be part of a valid block, so there is no report to punish anyone over and nothing is written to the disputes state  
   eq. 11.32 寫的是 r_a ∈ α[r_c]——α 沒有 prime；不合法就整塊被拒，不會寫進 disputes。
2. ❌ The posterior pool, because the report is being included in this very block and eq. 8.2's rotation is applied before guarantees are processed; failure then invalidates the block, which is why the dependency graph lists α′ among the inputs of ρ′ rather than the other way round  
   α′ 依賴 φ′、要等 accumulation 之後才存在；§4 的 ρ′ ≺ (E_G, ρ‡, κ, τ′) 根本不含 α′。
3. ❌ Either pool will do, because the rotation only removes the authorizer the report itself consumed; failure records the report's guarantors as culprits in the disputes state so that the next epoch's punishment set slashes them, their Ed25519 keys entering ψ_O through the offenders marker of this very header  
   每塊每個 core 都會 append 一項 φ′[c]↺[H_T]，兩個 pool 幾乎必然不同；ψ_O 也只能經 E_D 增長。
4. ❌ The prior pool, but the report is still admitted into the availability assignments with an empty authorizer trace and is silently discarded at accumulation time, which is why an absent authorizer costs a guarantor nothing and ρ‡ holds the entry until the U = 5 slot assurance timeout clears it  
   eq. 11.46 只把通過全部檢查的 guarantee 填進 ρ′；ρ‡ 早在 guarantee 進來前就定案了。

> **陷阱**　「檢查用 prior α、更新用 posterior φ′」同一章兩個方向；面試常問「同一個區塊裡剛 assign 的 authorizer 能不能馬上用」——不能。

<sub>`c3-ch08-unauthorized-report-outcome`</sub>

---

### 8-9　What are the shapes of the authorizer pool α and authorizer queue φ?

<sub>8.2 Pool and Queue — ●○○ · 概念 · eq. 8.1</sub>

**標準答案**　α ∈ [[H]_{:O}]_C with O = 8 (up to 8 authorizer hashes per core); φ ∈ [[H]_Q]_C with Q = 80 (exactly 80 per core)

eq. 8.1：α ∈ [[H]_{:O}]_C，O = C_authpoolsize = 8（最多 8 個）；φ ∈ [[H]_Q]_C，Q = C_authqueuesize = 80（固定 80 個）。pool 是「現在可以用的 authorizer」，queue 是「未來每個 slot 依序補進 pool 的 authorizer」，rotation 取的是 H_T mod Q。φ 只能由 accumulate 中具 assigner 權限（χ_A[c]）的 service 透過 `assign` host call 修改。編碼上 C(1) 為 [var(x) | x ∈ α]、C(2) 為 E(φ)。

**逐項辨析**

1. ✅ α ∈ [[H]_{:O}]_C with O = 8 (up to 8 authorizer hashes per core); φ ∈ [[H]_Q]_C with Q = 80 (exactly 80 per core)  
   eq. 8.1：pool 是最多 O = 8 的變長序列，queue 是固定長度 Q = 80，兩個常數不互換。
2. ❌ α ∈ [[H]_{:Q}]_C with Q = 80 (up to 80 pooled per core); φ ∈ [[H]_O]_C with O = 8 (exactly 8 queued per core)  
   O 與 Q 對調了：pool 上限 8、queue 剛好 80，這是最常見的記憶錯亂。
3. ❌ α ∈ [H]_C — one live authorizer per core, replaced every block; φ ∈ [[H]_E]_C with E = 600, one queue entry per epoch slot  
   α[c] 是可容多個 hash 的序列（eq. 8.3 才需要移除最左邊一個）；queue 長度是獨立常數 Q，不隨 E 走。
4. ❌ α and φ are both dictionaries D⟨H → N_C⟩ from authorizer hash to core index, so no hash ever repeats and no per-core length bound is needed  
   與 eq. 8.1 的型別直接衝突：兩者都是 per-core 序列，同一 hash 在同一 pool 重複完全合法。

> **陷阱**　O = 8、Q = 80；pool 是 :O（可少於 8），queue 是固定長度 Q。

<sub>`ch08-pool-queue-sizes`</sub>

---

### 8-10　Core c has pool α[c] = [a, b, a, d] (left = oldest) and a guarantee in E_G for core c whose report has authorizer a. With φ′[c][H_T mod Q] = x and O = 8, what is α′[c]?

<sub>8.2 Pool and Queue — ●●● · 計算 · eq. 8.2–8.3</sub>
...
### 8-11　How is an authorizer identified in GP 0.8.0, and where is the authorization decision actually made?
...
1. ✅ Authorizer = H(PVM code hash ⌢ configuration blob); the is-authorized decision is made entirely in-core by the guarantors (Ψ_I), while on-chain logic only checks that the authorizer is in the core's pool  
   §8.1 原文：authorizer 是 PVM code hash ⌢ config blob 的雜湊；鏈上只驗 w_a ∈ α[w_c]。
2. ❌ Authorizer = the is-authorized code hash alone, with the configuration blob passed to Ψ_I separately; the block author runs that code while building the block and other validators trust the trace it publishes  
   eq. 14.11 的 p_a ≡ H(p_u ⌢ p_f) 已把 config 包進雜湊，且出塊者不會在鏈上跑 authorizer。
3. ❌ Authorizer = H(token ⌢ trace), so the pool entry commits both to the argument supplied with the package and to the output it produced; every validator therefore re-runs Ψ_I at block import to recompute that hash  
   把識別碼與執行結果搞混：authorizer 必須在任何執行之前就能拿去和 α[c] 比對。
4. ❌ Authorizer = the Ed25519 key of the coretime purchaser, so α[c] is a list of up to O = 8 buyer keys and guarantors verify a signature by one of them over the work-package hash rather than executing any PVM code  
   eq. 8.1 的 α 裝的是 hash；§8.1 定義 authorizer 是跑在 G_I = 50M gas 下的 PVM 邏輯，不是驗簽。

> **陷阱**　on-chain 不執行 authorizer code；只查 pool。

<sub>`ch08-authorizer-identity`</sub>

---

### 8-12　What motivation does the GP give for the authorization system?

<sub>8 Authorization — ●○○ · 設計理由 · §8 intro</sub>

**標準答案**　To disentangle the intention of using some coretime from the specification and submission of a particular workload, so that JAM can support both Ethereum-style and Polkadot-style interaction patterns

§8 intro：Ethereum 的 gas 由交易作者當場購買（購買者 = 作者）；Polkadot 的 parachain slot 由團隊長期租用（購買者通常 ≠ 出塊者）。「On a principle of flexibility, we would wish JAM capable of supporting a range of interaction patterns both Ethereum-style and Polkadot-style… we introduce the authorization system, a means of disentangling the intention of usage for some coretime from the specification and submission of a particular workload」——這就是「買 coretime 的人」與「提交 work-package 的人」可以不同人的來源，連結到 §4.9.2 coretime 與 §14 的 is-authorized。

**逐項辨析**

1. ✅ To disentangle the intention of using some coretime from the specification and submission of a particular workload, so that JAM can support both Ethereum-style and Polkadot-style interaction patterns  
   §8 intro 的 disentangling the intention of usage…，買 coretime 與送 package 可分離。
2. ❌ To give each core its own gas market: guarantors bid for coretime block by block and the clearing price is recorded next to the authorizer hash in α[c], so a congested core costs more to use  
   eq. 8.1 的 α 只裝 32-byte hash、沒有價格欄位，G_I / G_R 是協定常數，鏈上沒有競標。
3. ❌ To replace Grandpa's finality votes with per-core authorizer votes: a work-report is final once more than two thirds of the authorizers pooled for its core have signed it, which is what the bound O = 8 sizes  
   authorizer 是一段 PVM 邏輯而不是簽名者；三分之二門檻屬於 availability assurance，與它無關。
4. ❌ To cap how much work one service may submit: the pool's O = 8 entries act as a per-service rate limiter, so a service that already had a package reported this timeslot cannot have a second one guaranteed on any core  
   α 是 per-core 而非 per-service，協定並未限制單一 service 能送幾份 package。

> **陷阱**　設計理念題常見；連結到 §4.9.2 coretime 與 §14 的 is-authorized。

<sub>`ch08-why-authorization`</sub>
...
## §9 Service Accounts　<sub>14 題</sub>
...
<sub>9.2 Preimage Lookups — ●○○ · 設計理由 · §9.2 (domain of Λ); App. B Refine Invocation (D ≡ L + 4,800 = 19,200)</sub>

**標準答案**　Ω_H must return the same answer at every moment when auditing may still occur; the lookup anchor may itself be up to L = 14,400 timeslots older than recent history, so the period is that anchor age plus a further 4,800 slots (eight hours) of safety margin.

GP 附錄 B（Refine Invocation）原文：「The historical-lookup host-call function, Ω_H, is designed to give the same result regardless of the state of the chain for any time when auditing may occur … The lookup anchor may be up to L timeslots before the recent history and therefore adds to the potential age at the time of audit. We therefore set D to have a safety margin of eight hours: D ≡ L + 4,800 = 19,200。」L = 14,400（max lookup anchor age，24 小時），加 4,800 slot（8 小時）＝ 19,200 slot（32 小時）。§9.2 的 Λ 定義域正是 (H_t − D … H_t)。這題考的是「理由」而不是算術：D 的來源是 lookup anchor 的最大年齡加上安全邊際，凡是把它歸給別的保存期的說法，即使數字湊得上也不是 GP 寫的那條式子。

**逐項辨析**

1. ❌ 19,200 timeslots is exactly 32 epochs, which is precisely how long the accumulation history ξ keeps work-package hashes, so a preimage may not be dropped before the work-package that used it can no longer be re-accumulated; D is fixed at 32·E for that reason.  
   19,200 / E = 32 只是數值巧合；ξ 只保存一個 epoch 的 package hashes（eq. 12.1），與 D 無關。
2. ❌ The period must exceed the two-epoch audit window (1,200 timeslots) by a factor of sixteen so that erasure-coded bundles can still be reconstructed from the 1,023 validators' shards during a dispute; D is derived from that retention period, not from any anchor age.  
   推導方向反了：驅動 D 的是 lookup anchor 的年齡，不是 dispute 期間的 erasure 重建。
3. ✅ Ω_H must return the same answer at every moment when auditing may still occur; the lookup anchor may itself be up to L = 14,400 timeslots older than recent history, so the period is that anchor age plus a further 4,800 slots (eight hours) of safety margin.  
   附錄 B 直接寫出 D ≡ L + 4,800 = 19,200，理由就是 Ω_H 在任何可審計時點都要給同一個答案。
4. ❌ The per-octet deposit B_L is refunded linearly over the period, and 19,200 slots is 32 hours at six seconds per slot — the standard deposit-refund schedule for elective state; B_L is credited back to a_b in equal instalments over those slots.  
   GP 沒有 deposit 攤提退款機制；a_t 是門檻式檢查，餘額只透過 transfer 與 accumulate 改變。

> **陷阱**　D 只是「forget 之後多久才能真的刪」；真正把 preimage 從 state 移除的動作在 `forget`（自家，Ω_F = 25）與 `eject`（別家，Ω_J = 22）兩個 accumulate host call 裡——注意別跟 0.8.0 的 `expunge`（Ω_X = 14）混淆，那是 refine-only 的 inner-PVM 拆除呼叫，與 preimage 無關。preimage 生命週期的狀態機本身在 §9.2.2。

<sub>`c3-ch09-expunge-delay`</sub>
...
### 9-3　This is the team's Ω_W after the fix for the 'write mutates StorageDict before the balance check' bug. Which statement matches GP 0.8.0?

<sub>9.3 Account Footprint and Threshold Balance — ●●○ · 程式碼 · eq. 9.8 (a_i, a_o, a_t); App. B `write` (Ω_W)</sub>

```go
// remove the old item's footprint, then add the new one's
...
```
<sub>PVM/host_call_general.go (write) + internal/service_account/service_account.go:207 (CalcStorageItemfootprint)</sub>

**標準答案**　One storage entry costs 1 against a_i and 34 + |k| + |v| against a_o — both the key and the value are charged; when the post-write threshold exceeds the balance the call yields FULL and the account must be returned completely untouched, which is why the map write is deferred until after the comparison.

eq. 9.8：a_i ≡ 2·|a_l| + |a_s|；a_o ≡ Σ_{(h,z)∈K(a_l)} (81 + z) + Σ_{(x,y)∈a_s} (34 + |y| + |x|)；a_t ≡ max(0, B_S + B_I·a_i + B_L·a_o − a_f)。求和是對 a_s 的「pair」跑——storage 一筆 = 1 item、34 + |k| + |v| octets；lookup request 一筆 = 2 items、81 + z octets。附錄 B 的 Ω_W：「⟨continue, FULL, s⟩ otherwhen a_t > a_b」——回傳的第三個元素是 **s**（原封不動的帳戶），不是被改過的 a；#979/#980 這個 bug 正是因為 Go map 是 reference，先寫再檢查會讓 FULL 分支殘留寫入，fuzzer seed 3785638964 step 15419 抓到。程式碼把 map 寫入延到比較之後，就是為了滿足這條「失敗即不留下任何狀態變更」的要求。

**逐項辨析**

1. ❌ One storage entry costs 1 against a_i and 32 + |v| against a_o — key length is never charged, because JAM stores only the hash of a storage key in the trie; on FULL the write still lands and the shortfall is taken out of the balance.  
   eq. 9.8 的求和是對 (x, y) 這一對跑，key 長度同樣計費；而且 FULL 分支不得留下任何寫入。
2. ❌ One storage entry costs 2 against a_i and 81 + |v| against a_o, exactly like a lookup-meta entry, on the grounds that both occupy precisely one leaf of the state trie; FULL is only ever produced once the balance itself has reached zero, so a solvent service can always complete a write.  
   把 lookup request 的 2 items / 81 + z 套到 storage 上；FULL 的條件是 a_t > a_b，不是餘額歸零。
3. ❌ The threshold is recomputed once at the end of accumulation rather than on every host call, so `write` can never yield FULL at all; services that over-commit their storage are pruned afterwards by whichever service holds their code-hash tombstone, and their deposit is returned to the parent.  
   門檻檢查是每次 host call 當場做的，Ω_W 明列 FULL 分支；GP 也沒有 tombstone 剪枝或退款機制。
4. ✅ One storage entry costs 1 against a_i and 34 + |k| + |v| against a_o — both the key and the value are charged; when the post-write threshold exceeds the balance the call yields FULL and the account must be returned completely untouched, which is why the map write is deferred until after the comparison.  
   §9.8 的計價是 34 + |key| + |value|，key 的長度也算，且 FULL 時必須回傳原封不動的 s。

> **陷阱**　FULL 與 CASH 不同：a_t > a_b 用 FULL（write / solicit）；轉帳後自己跌破 a_t 用 CASH（transfer / new）。兩者都不留下任何狀態變更。

<sub>`c3-ch09-write-threshold-go`</sub>

---

### 9-4　A service calls `forget` at time t for one of its own request keys (h, z). Taking the four shapes of a_l[(h, z)] in turn, which set of transitions matches GP 0.8.0?

<sub>9.2.2 Semantics — ●●○ · 概念 · §9.2.2 (four shapes of a_l); App. B `forget` (Ω_F), expunge period D = 19,200</sub>

**標準答案**　[] → the request entry is dropped; [x] → [x, t]; [x, y] → the request entry and a_p[h] are both expunged, but only once y < t − D; [x, y, w] → [w, t], again only once y < t − D; every other case returns HUH.

附錄 B 的 Ω_F（`forget` = 25）逐條列出 a = self except：(i) 當 a_l[(h,z)] 是 []（無條件），或是 [x, y] 且 y < t − D，同時從 K(a_l) 移除 (h,z)、從 K(a_p) 移除 h；(ii) 當 a_l[(h,z)] = [x]，改寫成 [x, t]；(iii) 當 a_l[(h,z)] = [x,y,w] 且 y < t − D，改寫成 [w, t]；其餘 → error → HUH。這裡要點名一個 GP 的 under-specification：原文只寫了一條 guard「(x_s)_l[h,z] ∈ {[], [x,y]}, y < t − D」，把兩種形狀併在同一個集合裡，但 [] 這一支根本沒有任何東西綁定 y，年齡條件在該支懸空、無從判定；唯一可實作、也就是本題所鍵的讀法，是把 [] 這個 case 直接刪掉、完全不加等待期——[] 代表「已 solicit、從未供應」，若它也要等 D，solicit 之後就永遠取消不掉、[] 會永久卡在 state 裡——所以 D 的等待期實際上只約束 [x, y] 與 [x, y, w] 這兩支。GP 0.8.0 對此並未明文釐清，這是已知的 under-specification。對照 §9.2.2 的四種語意：[] requested、[x] available since x、[x,y] 自 y 起 unavailable、[x,y,z] 自 z 起再度 available。貫穿全題的原則是「available 的東西不能一步刪掉」——它必須先降級成 [x, t]，等 D = 19,200 slot 後才可能被真正 expunge，這正是 §9.2 開頭說的「it goes through a process of being marked as unavailable, and only after a period of time may it be removed from state. This ensures that historical information on its existence is retained.」否則 in-core historical_lookup 就不再確定（auditor 在 lookup anchor 時點重算會拿到不同答案），服務也就能隨時自我抹除歷史。至於 [x,y,w] → [w, t]：舊的 [x,y] 那段歷史被丟掉，因為它已經超過 D、不再需要保存，服務仍然持有那份 preimage 且自 w 起可用。

**逐項辨析**

1. ✅ [] → the request entry is dropped; [x] → [x, t]; [x, y] → the request entry and a_p[h] are both expunged, but only once y < t − D; [x, y, w] → [w, t], again only once y < t − D; every other case returns HUH.  
   四支轉移與 Ω_F 逐條相符，且 D 的等待期只綁在 [x, y] 與 [x, y, w] 這兩支上。
2. ❌ [] → HUH, there is nothing to forget; [x] → the request entry and a_p[h] are both dropped at once; [x, y] → [x, y, t]; [x, y, w] → [w, t] with no age condition attached; every other case returns HUH.  
   [x, y] → [x, y, t] 是 `solicit` 的動作；available 的 preimage 也不能一步刪，[x, y, w] 那支同樣要 y < t − D。
3. ❌ [] → the request entry is dropped; [x] → [x, t]; [x, y] → the request entry and a_p[h] are expunged the moment y < t, with no D-slot wait; [x, y, w] → dropped outright rather than rewritten; every other case returns HUH.  
   少了 D 個 slot 的等待期會破壞 historical_lookup 的確定性；[x, y, w] 是被改寫成 [w, t] 而非整筆移除。
4. ❌ [] → dropped; [x] → dropped together with a_p[h]; [x, y] → dropped together with a_p[h]; [x, y, w] → dropped together with a_p[h]; every shape collapses at once and the D-slot delay governs only `eject`.  
   §9.2 開頭明說「only after a period of time may it be removed from state」，`forget` 自己就帶這道等待期。

> **陷阱**　`forget` 不看餘額，`solicit` 才有 FULL 檢查；反向的 `solicit` 只接受兩種輸入形狀（不存在 → []、[x,y] → [x,y,t]），其餘一律 HUH。

<sub>`c3-ch09-forget-lifecycle`</sub>
...
**標準答案**　The context's next free id starts as check((E⁻¹_4(H(E(s, η′_0, H_t))) mod (2^32 − S − 2^8)) + S) with S = 2^16, so it is folded into the public range only; check linearly probes forward, i ↦ (i − S + 1) mod (2^32 − 2^8 − S) + S, until it lands on an index outside K(δ); the registrar alone may instead name any index below S.

附錄 B 的 initializer I：a（next free id）= check((E⁻¹_4(H(E(s, η′_0, H_t))) mod (2^32 − S − 2^8)) + S)，用的是**posterior** entropy accumulator η′_0 與本區塊的 H_t；模數刻意扣掉 S = C_minpublicindex = 2^16（registrar 專屬的 protected range）與最高的 2^8，所以隨機落點一定在 public range。eq. B.14 的 check(i) = i 若 i ∉ K(δ)，否則 check((i − S + 1) mod (2^32 − 2^8 − S) + S)——就是往前線性探測到第一個空位。成功 `new` 之後 context 的下一個 id 再跳 42：i* = check(S + (a − S + 42) mod (2^32 − S − 2^8))。registrar 路徑另走一條：當 caller = χ_R 且要求的 id < S，直接用該 id（已被占用則 FULL）。GP 也補了一句保險：萬一同一個 index 真的被兩個 service 取走，「the block is considered invalid」，但「Since no service can predict the identifier sequence ahead of time, they cannot intentionally disadvantage the block author.」——不可預測性正是這個設計要守住的性質。

**逐項辨析**

1. ❌ The index is Blake2b of the new account's code hash reduced mod 2^32, so that identical code always lands on the same slot and duplicate deployments are cheap; there is no probing at all, and should that index already be occupied the whole block is rejected as invalid.  
   以 code hash 為種子會讓相同 code 的服務互撞，而 GP 給的是線性探測而非直接判區塊無效。
2. ❌ The index is the largest key currently present in δ plus one, so indices are handed out strictly sequentially across the whole chain and a clash is impossible by construction; the registrar merely reserves the first 2^16 of them at genesis and hands them out off-chain.  
   純遞增計數器在 parallel accumulation（Δ*）下無法決定性合併；protected range 也不是鏈下發放。
3. ✅ The context's next free id starts as check((E⁻¹_4(H(E(s, η′_0, H_t))) mod (2^32 − S − 2^8)) + S) with S = 2^16, so it is folded into the public range only; check linearly probes forward, i ↦ (i − S + 1) mod (2^32 − 2^8 − S) + S, until it lands on an index outside K(δ); the registrar alone may instead name any index below S.  
   種子是 H(E(s, η′_0, H_t))，模數扣掉 S 與最高 2^8 折進 public range，check 再往前線性探測。
4. ❌ The index is derived from H(E(s, η_0, H_t)) using the prior entropy accumulator and then probed downwards until a free slot appears; the reserved block is the top 2^16 of the index space rather than the bottom, and any service — not merely the registrar — may claim one of those reserved slots.  
   用 prior η_0 會讓 index 在區塊執行前就可預測；保留區是最低的 2^16，而且只有 registrar 能指定。

> **陷阱**　η′_0（posterior）不是 η_0；並且 `new` 只在 accumulate 內可用，refine 沒有 δ。

<sub>`c3-ch09-new-service-index`</sub>

---

### 9-6　Your Go node is on GP 0.7.2, where the 'Owned Privileges' model let each privileged service rewrite its own slot of χ. What does GP 0.8.0 change, and who may hand a brand-new account a non-zero gratis storage offset a_f?

<sub>9.4 Service Privileges — ●●● · 版本差異 · eq. 9.9–9.10 (χ); App. B `bless` (Ω_B), `assign` (Ω_A), `new` (Ω_N) · ⚠ 0.7.2→0.8.0</sub>

**標準答案**　Ω_B is the one host call that rewrites (χ_M, χ_A, χ_V, χ_R, χ_Z) wholesale, and it now yields HUH unless the caller is χ_M itself, so no service can promote itself to manager; a core's assigner may still hand over its own χ_A[c] through `assign`, and `new` yields HUH whenever f ≠ 0 and the caller is not the manager.

GP 0.8.0（PR #519「Restrict bless to manager service」）在 Ω_B 加了一條守衛：「⟨continue, HUH, …⟩ otherwhen x_s ≠ (x_e)_m」（x_s 是呼叫者的 service index，x_e 是 invocation context 裡的 partial state）——也就是呼叫者不是 χ_M 就整組 HUH。§9.4 原文也改成 χ_M「is the service able to effect an alteration of χ from block to block as well as bestow services with storage deposit credits」。0.7.1 的 Owned Privileges（#475）讓每個特權服務改自己那格，攻擊面是：某服務先 bless 自己成 manager，再用 `new` 配 gratis storage、或把自己設成 registrar 去搶 < S 的低位 index。要留意 0.8.0 並沒有把「自有權」全部收回：Ω_A（`assign`）仍然要求 x_s = (x_e)_a[c] 才能寫 φ[c]，而且它同時寫回 χ_A[c]，所以 assigner 依然能把自己那一核的權限交棒；Ω_D（`designate`）則只要求 x_s = (x_e)_v，能改 ι 但改不了 χ_V。gratis 的守衛則在 Ω_N：「⟨continue, HUH, …⟩ otherwhen f ≠ 0 ∧ x_s ≠ (x_e)_m」。

**逐項辨析**

1. ❌ Ω_B may still be invoked by any of the five privileged services, each rewriting only the slot it owns; the manager is special solely in that it may grant storage deposit credits, and `new` accepts f ≠ 0 from either the manager or the registrar.  
   這是 0.7.1 的 Owned Privileges；0.8.0 的 Ω_B 已限定呼叫者必須是 χ_M，gratis 也只認 manager。
2. ❌ Ω_B is open to any service whose own index sits below S = 2^16, since occupying the protected range is what confers privilege in the first place; χ_A[c] may then only be moved by the manager, and a service raises its own a_f through `upgrade`.  
   低 index 只是 registrar 能指定的保留區、與特權無關；Ω_U 只能改 a_c/a_g/a_m，碰不到 a_f。
3. ❌ χ is no longer mutable by host calls at all in 0.8.0; it may only be rewritten by the services listed in χ_Z as part of the automatic accumulation they receive in every block, and a_f is fixed for every account at genesis and can never be raised afterwards by anyone, manager included.  
   χ_Z 只是「每個區塊自動 accumulate 並配基本 gas」的字典，沒有任何寫 χ 的能力。
4. ✅ Ω_B is the one host call that rewrites (χ_M, χ_A, χ_V, χ_R, χ_Z) wholesale, and it now yields HUH unless the caller is χ_M itself, so no service can promote itself to manager; a core's assigner may still hand over its own χ_A[c] through `assign`, and `new` yields HUH whenever f ≠ 0 and the caller is not the manager.  
   #519 在 Ω_B 加上 x_s ≠ (x_e)_m → HUH，而 Ω_N 的 gratis 守衛是 f ≠ 0 ∧ x_s ≠ (x_e)_m。

> **陷阱**　0.7.2 → 0.8.0 的一句話：bless 只剩 manager 能叫；但 assign 仍保留 per-core 的自有權。

<sub>`c3-ch09-privilege-mutation`</sub>

---

### 9-7　Which of the following is NOT a field of a service account A in GP 0.8.0 (eq. 9.3)?

<sub>9 Service Accounts — ●●○ · 概念 · eq. 9.3</sub>

**標準答案**　a_n — a nonce, incremented once on every accumulation of the service
...
1. ❌ a_f — the gratis storage offset, a balance-denominated deposit credit  
   a_f 確實列在 eq. 9.3，就是 a_t = max(0, … − a_f) 裡被扣掉的那筆 deposit credit。
2. ❌ a_p — the parent service index, i.e. the service that created this one  
   a_p（parent）是 eq. 9.3 的欄位之一，記錄建立這個帳戶的 service index。
3. ✅ a_n — a nonce, incremented once on every accumulation of the service  
   service 不由私鑰控制、系統裡也沒有使用者交易，eq. 9.3 因此沒有 nonce 這一欄。
4. ❌ a_m — the minimum gas required for each deferred transfer (minmemogas)  
   a_m 是 §9.1 的「每筆 deferred transfer 的最低 gas」，確實在 eq. 9.3 的欄位表裡。

> **陷阱**　兩個 gas 下限：a_g 每個 work-item 的 accumulate 最低 gas；a_m 每筆 deferred transfer 的最低 gas。

<sub>`ch09-account-fields`</sub>

---

### 9-8　How are a service's code a_c and metadata a_m derived from its code hash?

<sub>9.1 Code and Gas — ●●○ · 概念 · eq. 9.4</sub>

**標準答案**　The preimage of a_c in the account's own lookup a_p must decode as E(var(m), c) — a var-length metadata blob followed by the code; otherwise both are ∅

eq. 9.4：(a_m, a_c) = (m, c) 當 E(var(m), c) = a_p[a_c]，否則 (∅, ∅)。即 code hash 指向的、且必須在服務自己的 preimage lookup a_p 裡的那份 preimage，內容是「var-length metadata ⌢ code」的編碼；metadata 是 opaque（JIP-6 討論標準化）。code 走 preimage 而不走 storage 的理由在 §9.2：只有 preimage 是「available also in-core」的那一種，refine 根本讀不到 a_s。eq. 9.6 的不變式 (h ↦ d) ∈ a_p ⟹ h = H(d) 則固定了雜湊的對象；preimage 不在 state 裡，服務就是不能執行，refine 會回 BAD（eq. 11.7 的 BAD = code unavailable at lookup anchor）。entry points：0 = refine（in-core）、1 = accumulate（on-chain）（0.7.1 起 on_transfer 併入 accumulate）。

**逐項辨析**

1. ✅ The preimage of a_c in the account's own lookup a_p must decode as E(var(m), c) — a var-length metadata blob followed by the code; otherwise both are ∅  
   eq. 9.4：(a_m, a_c) = (m, c) 當 E(var(m), c) = a_p[a_c]，兩者同來自這一份 preimage。
2. ❌ The code sits directly in the storage dictionary a_s under the key a_c, with the metadata under a_c ⌢ [0]; a missing entry leaves the service on its previous code  
   a_s 是 accumulate 才寫得到、refine 讀不到的 on-chain 存儲；查不到時給 (∅, ∅)，沒有沿用舊 code。
3. ❌ The preimage is fetched from the manager service χ_M's lookup, so all service code is held centrally; the leading 32 octets of that blob are the metadata  
   χ_M 的權力限於改 χ 與發 gratis credit，code 一律取自服務自己的 a_p；metadata 也不是固定 32 octets。
4. ❌ a_c is the Blake2b of the raw code alone and the metadata lives in the storage dictionary a_s; a missing preimage still leaves the code resolvable from its hash  
   a_c 雜湊的是整個 E(var(m), c) 編碼而非裸 code，且 hash 反推不出 preimage。

> **陷阱**　`new` 會在 requests 裡放 (c, l) → []，等待 preimage 被提供。

<sub>`ch09-code-metadata`</sub>

---

### 9-9　A request entry a_l[(h, len)] holds a sequence of up to 3 timeslots. What does [x, y] mean, and what does [x, y, z] mean?
...
1. ✅ [x, y]: the preimage was available from x and is unavailable since y; [x, y, z]: it was available from x until y and is available again since z  
   兩個形狀都對上 §9.2.2：第二個時刻是轉為 unavailable，第三個是再次可用的起點。
2. ❌ [x, y]: the preimage was requested at x and was first supplied at y; [x, y, z]: supplied at y and then expunged from state entirely at z  
   序列裡不記 request 時刻——requested 的狀態就是空序列 []；z 標記的也不是刪除。
3. ❌ [x, y]: the preimage was available from x and was expunged from state at y; [x, y, z]: an illegal shape that no host call can ever produce  
   [x, y] 只是標成 unavailable，preimage 仍留在 a_p；[x, y, z] 是 §9.2.2 明列的合法形狀。
4. ❌ [x, y]: two distinct services requested the same preimage, one at x and one at y; [x, y, z]: three distinct services each requested it  
   a_l 掛在單一服務帳戶底下，鍵是 (h, len)、值是這個服務自己的可用性歷史，不記請求者身分。

> **陷阱**　再次 solicit 一個 [x,y] 會變 [x,y,t]；forget 一個 [x] 變 [x,t]；forget [x,y]（y 夠舊）才真正刪除。

<sub>`ch09-lookup-status`</sub>

---

### 9-10　A service has preimage p (hash h, length 40) with a_l[(h, 40)] = [100, 250, 400]. Which statement about the historical lookup Λ(a, t, h) is correct?

<sub>9.2 Preimage Lookups — ●●○ · 計算 · eq. 9.7 (Λ)</sub>

**標準答案**　Λ returns p for t = 120 and t = 450, but ∅ for t = 300

eq. 9.7：[x,y,z] 的條件是 x ≤ t < y ∨ z ≤ t。逐一代入：t=120：100 ≤ 120 < 250 ✓；t=300：不在 [100,250) 且 300 < 400 ✗；t=450：450 ≥ 400 ✓。Λ 還要求 h ∈ keys(a_p)（preimage 還在 state 裡），而 t 的定義域是 (H_T − D) … H_T，D = 19,200 slots（32 小時）——超過這個期間的 preimage 可能已被 expunge。
...
### 9-11　How are the footprint a_i (items) and a_o (octets) and the threshold balance a_t defined?
...
### 9-12　A service has one preimage request of declared length 100, two storage entries each with a 4-octet key and a 10-octet value, and gratis a_f = 0. What is its threshold balance a_t?

<sub>9.3 Account Footprint and Threshold Balance — ●●○ · 計算 · eq. 9.8 with B_S=100, B_I=10, B_L=1</sub>

**標準答案**　417

eq. 9.8 逐步代入：a_i = 2·|a_l| + |a_s| = 2·1 + 2 = 4；a_o = (81 + 100) + 2·(34 + 4 + 10) = 181 + 96 = 277；a_t = max(0, B_S + B_I·a_i + B_L·a_o − a_f) = max(0, 100 + 10·4 + 1·277 − 0) = 417。三個係數 B_S = 100、B_I = 10、B_L = 1 出自 appendix I；81（每個 request）與 34（每個 storage entry）的固定開銷寫在 eq. 9.8 裡，而 storage 的 key 長度 |x| 同樣計費。

**逐項辨析**

1. ✅ 417  
   a_i = 2·1 + 2 = 4、a_o = 181 + 96 = 277，100 + 40 + 277 恰好是這個數。
2. ❌ 407  
   比正解少 10：把 preimage request 只算 1 個 item（a_i = 3）的結果。
3. ❌ 336  
   漏掉 request 的 81 octets 固定開銷，a_o 只剩 196 就會落到這裡。
4. ❌ 349  
   漏掉 storage 每筆 34 octets 的固定開銷，a_o 只剩 209 就會落到這裡。

> **陷阱**　面試可能請你當場算 threshold；記住 81/34 兩個常數。

<sub>`ch09-threshold-calc`</sub>

---

### 9-13　The privileges state χ ≡ (χ_M, χ_V, χ_R, χ_A, χ_Z). Which mapping of privilege → power is correct?
...
1. ✅ χ_M manager: may alter χ (bless) and grant gratis storage; χ_V delegator: may set ι (designate); χ_R registrar: may create services with indices below S = 2^16; χ_A assigners (one per core): may set φ[c] (assign); χ_Z: services that accumulate every block with a fixed gas allowance  
   完全對上 §9.4：manager 改 χ 兼發 gratis、delegator 設 ι、registrar 開 protected range、assigner 逐 core 改 φ[c]。
2. ❌ χ_M manager: may set ι (designate); χ_V delegator: may alter χ (bless); χ_R registrar: may set φ[c] (assign); χ_A assigners (one per core): may create services with indices below S = 2^16; χ_Z: services exempt from gas metering while they accumulate  
   四種權力的持有者被互相對調；而 χ_Z 拿到的是每塊一份固定 gas 額度，一樣照 PVM 計量，不是免計 gas。
3. ❌ χ_M: the service that pays the accumulation gas of every other service; χ_V: the current active validator set κ; χ_R: the service that registers preimages on behalf of others; χ_A: the auditors drawn afresh in each tranche; χ_Z: services allowed to hold a zero balance  
...
### 9-14　The GP lists three differences between preimage lookups and general storage. Which is NOT one of them?
...
1. ❌ Preimage data is supplied extrinsically (via E_P), whereas storage data originates from the service's own accumulation  
   這是 §9.2 列出的差異之一：preimage 由 E_P 外部提供，storage 由服務自己的 accumulate 寫入。
2. ❌ Preimages map a hash to its preimage, whereas storage maps arbitrary keys to values  
   這是 §9.2 的第一個差異：一邊是 hash 對應其 preimage，一邊是任意 key 對 value。
3. ✅ Preimages may be removed instantly by the service, whereas storage entries are retained for 28 days  
   方向剛好說反：GP 說的是 preimage「may not be removed freely」，被綁住的是 preimage 不是 storage。
4. ❌ Preimage data, once supplied, goes through an 'unavailable' period before it may be removed, so that its historical availability is retained  
   這正是 §9.2 的第三個差異：供應後要先標成 unavailable，過一段時間才能移除。

> **陷阱**　forget 後要等 D = 19,200 slots 才能真正刪除。

<sub>`ch09-preimage-vs-storage`</sub>
...
```
<sub>internal/extrinsic/verdict_controller.go (VerdictWrapper.VerifySignature)</sub>
...
1. ❌ Age = 1: because the check compares Age against both a and a−1 symmetrically, the next epoch's index also passes and the judgments are verified against κ as if the epoch had already rolled over  
   Age = 1 既不等於 a = 0 也不等於 wrap 後的 0xFFFFFFFF，這個輸入照樣會被擋下。
2. ❌ Age = 0 with judgments signed by λ keys: since a−1 cannot be matched at genesis, the code falls through to the λ branch and accepts λ signatures for the current epoch  
   Age = 0 走的是 `Age == a` 分支 → 用 κ 驗簽，不會落到 λ。
3. ✅ Age = 2^32 − 1: `a-1` wraps around in U32, so the age check passes and the judgments are verified against λ, whereas ⌊τ/E⌋ − N_2 has no previous epoch at epoch 0  
   a 宣告成 U32，a = 0 時 a-1 wrap 成 4294967295，而 Age 以 E_4 編碼，fuzzer 做得出這個值。
4. ❌ None: the wrapped value of `a-1` can never equal a decoded 32-bit Age, so at genesis only Age = 0 passes and the code is exactly the GP rule  
   U32 之間的比較就是會相等，wrap 後的值完全可能命中解碼出來的 Age。

> **陷阱**　用 unsigned 減 1 之前先想 a = 0；GP 的 ⌊τ/E⌋ − N_2 沒有負數，也沒有 2^32 − 1。

<sub>`ch10-code-genesis-age-underflow`</sub>
...
```
<sub>internal/extrinsic/fault_controller.go (FaultController.VerifyReportHashValidty)</sub>

**標準答案**　Invalid: the equivalence forces r into exactly one of ψ′_B, ψ′_G, so a wonky report admits no fault for either v; the code nevertheless accepts it, since it only rejects (v ∧ good) or (¬v ∧ bad)

eq. 10.7 是三向等價：r ∈ ψ′_B ⇔ r ∉ ψ′_G ⇔ v。真值表：r 是 bad → (⊤ ⇔ ⊤ ⇔ v) → v 必須是 ⊤（offender 對壞 report 投 valid）；r 是 good → (⊥ ⇔ ⊥ ⇔ v) → v 必須是 ⊥（對好 report 投 invalid）；r 是 wonky 或根本沒被判過 → r ∈ ψ′_B 為 ⊥、r ∉ ψ′_G 為 ⊤，前兩項就不等價 → 區塊無效。直覺：wonky 表示「無法判定」，沒有人能被證明投錯票，所以 wonky verdict 只會清 ρ†（eq. 10.14），不會產生 offender。這段 Go 放行之後，k 還會被 UpdatePsiO 寫進 ψ′_O 並出現在 H_O——與 reference 分歧；#1042 的「fault equivalence enforcement」就是補這個洞。另外 k 的來源也有限制：k ∈ {κ ∪ λ 的 Ed25519 key} \ ψ_O（prior），已在 punish-set 裡的 key 不能再報（你們回 offender_already_reported）。

**逐項辨析**

1. ❌ Valid: v = ⊥ does not contradict a non-good verdict, so the fault stands, k enters ψ′_O and H_O; the code agrees, since neither inGood nor inBad is set for a wonky target  
   wonky 時等價式的前兩項就已經不相等，無論 v 為何都不成立，fault 不可能合法。
2. ✅ Invalid: the equivalence forces r into exactly one of ψ′_B, ψ′_G, so a wonky report admits no fault for either v; the code nevertheless accepts it, since it only rejects (v ∧ good) or (¬v ∧ bad)  
   eq. 10.7 要求 r 必落在 ψ′_B 或 ψ′_G 之一；wonky 時程式兩個子句皆假 → 直接放行。
3. ❌ Invalid, and the code rejects it too: with inGood = inBad = false both clauses collapse to the vote itself, so fault_verdict_wrong is returned for v = ⊥  
   inGood = inBad = false 時，(vote ∧ false) 與 (¬vote ∧ false) 都是 false，不會回錯誤。
4. ❌ Valid only if v = ⊤ — a 'valid' vote on a report that is not in ψ′_G is the offense a fault documents — and the code rejects exactly that combination via inGood  
   v = ⊤ 只有對 bad report 才是 offense；good report 對應的 fault 投的是 ⊥。

> **陷阱**　fault 只能指向 good 或 bad 的 report；wonky 沒有 offender，只有 ρ† 清除。

<sub>`ch10-code-fault-equivalence-wonky`</sub>
...
**標準答案**　H(E(w)) — the hash of the encoded work-report: it is exactly what guarantors signed (X_G ⌢ H(w)), so a culprit proof is the original guarantee signature, and eq. 10.14 matches cores via H((ρ[c]_g)_w)
...
1. ❌ The work-package hash (w_s)_h: the package is the unit of work, so judging it bans every report ever made of that package, and eq. 10.14 compares the package hash held in ρ[c] against the verdict  
   package hash 只承諾輸入：同一個 package 可能產出一對一錯的兩份 report，分不開。
2. ❌ H(E(g)) — the hash of the whole guarantee including its credential and slot, so that the guarantors who signed are bound into the judged object and culprits can be derived from the verdict alone  
   若 hash 含 credential，同一份 report 換一組簽章就成了另一個物件，judgment 也無法獨立計算。
3. ✅ H(E(w)) — the hash of the encoded work-report: it is exactly what guarantors signed (X_G ⌢ H(w)), so a culprit proof is the original guarantee signature, and eq. 10.14 matches cores via H((ρ[c]_g)_w)  
   H(E(w)) 與 guarantor 簽的 X_G ⌢ H(w) 是同一個訊息，culprit proof 就是原本那把簽章。
4. ❌ The erasure root (w_s)_u, because a dispute is ultimately about whether the erasure-coded bundle behind the report reconstructs to valid data, which is what auditors re-execute  
   erasure root 只承諾資料可用性、不承諾計算結果，而 dispute 爭的正是結果。

> **陷阱**　三處同一個 hash：judgment 訊息 X_valid/X_invalid ⌢ H(w)、culprit 訊息 X_G ⌢ H(w)、ρ† 清除 H((ρ[c]_g)_w)。

<sub>`ch10-report-hash-identity`</sub>
...
1. ✅ ψ_G, ψ_B and ψ_W are sets of work-report hashes judged respectively correct, incorrect and impossible to judge; ψ_O is a set of the Ed25519 keys of validators found to have misjudged a report  
   eq. 10.1 的前三者都是 work-report hash 集合，ψ_O 則是 Ed25519 key 的 punish-set。
2. ❌ ψ_G, ψ_B and ψ_W are sets of the validator indices which respectively guaranteed, audited and disputed a report; ψ_O is a set of the report hashes that have been judged invalid  
   三個集合裝的是 report hash 本身而不是誰投過票，且把 ψ_O 說成 report hash 正好相反。
3. ❌ ψ_G, ψ_B and ψ_W are sets of work-package hashes judged respectively correct, incorrect and impossible to judge; ψ_O is a set of the Bandersnatch keys of validators found to have misjudged a report  
   judgment 針對的是 work-report（hash 為 H(E(w))），而 punish-set 用的是 Ed25519 key。
4. ❌ ψ_G and ψ_B are sets of work-report hashes judged correct and incorrect; ψ_W is the queue of verdicts still short of the ⌊2|k|/3⌋+1 threshold; ψ_O is the balance slashed from each offender  
   eq. 10.4 要求 verdict 進鏈時票數恰好，鏈上沒有待補佇列；JAM 也不在 ψ_O 記罰沒餘額。

> **陷阱**　報告是 report hash H(E(w))；offender 用 Ed25519 key（不是 index，因為 validator 集合每個 epoch 會變）。

<sub>`ch10-state`</sub>

---

### 10-6　Per GP 0.8.0, which statement about the verdicts component E_V of the disputes extrinsic is correct?
...
> **陷阱**　epoch index 用 prior τ；票數是「恰好」不是「至少」。

<sub>`ch10-verdict-structure`</sub>

---

### 10-7　A verdict's outcome depends on t, the number of positive judgments among the ⌊2|k|/3⌋+1 signatures. In the tiny config (|k| = 6), which (t → outcome) table is correct, and what happens for any other t?

<sub>10.2 Extrinsic — ●●○ · 計算 · eq. 10.12 (V)</sub>

**標準答案**　t = 5 → good (⊤); t = 0 → bad (⊥); t = 2 → wonky (∅); any other t makes the block invalid

eq. 10.12：V(a, j) = ⊤ 當 t = ⌊2|k|/3⌋+1；⊥ 當 t = 0；∅ 當 t = ⌊|k|/3⌋。tiny |k| = 6：⌊4⌋+1 = 5、0、⌊2⌋ = 2；full |k| = 1023：683、0、341。三個門檻都是**等式**而不是區間，落在它們之外的 t 讓 V 無定義 → 整個區塊無效（你們的 code 回 `bad_vote_split`），而不是忽略該 verdict。註腳：這三個門檻恰是三種動作的決策門檻，且安全假設要求 ≥ 2/3+1 的 validator 是 live 的（ELVES 論文）。

**逐項辨析**

1. ✅ t = 5 → good (⊤); t = 0 → bad (⊥); t = 2 → wonky (∅); any other t makes the block invalid  
   tiny |k| = 6 代入 eq. 10.12 的三個等式：⌊4⌋+1 = 5、0、⌊2⌋ = 2，其餘 t 皆無定義。
2. ❌ t ≥ 4 → good (⊤); t ≤ 1 → bad (⊥); t ∈ {2, 3} → wonky (∅); no other t can occur  
   三個門檻都是等式不是區間，而且分裂票（t = 4、t = 1）正是 dispute 最可能的結果。
3. ❌ t = 5 → good (⊤); t = 0 → bad (⊥); t = 3 → wonky (∅); any other t makes the block invalid  
   wonky 門檻是 ⌊|k|/3⌋ = ⌊6/3⌋ = 2 而不是 3；good 與 bad 這兩個值倒是對的。
4. ❌ t = 6 → good (⊤); t = 1 → bad (⊥); t = 2 → wonky (∅); any other t is silently ignored  
   good 是 ⌊2|k|/3⌋+1 = 5 不是全票 6，bad 是 0 不是 1，落空的 t 也不會被忽略。

> **陷阱**　wonky 是「剛好 1/3 正票」，不是「介於中間」。

<sub>`ch10-verdict-thresholds`</sub>

---

### 10-8　Which statement about culprits (E_C) and faults (E_F) is correct in GP 0.8.0?
...
1. ✅ Verdicts ordered & unique by report hash; culprits and faults each ordered & unique by Ed25519 key; judgments within a verdict ordered & unique by validator index; no verdict report hash may already be in ψ_G ∪ ψ_B ∪ ψ_W  
   三個排序鍵都對：verdict 用 report hash、offender 用 Ed25519 key、judgment 用 validator index。
2. ❌ Verdicts ordered & unique by report hash; culprits and faults each ordered & unique by report hash; judgments within a verdict ordered & unique by their ⊤/⊥ vote; a report hash already in ψ_G ∪ ψ_B ∪ ψ_W may be re-judged to overturn the earlier verdict  
   offender 序列的排序鍵是 Ed25519 key、judgment 用 index，而 eq. 10.10 不許翻案重判。
3. ❌ Verdicts ordered & unique by the verdict's epoch index; culprits and faults each ordered & unique by Ed25519 key; judgments ordered & unique by validator index; no verdict report hash may already be in ψ_O  
   epoch index 只有兩個可能值排不出唯一序；要比對的也是 ψ_G ∪ ψ_B ∪ ψ_W 而非裝 key 的 ψ_O。
4. ❌ Verdicts ordered by the timeslot at which each dispute was raised; culprits and faults ordered by the offender's index within κ; judgments ordered by signature; duplicates anywhere are silently dropped rather than invalidating the block  
   verdict 結構裡根本沒有 timeslot 欄位，offender 用 key 不用 index，違規也不會被靜默忽略。

> **陷阱**　culprits/faults 排序鍵是 key，不是 report hash。

<sub>`ch10-ordering`</sub>
...
**標準答案**　ρ†[c] = ∅ for any core whose pending report was judged bad or wonky; ψ′_G, ψ′_B and ψ′_W each absorb their verdict's report hashes; ψ′_O absorbs all culprit and fault keys; H_O = [culprit keys in order] ⌢ [fault keys in order]

eq. 10.14：∀c：ρ†[c] = ∅ 當 (H(ρ[c]_w), v) ∈ v 且 v ∈ {⊥, ∅}——GP：「Authoring a block with a non-positive verdict has the effect of cancelling its imminent accumulation」。eq. 10.15–10.18：ψ′_G/ψ′_B/ψ′_W 各自 ∪ 對應的 report hash（三者單調成長、永不清空，這正是「同一個 report 不能被重複 dispute」的基礎）；ψ′_O = ψ_O ∪ {culprit keys} ∪ {fault keys}。eq. 10.19：H_O ≡ [k | (…, k, …) ∈ E_C] ⌢ [k | ∈ E_F]，內容必須是「exactly the keys of all new offenders」，好讓輕客戶端從 header 直接讀出誰被罰。你們 ClearWorkReports 對 PositiveJudgmentsSum < 2V/3 的 verdict 清 ρ。

**逐項辨析**

1. ✅ ρ†[c] = ∅ for any core whose pending report was judged bad or wonky; ψ′_G, ψ′_B and ψ′_W each absorb their verdict's report hashes; ψ′_O absorbs all culprit and fault keys; H_O = [culprit keys in order] ⌢ [fault keys in order]  
   eq. 10.14 的條件是 v ∈ {⊥, ∅}，bad 與 wonky 一起清；H_O 是 culprits 在前、faults 在後。
2. ❌ ρ†[c] = ∅ only for cores whose report was judged bad, wonky ones staying pending; ψ′_G, ψ′_B and ψ′_W each absorb their verdict's report hashes; ψ′_O absorbs all culprit and fault keys; H_O lists the offenders' validator indices  
   wonky 代表無法判定，eq. 10.14 一樣要清掉；eq. 10.19 的 H_O 裝 Ed25519 key 不是 index。
3. ❌ ρ†[c] = ∅ for any core whose pending report was judged bad or wonky; ψ′_G is emptied at each epoch change while ψ′_B and ψ′_W grow; ψ′_O absorbs the culprit keys only; H_O = [fault keys in order] ⌢ [culprit keys in order]  
   eq. 10.15–10.17 三個集合只做 ∪、永不清空；ψ′_O 兩邊 key 都吸收；H_O 串接順序也反了。
4. ❌ ρ†[c] is left untouched, a non-positive verdict only stopping accumulation later; ψ′_G, ψ′_B and ψ′_W each absorb their verdict's report hashes; ψ′_O absorbs all culprit and fault keys; H_O = H(E(E_D)), one commitment to E_D  
   清 ρ† 正是 disputes 階段的效果；H_O 要列出所有新 offender 的 key，不是 E_D 的單一 hash。

> **陷阱**　ρ† 是 disputes 之後、assurances 之前的中間狀態；順序：ρ → ρ† (disputes) → ρ‡ (assurances) → ρ′ (guarantees)。

<sub>`ch10-effects`</sub>
...
### 10-12　Which of the following is NOT a purpose of the disputes system as stated in the GP?

<sub>10 intro — ●○○ · 設計理由 · §10 intro paragraphs</sub>

**標準答案**　Directly slashing the offending validator's balance inside the JAM state
...
1. ❌ Removing and banning invalid work-reports from the processing pipeline  
   §10 intro 原話：removing and banning invalid work-reports from the processing pipeline。
2. ❌ Removing troublesome keys from the validator set where there is consensus over their malfunction  
   §10 intro 原話：removing troublesome keys from the validator set，確實是列出的目的之一。
3. ❌ Coordinating nodes to revert chain-extensions containing invalid work-reports and aggregating offenders for punishment in a higher-level system (e.g. slashing on a staking parachain)  
   §10 intro 同樣明列 revert chain-extensions 與 aggregating offenders for punishment。
4. ✅ Directly slashing the offending validator's balance inside the JAM state  
   JAM 只記 ψ_O 並在下次 key rotation 把 key 清零；扣質押是上層 staking parachain 的事。

> **陷阱**　與 §13 statistics 同理：rewards/slashing 都交給 staking 子系統。

<sub>`ch10-rationale`</sub>
...
<sub>11.2 Package Availability Assurances / 11.5 Transitioning for Reports — ●●● · 概念 · eq. 11.16–11.18, 11.32, 11.46</sub>

**標準答案**　ρ‡[0] = ∅ because 45 ≥ 40 + U; g₁'s report never enters R and is dropped without accumulation; g₂ is accepted, so ρ′[0] = (g₂, 45)
...
1. ✅ ρ‡[0] = ∅ because 45 ≥ 40 + U; g₁'s report never enters R and is dropped without accumulation; g₂ is accepted, so ρ′[0] = (g₂, 45)  
   45 ≥ 40 + 5 觸發 timeout，g₁ 從未進入 R，同一塊的 core 就能換上 g₂。
2. ❌ ρ‡[0] = (g₁, 40): the 4 + 3 = 7 assurances gathered so far exceed the threshold of 5, so the report is in R and accumulates, while g₂ is rejected as core_engaged  
   assurance 不跨塊累加：ρ 只存 (g, t)，state 裡沒有任何計數器。
3. ❌ ρ‡[0] = ∅ because the cumulative 7 assurances made the report available; it accumulates in this block and ρ′[0] = (g₂, 45)  
   同樣把 4 + 3 當成 7 票；eq. 11.17 只數本塊 E_A 裡的 assurance。
4. ❌ ρ‡[0] = (g₁, 40): the timeout only fires once H_T > t + U, i.e. from slot 46, so g₂ is rejected as core_engaged  
   eq. 11.18 的比較子是 ≥ 而不是 >，slot 45 就已經逾時。

> **陷阱**　assurance 不累積；timeout 用 ≥；timeout 的 report 不 accumulate，但 core 同一塊就可再用——這正是 ρ‡ 存在的理由。

<sub>`ch11-rho-pipeline-worked`</sub>
...
<sub>11.3 Guarantor Assignments — ●●● · 計算 · eq. 11.23, 11.28</sub>
...
1. ❌ M* = (P(|κ′|, η′_2, 593), Φ(κ′)): the previous rotation always uses the current epoch's entropy and set, only the rotation index differs  
   τ′ − R = 593 落在上一個 epoch，eq. 11.23 因此改用 (λ′, η′_3) 而不是當期的 (κ′, η′_2)。
2. ❌ M = (P(|κ′|, η′_2, 603), Φ(κ′)) at rotation index 0, because t is within R slots of τ′  
   判準是兩個 rotation index 是否相同（60 ≠ 59），而不是 t 與 τ′ 的距離。
3. ❌ The guarantee is invalid: a guarantee may not straddle an epoch boundary because the validator set may have changed  
   GP 明講可以用前一個 rotation 的 t，M* 正是為此而存在。
4. ✅ M* = (P(|λ′|, η′_3, 593), Φ(λ′)): the previous rotation (index 59) belonged to the previous epoch, so it is recomputed from λ′ and η′_3  
   ⌊593/600⌋ = 0 ≠ ⌊603/600⌋ = 1 → 取 (λ′, η′_3)，P 內的 rotation index 是 59。

> **陷阱**　M* 的 entropy/set 由 τ′ − R 落在哪個 epoch 決定；(λ′, η′_3) 這組合只在 epoch 開頭的第一個 rotation 出現。

<sub>`ch11-rotation-epoch-boundary-mstar`</sub>

---

### 11-4　This is the team's P (GP 0.7.2). Under GP 0.8.0 (eq. 11.20–11.21), what changes when the validator sequence has |κ′| = 9 in a chain with C = 341 cores?

<sub>11.3 Guarantor Assignments — ●●○ · 程式碼 · eq. 11.20–11.22 — internal/extrinsic/guarantor_assignments.go permute · ⚠ 0.7.2→0.8.0</sub>
...
func permute(e types.Entropy, currentSlot types.TimeSlot) []types.CoreIndex {
    ...
    ...
```
<sub>internal/extrinsic/guarantor_assignments.go (rotateCores, permute)</sub>

**標準答案**　0.8.0 builds the base [⌊i/3⌋ | i ∈ N_9] = [0,0,0,1,1,1,2,2,2] and rotates modulo |κ′|/3 = 3; this code builds ⌊341·i/9⌋ = [0, 37, 75, …, 303] and rotates modulo 341, scattering guarantors over inactive cores

eq. 11.21：P(v, e, t) = R(F([⌊i/3⌋ | i ∈ N_v], e), ⌊(t mod E)/R⌋)；eq. 11.20：R(c, n) = [(x + n) mod (|c|/3) | x ∈ c]。0.8.0 的邏輯是「每個 core 3 個 guarantor，所以只有前 |κ′|/3 個 core 是 active」（§11.3）；|κ′| = 9 → base = [0,0,0,1,1,1,2,2,2]，用 η′_2 做 Fisher–Yates 後 rotation 取 mod 3。你們 permute 是 0.7.2 的 ⌊C·i/V⌋ 加 `% CoresCount`：i = 0..8 → ⌊341·i/9⌋ = [0, 37, 75, 113, 151, 189, 227, 265, 303]，再 mod 341——把 guarantor 分到 inactive core，且每個 core 只有 1 人，永遠湊不到 2 個簽章。為什麼現在 test vectors 都過？因為 tiny (6, 2) 與 full (1023, 341) 都滿足 V = 3C：⌊2i/6⌋ = ⌊341i/1023⌋ = ⌊i/3⌋，且 mod C = mod V/3，兩式完全重合；一旦 0.8.0 eq. 6.8 允許的其他 |κ| ∈ {3c} 出現就分歧（issue #1037「support variable validator-set size」）。

**逐項辨析**

1. ✅ 0.8.0 builds the base [⌊i/3⌋ | i ∈ N_9] = [0,0,0,1,1,1,2,2,2] and rotates modulo |κ′|/3 = 3; this code builds ⌊341·i/9⌋ = [0, 37, 75, …, 303] and rotates modulo 341, scattering guarantors over inactive cores  
   0.8.0 的 base 是 ⌊i/3⌋、modulus 是 |κ′|/3；⌊C·i/9⌋ 會把 guarantor 撒到 inactive core。
2. ❌ Nothing observable: ⌊C·i/V⌋ and ⌊i/3⌋ coincide for every validator count permitted by eq. 6.8, and mod C equals mod V/3 for the same reason, so only the constant names differ between the versions  
   只有 V = 3C 時兩式才重合；eq. 6.8 允許的其他 |κ| 一出現就分歧。
3. ❌ 0.8.0 keeps ⌊C·i/V⌋ = [0, 37, 75, …, 303] as the base but changes the rotation modulus from C to E/R = 60, the number of rotations per epoch, so the shuffle output cycles once per epoch  
   E/R = 60 是每個 epoch 的 rotation 次數，跟 rotation 的 modulus（core 數）是兩回事。
4. ❌ 0.8.0 replaces the Fisher–Yates shuffle by a plain rotation of the identity assignment [0, 1, 2, …, 8] modulo |κ′|/3, removing the dependence on η′_2 so that assignments are known a whole epoch ahead  
   F 與 η′_2 都還在：用 η′_2 而非 η′_1 正是為了避免 fork magnification。

> **陷阱**　0.8.0 的 base 與 modulus 都只看 |κ′|（⌊i/3⌋、mod |κ′|/3），C 只在 ρ 的長度出現。

<sub>`ch11-code-permute-080`</sub>

---

### 11-5　This is the team's refinement-context validation (GP 0.7.2). Which conditions required by GP 0.8.0 eq. 11.36 (anchor) and eq. 11.38 (lookup anchor) are missing from it?

<sub>11.4.1 Contextual Validity of Reports — ●●○ · 程式碼 · eq. 11.36, 11.38 — internal/extrinsic/guarantee_controller.go ValidateContexts · ⚠ 0.7.2→0.8.0</sub>
...
if len(ancestry) > 0 {
    for _, context := range contexts {
        ...
        ...
```
<sub>internal/extrinsic/guarantee_controller.go (GuaranteeController.ValidateContexts)</sub>

**標準答案**　Anchor: the context's anchor timeslot must also equal the β† entry's timeslot (x_t = y_t); lookup anchor: the child header h′ (h′_p = H(h)) must carry h′_r = l_s, verifying the lookup anchor's posterior state root

eq. 11.36（0.8.0）：∀x ∈ x：∃y ∈ β†：x_a = y_h ∧ x_s = y_s ∧ x_b = y_b ∧ x_t = y_t——四個等式，第四個 timeslot 是 #526 新增（β 的每筆 entry 在 0.8.0 也多了 timeslot，eq. 7.2/7.8；你們 #1031 加了 BlockInfo.timeslot）。eq. 11.38（0.8.0）：∃h, h′ ∈ A：h_t = l_t ∧ H(h) = l ∧ h′_p = H(h) ∧ h′_r = l_s——lookup anchor 的 posterior state root l_s 沒辦法從 h 自己讀到（header 只帶 prior state root H_R），所以透過它的子塊 h′ 的 H_R 來驗。這段 Go 只比對 Anchor/StateRoot/BeefyRoot（三項）以及 ancestry 的 (slot, hash)（兩項）；#1027 加了 anchor_slot 與 lookup_anchor_state_root 兩個欄位，驗證邏輯也要跟上。

**逐項辨析**

1. ❌ Anchor: compare against β instead of β†, since 0.8.0 no longer patches the parent's state root into recent history; lookup anchor: unchanged, since the ancestor set already stores (slot, hash) pairs  
   β† 仍然必要：它把父塊 entry 的 state root 換成本塊的 H_R（eq. 7.5），否則沒有正確的 root 可比。
2. ✅ Anchor: the context's anchor timeslot must also equal the β† entry's timeslot (x_t = y_t); lookup anchor: the child header h′ (h′_p = H(h)) must carry h′_r = l_s, verifying the lookup anchor's posterior state root  
   缺的正是 eq. 11.36 的第四個等式 x_t = y_t，以及 eq. 11.38 的 h′_r = l_s。
3. ❌ Anchor: drop the state-root comparison because 0.8.0 anchors carry a timeslot instead of a state root; lookup anchor: check only l_t ≥ H_T − L, since the ancestor-set requirement was removed in 0.8.0  
   state root 的比對並沒有被拿掉，祖先集合的要求也還在，eq. 11.38 仍是硬性條件。
4. ❌ Nothing beyond renaming BeefyRoot to the accumulation-output super-peak b, with the new BlockInfo timeslot being informational only; both anchor tuples are otherwise identical to 0.7.2  
   「只是改名」忽略了 0.8.0 新增的兩個欄位：anchor slot 與 lookup-anchor posterior root。

> **陷阱**　0.8.0 的 anchor 比四項（hash、state root、super-peak、slot）；lookup anchor 比三項，其中 l_s 要靠子塊的 H_R。

<sub>`ch11-code-anchor-checks-080`</sub>

---

### 11-6　Why can the lookup-anchor requirement (eq. 11.38) not be checked from the on-chain state σ alone, and how does the conformance fuzzer make it checkable for an M1 target?

<sub>11.4.1 Contextual Validity of Reports — ●●○ · 概念 · eq. 11.37–11.38</sub>

**標準答案**　It needs the headers of the last L slots (the ancestor set, not part of σ), deterministic from the header chain; the fuzzer's Initialize message therefore carries an ancestry list of (slot, header hash), a mandatory M1 feature

§11.4.1 原文：「this is one of the few conditions which cannot be checked purely with on-chain state and must be checked by virtue of retaining the series of the last L headers as the ancestor set. Since it is determined through the header chain, it is still deterministic and calculable.」eq. 11.37 只用 H_T 與 l_t 比（l_t ≥ H_T − L，L = 14,400 slots = 24 小時）；eq. 11.38 則要在祖先集合裡找到 h（h_t = l_t、H(h) = l）及其子塊 h′（h′_p = H(h)、h′ 的 prior-state-root = r）——後者是 β 完全沒有的資訊。Conformance fuzzer 因此把 Ancestry 列為 M1 必備 feature（「lookup anchors in guarantees are within last L imported headers」），Initialize（SetState）訊息附帶 ancestry 陣列，之後每次成功 import 再 append——這就是 #853/#854「add ancestry to store and fix guarantee slot validation」與 #892 的來源。你們 ValidateContexts 只在 `len(ancestry) > 0` 時比對 (slot, hash)，否則跳過（錯誤碼 LookupAnchorNotRecent）。

**逐項辨析**

1. ❌ It only needs β, the last H = 8 blocks, whose entries carry a header hash, a state root and a timeslot, so the check is purely on-chain; the fuzzer merely replays enough blocks to fill β before it submits any guarantees  
   β 只有 H = 8 筆，而 lookup anchor 可回溯 L = 14,400 slot，覆蓋範圍差了三個數量級。
2. ❌ It needs the full posterior state of the lookup-anchor block in order to recompute its state root r, which the fuzzer supplies on demand through GetState messages while the block is being imported  
   eq. 11.38 只比對 header 欄位、不必重算 state root；GetState 是匯入完成後的稽核介面。
3. ✅ It needs the headers of the last L slots (the ancestor set, not part of σ), deterministic from the header chain; the fuzzer's Initialize message therefore carries an ancestry list of (slot, header hash), a mandatory M1 feature  
   祖先集合不在 σ 裡，但由 header chain 決定，所以仍是 deterministic and calculable。
4. ❌ The fuzzer skips it: 0.8.0 made the lookup anchor optional in the refinement context, so only the age bound l_t ≥ H_T − L is exercised by the vectors and no ancestry list is transmitted at Initialize  
   eq. 11.4 的 ℂ 一定帶 l、t、r 三個 lookup-anchor 欄位，0.8.0 反而多要求了 posterior root r。

> **陷阱**　anchor 靠 β†（state 內），lookup anchor 靠祖先 header（state 外）；兩者的年齡上限不同：H = 8 塊 vs L = 14,400 slots。

<sub>`ch11-lookup-anchor-ancestry`</sub>

---

### 11-7　A work-report in E_G lists prerequisite p. Package p was guaranteed 10 blocks ago and accumulated 8 blocks ago, so p ∈ ξ but p no longer appears in any entry of β (H = 8). Is the guarantee valid?

<sub>11.4.1 Contextual Validity of Reports — ●●● · 概念 · eq. 11.42</sub>

**標準答案**　Invalid (dependency_missing): eq. 11.42 admits a prerequisite only if it is in this block's E_G or among the reported-package sets of β; ξ is not consulted at guarantee time
...
1. ❌ Valid: p ∈ ξ proves the package was accumulated, and eq. 11.42 accepts ξ alongside the extrinsic and β, so the dependency holds and the report accumulates as soon as it is available  
   把 §12 的規則搬到了 §11：ξ 只在 accumulation 端用來判斷 prerequisite 是否已滿足。
2. ✅ Invalid (dependency_missing): eq. 11.42 admits a prerequisite only if it is in this block's E_G or among the reported-package sets of β; ξ is not consulted at guarantee time  
   eq. 11.42 的來源集合只有本塊的 p 與 β 各 entry 的 reported 字典，ξ 不在其中。
3. ❌ Valid at guarantee time, but the report is parked in the ready queue ω until p is reported again inside the recent-history window, because eq. 11.42 is re-evaluated during accumulation  
   ω 只收已經通過 guarantee 檢查的 report，不存在「暫存等補件」這種狀態。
4. ❌ Invalid (dependency_missing): eq. 11.42 requires every prerequisite to be guaranteed in the very same block, so only mutual and self dependencies are admissible and β is never consulted  
   eq. 11.42 明列 β 各 entry 的 reported 字典，vectors 的「from history」測的正是這條路徑。

> **陷阱**　guarantee 時看 β（8 塊），accumulate 時看 ξ（E 個 slot）；兩個視窗、兩個問題。

<sub>`ch11-prerequisite-window`</sub>

---

### 11-8　Service s upgraded its code after a work-package was built: the work-item's code hash c was correct at build time and refine ran the code available at the lookup anchor, but the including block's prior state has δ[s]_c ≠ c. What happens to the guarantee at inclusion?

<sub>11.4.1 Contextual Validity of Reports — ●●● · 概念 · eq. 11.45</sub>

**標準答案**　Rejected (bad_code_hash): eq. 11.45 demands d_c = δ[d_s]_c in the prior state of the including block, so a report refined against superseded code is discarded rather than accumulated

§11.1.4：「We include the hash of the code of the service at the time of being reported c, which must be accurately predicted within the work-report according to equation 11.45」；eq. 11.45：∀w ∈ I, ∀d ∈ w_d：d_c = δ[d_s]_c，δ 是 prior state。§14 的 work-item 也說 c 是「the code hash of the service at the time of reporting (whose preimage must be available from the perspective of the lookup anchor block)」——refine 執行的是這個 hash 在 lookup-anchor 視角下 historical lookup 到的 preimage，而鏈上檢查的是包含區塊當下的 δ[s]_c（reports vectors 有 bad_code_hash 案例）。理由：accumulate 稍後會用當下的 code 執行，若 report 是用已被淘汰的 code 算出來的，不該進入 accumulation，而是讓 guarantor 重做。附帶一提，BAD 是 refine 階段在 lookup-anchor 視角查不到 code preimage 時就寫進 digest 的值（eq. 11.7），與這條鏈上檢查是兩回事。你們 CheckWorkResult：`w.CodeHash != delta[w.ServiceID].ServiceInfo.CodeHash` → BadCodeHash，δ 取自 prior state。

**逐項辨析**

1. ❌ Accepted: c only records which code refine actually executed, resolved against the lookup anchor's posterior state, and accumulation then simply runs whatever δ′[s]_c holds at that time  
   eq. 11.45 是硬性等式，比對基準是包含區塊的 prior δ，既不是 lookup anchor 也不是 δ′。
2. ❌ Accepted, but the digest's result is overwritten on-chain with the BAD error, since the recorded code hash no longer resolves to a preimage the service still holds in its lookup dictionary  
   鏈上規則從不改寫 digest：改寫會讓 report hash 對不上 guarantor 的簽章。
3. ❌ Accepted provided the lookup anchor is at most L = 14,400 slots old, because code availability is judged only at the lookup anchor and eq. 11.45 constrains the payload hash, not the code hash  
   11.45 管的是 d_c（code hash），payload hash 鏈上不檢查；年齡上限是另一條 eq. 11.37。
4. ✅ Rejected (bad_code_hash): eq. 11.45 demands d_c = δ[d_s]_c in the prior state of the including block, so a report refined against superseded code is discarded rather than accumulated  
   eq. 11.45 要求 d_c = δ[d_s]_c，service 一升級這個預測就落空，report 只能重做。

> **陷阱**　digest 的 c 是「預測」：refine 用 lookup-anchor 的 code，鏈上卻比對 prior δ 的 code hash。

<sub>`ch11-code-hash-prediction`</sub>

---

### 11-9　Full config (C = 341). An assurer sets only the bit for core 340. What does E(f) look like, and what exactly does the assurer sign?

<sub>11.2.1 The Assurances Extrinsic — ●●○ · 計算 · eq. 11.11, 11.14; App. C bit-sequence encoding</sub>

**標準答案**　E(f) is 43 octets whose last octet is 0x10 (bit 4 of octet 42, packed LSB-first, no length prefix); the signature is over X_A ⌢ H(H_P ⌢ E(f)), i.e. a hash of 75 octets

附錄 C 的 bit sequence encoding：「pack the bits into octets in order of least significant to most」，且只有變長序列才加 length prefix；eq. 11.11 的 f ∈ bits[C] 是定長，所以 E(f) 就是 ⌈341/8⌉ = 43 個 octet、沒有前綴。core 340：340 = 42·8 + 4 → 第 43 個 octet（index 42）的 bit 4 → 0x10。eq. 11.14：s ∈ Ed25519_{κ[v]_e}(X_A ⌢ H(E(H_P, f)))，X_A = $jam_available：先把 32-byte 的 H_P 與 43-byte 的 E(f) 串成 75 bytes 做 Blake2b，再把 32-byte 摘要接在 X_A 後面簽——不是直接簽 H_P ⌢ E(f)，否則訊息長度會隨 C 變動。你們的實作：AvailBitfieldBytes = 43（full）/ 1（tiny）；`ToOctetSlice`：`bytes[i/8] |= b << (i % 8)`（LSB-first）；ValidateSignature：`hash.Blake2bHash(append(anchor, bitfield...))` 後接在 JamAvailable 之後——與 GP 一致。tiny 兩個 core 都勾 = 0x03。

**逐項辨析**

1. ❌ E(f) is 43 octets whose last octet is 0x08 (bit 4 counted from the most significant end of octet 42, no length prefix); the signature is over X_A ⌢ H(H_P ⌢ E(f)), a hash of 75 octets  
   0x08 是把 bit 4 從 MSB 端數的結果；附錄 C 規定 LSB-first，正確值是 0x10。
2. ❌ E(f) is 341 octets, one per core, the last being 0x01 and no length prefix; the signature is over X_A ⌢ H_P ⌢ E(f) directly, i.e. 373 octets signed with no intermediate hash  
   bit sequence 一律 8 bits 打包成 1 octet，而且 eq. 11.14 明確要先 Blake2b 再簽。
3. ✅ E(f) is 43 octets whose last octet is 0x10 (bit 4 of octet 42, packed LSB-first, no length prefix); the signature is over X_A ⌢ H(H_P ⌢ E(f)), i.e. a hash of 75 octets  
   340 = 42·8 + 4 → index 42 的 octet、bit 4 → 0x10，定長序列不加 length prefix。
4. ❌ E(f) is a length-prefixed sequence: the discriminator 0x2B followed by 43 octets ending in 0x10; the signature is over X_A ⌢ H(H_P ⌢ E(f)), i.e. a hash of 76 octets  
   只有變長序列才有 length discriminator；多出的那個 octet 也會讓 preimage 變成 76 bytes。

> **陷阱**　bit i 在 octet ⌊i/8⌋ 的第 (i mod 8) 位（LSB-first）；簽的是 hash 的 hash：X_A ⌢ Blake2b(H_P ⌢ E(f))。

<sub>`ch11-assurance-bitfield-packing`</sub>

---

### 11-10　GP 0.8.0, C = 341, E = 600, R = 10. During epoch e the active set had |κ| = 12 (cores 0–3 active); at the epoch change the set shrinks to |κ′| = 9. The first block of the new epoch (τ′ = 600) carries a guarantee for core 3 with t = 595, signed by the three validators that M* assigns to core 3. Which statement is correct?
...
### 11-11　What does each entry of ρ (the availability assignments) hold in GP 0.8.0, and why did PR #494 change it?
...
### 11-12　A work-report (eq. 11.2, of the set ℝ) is a tuple (s, c, c, a, t, l, d, g) — bold c is the refinement context, plain c the core index. Which field description is WRONG?

<sub>11.1.1 Work Report — ●●○ · 概念 · eq. 11.2–11.3</sub>

**標準答案**　g — the total gas used by all refine invocations in the package

eq. 11.2：ℝ ≡ (s avspec, **c** refinement context, c core index, a authorizer hash, **t** authorizer trace, **l** segment-root lookup, **d** ∈ [𝔻]_{1:I} digests, g authgasused)——report 層級與 digest 層級各有自己的 gas 欄位，命名相近但語意不同，是這章最容易記混的地方。eq. 11.3：|l| + |c_p（prerequisites）| ≤ J = 8，兩者共用同一個 J 預算。

**逐項辨析**

1. ❌ s — the availability specification (package hash, bundle length, erasure root, shard count, segment root, segment count)  
   eq. 11.2 的第一欄 s 就是 avspec，六個子欄位 (p, l, u, v, e, n) 也列對了。
2. ❌ l — the segment-root lookup dictionary (work-package hash → segment root), which together with the context's prerequisites is limited to J = 8 entries  
   eq. 11.3 就是 |l| + |c_p| ≤ J = 8，這個共用預算的描述無誤。
3. ✅ g — the total gas used by all refine invocations in the package  
   g 記的是 Is-Authorized 消耗的 gas；refine 的用量在每個 digest 自己的 u 欄位。
4. ❌ d — the work-digests, between 1 and I = 16 of them  
   eq. 11.2 的 d ∈ [𝔻]_{1:I}，I = 16，digest 數的上下界都對。

> **陷阱**　report 層級的 g = auth gas；digest 層級的 u = refine gas；digest 的 g = accumulate gas limit。

<sub>`ch11-workreport-fields`</sub>

---

### 11-13　Which fields does the refinement context (eq. 11.4, of the set ℂ) contain in GP 0.8.0 (PR #526)?
...
### 11-14　The availability specification (eq. 11.5) is s ≡ (p, l, u, v, e, n). What do u, v and e denote, and what constraint does eq. 11.31 place on v?
...
eq. 11.5：s ≡ (p package hash, l bundle length, u erasure root, v ∈ 𝕍 erasure shards, e segment root, n segment count)。eq. 11.31：∀w ∈ I：(w_s)_v = |κ′|——GP 的理由是「As one chunk is distributed to each assurer, the number of chunks must equal the size of the assuring validator set」（0.8.0 因 validator 數可變而必須明確檢查）。兩個 root 分工不同：erasure root 承諾了審計所需的全部資料（bundle shard + segment shards），segment root 則讓後續 package 的 import 可驗證重建出來的 segment。

**逐項辨析**

1. ✅ u = erasure root, the Merkle root over the v chunks that erasure-code the bundle plus exported segments; v = the erasure-chunk count, which eq. 11.31 forces to equal |κ′|; e = segment root, a constant-depth tree over the exported segment hashes; n = the segment count  
   eq. 11.31 要求 (w_s)_v = |κ′|：一個 chunk 發給一個 assurer，數量必須等於 assurer 集合大小。
2. ❌ u = segment root over the exported segment hashes; v = the number of work-items in the package, which eq. 11.31 forces to be at most I = 16; e = erasure root over the coded chunks; n = the number of validators assuring the core  
   u 與 e 對調了；v 也不是 work-item 數（那是 eq. 11.2 的 d ∈ [𝔻]_{1:I}），n 更不是 validator 數。
3. ❌ u = erasure root, the Merkle root over the v chunks that erasure-code the bundle plus exported segments; v = the erasure-chunk count, which eq. 11.31 fixes at 1023 in every configuration; e = segment root, a constant-depth tree over the exported segment hashes; n = the segment count  
   eq. 11.31 比的是 |κ′|，而 validator 數是 6 到 1023 之間 3 的倍數，寫死 1023 正是移植 bug。
4. ❌ u = the hash of the auditable work bundle; v = the number of guarantors credentialing the report, which eq. 11.31 forces to be 2 or 3; e = the erasure root of the bundle's extrinsics; n = the index of the core the package was built for  
   p 才是 package hash、l 是 bundle 長度；2–3 是 credential 長度（eq. 11.24），core index 是 w_c。

> **陷阱**　erasure shard 數 v = |κ′|（posterior）：tiny 6、full 1023。別把 v（shards）跟 n（segment count）記反。

<sub>`ch11-avspec`</sub>

---

### 11-15　A work-digest's result is either a blob or a member of the error set 𝔼 = {∞, ☇, BADEXPORTS, OVERSIZE, BAD, BIG} (eq. 11.7). Which meaning is correct?

<sub>11.1.4 Work Digest — ●●○ · 概念 · eq. 11.6–11.7</sub>

**標準答案**　∞ out-of-gas; ☇ panic; BADEXPORTS the number of exports was misreported; OVERSIZE the refine output would exceed the size limit; BAD service code unavailable at the lookup-anchor; BIG code exceeds W_C = 4,000,000

eq. 11.7（錯誤集合是 𝔼，注意 𝕁 在 0.8.0 是 segment 的集合）與 §11.1.4：「The first two are special values concerning execution of the virtual machine, ∞ denoting an out-of-gas error and ☇ denoting an unexpected program termination（panic）. Of the remaining four, the first indicates that the number of exports made was invalidly reported, the second that the size of the digest（refinement output）would cross the acceptable limit, the third（BAD）indicates that the service's code was not available for lookup in state at the posterior state of the lookup-anchor block. The fourth（BIG）indicates that the code was available but was beyond the maximum size allowed W_C」。另外 authorizer 不接受的 package 根本進不了鏈（eq. 11.32 要求 w_a ∈ α[w_c]），不會以 error 的形式留在 digest 裡。你們 encode.go 的 WorkExecResult：0 ok、1 oog、2 panic、3 bad-exports、4 output-oversize、5 bad-code、6 code-oversize。

**逐項辨析**

1. ✅ ∞ out-of-gas; ☇ panic; BADEXPORTS the number of exports was misreported; OVERSIZE the refine output would exceed the size limit; BAD service code unavailable at the lookup-anchor; BIG code exceeds W_C = 4,000,000  
   六個值與 §11.1.4 逐字對應，關鍵在 BAD 是 code 不可得、BIG 是 code 存在但超過 W_C。
2. ❌ ∞ an infinite loop was detected by the guarantor; ☇ a host-call fault; BADEXPORTS the number of exports was misreported; OVERSIZE the refine output would exceed the size limit; BAD service code unavailable at the lookup-anchor; BIG the bundle exceeds W_B  
   PVM 不做迴圈偵測（停機問題），∞ 就是 gas 耗盡；W_B 是 §14 對 bundle 的檢查，不是 digest 結果。
3. ❌ ∞ out-of-gas; ☇ a PVM page fault; BADEXPORTS the number of imported segments was misreported; OVERSIZE the bundle would exceed W_B; BAD the authorizer rejected the package; BIG more than I = 16 work-items  
   page fault 只是 panic 的成因之一；BADEXPORTS 針對 export 數，OVERSIZE 針對 refine 的輸出。
4. ❌ ∞ out-of-gas; ☇ panic; BADEXPORTS the number of exports was misreported; OVERSIZE the refine output would exceed the size limit; BAD the code was available but exceeds W_C = 4,000,000; BIG the code was unavailable at the lookup-anchor  
   BAD 與 BIG 對調了：GP 是 the third… code was not available、the fourth… beyond W_C。

> **陷阱**　錯誤結果仍會被 accumulate（operand 的 r 是 error），只是沒有 output blob。

<sub>`ch11-work-errors`</sub>

---

### 11-16　What is the on-chain limit on the variable-size content of a single work-report?

<sub>11.1.4 Work Digest — ●○○ · 概念 · eq. 11.8</sub>

**標準答案**　|authorizer trace t| + Σ over digests of |result blob| (errors count as 0) ≤ W_R = 48·2^10 = 49,152 octets

eq. 11.8：∀w ∈ R：|w_o| + Σ_{i} L(w_d[i]_r) ≤ W_R，L(r) = |r| 若 r ∈ B（成功輸出），否則 0；W_R = 48·2^10 = 49,152。GP 給的理由：「In order to ensure fair use of a block's extrinsic space」。對照 W_B（13,791,360）是 work-package **bundle**（extrinsics + imports 等）的上限（§14）——兩者管的是不同層級的東西。你們 guarantor 端的 BIG/OVERSIZE 判斷是累積前面 item 的輸出一起算。

**逐項辨析**

1. ✅ |authorizer trace t| + Σ over digests of |result blob| (errors count as 0) ≤ W_R = 48·2^10 = 49,152 octets  
   eq. 11.8 的 L 對 r ∈ 𝔼 回 0，失敗的 digest 不佔 report 空間，out-of-gas 反而「便宜」。
2. ❌ |authorizer trace t| + Σ over digests of |result blob| (errors count as their encoded length) ≤ W_B = 13,791,360 octets  
   error 不按編碼長度計；而 W_B 套的是 §14 的 work-package bundle，不是 report 的 W_R。
3. ❌ Σ over digests of |result blob| alone, the authorizer trace being unbounded, ≤ W_R = 48·2^10 = 49,152 octets  
   |w_t| 是加總的第一項，否則 is-authorized 的 trace 可以無限撐大區塊。
4. ❌ |authorizer trace t| ≤ 32 octets and each digest's result blob ≤ W_G = 4,104 octets, giving at most I = 16 blobs  
   eq. 11.8 是單一總量上限、沒有逐項上限；W_G = 4,104 是 segment 大小，與 report 無關。

> **陷阱**　48 KiB 是 report 級總量，且 auth trace 也算在內。

<sub>`ch11-report-size-limit`</sub>

---

### 11-17　Which set of rules applies to the assurances extrinsic E_A?

<sub>11.2 Package Availability Assurances — ●●○ · 概念 · eq. 11.11–11.16</sub>

**標準答案**　Each assurance = (anchor a, bitfield f ∈ bits[C], validator index v, Ed25519 sig); a must equal H_P; assurances strictly ordered by v (so at most one per validator); the signature by κ[v]_e is over X_A ⌢ H(E(H_P, f)); a bit may be set only if ρ†[c] ≠ ∅

eq. 11.11：E_A ∈ [(a ∈ H, f ∈ B_C, v ∈ N_{|κ|}, s ∈ E)]；11.12：a = H_P（anchor 是 parent hash，這也讓 assurance 無法被搬到另一條分叉）；11.13：依 v 嚴格遞增；11.14：s ∈ Ed25519 由 κ[v]_e 對 X_A ⌢ H(E(H_P, f)) 簽署，X_A = $jam_available；11.16：f[c] ⇒ ρ†[c] ≠ ∅。注意 signer 用的是 **prior** κ（0.6.4 起「Assurances are checked with the prior validator set」），而 11.16 的前提是 ρ†（disputes 之後、assurances 之前）——ρ‡ 要處理完這些 assurance 才算得出來，拿它當前提會變成循環定義。bit = 1 的意義是「我保證我正在為它的 availability 出力」（軟性宣告，§11.2.1 footnote）。你們的錯誤碼：BadAttestationParent、NotSortedOrUniqueAssurers、CoreNotEngaged。

**逐項辨析**

1. ✅ Each assurance = (anchor a, bitfield f ∈ bits[C], validator index v, Ed25519 sig); a must equal H_P; assurances strictly ordered by v (so at most one per validator); the signature by κ[v]_e is over X_A ⌢ H(E(H_P, f)); a bit may be set only if ρ†[c] ≠ ∅  
   五個條件全中，關鍵是 anchor 綁 parent hash H_P、簽名用的是 prior 的 κ[v]_e。
2. ❌ Each assurance = (anchor a, bitfield f ∈ bits[C], validator index v, Ed25519 sig); a must equal the hash of the block being built; assurances strictly ordered by v; the signature by κ′[v]_e is over X_A ⌢ H(E(H_P, f)); a bit may be set only if ρ†[c] ≠ ∅  
   簽名當下本塊 hash 還不存在，eq. 11.12 因此綁 H_P；eq. 11.14 用的也是 prior 的 κ。
3. ❌ Each assurance = (anchor a, bitfield f ∈ bits[|κ|], validator index v, Bandersnatch sig); a must equal H_P; a validator may send several assurances so long as they stay sorted by v; the signature is over X_A ⌢ H(E(H_P, f)); a bit may be set only if ρ†[c] ≠ ∅  
   f ∈ bits[C] 是每個 core 一 bit、簽章是 Ed25519，而 eq. 11.13 的嚴格遞增讓每人至多一筆。
4. ❌ Each assurance = (anchor a, bitfield f ∈ bits[C], validator index v, Ed25519 sig); a must equal H_P; assurances strictly ordered by core index; the signature by κ[v]_e is over X_A ⌢ the erasure root of each assured report; a bit may be set only if ρ‡[c] ≠ ∅  
   排序鍵是 assurer 的 index v；一筆 assurance 只簽整個 bitfield 一次；前提是 ρ† 而非 ρ‡。

> **陷阱**　bitfield 長度 = C（cores），full 是 43 bytes（341 bits），tiny 1 byte；LSB-first。

<sub>`ch11-assurance-rules`</sub>

---

### 11-18　When does a report become available (R), and when is a pending assignment cleared from ρ‡ in GP 0.8.0?
...
4. ❌ Available iff the number of assurances with bit c set is > 1/2·|κ| (tiny: ≥ 4 of 6; full: ≥ 512); ρ‡[c] = ∅ only once the report has been accumulated, so an assignment never times out and never reacts to a change in |κ|  
   門檻是 2/3 super-majority；等 accumulate 才清會讓湊不到票的 report 把 core 永久卡死。

> **陷阱**　timeout 比較的是 H_T（本塊 slot）與 assignment 的 t（guarantee 進鏈的 slot）。

<sub>`ch11-availability-threshold`</sub>

---

### 11-19　How are validators assigned to cores for guaranteeing in GP 0.8.0?
...
> **陷阱**　rotation period R = 10 slots；每個 epoch 600/10 = 60 次 rotation。

<sub>`ch11-guarantor-assignment`</sub>

---

### 11-20　Which statement about a guarantee g = (w, t, a) in E_G is correct?

<sub>11.4 Work Report Guarantees — ●●● · 概念 · eq. 11.24–11.29</sub>

**標準答案**　a has 2 or 3 (validator index, Ed25519 sig) pairs ordered by index; every signer must be assigned to core w_c under M when t is in the current rotation and under M* otherwise; R·(⌊τ′/R⌋ − 1) ≤ t ≤ τ′; each signature is over X_G ⌢ H(E(w)); w_c < |κ′|/3

eq. 11.24：G ≡ (w ∈ R, t ∈ N_T, a ∈ [(N, E)]_{2:3})；11.25–11.26：E_G ∈ [G]_{:C}，依 core 排序且唯一；11.27：credential 依 validator index 排序；11.28：∀(v, s) ∈ a：v < |k| ∧ c[v] = w_c < |κ′|/3、s ∈ Ed25519_{k[v]_e}(X_G ⌢ H(w))、R(⌊τ′/R⌋ − 1) ≤ t ≤ τ′；(c, k) = M 當 ⌊τ′/R⌋ = ⌊t/R⌋，否則 M*。GP：「Use of an inactive core is not permitted even if a timeslot in the previous rotation is used」。訊息先 Blake2b 再冠上 context string X_G，是為了避免同一段位元組在別的協定情境下被重放。簽署者的 Ed25519 key 會進入 reporters 集合 R（給 π 統計）。

**逐項辨析**

1. ✅ a has 2 or 3 (validator index, Ed25519 sig) pairs ordered by index; every signer must be assigned to core w_c under M when t is in the current rotation and under M* otherwise; R·(⌊τ′/R⌋ − 1) ≤ t ≤ τ′; each signature is over X_G ⌢ H(E(w)); w_c < |κ′|/3  
   eq. 11.28 的五個條件全中：c[v] = w_c、w_c < |κ′|/3、簽 X_G ⌢ H(E(w))、t 可回溯一個 rotation。
2. ❌ a has exactly 3 (validator index, Ed25519 sig) pairs ordered by index; every signer must be assigned to core w_c under M when t is in the current rotation and under M* otherwise; t must equal τ′; each signature is over the encoded report E(w) itself; w_c < |κ′|/3  
   credential 型別是 [(index, sig)]_{2:3}，2 個就成立；t 可早於 τ′；訊息要先 hash 再冠 context。
3. ❌ a has 2 or 3 (validator index, Ed25519 sig) pairs ordered by index; signers may be any members of κ′ ∪ λ′ whatever core they are assigned to; R·(⌊τ′/R⌋ − 1) ≤ t ≤ τ′; each signature is over X_G ⌢ H(E(w)); any core index below C = 341 may be used  
   eq. 11.28 要求 c[v] = w_c，簽署者必須正好被指派到這個 core；active core 上限是 |κ′|/3。
4. ❌ a has 2 or 3 (validator index, Ed25519 sig) pairs ordered by index; every signer must be assigned to core w_c under M* when t is in the current rotation and under M otherwise; R·(⌊τ′/R⌋ − 1) ≤ t ≤ τ′; each signature is over X_G ⌢ H(E(w)); w_c < C/3  
   M 與 M* 對調了：t 落在當前 rotation 用 M；且 C/3 ≈ 113 不是任何 config 的 active core 數。

> **陷阱**　t 是 guarantee 的 slot，可比 τ′ 早最多一個 rotation；ρ′ 的 timestamp 卻是 τ′（eq. 11.46）。

<sub>`ch11-guarantee-validity`</sub>

---

### 11-21　Which on-chain checks apply to each incoming report w before it is placed in ρ′?

<sub>11.4 Work Report Guarantees — ●●○ · 概念 · eq. 11.31–11.33</sub>

**標準答案**　ρ‡[w_c] = ∅ (core free after disputes and assurances); w_a ∈ α[w_c] (authorizer in the PRIOR pool); Σ digest accumulate-gas ≤ G_A = 10,000,000 and each digest's gas ≥ δ[d_s]_g; the erasure-chunk count equals |κ′|

eq. 11.31：∀r ∈ I：(r_s)_v = |κ′|（erasure chunk 數 = posterior 的 validator 數，因為 chunk 是發給下一組 assurer 的）；11.32：∀r ∈ I：ρ‡[r_c] = ∅ ∧ r_a ∈ α[r_c]——core 空不空要等 disputes 清完（ρ†）、assurances 與 timeout 收完（ρ‡）才知道，而 authorizer 只能比 **prior** 的 α；11.33：Σ_{d ∈ r_d} d_g ≤ G_A（10M，每個 report 的 accumulate gas 上限）∧ ∀d：d_g ≥ δ[d_s]_g。對照另外兩個容易混的預算：G_T = 3.5·10^9 是**整塊** accumulation 的總預算，G_R = 5·10^9 是單一 work-package refine 的預算（§14，鏈下判定）。

**逐項辨析**

1. ✅ ρ‡[w_c] = ∅ (core free after disputes and assurances); w_a ∈ α[w_c] (authorizer in the PRIOR pool); Σ digest accumulate-gas ≤ G_A = 10,000,000 and each digest's gas ≥ δ[d_s]_g; the erasure-chunk count equals |κ′|  
   eq. 11.32 檢查的是 ρ‡，所以同一塊裡剛騰出的 core 可以馬上再被用。
2. ❌ ρ[w_c] = ∅ (core free in the prior state); w_a ∈ α′[w_c] (authorizer in the POSTERIOR pool); Σ digest accumulate-gas ≤ G_T = 3,500,000,000 and each digest's gas ≥ δ[d_s]_g; the erasure-chunk count equals |κ′|  
   用 prior ρ 會擋掉本塊剛騰出的 core；α′ 要 accumulation 之後才存在；G_T 是整塊的總預算。
3. ❌ ρ‡[w_c] = ∅ and the core must additionally have stayed free for U = 5 slots; w_a ∈ α[w_c]; Σ digest accumulate-gas ≤ G_A = 10,000,000 with no per-service floor; the erasure-chunk count equals |κ′|  
   eq. 11.32 只要求 ρ‡[r_c] = ∅、沒有「空滿 U 個 slot」；eq. 11.33 也明確有 d_g ≥ δ[d_s]_g。
4. ❌ ρ‡[w_c] = ∅ (core free after disputes and assurances); w_a ∈ α[w_c]; Σ digest refine-gas used ≤ G_R = 5,000,000,000 and each digest's gas ≥ δ[d_s]_g; the erasure-chunk count equals |κ| in the prior state  
   eq. 11.33 管的是 accumulate gas 而不是 refine 的 G_R；eq. 11.31 比的也是 posterior 的 |κ′|。

> **陷阱**　α（prior）用於檢查，α′ 在 accumulate 之後才算。

<sub>`ch11-report-checks-state`</sub>

---

### 11-22　Which of the following is NOT one of the contextual validity requirements for reports in E_G?

<sub>11.4.1 Contextual Validity of Reports — ●●● · 概念 · eq. 11.35–11.45</sub>

**標準答案**　Every digest's service must have been accumulated at least once within the last epoch E, and a service left idle for longer must be re-registered before it may be reported again

這題考的是 §11.4.1 的完整清單：eq. 11.35（同一塊不得有兩份同 package 的 report）、11.36–11.38（anchor 四元組對 β†、lookup anchor 的時間下限與祖先集合 A）、11.39–11.41（package hash 不得出現在 β 的 reported 集合、ξ、ready queue ω 或任何 pending 的 ρ）、11.42–11.44（prerequisites 與 srlookup 的 key 與 segment root）、11.45（d_c = δ[d_s]_c）。service 只會因 eject 而消失（§9），report 是否合法完全不看該 service 上次 accumulate 的時間。§11.4.1 附註：這些檢查刻意允許表面上的 dependency loop，因為 accumulation 端的 Q 函數永遠不會 accumulate 它們。

**逐項辨析**

1. ❌ No two reports in the extrinsic share a work-package hash, and no package hash may appear in β's reported sets, in ξ (accumulated), in the ready queue ω or in a pending assignment of ρ  
   eq. 11.35 加 11.39–11.41 就是這一項：不重複，且不在 β、ξ、ω、ρ 任何一段 pipeline 裡。
2. ❌ The anchor's (a, n, s, b) must match an entry of β†; the lookup-anchor time must be ≥ H_T − L, and the lookup-anchor header must be found in the ancestor set A  
   eq. 11.36（anchor 四元組對 β†）、11.37（l_t ≥ H_T − L）、11.38（header 在 A 裡）確有此三條。
3. ❌ Every prerequisite and every segment-root-lookup key must appear in this extrinsic or in β's reported sets, and the looked-up segment roots must match those records  
   eq. 11.42 與 11.43–11.44 正是這一項：prereq/srlookup 的 key 與 segment root 都要對得上。
4. ✅ Every digest's service must have been accumulated at least once within the last epoch E, and a service left idle for longer must be re-registered before it may be reported again  
   GP 沒有「service 閒置過久失效」這條規則，唯一相關的是 eq. 11.45 的 d_c = δ[d_s]_c。

> **陷阱**　面試官可能要求你列舉「所有」contextual checks——建議背下 anchor / lookup-anchor / no-dup / prereq / srlookup / code-hash 六類。

<sub>`ch11-contextual-validity`</sub>

---

### 11-23　After processing E_G, what does ρ′[c] hold for a core that received a new guarantee g?

<sub>11.5 Transitioning for Reports — ●○○ · 概念 · eq. 11.46</sub>

**標準答案**　(g, τ′) — the whole guarantee paired with the current block's timeslot τ′ as its assignment time

eq. 11.46：ρ′[c] ≡ (g, t: τ′) 當 ∃g ∈ E_G：(g_w)_c = c，否則 ρ‡[c]。§11.5 附註：對 c ≥ |κ′|/3 的 core，ρ′[c] 永遠是 ∅，保證待審計的 report 數不會超過 active validator 能負擔的量。你們 code-map 3.7.6 特別標了這個坑：「the assignment timestamp is τ′, not the guarantee's slot t」——它直接決定 eq. 11.18 的 timeout（H_T ≥ t + U）從哪一刻開始算。

**逐項辨析**

1. ✅ (g, τ′) — the whole guarantee paired with the current block's timeslot τ′ as its assignment time  
   eq. 11.46 存的時間戳是 τ′（本塊 slot），timeout 因此從進鏈那一刻起算。
2. ❌ (g, g_t) — the whole guarantee paired with the guarantee's own slot t, which may lie a rotation earlier  
   g_t 可比 τ′ 早一整個 rotation（R = 10），一進鏈就超過 U = 5，下一塊立刻被清掉。
3. ❌ (w, τ′) — only the work-report, the guarantor credential being discarded once it has been verified  
   0.8.0 (#494) 的 ρ 型別就是 (g, t)：簽章要留著抓 bundle、以及在 dispute 時構造 culprits。
4. ❌ (H(E(w)), τ′) — the report hash paired with the current block's timeslot, the guarantee moving into β  
   β 每筆從不存 guarantee（eq. 7.2）；而 eq. 11.17 之後要交出整份 report，只有 hash 不夠。

<sub>`ch11-rho-prime`</sub>

---

### 11-24　The team's availability check uses `totalAvailable[i] >= types.ValidatorsSuperMajority`. Which statement is accurate?
...
```
<sub>internal/extrinsic/assurance_controller.go (UpdateNewlyAvailableWorkReports)</sub>
...
eq. 11.17：Σ_a a_f[c] > (2/3)|κ|。對整數而言 > 2V/3 ⇔ ≥ ⌊2V/3⌋ + 1：V = 6 → > 4 → ≥ 5；V = 1023 → > 682 → ≥ 683。所以 `>= ValidatorsSuperMajority` 這個寫法本身是對的，真正的風險在該常數必須等於 ⌊2|κ|/3⌋+1 且隨 |κ| 變動（0.8.0 起 validator 數可變），不能是編譯期寫死的數字。另外 `rhoDagger[i] == nil` 的情況其實已被 eq. 11.16（bit 只能在有 pending report 的 core 上設）擋掉，這裡是防禦性檢查。

**逐項辨析**

1. ✅ It matches the GP's strict '> 2/3·|κ|' exactly when ValidatorsSuperMajority = ⌊2|κ|/3⌋ + 1 is derived from the live size of κ — 5 of 6 in tiny, 683 of 1023 in full — rather than from a compile-time constant  
   > 2V/3 對整數等價於 ≥ ⌊2V/3⌋+1，所以只要該常數隨 |κ| 變動，這個比較就是對的。
2. ❌ It is wrong: eq. 11.17 asks only for ≥ 2/3·|κ|, so 4 of 6 assurances suffice in tiny and 682 of 1023 in full; the comparison should be against ⌈2|κ|/3⌉ instead of a super-majority constant  
   eq. 11.17 寫的是嚴格大於，⌈2·6/3⌉ = 4 會少算一票，平手局面不該讓 report 變成可用。
3. ❌ It is wrong: eq. 11.17 counts, per core, the assurers that set that core's bit, whereas the loop must first count the distinct validators appearing anywhere in E_A and compare that with the super-majority  
   eq. 11.17 的 Σ 是逐 core 對 a_f[c] 求和；混著數會把只 assure 了別的 core 的人算進來。
4. ❌ It is wrong: a core whose rhoDagger entry is nil must still contribute its report, because eq. 11.18 only empties ρ‡ after the availability count for the block has already been taken  
   那個 core 根本沒有 report 可送；ρ‡ 是 eq. 11.18 的輸出，不是計票當下的狀態。

> **陷阱**　tiny 5/6、full 683/1023；同一個 super-majority 常數也用在 verdict 的 good 門檻。

<sub>`ch11-code-availability`</sub>
...
### 12-1　Among the state items in σ written by accumulation are ω, ξ and θ. A GP 0.7.2-era Go client still calls its ready-queue field `Vartheta`. Which mapping of symbol → meaning is right for GP 0.8.0?

<sub>12.1 History and Queuing — ●○○ · 版本差異 · eq. 12.1 & 12.3; eq. 7.4 (θ); §4.2 state σ · ⚠ 0.7.2→0.8.0</sub>

**標準答案**　ω ∈ ⟦⟦(ℝ, {H})⟧⟧_E is the ready (accumulation) queue of deferred reports each paired with its outstanding dependency set — GP 0.7.2 wrote this as ϑ; ξ ∈ ⟦{H}⟧_E is the epoch-long history of already-accumulated work-package hashes; θ ∈ ⟦(N_S, H)⟧ is the Accumulation Output Log, this block's (service, commitment) pairs

GP 0.8.0 的 preamble 明訂 ready = ω、accumulated = ξ、lastaccout = θ。eq. 12.3 給出 ω ∈ ⟦⟦(ℝ, {H})⟧⟧_E（ℝ 是 0.8.0 的 work-report 集合，0.7.2 記作 W；每個 slot 一串「report + 尚未滿足的 dependency 集合」），eq. 12.1 給出 ξ ∈ ⟦{H}⟧_E（一個 epoch 份量的已 accumulate work-package hash），eq. 7.4 / 12.25 給出 θ ∈ ⟦(N_S, H)⟧ 亦即本區塊的 accumulation output log。0.7.2 的 ready queue 符號是 ϑ（團隊 Go 欄位 `Vartheta`、state key C(14)），0.8.0 改成 ω（GP 與 0.8.0 release notes 都沒有說明改名理由，別在口試時把「為了避開 ϑ/θ 混淆」講成 GP 的說法）；state key 不變：C(14)=ω、C(15)=ξ、C(16)=θ。

**逐項辨析**

1. ✅ ω ∈ ⟦⟦(ℝ, {H})⟧⟧_E is the ready (accumulation) queue of deferred reports each paired with its outstanding dependency set — GP 0.7.2 wrote this as ϑ; ξ ∈ ⟦{H}⟧_E is the epoch-long history of already-accumulated work-package hashes; θ ∈ ⟦(N_S, H)⟧ is the Accumulation Output Log, this block's (service, commitment) pairs  
   0.8.0 preamble 把 ready 定為 ω、accumulated 定為 ξ、lastaccout 定為 θ，三個型別也全對。
2. ❌ ω ∈ ⟦(N_S, H)⟧ is the Accumulation Output Log, renamed from θ in 0.8.0; ξ ∈ ⟦⟦(ℝ, {H})⟧⟧_E is the ready queue of deferred reports each paired with its outstanding dependency set; θ ∈ ⟦(N_S, N_G)⟧ now holds the per-service gas usage u that the last block's Δ+ returned, which is what the accumulation statistics read  
   per-service gas usage u 是 eq. 12.17 的回傳分量，餵完 eq. 12.28 的 G(s) 就消失，進不了 σ。
3. ❌ ω ∈ ⟦{H}⟧_E took over from ξ as the epoch-long history of already-accumulated work-package hashes; the ready queue is still spelled ϑ ∈ ⟦⟦(ℝ, {H})⟧⟧_E in 0.8.0 and θ ∈ ⟦(N_S, H)⟧ is unchanged, so a 0.7.2 client only has to retire the symbol ξ and repoint the state key that held it  
   ξ 在 0.8.0 完全沒有改名或改義，「只要把 ξ 退休掉」的遷移會把 accumulated 歷史整個弄丟。
4. ❌ ω ∈ ⟦(N_S, N_S, N_B, B_128, N_G)⟧ is a new 0.8.0 state item holding the deferred transfers carried between accumulation rounds so that they survive a block boundary; ϑ ∈ ⟦⟦(ℝ, {H})⟧⟧_E remains the ready queue and ξ ∈ ⟦{H}⟧_E the epoch-long history of accumulated work-package hashes  
   deferred transfers 從不進 σ，只在 Δ+ 遞迴內部傳遞；eq. 12.14 只是 Ψ_A 的輸入型別。

> **陷阱**　面試常考：0.7.2→0.8.0 ready queue 是 ϑ→ω 的更名，state key C(14) 與型別結構都沒動；同章的 W!/W_Q/W* 也一併改成 R!/R^Q/R*；別把 ω 和 θ（output log）搞混。

<sub>`c3-ch12-omega-xi-theta-naming`</sub>

---

### 12-2　Δ1 builds i^U as one operand tuple per work-digest of service s, taken from the round's reports in order. Two of s's digests sit in different reports r₁ and r₂. Which description of what an operand tuple carries is right for GP 0.8.0?

<sub>12.2 Execution — ●○○ · 概念 · eq. 12.13 (U), 12.14 (X), 12.23 (Δ1)</sub>
...
1. ✅ The set is U (and deferred transfers are X, Ψ_A taking ⟦U ∪ X⟧). Each tuple mixes digest-level fields — y payload hash, g accumulate gas-limit, l the result, which is a blob or a member of the work-error set E — with fields lifted from its own report: p = (r_s)_p package hash, e = (r_s)_e segment root, a = r_a authorizer hash and t = r_t authorizer trace; so the two tuples differ in their report-level fields  
   eq. 12.23 明寫來源：y/g/l 取自 digest，t/e/p/a 取自所屬 report，兩筆的 report-level 欄位因此不同。
2. ❌ The set is U (and deferred transfers are X, Ψ_A taking ⟦U ∪ X⟧). An operand tuple is the work-digest verbatim, so besides y, g and l it also carries c the code hash, d_u the gas actually used during refine, and the import / extrinsic / export counts that feed the core statistics; nothing is lifted in from the enclosing report, so the two tuples differ only in their payload hashes  
   code hash c、refine 實際用掉的 gas 與 i/x/z/e 計數只留在 digest，餵 §13 統計與 §11.4 檢查，不進 accumulate。
3. ❌ The set is U (and deferred transfers are X, Ψ_A taking ⟦U ∪ X⟧). One operand tuple is built per report rather than per digest: its l field holds the concatenation of that report's digest results and its g field their summed gas-limits, with p, e, a and t lifted from the report as usual, and the service demultiplexes the results by payload hash — so s receives one tuple here, not two  
   與 eq. 12.23 的 d ↕ r_d 迭代直接矛盾：該 service 這一輪有幾個 digest 就有幾個 operand。
4. ❌ The set is U (and deferred transfers are X, Ψ_A taking ⟦U ∪ X⟧). Each tuple takes y, g and l from the digest and, from its report, the core index and the refinement-context anchor in place of the package hash and segment root, so that accumulate can re-verify availability; digests whose l falls in the work-error set E are dropped before i^U is assembled, so the two tuples survive only if both refined successfully  
   eq. 12.13 的七個欄位裡沒有 core index 與 refinement context；l 的型別是 B ∪ E，錯誤結果不會被濾掉。

> **陷阱**　口訣：operand = 「digest 三件（y, g, l）+ report 四件（p, e, a, t）」；refine 用掉多少 gas 不在裡面。

<sub>`c3-ch12-operand-tuple-fields`</sub>

---

### 12-3　The queue-editing function E(r, x) does two jobs: it strips dependencies that appear in x, and it also discards whole entries. A teammate implements only the first job. What actually goes wrong?

<sub>12.1 History and Queuing — ●●○ · 設計理由 · eq. 12.7 (E), 12.8 (Q), 12.12, 12.33 (ω′)</sub>
...
1. ✅ Q stops terminating — E(r, P(g)) hands back the same entries with still-empty dependency sets, so the next recursion picks the identical g forever — and at the block boundary ω′ (both the i = 0 case E(R^Q, ξ′[E−1]) and the i ≥ τ′ − τ case E(ω↺[m−i], ξ′[E−1])) keeps reports that were just accumulated, so they would be accumulated again in a later block  
   少了 (r_s)_p ∉ x 這一半，eq. 12.8 每一輪都算出同一批 g，遞迴永遠停不下來。
2. ❌ Only ω′ is affected — both the i = 0 case E(R^Q, ξ′[E−1]) and the i ≥ τ′ − τ case E(ω↺[m−i], ξ′[E−1]) would keep reports that were just accumulated, so the ready queue grows without bound until the epoch-long shift drops the stale entries. Q itself stays safe, because each recursion is fed a strictly shorter sequence once its dependency sets have been stripped  
   沒有刪除就不會變短：Q 每輪拿到的序列長度不變，遞迴照樣不收斂。
3. ❌ Nothing breaks. Q still terminates because its recursion is bounded by the epoch length E, and the §11.4 contextual check rejects any incoming report whose package hash is already in ξ, so an entry sitting in ω can never be accumulated a second time either; discarding whole entries is a pure optimisation that keeps ω small  
   §11.4 管的是 E_G 新進來的 guarantee，管不到早就躺在 ω 裡的項目；Q 也沒有 epoch 長度的界。
4. ❌ The dependency-stripping job becomes redundant instead: once entries are never removed, a report can only ever be blocked by another report sitting in the same queue, so every dependency E would have stripped belongs to an entry E would also have deleted. Q therefore still terminates and ω′ still drains, just more slowly — one epoch-long shift at a time rather than one accumulation round at a time  
   被依賴的那份可能走 R! 或早已只留在 ξ，剪依賴與刪項目作用在不同項目上，兩者不互相涵蓋。

> **陷阱**　E 是「刪項目 + 剪依賴」兩件事；只做剪依賴，Q 就不會收斂。

<sub>`c3-ch12-E-removes-entries-too`</sub>

---

### 12-4　A report w is sitting in ω with one outstanding dependency p, and package p is never accumulated — the report carrying p was guaranteed a few blocks ago but timed out on its core and never became available. Trace w's fate under GP 0.8.0.

<sub>12.1 History and Queuing — ●●○ · 概念 · eq. 12.8 (Q), 12.31–12.33; §12.1</sub>

**標準答案**　Q never emits it, because its dependency set is never emptied; each block it is simply carried forward by the i ≥ τ′ − τ case ω′↺[m−i] = E(ω↺[m−i], ξ′[E−1]), and it vanishes at most one epoch later when the slot index wraps round and the i = 0 case overwrites that very slot with the new block's R^Q

eq. 12.8 的 Q 只挑 dependency 集合為空的項目（g = [r | (r, ∅) ↕ r]），w 永遠不會被挑中。eq. 12.33 的三個 case 決定它的壽命：i = 0 用本區塊的 R^Q 覆寫目前 slot、1 ≤ i < τ′ − τ 把跳過的 slot 清成 []、i ≥ τ′ − τ 則保留舊內容但套用 E(·, ξ′[E−1])。由於索引是 cyclic 的 ω′↺[m − i]，w 所在的 slot 在最多 E = 600 個 slot 後就會輪到 i = 0 被整個覆寫，這正好呼應 §12.1 的敘述「Each of these were made available at most one epoch ago」。GP 說的 cancelled entirely 指的是這種靜默淘汰——§12.1 沒有任何條款把 dependency 問題升級成區塊無效（§12 另有區塊有效性條件：12.18 底下 n/m 的索引衝突、以及 eq. 12.35–12.36 對 E_P 的要求，但與 dependency 無關）。

**逐項辨析**

1. ❌ Q never emits it, because its dependency set is never emptied; it is then dropped at the very next block, because E(·, ξ′[E−1]) removes any entry still carrying a dependency that is absent from ξ — which is exactly how the GP cancels a report with an unsatisfiable dependency  
   把 E 的語意反了：第二個參數是「已完成」的 hash 集合，只刪自己 package hash 落在其中的項目。
2. ❌ Q never emits it, and the block that made w available is invalid: §12.1 says accumulation is cancelled entirely in the case of an invalid dependency, and cancellation is signalled by rejecting the block, so a report that times out on its core retroactively invalidates whichever later block first queued the report that depended on it  
   §12.1 的 cancelled entirely 指靜默淘汰；guarantor 在 core 上逾時本來就是 §11 允許的正常結果。
3. ❌ Q holds it back only for a while: after E = 600 slots the dependency is deemed expired, the i ≥ τ′ − τ case stops re-applying E to that slot, and Q then treats w's dependency set as empty and accumulates it with whatever operands it has — so the work is delayed by at most one epoch but never silently lost  
   「到期就視為依賴滿足、照樣 accumulate」是憑空發明的規則，eq. 12.8 只挑 dependency 集合為空者。
4. ✅ Q never emits it, because its dependency set is never emptied; each block it is simply carried forward by the i ≥ τ′ − τ case ω′↺[m−i] = E(ω↺[m−i], ξ′[E−1]), and it vanishes at most one epoch later when the slot index wraps round and the i = 0 case overwrites that very slot with the new block's R^Q  
   eq. 12.33 的 i ≥ τ′ − τ 逐塊把它搬過去，直到 slot index 繞回、i = 0 那格被新的 R^Q 整個覆寫。

> **陷阱**　ready queue 的淘汰不是超時計時器，而是 cyclic buffer 被同一個 slot index 繞回來覆寫。

<sub>`c3-ch12-starved-dependency-fate`</sub>

---

### 12-5　G_A = 10,000,000 and G_T = 3,500,000,000, while every service account carries a_g and a_m. Which statement correctly places each accumulate-gas limit — the ceilings and the per-service floors?

<sub>12.2 Execution — ●●○ · 概念 · §11.4 (report gas checks); eq. 12.23 (Δ1 g), 12.24 (block g); §9.1</sub>

**標準答案**　Per report, §11.4 requires Σ over its digests of d_g ≤ G_A and every digest's d_g ≥ δ[d_s]_g (the destination service's a_g), both checked at guarantee time against the prior δ; per block, eq. 12.24 sets Δ+'s budget to g = max(G_T, G_A·C + Σ values(χ_Z)); and the `transfer` host call refuses with LOW unless the transfer's gas l ≥ δ[d]_m, so a deferred transfer also arrives with enough gas for the recipient's Accumulate

三個層級要分清楚。(1) Report 層：§11.4 的條件是 ∀w ∈ I: Σ_{d ∈ w_d}(d_g) ≤ G_A ∧ ∀d ∈ w_d: d_g ≥ δ[d_s]_g，是在 guarantee 進來時用 prior δ 檢查，跟 §12 無關（真要在 Δ+ 重驗，posterior δ 裡的 a_g 可能已被本塊改過，反而會讓合法區塊被拒）。(2) Block 層：eq. 12.24 的 g = max(G_T, G_A·C + Σ_{x ∈ values(χ_Z)}(x))，definitions.tex 也說 G_T「should be no smaller than G_A·C + Σ」。(3) Service 層的兩個下限來自 §9.1：a_g 是「每個 work-item」執行 Accumulate 所需的最低 gas，a_m 是「每筆 deferred transfer」所需的最低 gas，後者由 `transfer` host call 以 LOW 錯誤把關。

**逐項辨析**

1. ✅ Per report, §11.4 requires Σ over its digests of d_g ≤ G_A and every digest's d_g ≥ δ[d_s]_g (the destination service's a_g), both checked at guarantee time against the prior δ; per block, eq. 12.24 sets Δ+'s budget to g = max(G_T, G_A·C + Σ values(χ_Z)); and the `transfer` host call refuses with LOW unless the transfer's gas l ≥ δ[d]_m, so a deferred transfer also arrives with enough gas for the recipient's Accumulate  
   三個層級各歸各位：report 層 G_A 與 a_g（對 prior δ 檢查）、block 層 eq. 12.24、transfer 層 a_m。
2. ❌ Per report, §11.4 requires Σ over its digests of d_g ≤ G_A; per service, G_A is additionally a per-block cap enforced inside Δ1, which is why eq. 12.23 clamps that service's g at G_A instead of summing its sources; and a_g is a refund floor rather than a minimum — whatever a service leaves unspent below a_g is credited back to its balance at the end of Δ+, with a_m playing the same role for deferred transfers  
   eq. 12.23 的 g 是三項相加、沒有任何 clamp；§9.1 的 a_g 是准入下限而不是事後回沖。
3. ❌ Both ceilings are enforced inside Δ+ at accumulation time rather than at guarantee time: eq. 12.24's budget covers the whole block and §11.4's per-report sum is re-checked there against the posterior δ, so a service whose a_g exceeds G_A can never be accumulated — which is why a_g ≤ G_A is maintained as a state invariant by the `new` host call, and a_m is bounded against G_T the same way  
   §11.4 兩條 gas 條件在 guarantee 時就對 prior δ 檢查過，Δ+ 不會重驗；GP 也沒有這條不變量。
4. ❌ Per report, §11.4 requires Σ over its digests of d_g ≤ G_T, G_T being the figure that bounds any single report; per block, eq. 12.24 builds Δ+'s budget out of G_A, the whole-block ceiling, which is why G_T is the larger constant of the two; and the two service-level floors run the other way round — a_g is the minimum gas per deferred transfer and a_m the minimum per work-item  
   數量級剛好反了：G_T 比 G_A 大 350 倍，當成單一 report 上限會讓一份 report 吃掉整塊預算。

> **陷阱**　上限往下走（block g → report G_A），下限往上走（a_g / a_m → 呼叫方必須配足）；別把兩個方向搞反。

<sub>`c3-ch12-gas-floor-vs-ceiling`</sub>

---

### 12-6　In one block: service 5 accumulates and calls `yield` with a 32-octet hash; service 6 accumulates and burns gas but never calls `yield`; service 9 has no work-digest at all and is reached only by a deferred transfer, and it calls `yield`. What ends up in θ′, and what consumes it?
...
### 12-7　The preimage-integration function I is applied twice during a block's transition. Where are the two applications, and what does its Y predicate silently discard?

<sub>12.2 Execution / 12.4 Preimage Integration — ●●○ · 概念 · eq. 12.18 (d′), 12.20 (I), 12.21 (Y), 12.37</sub>

**標準答案**　Once inside Δ* as e_d′ = I((e_d ∪ n) ∖ m, ⋃_{s ∈ s} Δ(s)_p) — folding in the blobs offered by the `provide` host call only after newly created and removed services have been merged — and once at the very end as δ′ = I(δ‡, E_P). Y(d, s, i) is ⊥ unless s ∈ K(d) and d[s]_l[(H(i), |i|)] = [], so a blob aimed at a service ejected in the same round, or at a request that is not merely solicited, is dropped without prejudice

eq. 12.18 的 e_d′ = I((e_d ∪ n) ∖ m, ⋃_{s ∈ s} Δ(s)_p)：`provide` host call 產生的 provisions Δ(s)_p 是在 Δ* 這一層、而且是在新建服務 n 併入、被刪服務 m 移除「之後」才整合，所以同一輪被 eject 掉的服務收不到 blob（否則 n 與 m 還沒算出來，就無法判斷目標 service 是否還存在）。eq. 12.37 的 δ′ = I(δ‡, E_P) 是第二次，處理 preimage extrinsic。eq. 12.21 的 Y(d, s, i) 只在 s ∈ K(d) 且 d[s]_l[(H(i), |i|)] = ⟦⟧ 時為真，GP 原文也明說「Preimage provisions into services which no longer exist or whose relevant request is dropped are disregarded」——是靜默略過，不是讓區塊無效。

**逐項辨析**

1. ✅ Once inside Δ* as e_d′ = I((e_d ∪ n) ∖ m, ⋃_{s ∈ s} Δ(s)_p) — folding in the blobs offered by the `provide` host call only after newly created and removed services have been merged — and once at the very end as δ′ = I(δ‡, E_P). Y(d, s, i) is ⊥ unless s ∈ K(d) and d[s]_l[(H(i), |i|)] = [], so a blob aimed at a service ejected in the same round, or at a request that is not merely solicited, is dropped without prejudice  
   eq. 12.18 的 I 在 n 併入、m 移除之後才收 provisions，eq. 12.37 的 I 再收 E_P，兩處各司其職。
2. ❌ Only once, at the very end as δ′ = I(δ‡, E_P): the `provide` host call writes straight into the service's a_p and a_l during accumulation, so Δ* has nothing left to integrate. Y(d, s, i) is ⊥ unless s ∈ K(d) and d[s]_l[(H(i), |i|)] = [], so a blob aimed at a service ejected in the same round is dropped without prejudice — but that can only ever bite the extrinsic, never a `provide`  
   漏掉 Δ* 這一層的合流：eq. 12.18 的 e_d′ 就是用 I 把 ⋃ Δ(s)_p 併進去的。
3. ❌ Twice — once inside Δ* and once at the very end — but both applications take E_P as their second argument, the blobs offered by `provide` having been merged into E_P before Δ* runs. Y(d, s, i) additionally requires the blob to be under W_C = 4,000,000 octets and the service to hold the storage deposit for it, and a failure of either condition makes the block invalid rather than dropping the blob  
   eq. 12.20–12.21 的 Y 只有兩個條件，沒有 W_C 與押金，且不合格是 disregarded 而非讓區塊無效。
4. ❌ Once inside Δ1 for each accumulating service — so each service folds in its own provisions before Δ* merges the account dictionaries — and once at the very end as δ′ = I(δ‡, E_P). Y(d, s, i) accepts a request slot of the form [] or [x, y], i.e. solicited, or previously available and then forgotten, which is what lets the same preimage be provided twice over a service's lifetime  
   Δ1 只把 provisions 當輸出回傳；[x, y] 代表曾 available 後被 forget，違反 Y 的等號條件。

> **陷阱**　I 出現兩次：Δ* 裡收 `provide`、區塊尾端收 E_P；兩次的丟棄都是「without prejudice」，不會讓區塊無效。

<sub>`c3-ch12-provide-two-places`</sub>

---

### 12-8　This is the team's GP 0.7.2 accumulation-statistics builder S. Which gap against GP 0.8.0 must be closed?

<sub>12.3 Final State Integration — ●●○ · 程式碼 · eq. 12.27–12.28; eq. 12.17 (Δ+ output) · ⚠ 0.7.2→0.8.0</sub>
...
```
<sub>internal/accumulation/deferred_transfers.go (calculateAccumulationStatistics), 0.7.2 main</sub>

**標準答案**　0.8.0 makes S ∈ ⟨N_S → (N, N, N_G)⟩ a three-tuple S(s) = (N(s), T(s), G(s)); the missing middle element T(s) counts the processed deferred transfers whose destination is s, which forces Δ+ to return a fifth component — the processed-transfer sequence t ⌢ t† — and the inclusion filter becomes S(s) ≠ (0, 0, 0) rather than a sum test

GP 0.8.0 eq. 12.27 寫的是 S ∈ ⟨N_S → (N, N, N_G)⟩，eq. 12.28 是 S ≡ {(s ↦ S(s)) | S(s) ≠ (0, 0, 0)}，其中 S(s) ≡ (N(s), T(s), G(s))，T(s) ≡ |[t | t ↕ t, t_d = s]| 就是「送到 s 的已處理 deferred transfer 筆數」（GP PR #502 “Add back processed transfer count to service statistics”）。要算得出 T(s)，eq. 12.17 的 Δ+ 必須多回傳一個 processed-transfer 序列（回傳值是 ⟨i + j, e′, b* ∪ b, u* ⌢ u, t ⌢ t†⟩），而這份 0.7.2 程式的 OuterAccumulationOutput 只有四個欄位、型別也只是 (Gas, NumAccumulatedReports) 兩元組，篩選條件寫成 Gs + Ns == 0 也不等價於三元組全零。S 只收非零項，正是為了讓 δ‡ 只對真的被 accumulate 的服務蓋上 a_a = τ′，而 S 又是 π_S 的來源之一（eq. 13.14 的 K(S)）。

**逐項辨析**

1. ❌ N(s) must count whole work-*reports*, not digests: eq. 12.28 iterates r ↕ R*[..n] and counts each report once for the service that owns it, whereas this code counts one entry per matching digest, so a report carrying two digests for the same service is double-counted  
   eq. 12.28 的 N(s) 明確是 d ↕ r_d 逐個 digest 數，不是逐 report 數。
2. ✅ 0.8.0 makes S ∈ ⟨N_S → (N, N, N_G)⟩ a three-tuple S(s) = (N(s), T(s), G(s)); the missing middle element T(s) counts the processed deferred transfers whose destination is s, which forces Δ+ to return a fifth component — the processed-transfer sequence t ⌢ t† — and the inclusion filter becomes S(s) ≠ (0, 0, 0) rather than a sum test  
   eq. 12.27–12.28 的三元組與 S(s) ≠ (0,0,0) 篩選，逼得 eq. 12.17 的 Δ+ 多回傳 t ⌢ t†。
3. ❌ G(s) must be the gas *allotted* — Σ d_g over the service's digests, the very figure §11.4 checks against G_A — rather than the gas actually used, otherwise a service that runs out of gas is under-charged in the statistics that feed π_S; the two-element shape and the sum-based inclusion filter are both already right for 0.8.0  
   G(s) ≡ Σ_{(s, u) ∈ u}(u) 就是實際用掉的 gas，不是 §11.4 檢查的宣告上限。
4. ❌ S must carry an entry for every service in δ, with the all-zero triple for untouched services, so that the dictionary's key set is stable from block to block and π_S's serialization keeps a fixed length; the shape does become a triple in 0.8.0, but the new element is the transfer count sitting *last*, and it is Δ*, not Δ+, that returns the processed-transfer sequence it is built from  
   S 只收非零項（δ‡ 才只對 K(S) 蓋 a_a）；T(s) 在三元組中間，且 t ⌢ t† 由 Δ+ 而非 Δ* 回傳。

> **陷阱**　0.8.0 的 accumulation statistics 是三元組 (N, T, G)；漏掉 T 會連帶讓 Δ+ 的回傳值少一個分量。

<sub>`c3-ch12-code-accumulation-statistics`</sub>

---

### 12-9　R, R!, R^Q and R* are all sequences rather than sets, even though only membership decides which reports are eligible. Why does the ordering matter for consensus, and where is it fixed?

<sub>12.1 History and Queuing — ●●○ · 設計理由 · eq. 11.17 (R), 12.4, 12.11 (R*), 12.17, 12.31</sub>
...
1. ✅ Because Δ+ accumulates only the gas-fitting prefix R*[..i] and then records ξ′[E−1] = P(R*[..n]): a different ordering yields a different prefix and hence a different posterior state. The order is pinned right back at eq. 11.17, where R is built by walking cores in ascending core index over ρ† and keeping those with more than 2/3·|κ| assurances; R! preserves that order and Q appends the queued reports in dependency-resolution rounds  
   eq. 12.17 只吃 gas 前綴、eq. 12.31 寫 ξ′[E−1] = P(R*[..n])，順序因此是共識的一部分。
2. ❌ It does not matter for the posterior state. Δ* accumulates services in parallel, and any permutation of R* selects the same gas-fitting prefix anyway, because eq. 12.17 picks the maximal i by a sum and addition is commutative; that is exactly why b is a set while u is merely a convenient sequence, and why the GP bothers to fix an order only at eq. 12.11, purely so the prose can talk about R* concretely  
   把「Δ* 內服務可平行」誤讀成「報告可任意重排」：eq. 12.17 選的是前綴，重排會換掉入選的那個集合。
3. ❌ Ordering only affects the accumulation statistics S and therefore π_S, because eq. 12.28's N(s) walks R*[..n] in order; the service state δ† is order-independent, since each service's Δ1 is a pure function of its own operands and its own prior account, and ξ′[E−1] = P(R*[..n]) is a set, so whatever order its members were produced in is lost the moment it is written  
   ξ′[E−1] 雖是集合，決定它內容的仍是前綴長度 n，所以 δ† 與 state root 同樣隨順序而變。
4. ❌ Because Δ+ accumulates only the gas-fitting prefix R*[..i], so when gas runs short the ordering decides what gets dropped. The order is pinned at eq. 11.17, where R is built by ranking cores on how many assurances each collected, most-assured first, so that the best-attested reports are the ones accumulated and the marginal ones are deferred into ω  
   eq. 11.17 走的是 c ↕ N_C；2/3·|κ| 只是布林門檻，跨過就入列，沒有任何名次可言。

> **陷阱**　「哪些會被 accumulate」由 gas 前綴決定，所以序列順序就是共識的一部分；core index 遞增是唯一的來源。

<sub>`c3-ch12-why-R-star-is-a-sequence`</sub>

---

### 12-10　Mid-recursion inside Δ+, the remaining report sequence is empty but the previous Δ* round emitted three deferred transfers t, and the free-accumulation map handed to this call is ∅. Does Δ+ do any more work, and what does it hand back in GP 0.8.0?
...
1. ❌ No. The GP's termination test is the prefix length i, and with r = ⟦⟧ the maximal gas-fitting prefix is i = 0, so the base case ⟨0, e, ∅, ⟦⟧, ⟦⟧⟩ fires and Δ* is never invoked; the three pending transfers are carried into the next block through ω, which is precisely why ω is typed to hold them alongside the deferred reports, and their gas is re-charged against that block's budget g rather than this one's  
   混淆了 i 與 n；ω 型別上只放 report 與 hash 集合，deferred transfer 從不進 σ。
2. ❌ Yes — the test n = i + |t| + |f| = 3 ≠ 0 fires — but the round is a no-op: Δ*'s service set is s = {d_s | r ∈ r, d ∈ r_d} ∪ K(f) alone, so with r[..i] empty and f = ∅ it is empty too, the three transfers are discarded, and only the balances already credited by the earlier `transfer` host calls survive into δ†; u and b therefore come back empty and the three destinations contribute nothing to eq. 12.28's T(s)  
...
<sub>`c3-ch12-accseq-n-with-transfers`</sub>

---

### 12-11　Inside Δ*, core c's posterior authorizer queue φ′[c] and the posterior staging keys ι′ are each lifted out of exactly one service's Δ1 output. Whose output, and selected by which version of χ?

<sub>12.2 Execution — ●●● · 概念 · eq. 12.18 (Δ*), 12.19 (R)</sub>

**標準答案**　φ′[c] = ((Δ(χ_A[c])_e)_q)[c] and ι′ = (Δ(χ_V)_e)_i — the *prior* per-core assigner and the *prior* delegator. Even when this same round rewrites χ′_A[c] or χ′_V, the queue and the keys still come from whoever held the privilege when the round began, and only that service's own copy of the component is read, core index by core index

eq. 12.18 分得很細。**誰擁有特權**用 R 合併：∀c, e_a′[c] = R(e_a[c], (e*_a)[c], ((Δ(e_a[c])_e)_a)[c])，以及 e_v′、e_r′ 同理；而 e_m′ 與 e_z′ 直接取自 manager 的輸出 e* = Δ(m)_e，不經過 R。**特權作用的內容**則另外一條路：e_q′[c] = ((Δ(e_a[c])_e)_q)[c] 與 e_i′ = (Δ(e_v)_e)_i，括號裡的 e_a[c]、e_v 是 Δ* 的輸入、也就是 prior 的 χ_A[c] 與 χ_V。另外 §8 的 α′ 之所以必須在 accumulation 之後才算，authorization.tex 給的理由是「Since α′ is dependent on φ′ … this step must be computed after accumulation, the stage in which φ′ is defined」——那是 α′ 讀 φ′ 造成的順序，與這條 prior/posterior 選擇規則無關。

**逐項辨析**

1. ✅ φ′[c] = ((Δ(χ_A[c])_e)_q)[c] and ι′ = (Δ(χ_V)_e)_i — the *prior* per-core assigner and the *prior* delegator. Even when this same round rewrites χ′_A[c] or χ′_V, the queue and the keys still come from whoever held the privilege when the round began, and only that service's own copy of the component is read, core index by core index  
   eq. 12.18 的 e_q′[c] 與 e_i′ 括號裡是 Δ* 的輸入 χ_A[c]、χ_V，也就是回合開始時的持有者。
2. ❌ φ′[c] = ((Δ(χ_M)_e)_q)[c] and ι′ = (Δ(χ_M)_e)_i — both are lifted out of the manager's output e* = Δ(χ_M)_e, because only the manager may alter any component of χ; the assigner and the delegator merely stage requests inside their own partial states, which the manager's Accumulate ratifies before Δ* merges them  
   會漏掉 eq. 12.19 註解「This allows privileges to be owned」的整個設計；e* 只供出 e_m′ 與 e_z′。
3. ❌ φ′[c] = ((Δ(χ′_A[c])_e)_q)[c] and ι′ = (Δ(χ′_V)_e)_i — the *posterior* per-core assigner and the *posterior* delegator. A service granted the assign privilege during this very block can therefore immediately install the queue the block will use, and only that service's own copy of the component is read, core index by core index  
   同一區塊剛拿到 assign 特權的服務要等下一塊才能改 φ′[c]，式子讀的就是 prior。
4. ❌ φ′[c] = R(φ[c], (e*_q)[c], ((Δ(χ_A[c])_e)_q)[c]) — it is merged across the manager and the prior assigner with R(o, a, b), exactly as χ′_A[c] is; and ι′ is taken from whichever service in s wrote it last when s is walked in ascending service index, so a higher-indexed service can overwrite the delegator's copy  
   R 只出現在 e_a′、e_v′、e_r′ 三行；讓任何服務都能覆寫 ι′ 等於誰都能改下一輪的 validator 金鑰。

> **陷阱**　χ′ 用 R 合併，但 φ′[c] 與 ι′ 只讀 prior χ_A[c] / χ_V 那一個服務的輸出——同區塊拿到特權，下一區塊才生效。

<sub>`c3-ch12-prior-privileged-index-reads`</sub>

---

### 12-12　In the team's Δ*, the service set s is a Go map and this loop collects u and t′ from the per-service results. Which GP 0.8.0 requirement does it violate, and what is the observable symptom?

<sub>12.2 Execution — ●●● · 程式碼 · eq. 12.18 (u, t′)</sub>

```go
func set_s(r []types.WorkReport, f types.AlwaysAccumulateMap,
...
```
<sub>internal/accumulation/accumulation.go (set_s + ParallelizedAccumulation result loop), condensed</sub>

**標準答案**　Eq. 12.18 writes u = ⟦(s, Δ(s)_u) | s ↕ s⟧ and t′ = ⟦Δ(s)_t | s ↕ s⟧ with the ordered-iteration marker, i.e. ascending service index, whereas Go randomises map iteration. The flattened ⌢⌢t′ therefore reaches the next Δ+ round permuted, so each destination's i^T — and hence Ψ_A's operand sequence, the balances it credits and the resulting state root — can differ between two nodes importing the very same block

eq. 12.18 對這兩個輸出都用了有序迭代記號：u = ⟦(s, Δ(s)_u) | s ↕ s⟧、t′ = ⟦Δ(s)_t | s ↕ s⟧，而 §3 的 notation 說明 ↕ 代表依元素本身遞增排序，對 s ∈ N_S 就是 service index 由小到大。Go 的 `for k := range m` 是刻意隨機化的，於是 tPrime 的串接順序在每個節點都不同；由於 eq. 12.17 會把 t* 餵回下一輪，而 eq. 12.23 的 i^T = ⟦t | t ↕ t, t_d = s⟧ 直接沿用該序列，最後 Ψ_A 收到的 operand 序列與 `fetch` 讀出的內容就不同——同一個區塊在不同節點算出不同 state root，典型症狀是 fuzzer / trace 測試偶發性不一致（團隊也用一個 `sort.Slice(iT, ...)` by SenderID 去掩蓋，但 sort.Slice 不穩定，同一 sender 的多筆轉帳仍可能被重排）。

**逐項辨析**

1. ❌ Nothing consensus-relevant: eq. 12.18 writes t′ as a set, and u is only ever consumed by eq. 12.28's G(s), which sums per service, so both are order-independent. The flattened ⌢⌢t′ does reach the next Δ+ round in an arbitrary order, but eq. 12.23 re-sorts each destination's i^T by sender before Ψ_A sees it, so two nodes importing the same block still agree on the state root  
   t′ 是序列不是集合；eq. 12.23 的 i^T = ⟦t | t ↕ t, t_d = s⟧ 沿用傳進來的順序，不會另外重排。
2. ✅ Eq. 12.18 writes u = ⟦(s, Δ(s)_u) | s ↕ s⟧ and t′ = ⟦Δ(s)_t | s ↕ s⟧ with the ordered-iteration marker, i.e. ascending service index, whereas Go randomises map iteration. The flattened ⌢⌢t′ therefore reaches the next Δ+ round permuted, so each destination's i^T — and hence Ψ_A's operand sequence, the balances it credits and the resulting state root — can differ between two nodes importing the very same block  
   §3.7.1 的 ↕ 是依元素本身遞增，s 的元素是 service index，而 Go map 迭代是刻意隨機化的。
3. ❌ Eq. 12.18's ↕ marker does demand a deterministic walk, but over *report* order — the order in which each service's first digest appears in r[..i] — so sorting by ascending service index would be exactly as wrong as the map order. The symptom is the same either way: the flattened ⌢⌢t′ reaches the next Δ+ round permuted and two nodes importing the same block disagree on the state root  
   GP 沒有第二套排序規則：↕ 依元素本身遞增，正解就是 ascending service index。
4. ❌ The violation is in set_s rather than in the loop: only services carrying a work-digest may contribute to u and t′, so transfer destinations and always-accumulate services have to be filtered back out of s before this loop runs. The map iteration itself is harmless, because eq. 12.18 marks u and t′ with ∈ rather than ↕ and both are consumed order-insensitively  
   s 的定義本來就含 K(f) 與 {t_d}，濾掉會讓 always-accumulate 與純收款服務完全不被 accumulate。

> **陷阱**　GP 的 ↕ 不是裝飾：任何從集合展開成序列的地方，Go map 迭代都是共識殺手。

<sub>`c3-ch12-code-delta-star-map-order`</sub>

---

### 12-13　What are ξ (accumulated) and ω (ready) and how big are they?
...
<sub>`ch12-history-queue-state`</sub>

---

### 12-14　How is the sequence of accumulatable reports R* built from the newly available reports R?

<sub>12.1 History and Queuing — ●●● · 概念 · eq. 12.4–12.12 (W!, W^Q, E, Q, W*)</sub>
...
3. ❌ R! = reports with no prerequisites, whatever their segment-root lookup holds, accumulated immediately; R^Q = the rest, each paired with its prerequisite set alone, pruned by ξ[E−1] (only the packages accumulated in the previous slot); R* = R! ⌢ Q(E(ω[m..] ⌢ ω[..m] ⌢ R^Q, P(R!))) with Q extracting reports whose dependency set is empty  
...
> **陷阱**　E 的參數 x 在第一次是 P(R!)（本塊立即 accumulate 者的 hash），不是整個 ξ（R^Q 已先用 ξ_∪ 修剪過）。

<sub>`ch12-W-partition`</sub>

---

### 12-15　What total gas budget g is handed to the outer accumulation Δ+ in a block, and how does Δ+ choose how many reports to accumulate in a round?
...
> **陷阱**　遞迴時 f（always-accumulate）傳空——特權 service 只在第一輪免費 accumulate。

<sub>`ch12-gas-budget`</sub>
...
**標準答案**　s = {services with a digest} ∪ keys(f) ∪ {destinations of deferred transfers}; each is accumulated exactly once via Δ1; the manager's output alone decides χ′_M and χ′_Z; for assigners, delegator and registrar the conflict resolver is R(o, a, b) = b when a = o, else a

eq. 12.18：s = {d_s | r ∈ r, d ∈ r_d} ∪ K(f) ∪ {t_d | t ∈ t}；每個 s 呼叫 Δ1(e, t, r, f, s) 一次。(χ′_M, χ′_Z) 直接取 manager m 的輸出 e*；∀c：χ′_A[c] = R(χ_A[c], e*_A[c], (Δ1(χ_A[c]) 輸出)_A[c])；同理 χ′_V、χ′_R；eq. 12.19：R(o, a, b) = b 若 a = o 否則 a。§12.2：「This allows privileges to be 'owned' and facilitates the removal of the manager service」——assigner 可以自己把 assign 權轉走（owned privileges，0.7.1 #475），但 manager 的變更優先。ι′ 來自 delegator 的輸出，φ′[c] 來自 assigner c 的輸出。新建帳戶 n 與刪除帳戶 m 合併進 δ；若兩個 service 產生同一個新 index（理論上不會）則區塊無效。

**逐項辨析**

1. ✅ s = {services with a digest} ∪ keys(f) ∪ {destinations of deferred transfers}; each is accumulated exactly once via Δ1; the manager's output alone decides χ′_M and χ′_Z; for assigners, delegator and registrar the conflict resolver is R(o, a, b) = b when a = o, else a  
   eq. 12.18 的三個聯集與 eq. 12.19 的 R 都對：manager 沒動時（a = o）才採用持有者自己的 b。
2. ❌ s = {services with a digest} only, so a service that merely receives a deferred transfer or enjoys free accumulation is skipped; each is accumulated exactly once via Δ1; the manager's output alone decides χ′_M and χ′_Z, and assigners, delegator and registrar are changeable only by the manager  
...
4. ❌ s = {services with a digest} ∪ keys(f) ∪ {destinations of deferred transfers}; each is accumulated exactly once via Δ1; χ′_M and χ′_Z are taken from the registrar's output; the conflict resolver is R(o, a, b) = a when a = o, else b  
   這兩項直接取 manager 的輸出；顛倒後的 R 在 manager 沒動時回傳 prior 值 o，等於忽略持有者。

> **陷阱**　每個 service 一輪只 accumulate 一次，其 inputs = 給它的 transfers ⌢ 它的 operand tuples（eq. 12.23）。

<sub>`ch12-delta-star`</sub>

---

### 12-17　For a single service s, Δ1 invokes Ψ_A(e, τ′, s, g, i^T ⌢ i^U). How is the gas g computed and what are the inputs?

<sub>12.2 Execution — ●●○ · 概念 · eq. 12.23 (Δ1)</sub>

**標準答案**　g = f[s] (free allowance, or 0) + Σ gas of deferred transfers destined to s + Σ accumulate gas-limits of s's digests across the round's reports; i^T = the transfers to s (in order), i^U = one operand tuple per digest of s (report order)

eq. 12.23：g = f[s]（若無則 0）+ Σ_{t: t_d = s} t_g + Σ_{d: d_s = s} d_g；i^T = [t | t ∈ t, t_d = s]；i^U = [(r_d result, g_d gas limit, y payload hash, o auth trace, e segroot, h package hash, a authorizer) | r ∈ r, d ∈ r_d, d_s = s]，產生式同時走 r ⊰ r 與 d ⊰ r_d，所以同一 report 裡屬於 s 的兩個 digest 會給出兩筆。0.7.1 起 on_transfer 被併入 accumulate：transfer 以 input 形式（在 operand 之前）交給同一次 Ψ_A（你們 deferred_transfers.go 的註解「v0.7.1 has removed deferred transfers & Ψ_T」）。operand tuple 定義在 eq. 12.13：(h, e, a, y, g, o, r)。

**逐項辨析**

1. ✅ g = f[s] (free allowance, or 0) + Σ gas of deferred transfers destined to s + Σ accumulate gas-limits of s's digests across the round's reports; i^T = the transfers to s (in order), i^U = one operand tuple per digest of s (report order)  
   eq. 12.23 的 g 是 f[s]、transfer gas 與 d_g 三項相加、缺一不可，引數順序也明寫 i^T ⌢ i^U。
2. ❌ g = f[s] (free allowance, or 0) + Σ gas of deferred transfers destined to s, the digests' accumulate gas-limits NOT being added because Δ+ already charged them against the block budget; i^T = the transfers to s (in order), i^U = one operand tuple per digest of s (report order)  
...
<sub>`ch12-delta-one-gas`</sub>

---

### 12-18　A deferred transfer T = (s, d, a, m, g). What are the fields, and when is the balance moved?

<sub>12.2 Execution — ●○○ · 概念 · eq. 12.14 & B.transfer</sub>
...
1. ✅ s source service, d destination, a amount, m memo of W_T = 128 octets, g gas limit for the recipient's handling; the sender's balance is deducted when `transfer` is called, and the amount is credited to the destination when it is accumulated in a later round  
   五個欄位全對；Ω_T 成功時當場從 sender 扣款，收方要到下一輪 Δ+ 以 input 形式收到才入帳。
2. ❌ s the sender's Ed25519 public key, d the destination service, a the amount, m a 32-octet memo hash, g the gas price the sender is willing to pay; the sender's balance is deducted when `transfer` is called and credited to the destination inside the very same Δ* round  
...
<sub>`ch12-deferred-transfer`</sub>

---

### 12-19　After Δ+ returns (n, e′, b, u, t), which statements about the final integration are correct?
...
<sub>`ch12-outputs`</sub>

---

### 12-20　Which rule governs the preimages extrinsic E_P and its integration into δ′?

<sub>12.4 Preimage Integration — ●●○ · 概念 · eq. 12.34–12.37</sub>
...
1. ✅ E_P ∈ [(s, d)] ordered & unique; each (s, d) must be providable in the PRIOR δ — the service exists and δ[s]_l[(H(d), |d|)] = [] (requested, not yet provided); integration happens after accumulation: δ′ = I(δ‡, E_P), setting a_l[(H(d),|d|)] = [τ′] and a_p[H(d)] = d, silently dropping any preimage that is no longer useful  
   eq. 12.36 的 Y 吃 prior δ 判斷 providable，eq. 12.37 的整合作用在 δ‡，兩個 staging 都擺對了。
2. ❌ E_P ∈ [(s, d)] ordered & unique; each (s, d) must be providable in the POSTERIOR δ‡ — the service must still exist there and δ‡[s]_l[(H(d), |d|)] = [] — so a request that accumulation dropped in the same block makes the whole block invalid; integration is then δ′ = I(δ‡, E_P), setting a_l[(H(d),|d|)] = [τ′] and a_p[H(d)] = d  
   eq. 12.36 的 Y 明確吃 prior δ；被 forget 掉的 preimage 是 disregarded, without prejudice。
3. ❌ E_P ∈ [(s, d)] ordered & unique; any preimage may be included whether or not the service solicited it, since I stores every pair it is handed; integration happens BEFORE accumulation, δ† = I(δ, E_P), so that a service's Accumulate can read a preimage supplied in the same block, setting a_l[(H(d),|d|)] = [τ′] and a_p[H(d)] = d  
   Y 要求 δ[s]_l[(H(d), |d|)] = []；§12.4 開頭就是 After accumulation, we must integrate…。
4. ❌ E_P ∈ [(s, d)] need only be free of duplicates, the ordering being unconstrained; each (s, d) must be providable in the prior δ and must additionally be signed by the service's manager; integration happens after accumulation: δ′ = I(δ‡, E_P), setting a_l[(H(d),|d|)] = [τ′] and a_p[H(d)] = d, silently dropping any preimage that is no longer useful  
   eq. 12.35 要求 ordered 且 unique，而 GP 從頭到尾沒有任何簽章條件，只看 request 是否存在。

> **陷阱**　驗證看 prior δ、整合作用於 δ‡；順序題在 fuzzer 很常出現。

<sub>`ch12-preimage-integration`</sub>

---

### 12-21　This is the team's prefix selection in Δ+. Compared with GP 0.8.0 eq. 12.17, what is missing?
...
1. ❌ JAM pays validators directly: each block the STF debits a treasury service in proportion to π_V, exactly as it does for punishment; block production, guaranteeing and assurances are trackable on-chain, and so are GRANDPA, BEEFY and auditing, since every message a validator signs is eventually included in some block  
   與 §13.1 第一句 does not explicitly issue rewards 正面牴觸；GRANDPA/BEEFY 簽章根本不進區塊。
2. ✅ JAM issues no rewards itself but oracles activity data to a staking subsystem, as it does with punishment data; block production, guaranteeing and assurances are trackable on-chain, whereas GRANDPA, BEEFY and auditing are not and are instead covered by validators voting on each other's efforts, with a median accepted under a 50%-honest assumption  
   §13.1 原文：JAM 只 facilitate the arrival of information，untrackable 的部分靠互評取 median。
3. ❌ JAM issues no rewards itself but feeds π into Safrole, which drops from ι′ any validator whose counters fall below a threshold; block production, guaranteeing and assurances are trackable on-chain, whereas GRANDPA and BEEFY activity is read straight from the header's finality field and auditing is covered by validators voting on each other's efforts  
   ι′ 來自 delegator service 的 accumulate 輸出（§12），與 π 無關；§5 的 header 也沒有 finality 欄位。
4. ❌ JAM issues no rewards itself and the statistics exist only for telemetry (JIP-3) and RPC: π sits outside the consensus state and is not covered by the state root, as is the punishment record ψ; GRANDPA, BEEFY and auditing activity is therefore simply omitted rather than oracled to any external subsystem  
   π 是 App. D 的 C(13)、ψ 是 C(5)，兩者都在 state root 底下；untrackable 的活動也沒有被略過。

> **陷阱**　JAM 不發獎勵、不追蹤 GRANDPA/BEEFY/audit；這些靠 validator voting 的 median，假設只需 50% 誠實。

<sub>`ch13-why-statistics`</sub>
...
1. ❌ Everything this block contributes lands in the freshly zeroed accumulator, because eq. 13.4 applies the assurance pass to π_V‡ rather than to π_V; validator 7's block/ticket/preimage counters and validator 22's guarantee credit therefore sit in the current-epoch record alongside validators 4 and 9's assurance steps, and the archive keeps the previous epoch's totals untouched.  
   eq. 13.4 白紙黑字是 π_V† ≡ π_V except …，起點是 prior 的 π_V；π_V‡ 要到 eq. 13.5 才存在。
2. ✅ The two assurance increments land in the last-epoch archive, because they are added to the prior record first and it is that already-incremented record which is moved across at the rollover; validator 7's block/ticket/preimage counters and validator 22's guarantee credit start from the freshly zeroed accumulator and so land in the current-epoch record. One block is deliberately split across the two records.  
   eq. 13.4 先加在 π_V 上，eq. 13.5 的 otherwise 分支再把整包 π_V† 搬進 π′_L，一塊被劈成兩半。
3. ❌ Everything this block contributes lands in the last-epoch archive, because eq. 13.6 applies its five remaining counters to π_V† as well and it is the result of that whole pass which is moved across at the rollover; validator 7's block/ticket/preimage counters, validator 22's guarantee credit and validators 4 and 9's assurance steps therefore travel together into π′_L. The current-epoch record stays an all-zero sequence until the second block of the new epoch.  
   eq. 13.6 明寫 π′_V ≡ π_V‡ except …，五個 counter 的起點是 rollover 之後的 π_V‡。
4. ❌ The rollover is triggered by the header's epoch marker H_E rather than by comparing ⌊τ/E⌋ with ⌊τ′/E⌋, and when H_E ≠ ∅ both records are zeroed rather than one being overwritten by the other; validator 7's block/ticket/preimage counters, validator 22's guarantee credit and validators 4 and 9's assurance steps are therefore all first entries of the current-epoch record, and the archive contributes nothing to this block at all.  
   π_L 從來不會被歸零：eq. 13.5 的 otherwise 分支是把 π_V† 整包搬過去覆寫它。

> **陷阱**　口訣：assurance 在 rollover 之前，b/t/p/d/g 在 rollover 之後；epoch 邊界那一塊會被劈成兩半。

<sub>`c3-ch13-epoch-boundary-split`</sub>
...
> **陷阱**　GP 字面寫 κ′[v] ∈ G，但 κ′[v] ∈ K 是四欄位金鑰 tuple 而 G ⊂ H；實作一律讀成 (κ′[v])_e ∈ G。

<sub>`c3-ch13-guarantee-credit-set`</sub>
...
### 13-6　In one block, core 5 sees two things. (a) The guarantee extrinsic brings in a fresh work-report for core 5 whose availability spec has bundle length 100,000 octets and export-segment count 200. (b) An older report already assigned to core 5 crosses its two-thirds assurance threshold in this same block; its availability spec has bundle length 262,144 octets and export-segment count 130. With W_G = 4,104, what value does π′_C[5]_d take?

<sub>13.2 Cores and Services — ●●● · 計算 · eq. 13.9, 13.12; eq. 11.17 (R); eq. 11.30 (I)</sub>

**標準答案**　807,976

eq. 13.12：D(c) ≡ Σ_{w ∈ R, w_c = c} ((w_s)_l + W_G⌈(w_s)_n · 65/64⌉)，求和範圍是 **R**——eq. 11.17 定義的「newly available」報告（本塊剛跨過 2/3 super-majority 的），**不是** eq. 11.30 的 I（本塊 E_G 帶進來的報告）。I 只餵 eq. 13.10 的 R(c)（i/x/z/e/u）與 eq. 13.11 的 L(c)（l）。所以用 (b) 那筆：130 × 65 = 8,450；8,450 ÷ 64 = 132.03125，⌈·⌉ = 133；4,104 × 133 = 545,832；262,144 + 545,832 = **807,976**。65/64 的來由：每 64 個 export segment 會多一個 paged-proof segment（§14），所以 DA 佔用是 ⌈65n/64⌉ 個 segment，每個 W_G = 4,104 octets。

**逐項辨析**

1. ❌ 803,872  
   漏掉 ceiling：把 8,450/64 = 132.03 取成 ⌊·⌋ = 132 而不是 133。
2. ❌ 937,216  
   誤用了 (a) 那筆剛 guaranteed 的報告；那組數字只會出現在 l = L(c) 這個欄位。
3. ✅ 807,976  
   取 (b) 那筆 newly available 的報告：262,144 + 4,104 × 133 = 807,976。
4. ❌ 795,664  
   完全漏掉 65/64 係數，把 130 個 export segment 直接算成 4,104 × 130。

> **陷阱**　DA load 看 R（剛 available），bundle length 與 refine 統計看 I（剛 guaranteed）——同一塊同一個 core 可以兩者都有。

<sub>`c3-ch13-da-load-calc`</sub>

---

### 13-7　This is the team's GP 0.7.2 statistics code, condensed. Measured against GP 0.8.0 eq. 13.4–13.6, on which blocks does it place a validator's assurance increment in the wrong record, and what is the minimal fix?

<sub>13.1 Validator Activity — ●●○ · 程式碼 · eq. 13.4–13.6 · ⚠ 0.7.2→0.8.0</sub>

```go
// (13.3)  π ≡ (πV , πL, πC , πS);  (πV, πL) => (current, last)
...
```
<sub>internal/statistics/statistics.go:109-133, 460-504 (UpdateValidatorActivityStatistics / UpdateCurrentStatistics, GP 0.7.2, condensed)</sub>

**標準答案**　Only on blocks where ⌊τ/E⌋ ≠ ⌊τ′/E⌋. eq. 13.4 applies the assurance pass to the prior record, and eq. 13.5 then moves that already-incremented record into the archive, so on an epoch-boundary block each assurer's step belongs in the archive rather than in the newly allocated slice this code writes it into. The fix is to run the assurance pass on the prior record before the branch and to hand the incremented record to the else-branch's archive assignment.
...
1. ✅ Only on blocks where ⌊τ/E⌋ ≠ ⌊τ′/E⌋. eq. 13.4 applies the assurance pass to the prior record, and eq. 13.5 then moves that already-incremented record into the archive, so on an epoch-boundary block each assurer's step belongs in the archive rather than in the newly allocated slice this code writes it into. The fix is to run the assurance pass on the prior record before the branch and to hand the incremented record to the else-branch's archive assignment.  
   非邊界塊 π_V‡ = π_V†，兩種寫法同值；只有 ⌊τ/E⌋ ≠ ⌊τ′/E⌋ 的那一塊才會顯現差異。
2. ❌ On every block. eq. 13.4 attributes each assurance to the block author rather than to the assurer who signed it, so the availability pass credits the wrong validator on every block and not merely at the boundary; which of the two records receives that credit is a separate question this code already answers correctly. The fix is to index the increment by authorIndex exactly as the ticket and preimage passes already do.  
   與 UpdateAvailabilityStatistics 目前用 assurance.ValidatorIndex 的作法相反，也違反 §13.1 對 a 的定義。
3. ❌ Only on epoch-boundary blocks, but in the opposite direction. eq. 13.6 applies all six counters to π_V† before eq. 13.5 performs the rollover, so on a boundary block the author's block, ticket and preimage steps belong in the archive too, and the newly allocated slice must stay an all-zero sequence until the second block of the new epoch. The fix is to hoist the whole of UpdateCurrentStatistics above the epoch branch, so that every counter is credited before the reset rather than only the assurance pass.  
   eq. 13.6 的 b/t/p/d/g 全部以 π_V‡ 為起點，提前做等於讓新 epoch 的第一塊完全不計分。
4. ❌ On no block at all: the placement of the assurance increment already matches 0.8.0, because eq. 13.4 is itself applied to π_V‡ and this code likewise runs its availability pass on the posterior record. The only genuine 0.8.0 gap visible here is eq. 13.3, which sizes the two records from |κ| and |λ| rather than from the constant ValidatorsCount. The fix is to allocate the reset slice with len(kappa) instead.  
   |κ|/|λ| 確實是另一個缺口，但這說法把 eq. 13.4 的起點誤讀成 π_V‡，與式子本身矛盾。

> **陷阱**　非 epoch 邊界的 fuzz trace 全綠不代表對——這個 bug 只在 600 塊裡的那 1 塊現形。

<sub>`c3-ch13-rollover-code`</sub>

---

### 13-8　The team's GP 0.7.2 code builds each service record's accumulation entry as the pair shown. GP 0.8.0 reshapes that entry. What exactly changes, and what observable consequence does it have for which services appear in π′_S?

<sub>13.2 Cores and Services — ●●○ · 程式碼 · eq. 13.8, 13.13–13.14; eq. 12.27–12.28 · ⚠ 0.7.2→0.8.0</sub>
...
```
<sub>internal/statistics/statistics.go:386-450 (CalculateAccumulationStatistics + record assembly, GP 0.7.2)</sub>

**標準答案**　The entry becomes a three-element value — work-digests accumulated for the service, deferred transfers delivered to it, and accumulation gas — obtained as the substitute-if-nothing of the service's entry in S against an all-zero triple. Since S now keeps every service whose triple is not all-zero, a service touched only by incoming transfers has a non-zero middle element, becomes a key of S, and therefore joins the union that forms π′_S's domain.

eq. 13.8：π_S ∈ ⟨N_S → (p ∈ (N, N), r ∈ (N, N_G), i ∈ N, x ∈ N, z ∈ N, e ∈ N, a ∈ (N, N, N_G))⟩——a 在 0.8.0 是**三元組**。eq. 13.13：a = 𝒰(S[s], (0, 0, 0))，𝒰 是 §3.2 / eq. 3.2 的 substitute-if-nothing。S 的定義在 eq. 12.27–12.28：S ∈ ⟨N_S → (N, N, N_G)⟩、S ≡ {s ↦ S(s) | S(s) ≠ (0, 0, 0)}，其中 S(s) = (N(s), T(s), G(s))：N(s) 是本塊被 accumulate 的 work-digest 數、T(s) ≡ |[t | t ↕ t, t_d = s]| 是 destination 為 s 的 processed deferred transfer 數、G(s) 是 accumulation 用掉的 gas。eq. 13.14：s = s^R ∪ s^P ∪ K(S)。歷史脈絡：0.7.1（team #656）因為 Ψ_T 被移除而把 OnTransfersCount / OnTransfersGasUsed 從 ServiceActivityRecord 刪掉，0.8.0（GP #502「Add back processed transfer count to service statistics」）只把 transfer **筆數**放回三元組中間，並沒有復活 on-transfer 的 gas 欄位，也沒有復活 Ψ_T——轉帳的 gas 是在 eq. 12.23 的 g 裡與 work-digest 的 gas 加總後一次進 Ψ_A。另一個要留意的編輯瑕疵：eq. 13.13 的 p = Σ_{(s, d) ∈ E_P}(1, |d|)，comprehension 綁定的 s 字面上**遮蔽**了外層的 ∀s ∈ s，照字面讀會退化成與 service 無關的全域總和；實際意思（也是所有實作的作法）是「E_P 中 service index 等於該 s 的那些 preimage 的 (筆數, 總 octet 數)」，同樣的遮蔽寫法也見於 eq. 12.28 的 G(s)。

**逐項辨析**

1. ✅ The entry becomes a three-element value — work-digests accumulated for the service, deferred transfers delivered to it, and accumulation gas — obtained as the substitute-if-nothing of the service's entry in S against an all-zero triple. Since S now keeps every service whose triple is not all-zero, a service touched only by incoming transfers has a non-zero middle element, becomes a key of S, and therefore joins the union that forms π′_S's domain.  
   eq. 13.13 的 a = 𝒰(S[s], (0,0,0))；T(s) > 0 讓純收款的 service 經 K(S) 進入 domain。
2. ❌ The entry keeps its two elements — work-digests accumulated for the service and accumulation gas — but the gas element is re-typed from N_G to a plain natural, and it is now read out with a bare dictionary subscript rather than through the substitute-if-nothing wrapper. The domain of π′_S is unchanged, still the union of reported services, preimage-provided services and the keys of the accumulation statistics, so a service touched only by a transfer never appears.  
   eq. 13.8 的 a 第三個元素仍是 N_G，且用的是 𝒰 包裝——裸下標查不到會得 ∅，型別不合。
3. ❌ 0.8.0 restores the separate on-transfer PVM invocation Ψ_T, so the accumulation entry keeps its two elements and the record regains a transfer count and a transfer gas figure as two further top-level fields beside it, drawn from a second dictionary of on-transfer statistics. Every service that received a transfer therefore appears in π′_S carrying its own on-transfer gas figure, beside the reported and preimage-provided services.  
   accumulation.tex 全篇沒有 Ψ_T；#502 只放回 transfer 筆數，沒有第二本 on-transfer 統計字典。
4. ❌ The entry becomes a three-element value — work-digests accumulated for the service, preimages provided to it, and accumulation gas — obtained as the substitute-if-nothing of the service's entry in S against an all-zero triple. Since that middle element already counts provisions the separate provision pair becomes redundant, and the domain of π′_S narrows to exactly the services which appear as keys of the accumulation statistics.  
   eq. 12.28 的 T(s) 數的是 transfer 筆數而非 preimage；eq. 13.14 的三方聯集也沒動過。

> **陷阱**　0.8.0 加回來的是 transfer 的「筆數」，不是 on-transfer 的 gas；Ψ_T 沒有回來。

<sub>`c3-ch13-accum-triple-code`</sub>

---

### 13-9　An interviewer pushes back: 'Validator counters look like telemetry to me. Why are they a component of σ and Merklized into the state trie at all, and why keep two validator records rather than one running counter?' What is the GP-grounded answer?

<sub>13.1 Validator Activity — ●●○ · 設計理由 · §13.1; eq. 4.4 (state composition); eq. 13.1–13.3; App. D key C(13)</sub>

**標準答案**　Because the figures must be bit-identical on every node and provable to a consumer outside the chain: JAM pays no rewards itself but has to deliver activity data to a staking subsystem, so the record is a state component with its own trie key and a node computing it differently produces a different state root. The pairing exists because settlement is per-epoch — the archive holds the just-completed epoch's totals unchanged for the whole of the next epoch, giving a stable snapshot to read while the accumulator fills.

§13.1 開頭：「The JAM chain does not explicitly issue rewards—we leave this as a job to be done by the staking subsystem…However…it is important for the JAM chain to facilitate the arrival of information on validator activity in to the staking subsystem so that it may be acted upon.」要讓鏈外（甚至跨鏈）的 staking 系統能**信任**這些數字，它就必須是共識狀態：eq. 4.4 把 π 列為 σ 的十七個分量之一，App. D 以 C(13) 當 state key 併入 state trie，因此 π′ 併入 M_σ(σ′) 並可被 light client 證明——但要記住 JAM 的 header 帶的是 **prior** state root（§5：H_R ≡ M_σ(σ)，GP 明說這是與 Ethereum／Polkadot 相反的設計，為了 pipelining），所以含 π′ 的 σ′ 之根**不在本塊的 H_R 裡**，要等**下一塊**的 header 才會出現，剛好落後一塊；任何節點算錯 π 就會算出不同的 state root，直接是 invalid block。雙份 record 的理由在 §13.1：「The validator statistics are made on a per-epoch basis and we retain one record of completed statistics (π_L) together with one record which serves as an accumulator for the present epoch (π_V)」——結算以 epoch 為單位，π_L 在整個下一個 epoch 都不再變動，消費者才有穩定快照可讀。§13.2 明講 π_C/π_S「are tracked only on a per-block basis」，所以不需要這種配對；而懲罰在 §10 的 judgements，依據是 ψ 不是 π。

**逐項辨析**

1. ❌ It is essentially a serialization convenience: the JIP-2 RPC's statistics endpoint and the conformance vectors want a fixed-shape record, so π rides along in σ purely to give those consumers a stable layout, and because it is excluded from the state-trie construction of appendix D a node that skipped the update entirely would still agree on the state root. The pairing exists only so that a test vector can compare a whole epoch's totals in one shot.  
   與 App. D 直接衝突：C(13) 就是 π 的 state key，π′ 一定會被併進 M_σ(σ′)。
2. ❌ Because Grandpa weights finality votes by validator performance, so the figures have to sit in σ where the finality gadget can read them and a stale accumulator would let a validator vote with the previous epoch's weight; that is also why §13.1 has validators vote on each other's auditing efforts, since audits carry the same weight in that ballot. The pairing exists so that a validator removed from the active set mid-epoch can still be slashed against its own history rather than against whoever replaced it, and the core records are paired for exactly the same reason.  
   GRANDPA、BEEFY 與 auditing 恰恰是 §13.1 說無法直接鏈上追蹤、要靠互評取中位數的部分。
3. ❌ Because the author's reward is paid out of the block's accumulation gas budget in the same block, so the counters have to be readable from inside the state transition rather than from an off-chain index. The pairing is a rollback buffer: when a fork is reverted the archive is copied back over the accumulator, which is exactly why both halves have to be Merklized.  
   §13.1 第一句就是 does not explicitly issue rewards；分叉靠整個 σ 版本切換，沒有回拷 π 的動作。
4. ✅ Because the figures must be bit-identical on every node and provable to a consumer outside the chain: JAM pays no rewards itself but has to deliver activity data to a staking subsystem, so the record is a state component with its own trie key and a node computing it differently produces a different state root. The pairing exists because settlement is per-epoch — the archive holds the just-completed epoch's totals unchanged for the whole of the next epoch, giving a stable snapshot to read while the accumulator fills.  
   §13.1 原文：JAM 不發獎勵、只把活動資料送進 staking subsystem，配對是為了 per-epoch 結算的穩定快照。

> **陷阱**　「為什麼在鏈上」的標準答法：因為要能被證明且必須共識一致，π 進 state root；不是為了 RPC 好看。

<sub>`c3-ch13-consensus-state-rationale`</sub>

---

### 13-10　π ≡ (π_V, π_L, π_C, π_S). Which statement about the validator statistics is correct in GP 0.8.0?
...
### 13-11　Core statistics π_C and service statistics π_S are per-block (not per-epoch). Which description of the core statistics is correct?
...
1. ✅ Per core: d (DA load) = Σ over the reports that became AVAILABLE this block (the set R) on that core of bundle length + W_G·⌈65·segment_count/64⌉; p (popularity) = the number of assurances whose bitfield has that core set; i, x, z, e, u and l are summed over the reports GUARANTEED this block (the set I) on that core  
   d 對 R（本塊剛 available）求和、i/x/z/e/u/l 對 I（本塊 guaranteed）求和，兩個來源分得清楚。
2. ❌ Per core: d (DA load) = Σ over the reports GUARANTEED this block (the set I) on that core of bundle length + W_G·⌈65·segment_count/64⌉; p (popularity) = the number of reports guaranteed on that core; i, x, z, e, u and l are summed over I as well, so every field of π′_C comes from one and the same source  
   d 的來源是 R 而非 I；p 是對 E_A 的 availability bitfield 求和（Σ_a a_f[c]），與 report 數無關。
3. ❌ Per core: d (DA load) = Σ over the reports that became AVAILABLE this block (the set R) on that core of bundle length + W_G·segment_count, the paged proofs not being counted; p (popularity) = the number of assurances whose bitfield has that core set; i, x, z, e, u and l are summed over R as well, so all eight fields follow availability  
   65/64 正是把每 64 個 export segment 多出的那一頁 paged proof 算進去；R(c)、L(c) 的範圍是 I。
4. ❌ Per core: d (DA load) = Σ over the reports that became AVAILABLE this block (the set R) on that core of bundle length + W_G·⌈65·segment_count/64⌉; p (popularity) = the number of guarantor signatures backing that core's reports; i, x, z, e and u are summed over the reports GUARANTEED this block (the set I), but l is the largest bundle length on that core rather than their sum  
   p 仍然是 assurance 打勾數；L(c) 是該 core 上所有 report 的 bundle length 總和而不是最大值。

> **陷阱**　兩個不同來源（guaranteed vs available）——面試容易考。

<sub>`ch13-core-service-stats`</sub>

---

### 13-12　Which services appear in π′_S for a block, and what does the accumulation entry hold?
...
> **陷阱**　π_S 的 accumulation 是 (N, T, G) 三元組。

<sub>`ch13-service-stats`</sub>
...
### 14-1　A work-package (eq. 14.2, of the set ℙ) is ⟨j, h, u, f, c, w⟩. Which description is correct?

<sub>14.2 Work Packages — ●●○ · 概念 · eq. 14.2–14.3 (§14.3)</sub>

**標準答案**　j authorization token; h auth-service index (the service hosting the authorizer code); u authorizer code hash; f configuration blob (parameterization); c refinement context; w work-items (1..I = 16) each with service, code hash, payload, refine & accumulate gas limits, imports (segment root/hash + index), extrinsic (hash, length) pairs, and export count

eq. 14.2：ℙ ≡ (j ∈ B token, h ∈ N_S auth service, u ∈ H auth code hash, f ∈ B config, c ∈ ℂ context, w ∈ [𝕀]_{1:I} items)；eq. 14.3：work-item = (s service, c code hash, y payload, g refine gas limit, a accumulate gas limit, i imports [(H ∪ hash⊞, N)], x extrinsics [(H, N)], e export count)。authorizer a = H(u ⌢ f)（0.8.0 #522）；h 是「hosts the authorization code」的 service，c 是 refinement context（anchor hash/state root/beefy root、lookup anchor hash 與 slot、prerequisites），不要跟 work-report 的 availability specification 混。限制（eq. 14.5–14.9）：總 extrinsics ≤ T = 128、imports/exports ≤ W_M/W_X = 3,072、bundle ≤ W_B = 13,791,360。§14.2 的設計原則是「Rather than having much data inline, they instead reference data through commitments」——import 用 (Merkle root, index) 承諾，export「we do not require any particular commitment to them in the work-package beyond knowing how many」。§14 之後（15 guaranteeing、16 assurance、17 auditing）是 M2 範圍，但 M1 面試仍可能問 refine 的輸入輸出概念。

**逐項辨析**

1. ✅ j authorization token; h auth-service index (the service hosting the authorizer code); u authorizer code hash; f configuration blob (parameterization); c refinement context; w work-items (1..I = 16) each with service, code hash, payload, refine & accumulate gas limits, imports (segment root/hash + index), extrinsic (hash, length) pairs, and export count  
   六個欄位逐一對上 eq. 14.2，work-item 的 g/a 兩個 gas limit 與只記數量的 e 也對上 eq. 14.3。
2. ❌ j the guarantors' signatures over the package; h the hash of the package itself; u the index of the service whose refine code runs; f the gas limit shared by every item; c the availability specification (erasure-root, segment-root, length); w work-items (1..I = 16) each with service, payload, extrinsic hashes and an authorization token of its own  
   把 work-report 的欄位搬進 package：guarantor 簽章在 E_G 裡，availability specification 是 report 的欄位。
3. ❌ j authorization token; h the index of the service that will accumulate the results; u the refine code hash of the first work-item; f configuration blob; c the availability context: erasure-root plus the chunk indices; w work-items (1..T = 128) each with service, code hash, payload, one combined gas limit, imported segment hashes, and its exported segments inline  
   item 上限是 I = 16（T = 128 是整個 package 的 extrinsic 總數），而且 export 只記數量、不內嵌。
4. ❌ j authorization token (an opaque blob); h the index of the core the package is destined for; u authorizer code hash; f configuration blob (parameterization); c refinement context; w work-items (1..I = 16) each with service, code hash, payload, refine & accumulate gas limits, the imported segments' data inline, extrinsic (hash, length) pairs, and the exported segments' hashes  
   h 的型別是 N_S（service index），core 由 guarantor 決定並寫在 report 上；import 也是承諾而非內嵌資料。

> **陷阱**　auth code 是從 δ[h] 的 preimage 以 lookup-anchor 時間做 historical lookup 取得（eq. 14.4 附近）。

<sub>`ch14-work-package`</sub>

---

### 14-2　This is the team's item-to-digest function (GP 0.8.0 eq. 14.10, C). Which statement about it is correct?

<sub>14.3 Packages and Items — ●●○ · 程式碼 · eq. 14.10 (item-to-digest C); eq. 11.6 (D); eq. 13.10/13.17 — internal/work_package/work_package.go C</sub>
...
```
<sub>internal/work_package/work_package.go (C; the '14.8' in the comment is the 0.7.2 number of eq. 14.10)</sub>

**標準答案**　Three load counters are rotated: eq. 14.10 sets x = |w_x| (number of extrinsics), z = Σ of the extrinsic lengths and e = w_e (declared export count), but the code puts w_e into ExtrinsicCount, |w_x| into ExtrinsicSize and the length sum into Exports — payload hash, accumulate gas limit, import count and gas used are right

eq. 14.10 C：((s, c, y, a, e, i, x), l, u) ↦ (s, c, y ↦ H(y), g ↦ w_a, l, u, i ↦ |w_i|, e, x ↦ |w_x|, z ↦ Σ_{(h,z)∈w_x} z)。對照 code：Imports = len(ImportSegments)、AccumulateGas = w_a、PayloadHash = H(payload)、GasUsed = u 都正確；但 ExtrinsicCount 被塞了 item.ExportCount（應為 len(item.Extrinsic)）、ExtrinsicSize 被塞了 len(item.Extrinsic)（應為 Σ Len，也就是 zSum）、Exports 被塞了 zSum（應為 ExportCount）——三個欄位輪轉錯位。三個干擾項共用同一個破綻：它們都順帶宣稱 x、z、e 指派正確。後果：digest 的 x/z/e 錯 → report hash 與其他實作不同（guarantor 簽章不一致）→ π_C 與 π_S（eq. 13.10、13.17 的 R(c)、R(s)）也錯。STF conformance fuzzer 沒抓到，因為 test vectors 直接提供現成的 work-report（code-map §5 第 16 點：這條路徑不在 STF 向量裡）。另外 e 是 work-item **宣告**的 w_e，實際 export 數不符時 I 函數（eq. 14.13）會給 ⊚ BADEXPORTS。

**逐項辨析**

1. ❌ The only defect is the payload hash: eq. 14.10 requires y = H(E(w)), the hash of the whole encoded work-item rather than of the payload alone, so PayloadHash must be taken over E(item); the accumulate gas limit, the import count and the three load counters x = |w_x|, z = Σ of the extrinsic lengths and e = w_e are all assigned correctly  
   eq. 11.6 說 y 是原樣交給 accumulation 的 payload，digest 取的就是 H(w_y)，不是整個 work-item。
2. ❌ AccumulateGas is the only defect: a digest's g must be the refine gas limit w_g, because the accumulation gas is decided on-chain by Δ+ rather than by the package; the payload hash H(w_y), the import count |w_i| and the three load counters x = |w_x|, z = Σ of the extrinsic lengths and e = w_e are all assigned correctly  
   digest 的 g 是 accumulate gas limit w_a；refine 的 w_g 只出現在 eq. 14.9 與 Ψ_R。
3. ❌ The code is correct: eq. 14.10 pins down only the payload hash, the accumulate gas limit, the import count and the gas used, leaving the three manifest counters x, z and e to whatever Ψ_R actually did, so any self-consistent assignment is acceptable provided the guarantors agree on the resulting report hash  
   eq. 14.10 對 x、z、e 是精確定義，沒有「任意自洽即可」的空間，而 code 的這三欄確實錯位。
4. ✅ Three load counters are rotated: eq. 14.10 sets x = |w_x| (number of extrinsics), z = Σ of the extrinsic lengths and e = w_e (declared export count), but the code puts w_e into ExtrinsicCount, |w_x| into ExtrinsicSize and the length sum into Exports — payload hash, accumulate gas limit, import count and gas used are right  
   ExtrinsicCount 收了 w_e、ExtrinsicSize 收了 |w_x|、Exports 收了 Σz，正是三欄整組輪轉。

> **陷阱**　digest 五個負載欄位：u gas used、i imports、e exports、x extrinsic count、z extrinsic size——順序與名稱要背熟。

<sub>`ch14-code-digest-mapping`</sub>

---

### 14-3　Below is how the team's Ψ_R builds the argument blob a handed to Ψ_M. Which statement is correct under GP 0.8.0 (eq. B.5)?

<sub>14.4 Computation of Work-Report — ●●● · 程式碼 · eq. B.5 (Ψ_R argument a and gas w_g); eq. B.1 (Ψ_I uses E_2(c)); eq. 14.9 (G_R) — PVM/refine_invocation.go RefineInvoke</sub>
...
1. ❌ The core index must be encoded as E_2(c), two fixed octets, exactly as Ψ_I does, so the compact EncodeUint is wrong; the service index w_s, the compact i, the length-prefixed payload, H(p) over the encoded package and the gas limit w_g all match the GP  
   E_2(c) 是 Ψ_I 的參數（eq. B.1：Ψ_M(p_u, 0, G_I, E_2(c), …)），Ψ_R 的 c 用的是 compact E。
2. ✅ The third component of a must be the service index w_s as a compact natural, but the code appends the 32-octet code hash w_c; the compact c and i, the length-prefixed payload, H(p) over the encoded package and the gas limit w_g all match the GP  
   註解寫 w_s 的那行編進去的是 workItem.CodeHash，32-byte 的 w_c 頂替了 compact 的 service index。
3. ❌ The final hash must be H(E(w)), the hash of the encoded work-item being refined, rather than H(p); and the gas limit handed to Ψ_M must be G_R = 5·10⁹, the per-package refine allowance, not one item's w_g — the compact c and i, the service index w_s and the length-prefixed payload are all correct  
   H(p) 就是整包 package 的 hash；G_R 是 eq. 14.9 的 package 級 Σ w_g 上限，單一 item 傳的是 w_g。
4. ❌ The payload must be appended without a length prefix because Refine obtains it through the fetch host-call instead, so the ↕ is spurious; the compact c and i, the service index w_s, H(p) over the encoded package and the gas limit w_g all match eq. B.5  
   eq. B.5 的 ↕w_y 明確帶長度前綴，fetch selector 13 只是另一個取得管道，不能取消 ↕。

> **陷阱**　Ψ_I：E_2(c) + G_I = 50M；Ψ_R：E(c, i, w_s, ↕w_y, H(p)) + w_g（Σ w_g < G_R = 5·10⁹）。

<sub>`ch14-code-refine-args`</sub>

---

### 14-4　GP 0.8.0 redefines the work-report computation function Ξ. Which statement about its arguments and its failure condition E is correct?
...
### 14-5　A guarantor computes a report for a package whose authorization trace t is 1,000 octets and whose four work-items all refine successfully (export counts correct) with output blobs of 20,000, 25,000, 5,000 and 3,000 octets, in that order. With W_R = 49,152, what results do the four digests carry?

<sub>14.4 Computation of Work-Report — ●●● · 計算 · eq. 14.13 (I(p, j), z); eq. 11.8 (W_R)</sub>

**標準答案**　ok (20,000), ok (25,000), ⊖ OVERSIZE, ok (3,000) — the size check is cumulative (|r| + |t| + Σ of earlier successful outputs > W_R) and an item replaced by ⊖ contributes 0 to the sums of later items

eq. 14.13 的 I(p, j)：若 |r| + z > W_R 則 (⊖, u, [J_0, …]_{m_e})，其中 z = |t| + Σ_{k<j, (r ∈ B, …) = I(p,k)} |r|——只累加**前面成功（r ∈ B）**的輸出，再加上 authorizer trace t。逐項：item 0：z = 1,000，20,000 + 1,000 = 21,000 ≤ 49,152 → ok；item 1：z = 21,000，25,000 + 21,000 = 46,000 → ok；item 2：z = 46,000，5,000 + 46,000 = 51,000 > 49,152 → ⊖ OVERSIZE，輸出換成 error、exports 換成 m_e 個零 segment（J_0 = [0]^{W_G}），gas u 照記；item 3：z 仍是 46,000（item 2 的 r 不是 blob，不計入），3,000 + 46,000 = 49,000 ≤ 49,152 → ok。最終 report 的 |t| + Σ L(r) = 49,000，自動滿足鏈上 eq. 11.8——guarantor 端正是靠 I 的 ⊖ 保證 report 不會超過，這正是 GP 要「gracefully handle」的情形。同理 ⊚ BADEXPORTS（|e| ≠ m_e）也只影響該 item。你們 work_package.go：z := len(o) + rSum、rSum += len(r.Data)，error 結果的 Data 為空所以不累加——與 GP 一致。W_R = 48·2^10 = 49,152。

**逐項辨析**

1. ❌ ok, ok, ok, ok — each output is individually far below W_R, so I never fires; the 48 KiB ceiling is enforced only on-chain, by eq. 11.8 against the report as assembled from the digests  
   I 在 guarantor 端就會觸發；真等到 eq. 11.8 才擋，整份 report 會被拒、guarantor 白做工。
2. ❌ Ξ returns ∇ for the whole package: 1,000 + 20,000 + 25,000 + 5,000 + 3,000 = 54,000 exceeds W_R, and E is true whenever the authorization trace plus all refinement outputs cross the limit, so no report is produced at all  
   輸出總長度從來不是 E 的判斷條件，eq. 14.13 的 E 只有 Ψ_I 失敗與 keys(l) 不對兩個 disjunct。
3. ✅ ok (20,000), ok (25,000), ⊖ OVERSIZE, ok (3,000) — the size check is cumulative (|r| + |t| + Σ of earlier successful outputs > W_R) and an item replaced by ⊖ contributes 0 to the sums of later items  
   第三項 5,000 + 46,000 = 51,000 > W_R 觸發 ⊖，其 r 不計入 z，第四項的 z 仍停在 46,000。
4. ❌ ok (20,000), ok (25,000), ⊖ OVERSIZE, ⊖ OVERSIZE — the check is cumulative (|r| + |t| + Σ of earlier outputs > W_R) but an item replaced by ⊖ still contributes its own 5,000 octets, so every later item is oversize as well  
   把 oversize item 的 5,000 也算進 z 了；I 的 z 只累加 (r ∈ B, …) 也就是成功的輸出。

> **陷阱**　z 只累計成功輸出 + |t|；⊖ 的 item 不計入，後面的 item 還有機會成功。

<sub>`ch14-oversize-cumulative-calc`</sub>

---

### 14-6　Exported segments are committed to by the segments-root e of the availability specification and justified through paged proofs. Which statement is correct?

<sub>14.3.1 Exporting / 14.4.1 Availability Specifier — ●●○ · 概念 · eq. 14.12 (P), 14.18 (A: e = M(s), s♣); eq. E.4–E.6 (M, J_x, L_x); eq. 13.12</sub>

**標準答案**　e = M(s) is the root of a constant-depth binary Merkle tree over the exported segments, its leaves '$leaf'-prefixed and zero-hash-padded to a power of two; P(s) yields ⌈|s|/64⌉ extra segments, each the zero-padded encoding of the Merkle path J_6(s, i) to a 64-leaf subtree plus that subtree's page of 64 leaf hashes L_6(s, i); both are erasure-coded into the long-lived D³L

§14.3.1：segments-root「is formed as the root of a constant-depth binary Merkle tree as defined in equation E.4」，即 A 的 e = M(s)（eq. 14.18）；E.4/E.7：M 先把每個 leaf 做 H($leaf ⌢ v) 再零填到 2 的冪——固定深度讓 import 證明的大小固定（eq. 14.7 的 32·⌈log₂ W_X⌉）。eq. 14.12：P(s) = [P_{W_G}(E(↕J_6(s, i), ↕L_6(s, i))) | i < ⌈|s|/64⌉]——每頁一個 segment，內容是 root 到該 64-leaf 子樹的路徑 J_6（E.5）加上這 64 個 leaf hash 的頁 L_6（E.6），零填到 4,104；§14.4.1：「exactly ⌈1/64⌉ paged-proof segments as the number of yielded segments, each composed of a page of 64 hashes of segments, together with a Merkle proof from the root to the subtree-root which includes those 64 segments」。它們與 exported segments 串接後（s ⌢ P(s)）一起被 C_v 切片、轉置後做成 s♣ → erasure root（eq. 14.18），存在長期的 D³L；importer「each justification can be derived through a single paged-proof」，最壞每個 import 多抓一個 segment，連續 import 共用一頁。這也是 eq. 13.12 DA-load 65/64 的由來。你們 work_package.go PagedProofs：Jx(6,…)、Lx(6,…)、PadToMultiple(SegmentSize)；A 用 merkle_tree.M 算 exportsRoot。

**逐項辨析**

1. ✅ e = M(s) is the root of a constant-depth binary Merkle tree over the exported segments, its leaves '$leaf'-prefixed and zero-hash-padded to a power of two; P(s) yields ⌈|s|/64⌉ extra segments, each the zero-padded encoding of the Merkle path J_6(s, i) to a 64-leaf subtree plus that subtree's page of 64 leaf hashes L_6(s, i); both are erasure-coded into the long-lived D³L  
   §14.3.1 明指 segment root 用 eq. E.4 的固定深度 M，eq. 14.12 每 64 個 segment 一頁，兩者同進 D³L。
2. ❌ e = M_B(s), the well-balanced binary Merkle root taken over the raw segments with no padding; P(s) yields ⌈|s|/64⌉ extra segments, each the zero-padded encoding of the Merkle path J_6(s, i) to a 64-leaf subtree plus that subtree's page of 64 leaf hashes L_6(s, i); both are stored inside the auditable bundle rather than in the D³L  
   M_B（E.3）是 well-balanced 無填充版，用在 erasure root 與 extrinsic hash；bundle 裝的是本包 import 的 justification。
3. ❌ e = M(s) is the root of a constant-depth binary Merkle tree over the exported segments, its leaves '$leaf'-prefixed and zero-hash-padded to a power of two; P(s) yields one 4,104-octet segment per 32 exported segments, each carrying a page of 32 leaf hashes plus its subtree path; they live only in the short-term Audit DA, since auditors are the only ones who verify imports  
   頁大小是 2^6 = 64（DA load 才會是 65/64）；逐一驗 import justification 的其實是 guarantor。
4. ❌ e = M(s) is the root of a constant-depth binary Merkle tree over the exported segments, its leaves '$leaf'-prefixed and zero-hash-padded to a power of two; P(s) yields one extra segment per exported segment, each holding that segment's own ≈350-byte justification, and every validator stores all of them so that any node can verify any import without fetching chunks  
   §14.2.2 正因這種資料 too voluminous to have all validators store all data，才改成 64 個 segment 一頁。

> **陷阱**　segment root = M（constant depth，$leaf 前綴 + 零填）；paged proof 每 64 個 segment 一頁、存 D³L；65/64。

<sub>`ch14-paged-proofs`</sub>

---

### 14-7　What exactly goes into the auditable work-bundle B(p, l) that guarantors erasure-code into the Audit DA, and how does that differ from what goes into the D³L?
...
## 附錄 A · PVM　<sub>17 題</sub>

### A-1　The PVM invocation Ψ returns an exit reason ε. Which is the complete set of possible exit reasons?

<sub>A.1 Basic Definition — ●○○ · 概念 · eq. A.1 (Ψ) & §4.7</sub>

**標準答案**　∎ halt (regular termination), ☇ panic, ∞ out-of-gas, F̄ × address (page fault, with the lowest inaccessible page address), h̄ × id (host call, with the host-call identifier)

eq. A.1：ε ∈ {∎, ☇, ∞} ∪ ({F̄, h̄} × N_R)，剛好五種。§A.1：「In the case of a final halt, either through panic or success, the instruction counter returned is zero. In all other cases, the return value of the instruction counter indexes the one which caused the exit to happen and the machine state represents the prior state of said instruction」——所以 host call / page fault / OOG 都是可以「續跑」的：改 RAM、補 gas、或執行 host call 並把 PC 前進 1 + skip。stack 只是標準初始化（eq. A.46）配置的一段 W 記憶體，不是 PVM 的原生概念。

**逐項辨析**

1. ✅ ∎ halt (regular termination), ☇ panic, ∞ out-of-gas, F̄ × address (page fault, with the lowest inaccessible page address), h̄ × id (host call, with the host-call identifier)  
   與 eq. A.1 的 {∎, ☇, ∞} ∪ ({F̄, h̄} × N_R) 完全一致，fault 帶的正是對齊後的 page 位址。
2. ❌ ∎ halt (regular termination), ☇ panic, ∞ out-of-gas, F̄ × address (page fault, carrying the exact octet address that was accessed), h̄ × id (host call, with the host-call identifier)  
   eq. A.9 回的是 Z_P⌊·/Z_P⌋ 對齊後的最低不可存取 page 位址，不是實際存取的 octet 位址。
3. ❌ ∎ halt (regular termination), ☇ panic, ∞ out-of-gas, F̄ × address (page fault, with the lowest inaccessible page address), ⊘ divide-by-zero (raised when div_u_64 is given φ_B = 0)  
   §A.5.1 的除法從不 panic（除以 0 得 2^64 − 1），⊘ 不存在，這組還漏掉 host call。
4. ❌ ∎ halt (regular termination), ☇ panic, ∞ out-of-gas, h̄ × id (host call, with the host-call identifier), ⊗ stack-overflow (raised when φ_1 falls below the stack’s lowest page)  
   PVM 沒有 stack 概念，越界只是 F̄（低於 2^16 才 ☇），這組還漏掉 page fault。

> **陷阱**　PVM 內部沒有 stack overflow 概念——stack 只是記憶體區段，越界就是 page fault（或 < 2^16 時 panic）。

<sub>`appA-exit-reasons`</sub>
...
**標準答案**　(a) the machine panics immediately regardless of page accessibility; (b) the machine state is unchanged and the exit is a page fault F̄ × (Z_P·⌊addr/Z_P⌋), reporting the lowest inaccessible page address

§A.5：「When an index of RAM below 2^16 is required, the machine always panics immediately… regardless of the apparent (in)accessibility of the value. Otherwise, should the given index of RAM not be accessible then machine state remains unchanged and the exit reason is a fault with the lowest inaccessible page address」。eq. A.10：ε^μ = ☇ 當 min(x) mod 2^32 < 2^16；F̄ × Z_P·⌊min(x) mod 2^32 / Z_P⌋ 否則，x 是所有不可讀/不可寫的存取位址集合。第一個 64 KiB zone 永遠不可存取（標準初始化把 RO data 放在 Z_Z = 2^16 開始）——null-pointer 保護。你們 code-map 3.11：「page fault reports the lowest faulting address; any address in the first 64 KiB zone is a panic not a fault」。

**逐項辨析**

1. ✅ (a) the machine panics immediately regardless of page accessibility; (b) the machine state is unchanged and the exit is a page fault F̄ × (Z_P·⌊addr/Z_P⌋), reporting the lowest inaccessible page address  
   低位址無條件 ☇、高位址回 eq. A.10 的 F̄ × Z_P⌊·/Z_P⌋，且狀態完全不變因此可續跑。
2. ❌ (a) the access becomes a page fault F̄ × 0 so that the host can tell null-pointer bugs from ordinary faults; (b) the machine panics, because a page whose access mode in μ_a is ∅ can never be made accessible again and so the exit must be final  
   §A.5 明寫低於 2^16「always panics immediately」；不可存取頁反而是可續跑的 fault 而非終局 panic。
3. ❌ (a) the machine panics immediately regardless of page accessibility; (b) the exit is a page fault F̄ × addr carrying the exact octet address requested, and the octets of the access that did fall in accessible pages are written before the fault is raised  
   eq. A.10 在 ε^μ ≠ ▸ 時 (ı*, φ*, μ*) = (ı, φ, μ)，記憶體一個 byte 都不能先寫下去。
4. ❌ (a) reads below 2^16 return zero and writes there are silently dropped; (b) the exit is a page fault F̄ × (Z_P·⌊addr/Z_P⌋) for reads only — a write to a page whose access mode is R rather than W panics instead of faulting  
   低於 2^16 一律 ☇ 而非靜默；寫入判定用 V*_μ（μ_a = W），寫 R 頁同樣是 F̄ 不是 ☇。

> **陷阱**　位址先 mod 2^32（暫存器是 64-bit，記憶體是 32-bit 定址）。

<sub>`appA-memory-access`</sub>

---

### A-5　Under standard program initialization Y(p, a), which register/memory layout is correct?
...
**標準答案**　Because LLVM 'requires and assumes in its code generation that dynamically computed jump destinations always have a certain memory alignment', and JAM's tooling depends on LLVM, so the GP acquiesces to that assumption

eq. A.22 的 footnote：「The popular code generation backend LLVM requires and assumes in its code generation that dynamically computed jump destinations always have a certain memory alignment. Since at present we depend on this for our tooling, we must acquiesce to its assumptions.」djump(a)：a = 2^32 − 2^16 → halt；a = 0 ∨ a > |j|·Z_A ∨ a mod Z_A ≠ 0 ∨ j[a/Z_A − 1] ∉ ϖ → panic；否則 PC = j[a/Z_A − 1]。靜態 jump 與 branch 的目標（含 not-taken 的下一條）也都必須是 basic block 起點，否則 panic（§A.5）。

**逐項辨析**

1. ✅ Because LLVM 'requires and assumes in its code generation that dynamically computed jump destinations always have a certain memory alignment', and JAM's tooling depends on LLVM, so the GP acquiesces to that assumption  
   eq. A.22 的 footnote 直說是遷就 LLVM 對動態計算跳躍目標的對齊假設，純屬工具鏈妥協。
2. ❌ Because every jump-table entry occupies exactly Z_A = 2 octets: the factor of two is simply the entry width, so scaling by Z_A converts a table position into an octet offset inside j, and wider entries would need a larger alignment factor  
   把 Z_A 與 entry 寬度 z 搞混——z 由 blob 的 E_1(z) 決定，a/Z_A − 1 取的是 j 的元素索引。
3. ❌ Because RISC-V hardware traps on an unaligned instruction fetch and the PVM must stay directly executable on RISC-V hosts; the same rule is why sjump also rejects any static jump target b with b mod Z_A ≠ 0 before checking b ∈ ϖ  
   PVM 從不由 RISC-V 硬體直接執行，且 eq. A.20 的 sjump 只有 b ∉ ϖ 這一個 panic 條件。
4. ❌ Because odd dynamic addresses are reserved as sentinels: a = 1 means halt and every other odd value encodes a host-call identifier, so demanding a mod Z_A = 0 keeps genuine code targets disjoint from those encodings  
   halt sentinel 是偶數的 2^32 − 2^16，host-call id 來自 ecalli 的 ν_X 而非 djump 的位址空間。

> **陷阱**　jump table index 是 a/Z_A − 1（從 1 開始算），因為 a = 0 保留為無效。

<sub>`appA-djump-alignment`</sub>
...
### A-9　Every conditional branch in the team's 0.7.2 interpreter (instBranch for 170–175 and the branch_*_imm handlers) resolves through this helper. Under GP 0.8.0 eq. A.21, which statement about the branch rule is correct?
...
### A-10　The PVM has four unconditional control-transfer instructions: jump (40), jump_ind (50), load_imm_jump (80) and load_imm_jump_ind (180). Which description of their operands and target resolution is correct?

<sub>A.5.1 Instruction Tables (jump, jump_ind, load_imm_jump, load_imm_jump_ind) — ●●○ · 概念 · §A.5.1 opcodes 40/50/80/180 (eq. A.27, A.28, A.30, A.34 tables); eq. A.20 (sjump), A.22 (djump)</sub>

**標準答案**　jump and load_imm_jump carry a signed PC-relative offset (target = ı + Z_l(…)) resolved statically by sjump; jump_ind and load_imm_jump_ind compute (φ_reg + ν) mod 2^32 and resolve it through the jump table via djump; the two load_imm_* forms additionally write φ′_A = ν_X, which is how a call saves its return address

四條都在 §A.3 的 terminator 集合 T 裡。jump（40，eq. A.27 One Offset）：ν_X ≡ ı + Z_{l_X}(E^{-1}_{l_X}(ζ_{ı+1…}))，是相對於 ı 的帶號 offset，執行 sjump(ν_X)（eq. A.20：b ∉ ϖ → ☇）。load_imm_jump（80，eq. A.30 One Register, One Immediate and One Offset）：sjump(ν_Y) 且 φ′_A = ν_X——ν_Y 是 offset、ν_X 是要載入的立即值。jump_ind（50，eq. A.28）：djump((φ_A + ν_X) mod 2^32)。load_imm_jump_ind（180，eq. A.34 Two Registers & Two Immediates）：djump((φ_B + ν_Y) mod 2^32) 且 φ′_A = ν_X。djump（eq. A.22）：a = 2^32 − 2^16 → ∎；a = 0 ∨ a > |j|·Z_A ∨ a mod Z_A ≠ 0 ∨ j[a/Z_A − 1] ∉ ϖ → ☇；否則 ı′ = j[a/Z_A − 1]。典型用法：call = load_imm_jump（ra ← 返回用的 jump-table 位址，同時跳到函式）；return = jump_ind ra + 0；標準初始化（eq. A.47）把 φ_0 設成 2^32 − 2^16，entry function 一 return 就 halt。你們的 instJump 走 branch(pc, vX, true, …)，instJumpInd / instLoadImmJumpInd 走 djump（branch.go 的 DjumpResolve）。

**逐項辨析**

1. ❌ All four carry an absolute code offset; the _ind forms differ only in reading that offset from a register instead of from an immediate, and all four are validated by the same sjump rule (target ∈ ϖ), so the jump table j is consulted only for the halt sentinel 2^32 − 2^16 and never for an ordinary jump  
   jump/load_imm_jump 的 ν ≡ ı + Z_l(…) 是相對位移，而 j 是 _ind 兩條每次跳躍的必經之路。
2. ❌ jump and jump_ind form the static pair, whose targets v_blob already checks at deblob time so that no runtime ϖ test is needed; load_imm_jump and load_imm_jump_ind form the dynamic pair, going through the jump table j, writing φ′_A = ν_X and recognising the halt sentinel 2^32 − 2^16  
   配對與時機都錯：分界在 _ind（動態）而非 load_imm_（寫暫存器），且 v_blob 一個跳躍目標都不驗。
3. ✅ jump and load_imm_jump carry a signed PC-relative offset (target = ı + Z_l(…)) resolved statically by sjump; jump_ind and load_imm_jump_ind compute (φ_reg + ν) mod 2^32 and resolve it through the jump table via djump; the two load_imm_* forms additionally write φ′_A = ν_X, which is how a call saves its return address  
   分界正是 _ind：sjump 收相對 offset、djump 拿 (φ + ν) mod 2^32 查 j，load_imm_ 另外寫 φ′_A。
4. ❌ jump_ind and load_imm_jump_ind index the jump table directly with the register value, j[φ_A], their immediate serving only as a gas hint; jump and load_imm_jump carry a signed PC-relative offset resolved by sjump, and the halt sentinel 2^32 − 2^16 is recognised by sjump rather than by djump  
   eq. A.22 是先算 (φ + ν) mod 2^32 再取 a/Z_A − 1；sjump 只有 b ∉ ϖ 一個條件、不認 sentinel。

> **陷阱**　jump_ind 跳的是「jump table 位址」不是 code offset：a → j[a/2 − 1]；a = 0 保留為無效。

<sub>`b2-appA-jump-family`</sub>

---

### A-11　GP 0.8.0 threads a boolean 'gas charged' flag through Ψ. A basic block has an ecalli in its middle; the host call returns ▸ and Ψ_H resumes at ı″ = ı′ + 1 + skip(ı′). Which statement about gas charging in this situation is correct?
...
1. ❌ Only the suffix of the block is re-charged: every non-▸ exit — a host call included — clears the flag to ⊥, and Ψ_1 then prices just the instructions from ı″ up to the block’s terminator, so a block interrupted by n host calls costs its ϱ^Δ plus n partial re-charges  
...
> **陷阱**　flag 只被「執行完的 terminator」與「OOG」清成 ⊥；ecalli 不是 terminator，host call 續跑不重扣。

<sub>`b2-appA-gas-charged-flag`</sub>

---

### A-12　The recompiler contains a prepared-but-disabled block-level gas path for GP 0.8.0 (shown together with the current 0.7.2 per-instruction landing pad). Measured against eq. A.8, what is still wrong with the prepared path?

<sub>A.5 Single-Step State Transition (gas charging) — x86-64 recompiler — ●●● · 程式碼 · eq. A.8 (ε^ϱ, ϱ* unchanged on ∞), A.10 (ı* = ı), A.54 (ϱ^Δ) · ⚠ 0.7.2→0.8.0</sub>
...
```
<sub>PVM/recompiler/gas.go (emitOutOfGasExit, emitBlockGasCheck, emitBlockOutOfGasExit)</sub>

**標準答案**　Two things: the amount deducted must be the block's ϱ^Δ from the A.9 pipeline simulation (max(cycles − 3, 1)), not its instruction count; and on the out-of-gas path the deduction must be undone, because A.8 leaves ϱ unchanged when ϱ < ϱ^Δ — the per-instruction pad restores its charge, the block pad does not
...
1. ❌ Nothing: in 0.8.0 each instruction still costs 1 gas and is merely aggregated per block, so subtracting the instruction count at block entry is exactly ϱ^Δ; and leaving the counter negative on the out-of-gas path is intended, because Ψ_M's R function reports consumption as u = ϱ − max(ϱ′, 0) and clamps it anyway  
   0.8.0 的 ϱ^Δ 來自 §A.9 管線模擬，與指令數沒有固定比例；u 的 clamp 也救不了寫回外層的負 counter。
2. ❌ Only the exit PC: on a block-entry out-of-gas exit eq. A.10 requires ı* to point at the block's terminating instruction — the last one the charge would have paid for — rather than at the block start; the amount deducted and the untouched-gas rule are both already handled correctly by this code  
   eq. A.10 在 ε^ϱ ≠ ▸ 時 ı* = ı（嘗試收費的那條指令）；而金額與不扣款兩點正是真正的缺陷所在。
3. ✅ Two things: the amount deducted must be the block's ϱ^Δ from the A.9 pipeline simulation (max(cycles − 3, 1)), not its instruction count; and on the out-of-gas path the deduction must be undone, because A.8 leaves ϱ unchanged when ϱ < ϱ^Δ — the per-instruction pad restores its charge, the block pad does not  
   金額要換成 eq. A.54 的 max(cycles − 3, 1)，且 A.8 的 ϱ* = ϱ 要求 landing pad 把預扣的錢加回去。
4. ❌ The direction: 0.8.0 charges a block only once its terminator has executed, so the SubMemImm32 belongs after the terminator with the sign check reversed; charging at block entry would bill a block that panics half-way through, which eq. A.8 avoids precisely by deferring the deduction  
   事後收費等於讓 panic 的 block 免費，正是 GP 要求「預先扣款」所要防的事。

> **陷阱**　OOG 時 ϱ 不變（A.8）——「先扣再判斷」的 JIT 寫法一定要在 landing pad 把錢加回去，而且扣的是 ϱ^Δ 不是指令數。

<sub>`b2-appA-recompiler-block-gas-stub`</sub>

---

### A-13　In this implementation of load_imm_jump_ind the register write φ_A = ν_X happens even when the dynamic jump panics. Is that what GP 0.8.0 prescribes, and can the difference ever be observed?
...
```
<sub>PVM/instructions.go (instLoadImmJumpInd, opcode 180)</sub>

**標準答案**　Yes: the table row defines djump((φ_B + ν_Y) mod 2^32) and φ′_A = ν_X as independent mutations, and on ☇ Ψ returns the posterior registers φ′ with only ı reset to 0; it is observable because the invoke host call writes the inner machine's registers back to memory even when the inner exit is PANIC, and at top level via φ_7/φ_8 when the sentinel halts

eq. A.34 表列 load_imm_jump_ind：djump((φ_B + ν_Y) mod 2^32)，φ′_A = ν_X——兩個 mutation 並列、互不條件化；eq. A.22 的 djump 只決定 (ε, ı′)。eq. A.10：ε^ϱ、ε^μ 都是 ▸ 時 (ε*, ı*, φ*, μ*) = (ε, ı′, φ′, μ′)，所以 ε = ☇ 時 φ* 仍含這次寫入；eq. A.1 的 Ψ 在 ε ∈ {☇, ∎} 回傳 (ε, 0, ϱ′, flag′, φ′, μ′)——posterior 的暫存器、只有 ı 歸零。§A.1：「In the case of a final halt, either through panic or success, the instruction counter returned is zero」，而那句「the machine state represents the prior state of said instruction」講的是「In all other cases」，即 ∞ / F̄ / h̄ 這三種可續跑的 exit（Ψ 回傳 (ε, ı, ϱ′, flag′, φ, μ)）。可觀察性：App. B 的 invoke 在 c = ☇ 時仍回 (▸, PANIC, φ_8, μ*, m*)，而 μ* 把 E_8(g′_R) ⌢ E_8(φ′) 寫回外層記憶體，外層程式讀得到內層 panic 後的暫存器；頂層 Ψ_M（eq. A.48 的 R）雖對 ☇ 回 panic，但 sentinel halt 時回傳 μ′[φ′_7 …+φ′_8]，若 A ∈ {7, 8} 也直接可見。你們程式碼註解引用的 koute 測試向量 inst_load_imm_and_jump_indirect_invalid_djump_to_zero_different_regs_without_offset_nok 正是要求 panic 後暫存器已更新。load_imm_jump（80）同理：sjump(ν_Y) 與 φ′_A = ν_X 並列。

**逐項辨析**

1. ❌ No: §A.1's 'the machine state represents the prior state of said instruction' covers ☇ just as it covers ∞, F̄ and h̄, so φ_A must stay untouched when the djump panics; the write is a latent bug that goes unnoticed only because Ψ_M's R function maps ☇ to the panic result and throws the register file away  
   那句 prior state 的主詞是「In all other cases」，☇/∎ 回的是 posterior 狀態、只有 ı 歸零。
2. ❌ Yes, but only for the halt sentinel a = 2^32 − 2^16, where φ_A = ν_X is part of what the caller reads back; eq. A.22 evaluates djump before the register write, so on the panic branch the write is skipped, and no caller could observe it anyway because a panicking inner machine leaves the outer memory untouched  
   eq. A.34 把兩個 mutation 並列、沒有任何條件化；invoke 在 c = ☇ 時照樣把 φ′ 寫回外層 RAM。
3. ❌ No: eq. A.10 rolls back every mutation of a panicking instruction — ε* = ☇ forces (ı*, φ*, μ*) = (ı, φ, μ) exactly as it does for a page fault — so φ_A keeps its old value; the koute PVM test vectors bear this out, expecting φ_A unchanged in every *_nok djump case — the jump-to-zero, the over-length and the misaligned variants alike  
   回捲只發生在 ε^ϱ ≠ ▸ 與 ε^μ ≠ ▸ 兩支，指令層級的 ☇ 走的是 otherwise 支 (ε, ı′, φ′, μ′)。
4. ✅ Yes: the table row defines djump((φ_B + ν_Y) mod 2^32) and φ′_A = ν_X as independent mutations, and on ☇ Ψ returns the posterior registers φ′ with only ı reset to 0; it is observable because the invoke host call writes the inner machine's registers back to memory even when the inner exit is PANIC, and at top level via φ_7/φ_8 when the sentinel halts  
   表列把 djump 與 φ′_A = ν_X 並列，而 eq. A.1 在 ☇ 回傳 posterior 的 φ′，只有 ı 歸零。

> **陷阱**　☇ 與 ∎ 回傳 posterior 狀態（只有 ı 歸零）；∞ / F̄ / h̄ 才回傳 prior 狀態以便續跑。

<sub>`b2-appA-load-imm-jump-ind-reg-write`</sub>

---

### A-14　A store_u64 (8 bytes) targets address 0x2_0FFC: page 0x20 (0x20000–0x20FFF) is writable, page 0x21 is unallocated. What exit does GP 0.8.0 eq. A.9 require, and how does this implementation differ?

<sub>A.5 Single-Step State Transition (memory faults) — ●●● · 程式碼 · eq. A.9 (ε^μ), A.10 (state unchanged on F̄); §A.5.1 store_u64 row</sub>
...
```
<sub>PVM/decode.go (storeIntoMemory), used by every store_* / store_ind_* / store_imm_* handler</sub>

**標準答案**　GP: F̄ × 0x2_1000 — the lowest inaccessible page address among the written indices (x = {0x21000…0x21003}, min(x) rounded down to a multiple of Z_P); the code correctly leaves memory untouched but reports memIndex = 0x2_0FFC, an address that lies in an accessible page and is not page-aligned

eq. A.9：x = {x ∈ w : x mod 2^32 ∉ V*_μ}（w 是這條指令要寫的所有索引；V*_μ = {i : μ_a[⌊i/Z_P⌋] = W} 是可寫索引集），ε^μ = ☇ when min(x) mod 2^32 < 2^16；F̄ × Z_P⌊min(x) mod 2^32 ÷ Z_P⌋ otherwise。這裡 w = {0x20FFC, …, 0x21003}，只有落在 page 0x21 的四個位址不可寫，x = {0x21000, …, 0x21003}，min(x) = 0x21000，對齊 Z_P = 4096 後仍是 0x21000。§A.5 原文：「the exit reason is a fault with the lowest page address to be written which is inaccessible」。eq. A.10：ε^μ ≠ ▸ 時 (ε*, ı*, φ*, μ*) = (ε^μ, ı, φ, μ)——狀態完全不變（r 與 w 是「為了算出 Ψ_1 的結果而需要下標的全部索引」，要先整組檢查才可能有任何 mutation）。你們的實作先檢查兩頁再 copy，記憶體不變這點正確，但每個 fault 分支都回 `ExitPageFault | ExitReason(memIndex)`，回報的是存取起始位址 0x20FFC：它在可寫的 page 裡、也不是 page 對齊；即使不跨頁（例如 fault 在 0x21004）也應回 0x21000 而非 0x21004。recompiler 的 SIGSEGV handler 同樣塞入原始 guest_addr。可觀察性：invoke host call 把 F̄ 的位址放進 φ_8（FAULT），koute 的 PVM 測試向量也帶 expected-page-fault-address 欄位；正確做法是回報前先 `addr &^ (ZP-1)`，並在跨頁時選第一個不可存取的 page。（loadFromMemory 同樣回原始 vx。）

**逐項辨析**

1. ✅ GP: F̄ × 0x2_1000 — the lowest inaccessible page address among the written indices (x = {0x21000…0x21003}, min(x) rounded down to a multiple of Z_P); the code correctly leaves memory untouched but reports memIndex = 0x2_0FFC, an address that lies in an accessible page and is not page-aligned  
   x 只收不可存取的索引，min(x) = 0x21000 再套 Z_P 對齊；實作回報的 0x20FFC 落在可寫頁又未對齊。
2. ❌ GP: F̄ × 0x2_0FFC — eq. A.9 reports min(x) itself, and x is seeded with the address the instruction was handed rather than with only the inaccessible indices; the implementation is therefore exactly compliant here and in the single-page case, where the reported value is likewise the first octet touched  
   x 的定義只收「不可存取」的索引，可寫的 0x20FFC 根本不在 x 裡，外面還套了 Z_P⌊· ÷ Z_P⌋ 對齊。
3. ❌ GP: ☇ (panic) — a multi-octet access may not straddle a page boundary, and the GP treats such a straddling access exactly like an access below 2^16; the code is wrong to report a page fault at all, and would be equally wrong to commit the four octets that do fit inside page 0x20  
   eq. A.9 只有 min(x) mod 2^32 < 2^16 這一個 ☇ 條件，跨頁與未對齊本身都是合法存取。
4. ❌ GP: the four octets that fit are committed to page 0x20 and only then is F̄ × 0x2_1000 raised for the remainder, because eq. A.9 is evaluated octet by octet as the store proceeds; the code is wrong to leave memory completely unchanged and wrong to test both pages before copying anything  
   eq. A.10 在 ε^μ ≠ ▸ 時狀態完全不變，因此必須先整組檢查才可能有任何 mutation。

> **陷阱**　回報的是「最低不可存取的 page 位址」（Z_P 對齊、且必須真的不可存取），不是存取的起始位址。

<sub>`b2-appA-store-cross-page-fault-address`</sub>

---

### A-15　The x86-64 recompiler maps guest memory as one 4 GiB region based at R15 with real mprotect permissions and emits no software bounds check above 2^16; this signal handler turns hardware events inside JIT code into PVM exits. Which statement is correct?

<sub>A.5 (memory faults & division semantics) — recompiler design — ●●● · 程式碼 · eq. A.9 (ε^μ: < 2^16 → ☇, else F̄), §A.5.1 div/rem rows (b = 0, MIN/−1)</sub>
...
```
<sub>PVM/recompiler/x86signal/x86_signal_linux.c (signal_handler)</sub>

**標準答案**　A fault at guest offset < 2^16 becomes ☇ and any other fault inside the guest window becomes F̄ carrying the faulting guest address, after the 13 pinned x86 registers are spilled to the control region and RIP/RSP are rewritten to the Go return point; SIGFPE from JIT code becomes ☇ only as a safety net, because correct div/rem codegen never lets IDIV see b = 0 or MIN/−1

對照 eq. A.9：min(x) mod 2^32 < 2^16 → ☇（GP 特別強調 regardless of the apparent (in)accessibility），否則 F̄——handler 的 `guest_addr < 0x10000u → jit_exit_panic`、否則 `(EXIT_PAGE_FAULT_TYPE << 56) | guest_addr` 正是這條規則的硬體版：前 64 KiB 永遠不映射（PROT_NONE），存取就 SIGSEGV。恢復方式不是「重試」：handler 先用 store_pvm_regs 把 ucontext 裡 13 個 pinned 暫存器（RAX/RDX/RBX/…/RBP，對應 register_map.go 的 PVMToX86）寫回 R15 下方的 control region，再把 RIP/RSP 改成 entry trampoline 存好的 OFF_RETURN_ADDR / OFF_RETURN_STACK，於是 sigreturn 後直接「回到 Go」，BlockBasedInvoke 讀 ExitReason/ExitPC 後結束（這也是為何要 runtime.LockOSThread：guest_base_ptr 是 per-OS-thread 的 TLS）。GP 的 F̄ 本來就是可續跑的 exit（改 RAM 後再 Ψ），但這由 host（例如 invoke + pages）決定，不是 handler 偷偷補頁。SIGFPE：x86 IDIV 在除以 0 與 MIN/−1 會發 #DE，而 GP 規定 div 除零得 2^64 − 1、rem 除零得被除數、MIN/−1 得 φ_A / 0（§A.5.1），不能 panic——所以 emit_arith_three.go 的 emitDivS64 先 `TestRegReg` 判 0、再 `CmpRegImm32(-1)` 判 −1 才走 IDIV，handler 的 SIGFPE → ☇ 只是防線。剩下的差距：回報的是原始 guest_addr，而 eq. A.9 要求 Z_P⌊·/Z_P⌋ 對齊的最低不可存取 page 位址。其他設計選擇：executable.go 用 memfd 雙映射（RW 視圖寫碼、RX 視圖執行）避免每個 block mprotect 切換 W^X。

**逐項辨析**

1. ❌ Every fault is retried in place: the handler mprotects the missing page read-write, restores the spilled registers and returns from the signal so the faulting x86 instruction re-executes — the JIT analogue of eq. A.9's 'RAM must be changed' recovery; SIGFPE is chained to the previously installed handler, since an IDIV trap can only ever be a host bug  
   F̄ 要回到 host 決定是否改 RAM，handler 私自補頁等於憑空給服務記憶體；SIGFPE 轉交舊 handler 會讓 process 直接死掉。
2. ✅ A fault at guest offset < 2^16 becomes ☇ and any other fault inside the guest window becomes F̄ carrying the faulting guest address, after the 13 pinned x86 registers are spilled to the control region and RIP/RSP are rewritten to the Go return point; SIGFPE from JIT code becomes ☇ only as a safety net, because correct div/rem codegen never lets IDIV see b = 0 or MIN/−1  
   前 64 KiB 永遠 PROT_NONE，硬體 SIGSEGV 恰好對上 eq. A.9 的 ☇ / F̄ 二分；除法特例則靠 codegen 先擋。
3. ❌ A fault at guest offset < 2^16 becomes F̄ × 0 so that the host can tell null-pointer bugs apart from ordinary faults, and any other fault becomes F̄ carrying the faulting guest address; SIGFPE becomes ∞, because div/rem is the most expensive row of the §A.10 table (60 cycles, 1 DIV unit) and a trapping divide means the block was mispriced  
   eq. A.9 要求 < 2^16 一律 ☇；∞ 只由 eq. A.8 的 gas 不足產生，div/rem 也不是 A.10 最貴的一列。
4. ❌ The handler cannot know which PVM registers were live at the fault, so it always reports ☇ and discards register state; only the interpreter backend can ever produce F̄, which is why the recompiler emits a software bounds check before every load and store and bails out to the interpreter whenever the checked address leaves the 4 GiB window  
   store_pvm_regs 從 ucontext 撈得到全部 13 個 pinned 暫存器，配真實 mprotect 正是為了免掉軟體檢查。

> **陷阱**　硬體 page fault 只能替代「≥ 2^16 的可存取性檢查」；< 2^16 → ☇ 與除法特例是語意規則，要在 codegen / handler 明確處理。

<sub>`b2-appA-sigsegv-handler`</sub>

---

### A-16　GP 0.8.0's deblob(p, ı) returns error — and Ψ then panics without executing anything — unless v_blob(c, k, 0) and v_inst(c, k, ı) both hold. Which set of conditions do these two validators actually enforce?
...
1. ❌ Only that |k| = |c| and that every jump-table entry points at a basic-block start; opcode validity is left to execution time, where an unknown opcode behaves like opcode 0 and traps, and the entry point ı is accepted wherever it lands, since Ψ re-derives the nearest opcode boundary from k  
   deblob 另外要求 v_inst(c, k, ı)：入口必須 k_ı = 1 且 c_ı ∈ U，Ψ 不會替你重新對齊到最近邊界。
2. ❌ That every branch and jump target lies in ϖ and that no instruction is longer than 16 octets, both established during a single walk of the code; the final instruction may be anything, because ζ pads the code with zeros so that running off the end simply executes opcode 0 and traps  
   跳躍目標是 eq. A.20/A.21 在 runtime 查 ϖ；真正的關鍵被漏掉了——走訪必須停在 |k| 且尾端 ∈ T。
3. ✅ |k| = |c|; walking the code from index 0 by 1 + skip(·), every visited octet is flagged in k and is a valid opcode (∈ U); the walk ends exactly at |k| with the last instruction a terminator (∈ T); and the entry point ı is itself a flagged, valid opcode  
   四項正是 eq. A.2 的 v_blob 與 v_inst：|k| = |c|、沿途皆合法 opcode、剛好停在 |k|、尾端 ∈ T。
4. ❌ That the code contains at least one ecalli and ends with a jump_ind to the halt sentinel 2^32 − 2^16, so that every program has a well-defined return path to the host; the bitmask itself needs no check, since k is regenerated from each opcode's length while deblob decodes  
   GP 沒有這兩條結構要求；k 是 blob 實際傳來的欄位，正因指令長度隱含才需要它，無法反推。

> **陷阱**　v_blob 驗的是「指令流的形狀」（合法 opcode + terminator 收尾），不驗跳躍目標；跳錯位置仍是 runtime panic。

<sub>`b2-appA-vblob-terminator`</sub>

---

### A-17　This is the team's GP 0.7.2 opcode table for the no-argument and two-register groups. What must change to match GP 0.8.0?

<sub>A.3 Basic Blocks & A.5.1 (opcodes 2 and 100–110) — ●●○ · 程式碼 · §A.5.1 tables (opcodes 0–2, 100–110), eq. A.65 (𝔟 branch cost), A.10 table (unlikely = 40 cycles) · ⚠ 0.7.2→0.8.0</sub>
...
```
<sub>PVM/opcode_info.go (opcodeInfoTable, excerpt)</sub>

**標準答案**　sbrk (101) disappears — heap growth becomes the grow_heap host call — so count_set_bits_64 … reverse_bytes shift down to 101–110; and a new no-argument opcode unlikely = 2 appears: a no-op that is not a terminator but is priced at 40 cycles and makes any branch targeting it (or trap) cost 𝔟 = 1 instead of 20

GP 0.8.0 §A.5.1：無引數指令 0 trap（ε = ☇）、1 fallthrough（sjump(ı + 1 + skip(ı))）、2 unlikely（沒有任何 mutation）；Two Registers 表：100 move_reg、101 count_set_bits_64、102 count_set_bits_32、103 leading_zero_bits_64、104 leading_zero_bits_32、105 trailing_zero_bits_64、106 trailing_zero_bits_32、107 sign_extend_8、108 sign_extend_16、109 zero_extend_16、110 reverse_bytes——sbrk 不存在了（PR #508：「sbrk replaced by grow_heap host call；opcodes and host calls renumbered」），heap 成長改由 App. B 的 grow_heap host call 處理。§A.3 的 T 沒有 unlikely，也沒有 sbrk：unlikely 只是提示，執行到它就 ı′ = ı + 1 + skip(ı) 繼續同一個 block；trap 仍是 opcode 0，所以 ζ 的零填充（eq. A.4）依然是 trap。價格：A.10 表 unlikely = 40 cycles、1 decode slot、不佔 unit；trap / fallthrough = 2 cycles；eq. A.65：𝔟(c, ı) = 1 when {ζ_{ı+1+skip(ı)}, ζ_{ı_target}} ∩ {unlikely, trap} ≠ ∅，否則 20——編譯器把 unlikely 放在冷路徑（錯誤處理）入口，熱路徑上的 branch 幾乎免費，真的走進冷 block 才付 40。你們要做的：刪掉 instSbrk（code-map 3.11.8）、整組 101–111 往下移一格、新增 opcode 2（IsTerminator: false）並在 gas 表加上 40 cycles 與 𝔟 的目標檢查。

**逐項辨析**

1. ❌ Nothing in these groups: 0.8.0 renumbered only the host calls, inserting grow_heap at id 1, while PVM opcodes stay frozen across GP versions so that already-compiled service blobs remain valid; the new unlikely hint is encoded as an ecalli with ν_X = 0 rather than as an opcode of its own  
   0.8.0 這次確實重新編號了 PVM opcode；ecalli 的語意是 ε = h̄ × ν_X，與純 no-op 的提示指令無關。
2. ❌ sbrk stays at 101 but becomes a terminator, because growing the heap can now page-fault and must therefore end a basic block; unlikely = 2 is added as a second new terminator, so a compiler can close a block wherever it marks cold code, and both are priced at 2 cycles just like trap and fallthrough  
   sbrk 已被移除，§A.3 的 T 也不含這兩者，而 A.10 表給 unlikely 的是 40 cycles 不是 2。
3. ❌ The two-register group is renumbered to start at 101 and trap moves from opcode 0 to opcode 2, swapping places with unlikely, so that a zero octet is no longer a valid opcode and running off the end of the code is caught by v_blob at deblob time rather than by ζ's zero padding at run time  
   trap 仍是 opcode 0，正因如此 eq. A.4 的 ζ 補零才等同 trap；兩暫存器組仍從 100 的 move_reg 起算。
4. ✅ sbrk (101) disappears — heap growth becomes the grow_heap host call — so count_set_bits_64 … reverse_bytes shift down to 101–110; and a new no-argument opcode unlikely = 2 appears: a no-op that is not a terminator but is priced at 40 cycles and makes any branch targeting it (or trap) cost 𝔟 = 1 instead of 20  
   sbrk 被 grow_heap host call 取代使 101–110 整組下移，而 unlikely 讓指向它的 branch 只付 𝔟 = 1。

> **陷阱**　unlikely 是 hint 不是 terminator；便宜的是「指向它的 branch」（𝔟 = 1），貴的是「真的跑進去」（40 cycles）。

<sub>`b2-appA-opcode-renumbering-unlikely`</sub>
...
### B-2　The PVM has three invocation types. Which statement about their host-call sets is correct in GP 0.8.0?

<sub>B.2–B.4 Invocations — ●●○ · 概念 · eq. B.1–B.2, B.5–B.6, B.9–B.11</sub>
...
<sub>`appB-three-invocations`</sub>

---

### B-3　Which host-call index assignment is correct for GP 0.8.0?

<sub>B.5–B.7 host-call tables — ●●○ · 版本差異 · §B.5–B.7 (0.8.0 numbering) · ⚠ 0.7.2→0.8.0</sub>

**標準答案**　gas 0, grow_heap 1, fetch 2, lookup 3, read 4, write 5, info 6, historical_lookup 7, export 8, machine 9, peek 10, poke 11, pages 12, invoke 13, expunge 14, bless 15, assign 16, designate 17, checkpoint 18, new 19, upgrade 20, transfer 21, eject 22, query 23, solicit 24, forget 25, yield 26, provide 27, log 100 (JIP-1)

0.8.0 附錄 B 的表格：gas=0、grow_heap=1、fetch=2、lookup=3、read=4、write=5、info=6、historical_lookup=7、export=8、machine=9、peek=10、poke=11、pages=12、invoke=13、expunge=14、bless=15、assign=16、designate=17、checkpoint=18、new=19、upgrade=20、transfer=21、eject=22、query=23、solicit=24、forget=25、yield=26、provide=27；JIP-1 的 log = 100。grow_heap 插在 1 使 0.7.2 之後的所有 index 都 +1（你們 issue digest：「grow_heap at id 1」、code-map 的「0.7.2 host-call numbering vs 0.8.0 shift」），這是 0.7.2 code 跑 0.8.0 service 時最直接的 mismatch 來源。

**逐項辨析**

1. ✅ gas 0, grow_heap 1, fetch 2, lookup 3, read 4, write 5, info 6, historical_lookup 7, export 8, machine 9, peek 10, poke 11, pages 12, invoke 13, expunge 14, bless 15, assign 16, designate 17, checkpoint 18, new 19, upgrade 20, transfer 21, eject 22, query 23, solicit 24, forget 25, yield 26, provide 27, log 100 (JIP-1)  
   grow_heap 插在 1，使 0.7.2 之後的所有 index 全部 +1，log = 100 由 JIP-1 定。
2. ❌ gas 0, fetch 1, lookup 2, read 3, write 4, info 5, historical_lookup 6, export 7, machine 8, peek 9, poke 10, pages 11, invoke 12, expunge 13, bless 14, assign 15, designate 16, checkpoint 17, new 18, upgrade 19, transfer 20, eject 21, query 22, solicit 23, forget 24, yield 25, provide 26, log 100 (JIP-1)  
   這正是 0.7.2 的編號，拿它跑 0.8.0 會讓每個 ecalli 都打到相鄰的 host call。
3. ❌ gas 0, grow_heap 1, fetch 2, lookup 3, read 4, write 5, info 6, historical_lookup 7, export 8, compile 9, machine 10, peek 11, poke 12, pages 13, invoke 14, expunge 15, bless 16, assign 17, designate 18, checkpoint 19, new 20, upgrade 21, transfer 22, eject 23, query 24, solicit 25, forget 26, yield 27, provide 28, log 100 (JIP-1)  
   GP 0.8.0 附錄 B 沒有 compile——PVM 程式直接以 JAM blob 餵給 machine，沒有編譯步驟。
4. ❌ assign 0, bless 1, checkpoint 2, designate 3, eject 4, expunge 5, export 6, fetch 7, forget 8, gas 9, grow_heap 10, historical_lookup 11, info 12, invoke 13, lookup 14, machine 15, new 16, pages 17, peek 18, poke 19, provide 20, query 21, read 22, solicit 23, transfer 24, upgrade 25, write 26, yield 27, log 100 (JIP-1)  
   編號依 general / refine / accumulate 三段功能分區給定，gas 恆為 0、assign 落在 16。

> **陷阱**　遷移時 host-call 編號全部位移——這是 0.7.2 code 跑 0.8.0 service 時最直接的 mismatch。

<sub>`appB-host-call-indices`</sub>

---

### B-4　Which statement about the accumulate invocation Ψ_A(e, t, s, g, i) is correct?

<sub>B.4 Accumulate Invocation — ●●● · 概念 · eq. B.7–B.14</sub>

**標準答案**　If the service's code is unavailable or > W_C it returns the state with incoming transfer amounts already credited and no other effect; otherwise it runs Ψ_M(code, entry 5, g, E(t, s, |i|), F, I(s, s)^2) with a regular context x and an exceptional context y (only `checkpoint` copies x into y); on ☇ or ∞ the result collapses to y; a 32-octet return blob becomes the yield hash; inputs are read via `fetch`

eq. B.9：Ψ_A(e, t, s, g, i)：若 code = ∅ 或 |code| > W_C → (s（餘額已加上 Σ transfer 金額）, [], ∅, 0, [])；否則 C(Ψ_M(code, 5, g, E(t, s, |i|), F, I(s, s)^2))——entry point 是 **5**（refine 與 is-authorized 是 0），參數只有 (timeslot, service id, input 數) 共 12 octets，正是為了「avoid introducing unbounded data as part of the basic invocation arguments」（否則 RAM 配置無界、gas 會被灌水），實際 inputs 用 fetch(14/15) 讀。eq. B.7：context **x** = (s id, u state, i nextfreeid, t transfers, y yield, p provisions)；例外維度是 **y**；eq. B.10：I(s, s) 初始化 nextfreeid = check(decode_4(H(E(s, η′_0, H_T))) mod (2^32 − S − 2^8) + S)。collapse C（B.13）：o ∈ {∞, ☇} → 用 **y**（checkpoint 過的狀態）；o ∈ H（32 bytes）→ yield = o；其他 → X 原樣。你們 code-map 3.8.7：「accumulate entry point is PC 5；on PANIC/OOG the checkpointed context y is committed；a 32-byte return blob becomes the accumulation output hash」。

**逐項辨析**

1. ✅ If the service's code is unavailable or > W_C it returns the state with incoming transfer amounts already credited and no other effect; otherwise it runs Ψ_M(code, entry 5, g, E(t, s, |i|), F, I(s, s)^2) with a regular context x and an exceptional context y (only `checkpoint` copies x into y); on ☇ or ∞ the result collapses to y; a 32-octet return blob becomes the yield hash; inputs are read via `fetch`  
   entry point 是 5、參數只有 12 octets 的 E(t, s, |i|)，例外時 collapse 取 checkpoint 過的 y。
2. ❌ If the service's code is unavailable or > W_C it returns the state with incoming transfer amounts already credited and no other effect; otherwise it runs Ψ_M(code, entry 1, g, E(t, s, |i|) ⌢ E(i), F, I(s, s)^2) with a regular context x and an exceptional context y (only `checkpoint` copies x into y); on ☇ or ∞ the result collapses to x, so everything done before the fault is kept; a 32-octet return blob becomes the yield hash  
   GP 明寫 entry point 5 且參數不接 E(i)；例外時若取 x，checkpoint 就完全沒有意義。
3. ❌ If the service's code is unavailable or > W_C it returns the state with incoming transfer amounts already credited and no other effect; otherwise it runs Ψ_M(code, entry 0, g, E(t, s, |i|), F, I(s, s)^2) with a regular context x and an exceptional context y (only `checkpoint` copies x into y); on ☇ or ∞ the result collapses to y; the deferred transfers among i are instead delivered by a separate Ψ_T on-transfer invocation  
   entry point 應為 5，且 0.8.0 只有三種 invocation，deferred transfer 直接混在 i 裡當 input。
4. ❌ If the service's code is unavailable or > W_C it halts with the work error BAD or BIG exactly as Ψ_I and Ψ_R do; otherwise the guarantors run it in-core as Ψ_M(code, entry 5, g, E(c, i, s, payload, H(p)), F, I(s, s)^2) and the block author merely records the resulting hash; a regular context x and an exceptional context y are kept (only `checkpoint` copies x into y) and on ☇ or ∞ the result collapses to y  
   BAD/BIG 是 Ψ_I/Ψ_R 的 workerror，E(c, i, s, payload, H(p)) 也是 refine 的參數。

> **陷阱**　transfer 的金額在執行**前**就加到餘額（s = e except balance += Σ x_a）。

<sub>`appB-accumulate-invocation`</sub>

---

### B-5　How are new service indices chosen by the `new` host call in GP 0.8.0?

<sub>B.4 Accumulate Invocation — ●●● · 概念 · eq. B.10, B.14 & `new` (§B.7)</sub>

**標準答案**　The context's next-free id starts at check(decode_4(H(E(s, η′_0, H_T))) mod (2^32 − S − 2^8) + S) with S = 2^16; check() linearly probes (+1 wrapping inside [S, 2^32 − 2^8)) until an index not in keys(δ); after each `new` the next candidate is check(S + (i − S + 42) mod (2^32 − S − 2^8)); only the registrar may instead request a specific index < S, which returns FULL if that index already exists

eq. B.10：i_nextfree = check((decode_4(H(E(s, η′_0, H_T))) mod (2^32 − S − 2^8)) + S)；eq. B.14：check(i) = i 若 i ∉ keys(δ)，否則 check((i − S + 1) mod (2^32 − 2^8 − S) + S)。`new`（index 19）：registrar 且 desired id < S → 直接用該 id（若已存在 → FULL）；否則用 nextfreeid 並把 nextfreeid 更新為 check(S + (i − S + 42) mod (2^32 − S − 2^8))。§B.4 的設計目的：「no service can predict the identifier sequence ahead of time, they cannot intentionally disadvantage the block author」——正因為種子綁 η′_0，service 無法預先卡位。另外（本題選項未涵蓋，但同屬 Ω_N）：f ≠ 0 且非 manager → HUH；扣掉新帳戶的 a_t 後餘額低於自身 threshold → CASH。新帳戶：storage 空、requests = {(c, l) → []}、balance = 其 threshold、created = t、lastacc = 0、parent = 建立者。

**逐項辨析**

1. ✅ The context's next-free id starts at check(decode_4(H(E(s, η′_0, H_T))) mod (2^32 − S − 2^8) + S) with S = 2^16; check() linearly probes (+1 wrapping inside [S, 2^32 − 2^8)) until an index not in keys(δ); after each `new` the next candidate is check(S + (i − S + 42) mod (2^32 − S − 2^8)); only the registrar may instead request a specific index < S, which returns FULL if that index already exists  
   種子綁 posterior η′_0、每次配置跳 42 讓序列不可預測，撞號回 FULL（resource already allocated）。
2. ❌ The context's next-free id starts at check(decode_4(H(E(s, η′_0, H_T))) mod (2^32 − S − 2^8) + S) with S = 2^16; check() linearly probes (+1 wrapping inside [S, 2^32 − 2^8)) until an index not in keys(δ); after each `new` the next candidate is simply check(i + 1); only the registrar may instead request a specific index < S, which returns WHO if that index already exists  
   跳 42 是刻意讓連續配置不相鄰；撞號的語意是 FULL，WHO 指的是 index unknown。
3. ❌ The context's next-free id starts at check(decode_4(H(E(s, η_0, H_T))) mod (2^32 − S − 2^8) + S) with S = 2^8; check() linearly probes (+1 wrapping inside [S, 2^32 − 2^8)) until an index not in keys(δ); after each `new` the next candidate is check(S + (i − S + 42) mod (2^32 − S − 2^8)); only the registrar may instead request a specific index < S, which returns FULL if that index already exists  
   GP 用的是本區塊的 posterior η′_0，且 S = 2^16；2^8 是保留在 2^32 頂端的另外 256 個索引。
4. ❌ The context's next-free id starts at S itself with S = 2^16, so the sequence is fixed by the block author rather than by entropy; check() linearly probes (+1 wrapping inside [S, 2^32 − 2^8)) until an index not in keys(δ); after each `new` the next candidate is check(i + 1); only the registrar may instead request a specific index < S, which returns FULL if that index already exists  
   序列若從 S 固定推進就可預測，service 便能預先卡位，正是 §B.4 要防的事。

> **陷阱**　S = 2^16 = 65536 是 public index 下限；2^32 − 2^8 上限保留最後 256 個。

<sub>`appB-new-service-index`</sub>

---

### B-6　The `transfer` host call (φ_7 = d, φ_8 = a, φ_9 = l gas, φ_10 = o memo ptr) can fail in several ways. Which order/meaning is correct?

<sub>B.7 Accumulate Functions — transfer — ●●○ · 概念 · `transfer` = 21</sub>
...
1. ✅ Memo unreadable → panic; d ∉ keys(δ) → WHO; l < δ[d]_m (recipient's minmemogas) → LOW; balance − a < own threshold a_t → CASH; otherwise OK: the transfer is appended to the context's transfer list, the sender's balance is reduced by a, and gas g = M_T + l is charged  
   四個檢查依序是 panic/WHO/LOW/CASH，且 g = M_T + l 把預付給接收方的 gas 一起收。
2. ❌ Memo unreadable → panic; d ∉ keys(δ) → WHO; l < δ[d]_m (recipient's minmemogas) → CASH; balance − a < own threshold a_t → LOW; otherwise OK: the transfer is appended to the context's transfer list, the sender's balance is reduced by a, and gas g = M_T + l is charged  
   語義反了：LOW 對應的是 l 不足（gas limit too low），CASH 對應餘額跌破 threshold。
3. ❌ Memo unreadable → HUH; d ∉ keys(δ) → WHO; l < the sender's own minmemogas a_m → LOW; balance − a < 0 → CASH; otherwise OK: the transfer is appended to the context's transfer list, the sender's balance is reduced by a, and gas g = M_T alone is charged  
   記憶體不可讀在 Ω_T 一律是 ☇；比的是接收方的 d[d]_m，門檻是自身 a_t 而不是 0。
4. ❌ Memo unreadable → panic; d ∉ keys(δ) → WHO; l < δ[d]_m (recipient's minmemogas) → LOW; balance − a < own threshold a_t → CASH; otherwise OK: a is moved into δ[d]'s balance inside this host call and the recipient is entered at once with the 128-octet memo, gas g = M_T + l being charged  
   transfer 是 deferred 的：只附加到 x_t，接收方下一輪才被當成 accumulate input 叫起來。

> **陷阱**　LOW 對照的是**接收方**的 a_m，不是發送方。

<sub>`appB-transfer-rules`</sub>

---

### B-7　Given request status l = a_l[(h, z)] and current slot t, which transitions do `solicit` and `forget` perform?

<sub>B.7 — solicit / forget / eject — ●●● · 概念 · `solicit` = 24, `forget` = 25, `eject` = 22</sub>

**標準答案**　solicit: no entry → [] (FULL if the new footprint is unaffordable); [x, y] → [x, y, t]; anything else → HUH. forget: [] or [x, y] with y < t − D → delete request and preimage; [x] → [x, t]; [x, y, w] with y < t − D → [w, t]; anything else → HUH
...
1. ✅ solicit: no entry → [] (FULL if the new footprint is unaffordable); [x, y] → [x, y, t]; anything else → HUH. forget: [] or [x, y] with y < t − D → delete request and preimage; [x] → [x, t]; [x, y, w] with y < t − D → [w, t]; anything else → HUH  
   兩者正是 preimage lattice 的兩個方向：過期判定看的是 y，付不起新 footprint 回 FULL。
2. ❌ solicit: no entry → [] (CASH if the new footprint is unaffordable); [x, y] → [x, y, t]; anything else → HUH. forget: [] or [x, y] with y < t − D → delete request and preimage; [x] → [] (unavailable at once); [x, y, w] with y < t − D → [w, t]; anything else → HUH  
   付不起 footprint 在 GP 裡是 FULL 而非 CASH；[x] → [x, t] 保留了 historical lookup 所需的可用區間。
3. ❌ solicit: no entry → [t]; [x, y] → [x, y, t]; anything else → HUH. forget: [] or [x, y] with y < t − D → delete request and preimage; [x] → [x, t]; [x, y, w] with w < t − D → [w, t]; anything else → HUH. Either call returns FULL when the account cannot afford its new threshold  
   新 request 一定是空序列 []；三元素的過期判定看中間的 y，而且 forget 從不回 FULL。
4. ❌ solicit and forget are refine host calls managing D3L segments: solicit reserves the next export index (FULL once W_X is spent), forget releases one, both HUH on a bad index; the request lattice [] → [x] → [x, y] → [x, y, w] is driven purely by the preimages extrinsic E_P  
   eq. B.6 的 refine 表裡沒有 solicit/forget，D3L segment 是 export（index 8）在管。

> **陷阱**　刪除必須等 D 個 slot（32h）——確保 refine 的 historical lookup 仍可判定。

<sub>`appB-solicit-forget`</sub>

---

### B-8　Which privilege checks do bless, assign and designate perform in GP 0.8.0?
...
### B-9　The `fetch` host call (index 2) is available in all three invocations. Which statement is correct?

<sub>B.5 General Functions — fetch — ●●○ · 概念 · `fetch` = 2</sub>

**標準答案**　φ_10 selects the datum: 0 = the encoded protocol constants, 1 = η′_0 (in accumulate), 2 = authorizer trace, 3–6 = extrinsics/imports, 7–13 = package, config, token, context, work-item summaries and payload, 14–15 = accumulate inputs — availability depends on the invocation; the datum is copied into memory at φ_7 with offset φ_8 and length φ_9, and φ′_7 = the datum's full length (or NONE if the selector is unavailable)

Ω_Y（fetch, 2）是 0.6.0 起取代 import 的「super-fetch」（0.6.7）：selector（φ_10）0 = protocol constants 的編碼（B_I, B_L, B_S, C, D, E, G_A, G_I, G_R, G_T, H, I, J, K, L, O, P, Q, R, T, U, W_A, W_B, W_C, W_M, W_R, W_T, W_X, Y，JIP-4 chainspec 的 protocol_parameters 用同樣格式），1 = η′_0（accumulate；refine 傳入的是零 hash），2 = authorizer trace，3 = extrinsic [φ_11][φ_12] / 4 = 本 item 的 extrinsic [φ_11]，5 = import segment [φ_11][φ_12] / 6 = 本 item 的 import [φ_11]，7 = 整個 package 編碼，8 = auth config，9 = auth token，10 = context，11 = 所有 work-item 摘要 / 12 = item φ_11 的摘要，13 = item φ_11 的 payload，14 = 所有 accumulate inputs / 15 = input φ_11；不可用時回 NONE。輸出採「寫入記憶體視窗、回傳完整長度」模式（與 lookup/read/info/historical_lookup 相同）：[o, f_0, z] = φ_7…φ_9，f = min(f_0, |v|)、l = min(z, |v| − f)，回傳 |v|。把資料改由 invocation argument blob 傳入會讓 §B 開頭的「avoid introducing unbounded data as part of the basic invocation arguments」失效。你們 code-map 3.12.4 有 getFetchConstantsData 的欄位順序（與 JIP-4 chainspec 的 protocol_parameters 同一套編碼）。

**逐項辨析**

1. ✅ φ_10 selects the datum: 0 = the encoded protocol constants, 1 = η′_0 (in accumulate), 2 = authorizer trace, 3–6 = extrinsics/imports, 7–13 = package, config, token, context, work-item summaries and payload, 14–15 = accumulate inputs — availability depends on the invocation; the datum is copied into memory at φ_7 with offset φ_8 and length φ_9, and φ′_7 = the datum's full length (or NONE if the selector is unavailable)  
   採「寫入記憶體視窗、回傳完整長度 |v|」模式，讓呼叫者知道要配置多大緩衝區。
2. ❌ φ_10 selects the datum, but the whole table is refine-only: 0 = the encoded work-package, 1 = the authorizer trace, 2 = η′_0, 3–6 = extrinsics/imports, 7–13 = config, token, context, item summaries and payload, 14–15 = accumulate inputs; is-authorized and accumulate get theirs from the argument blob; the datum is copied into memory at φ_7 with offset φ_8 and length φ_9, and φ′_7 = the datum's full length (or NONE if the selector is unavailable)  
   三張 mutator 表都列出 Ω_Y：is-authorized 靠它讀 0 與 7–13，accumulate 靠 14/15 讀 operands。
3. ❌ φ_10 selects the datum: 0 = the encoded protocol constants, 1 = η′_0 (in accumulate), 2 = authorizer trace, 3–6 = extrinsics/imports, 7–13 = package, config, token, context, work-item summaries and payload, 14–15 = accumulate inputs — availability depends on the invocation; the datum is copied into memory at φ_7 with length φ_8 and offset φ_9, and φ′_7 = the number of octets actually written (or 0 if the selector is unavailable)  
   暫存器約定是先 offset（φ_8）後長度（φ_9）；回 0 會與「長度為 0 的合法 datum」混淆。
4. ❌ φ_10 selects the datum and the queue-like selectors are destructive: 14 = the next pending accumulate operand and 15 = the next deferred transfer, each removed from the input sequence once read; 0 = protocol constants, 1 = η′_0, 2 = authorizer trace, 3–13 = extrinsics, imports and package metadata; the datum is copied into memory at φ_7 with offset φ_8 and length φ_9, and φ′_7 = the datum's full length (or NONE if the selector is unavailable)  
   Ω_Y 的 mutation 只有 (ε′, φ′_7, μ′)、不動 context，14/15 可以重複讀而不消耗。

> **陷阱**　accumulate 的 inputs（operands/transfers）不是透過參數傳入，而是 fetch 14/15。

<sub>`appB-fetch`</sub>

---

### B-10　What do `checkpoint`, `yield` and `provide` do inside accumulate?
...
1. ✅ checkpoint: copies the regular context x into the exceptional context y (so a later panic/OOG commits the checkpointed state) and returns remaining gas in φ_7; yield(o): records the 32-octet hash at o as the service's accumulation output (θ entry / BEEFY-visible commitment); provide(s, o, z): supplies preimage bytes for a request [] of service s (or the caller if φ_7 = 2^64−1), returning WHO/HUH on errors, integrated after the round  
   checkpoint 是唯一會改 y 的 host call，因此 ☇/∞ 時 collapse 才能 commit 到存檔點。
2. ❌ checkpoint: copies the exceptional context y into the regular context x (so a later panic/OOG rewinds to the start of the accumulation) and returns the gas already used in φ_7; yield(o): records the 32-octet hash at o as the service's accumulation output (θ entry / BEEFY-visible commitment); provide(s, o, z): supplies preimage bytes for a request [] of service s (or the caller if φ_7 = 2^64−1), returning WHO/HUH on errors, integrated after the round  
   方向相反：存檔是把當下的 x 複製進 y，反過來會把先前的進度倒回去，回傳的也是剩餘 gas。
3. ❌ checkpoint: copies the regular context x into the exceptional context y (so a later panic/OOG commits the checkpointed state) and returns remaining gas in φ_7; yield(o): halts the invocation there and then, committing x and forfeiting the unspent gas, with the 32 octets at o as the output; provide(s, o, z): writes the preimage straight into δ[s]_p and moves a_l[(H(i), z)] to [t] inside this same host call  
   Ω_Taurus 只設 x_y 一個欄位（停機得靠 halt 指令），provide 也只加進 x_provisions 等 Δ* 整合。
4. ❌ All three belong to the refine invocation: checkpoint snapshots the inner-PVM map m so that a later panic can restore it and returns remaining gas in φ_7; yield(o) appends the 32 octets at o to the export sequence e; provide(s, o, z) hands preimage bytes to the guarantor's local cache, returning WHO/HUH on errors; accumulate reaches the same effects through write, export and transfer instead  
   eq. B.11 才有這三個；refine 的 export 附加的是 4,104 octet segment，不是 32-octet hash。

> **陷阱**　yield 的 hash 進 θ′ → β′_B belt → BEEFY；這是 service 對外（bridge）承諾狀態的管道。

<sub>`appB-checkpoint-yield`</sub>

---

### B-11　How is host-call gas charged in GP 0.8.0 (PR #517), compared with 0.7.2?

<sub>B.5 General Functions — ●●○ · 版本差異 · eq. B.17 (memgas) & Appendix I host-function gas costs · ⚠ 0.7.2→0.8.0</sub>

**標準答案**　Each host call has a base cost M_□ (e.g. gas 48, checkpoint 103, transfer 575 + l, new 3855, export 3521) plus, where applicable, a per-KiB term ⌈L·ℓ/1024⌉ (e.g. lookup 600 + 248/KiB, write 2442 + key 3358/KiB + value 216/KiB) or a per-page term; if ϱ < g the call exits ∞ before doing anything; unknown calls cost M_∅ = 1000

0.7.2：每個 host call 固定 10 gas（transfer 10 + φ_9，log 0，jamtestvectors 用的也是此模型）。0.8.0 附錄 I「Host-function gas costs」：M_G(gas)=48、M_♊(grow_heap)=275 + 121/page、M_Y(fetch)=依 selector 而定（constants 390、entropy 103、無效 selector 80）、M_L(lookup)=600 + 248/KiB、M_R(read)=2407 + key 1736/KiB + value 248/KiB、M_W(write)=2442 + key 3358/KiB + value 216/KiB、M_I(info)=703、M_H(historical_lookup)=1125 + 264/KiB、M_E(export)=3521、M_M(machine)=1862 + 112/KiB、M_P(peek)=377+336/KiB、M_O(poke)=297+224/KiB、M_K(invoke)=968、M_X(expunge)=335、M_B(bless)=422 + 20/item、M_A(assign)=1818、M_D(designate)=1100 + 302/validator、M_C(checkpoint)=103、M_N(new)=3855、M_U(upgrade)=1028、M_T(transfer)=575 + l、M_J(eject)=458、M_Q(query)=643、M_S(solicit)=2193、M_F(forget)=3250、M_∅=1000。M_Taurus(yield)=98、M_Aries(provide)=3980 + 2264/KiB；`pages` 還依 r 分三檔：free 212 + 118/頁、alloc 275 + 121/頁、setmode 130 + 29/頁、無效 r 固定 80。eq. B.17：memgas(L, ℓ) = ⌈L·ℓ/1024⌉。§B.5：ϱ < g → 直接 ∞，不做任何事。未知 host call 一次就能燒掉 M_∅ = 1000 gas，這正是 accumulate 誤呼叫 refine-only host call 時會 ∞ 的原因。

**逐項辨析**

1. ✅ Each host call has a base cost M_□ (e.g. gas 48, checkpoint 103, transfer 575 + l, new 3855, export 3521) plus, where applicable, a per-KiB term ⌈L·ℓ/1024⌉ (e.g. lookup 600 + 248/KiB, write 2442 + key 3358/KiB + value 216/KiB) or a per-page term; if ϱ < g the call exits ∞ before doing anything; unknown calls cost M_∅ = 1000  
   模型是 base + per-KiB/per-page（eq. B.17），且 §B.5 要求動作前先檢查 gas 夠不夠。
2. ❌ Each host call has a base cost M_□ (e.g. gas 48, checkpoint 103, transfer 575 + l, new 3855, export 3521) plus, where applicable, a per-KiB term ⌈L·ℓ/1024⌉ (e.g. lookup 600 + 248/KiB, write 2442 + key 3358/KiB + value 216/KiB) or a per-page term; if ϱ < g the call still runs and the counter simply goes negative; unknown calls cost M_∅ = 10 as in 0.7.2  
   §B.5 明定 ϱ < g 直接 ∞、什麼都不做；M_∅ 在 0.8.0 也已從 10 提高到 1000。
3. ❌ Each host call has one base cost M_□ and nothing else (e.g. gas 48, checkpoint 103, transfer 575, new 3855, export 3521): 0.8.0 merely replaced the flat 10 with per-call constants, so lookup, read and write cost the same whatever the key and value lengths; if ϱ < g the call exits ∞ before doing anything; unknown calls cost M_∅ = 1000  
   漏掉 eq. B.17 的 memory-sized gas，否則一次 write 塞進 MB 級 value 卻只付 base cost。
4. ❌ Each host call has a base cost M_□ plus, where applicable, a per-KiB term ⌈L·ℓ/1024⌉ or a per-page term, but the base costs are still 0.7.2's flat 10 (gas 10, checkpoint 10, transfer 10 + l, new 10, export 10) and only lookup (248/KiB) gained a size term; if ϱ < g the call exits ∞ before doing anything; unknown calls cost M_∅ = 10  
   版本沒跟上：附錄 I 對每一個 host call 都給了獨立 base 常數，不再是 flat 10。

> **陷阱**　面試不會要你背全部數字，但要知道模型：base + per-KiB/per-page，且先檢查 gas。

<sub>`appB-host-gas-costs`</sub>

---

### B-12　The excerpt is the team's Ω_W (`write`) after fix #980. A service overwrites a key that currently holds a 100-octet value with a 5,000-octet value (φ_7…φ_10 = k_O, k_Z, v_O, v_Z). Which statement matches GP 0.8.0?
...
```
<sub>PVM/host_call_general.go (write, after PR #980)</sub>

**標準答案**　If the post-write a_t ≤ a_b the value is stored and φ′_7 = 100, the length of the value being replaced; if a_t > a_b, φ′_7 = FULL and the account is handed back unchanged (s′ = s) — the pre-fix code had already mutated the shared Go map, so the FULL write leaked (#979)

Ω_W（write = 5）定義：[k_O, k_Z, v_O, v_Z] = φ_7…φ_10；k = μ[k_O..+k_Z]（不可讀 → error）；a = s 但 a_storage[k] = μ[v_O..+v_Z]（v_Z = 0 時改為刪除 k；value 範圍不可讀 → error）；l = |s_storage[k]| 若 k ∈ keys(s_storage)，否則 NONE；(ε′, φ′_7, s′) = (☇, φ_7, s) 當 k = error ∨ a = error；(▸, FULL, s) 當 a_minbalance > a_balance；(▸, l, a) 否則。所以成功時回傳的是「被取代的舊值長度」，FULL 時 s′ = s 一字不改。a_t（eq. 9.8）= max(0, B_S + B_I·a_i + B_L·a_o − a_f)，而 a_o 含每筆 storage 的 34 + |k| + |v|，所以把 100 octets 換成 5,000 octets 會拉高 a_t。你們的 bug #979（fuzz seed 3785638964 step 15419）：write 先改了 StorageDict 再檢查門檻，Go map 淺複製共用底層 map，FULL 的寫入仍被持久化 → state root mismatch；PR #980 改成先算 newMinBalance 再改 map（即上面的順序）。另外 accumulate 中 write 經 G（eq. B.12）包裝：x*_self = 回傳的 s′，所以同一次 accumulate 之後的 read/info 看得到新值。

**逐項辨析**

1. ❌ On success φ′_7 = 5000, the length of the value just written; if a_t > a_b the value is still stored, φ′_7 = FULL and the shortfall is debited from the balance at the service's next accumulation — so all the fix had to change was the register value, not the order of the map mutation  
   回傳的是被取代的舊值長度而不是新長度；GP 的 FULL 分支回的是原封不動的 s，沒有欠款概念。
2. ✅ If the post-write a_t ≤ a_b the value is stored and φ′_7 = 100, the length of the value being replaced; if a_t > a_b, φ′_7 = FULL and the account is handed back unchanged (s′ = s) — the pre-fix code had already mutated the shared Go map, so the FULL write leaked (#979)  
   成功回舊值長度 100（key 原本不存在才回 NONE），FULL 時 s′ = s——所以必須先算門檻再改 map。
3. ❌ On success φ′_7 = OK (0); a_t > a_b cannot occur for an overwrite because the storage deposit (B_I per item, B_L per octet) is charged only when a key is first created, never when its value changes size, so Ω_W needs no threshold check on this path  
   a_t 隨 a_o 變動（每筆 storage 計 34 + |k| + |v|），改變 value 大小就會拉高門檻。
4. ❌ If a_t > a_b the host call exits ☇ (panic) so that the whole accumulation collapses to the checkpointed context y; otherwise φ′_7 = 100, the length of the value being replaced, and the value is stored — the collapse is exactly what made the pre-fix map mutation harmless  
   ☇ 只保留給記憶體錯誤；餘額不足是可續行的 FULL，context 不會被丟棄，污染會一路寫進 δ′。

> **陷阱**　write 回傳「舊長度或 NONE」，不回傳 OK；v_Z = 0 是刪除；FULL 時必須零副作用。

<sub>`b2-appB-write-prev-length-full`</sub>

---

### B-13　Excerpt from the team's Ω_R (`read`) after fix #938, which added the `callerServiceID == serviceID` guard (s* = φ_7, or the caller when φ_7 = 2^64−1). Accumulating service A reads a storage key of service B, which is not accumulating in this block. Which statement is correct?
...
```
<sub>PVM/host_call_general.go (read, after PR #938)</sub>

**標準答案**　Ω_R(ϱ, ω, μ, s, s, d) may look up any account in d but hands back only the caller's account s (mirrored into x by G); a read of B is a pure lookup that must leave B untouched — before the guard, B's key-val was evicted from the unmatched pool, B's entry vanished from the merged δ′ and the state root diverged

Ω_R（read = 4）：s* = s 若 φ_7 = 2^64−1，否則 φ_7；a = s（呼叫者自己，含本次已 write 的內容）若 s* = s；d[s*] 若 s* ∈ keys(d)；否則 none。[k_O, k_Z, o] = φ_8…φ_10、v_Z = φ_12、f = min(φ_11, |v|)、l = min(v_Z, |v| − f)；v = error（key 範圍不可讀）→ ☇；a = none 或 k ∉ keys(a_storage) → NONE（不是 WHO——WHO 是 transfer/eject/provide 等找不到 service 時用的）；否則寫 v[f..f+l] 到 μ[o..+l] 並回傳 |v|。read 的 mutation 只有 (ε′, φ′_7, μ′)，s′ = s；在 accumulate 的 F（eq. B.11）中 read 被 G 包起來，G 只把「呼叫者自己的帳戶」寫回 x_self（eq. B.12），所以跨 service 的讀取在 GP 裡是純函數，不可能改動 B。d 是 (x_u)_d，即 Ψ_A 拿到的 partial state 快照（Δ1 對同一輪的每個 service 傳同一份 e，§12 平行累加），因此同一輪其他 service 的寫入本來就看不到。你們的 #938（bug id 1776702160_6218）：A 讀 B 的 key 時，把 B 的 entry 從 UnmatchedKeyVals cache 移除當成最佳化，但 B 這個 block 沒有 accumulate，其本地帳戶副本從未寫回 d′，而 #880 的 merge 採交集語意（任何輸出缺少的 key 視為被刪除）→ B 的 storage 全局消失、state root mismatch；修法就是只在 callerServiceID == serviceID 時才快取並移除。

**逐項辨析**

1. ✅ Ω_R(ϱ, ω, μ, s, s, d) may look up any account in d but hands back only the caller's account s (mirrored into x by G); a read of B is a pure lookup that must leave B untouched — before the guard, B's key-val was evicted from the unmatched pool, B's entry vanished from the merged δ′ and the state root diverged  
   跨 service 讀取在 GP 裡是純函數：G 只把呼叫者自己的帳戶寫回 x_self，B 不可能被改動。
2. ❌ Reading another service's storage is not permitted by Ω_R: for s* ≠ s the call must return WHO without touching memory or the key-val pool, so in a compliant implementation the branch that reaches B's account is unreachable and the guard is merely defensive — cross-service inspection is what `info` and `historical_lookup` are for  
   d（partial state 的 accounts）正是為了讓 s* ≠ s 能查而傳進來的，而且查不到時回的是 NONE。
3. ❌ The read must observe the posterior state being built in this block — including writes by services accumulated earlier in the same round — so the cached value has to be refreshed from the accumulation context on every read rather than removed from the pool, and the guard is itself a bug because it suppresses that refresh for every s* ≠ s  
   d 是 Ψ_A 拿到的 partial state 快照，同一輪其他 service 的寫入本來就看不到，refresh 也救不了。
4. ❌ The read must be served from the lookup-anchor snapshot exactly as `historical_lookup` does, so mutating the current-state cache is only a performance concern and can never change δ′; the guard merely avoids decoding B's key-vals twice, and the state-root divergence came from the merge rule alone  
   historical_lookup 只存在於 refine；且因果顛倒——交集式 merge 是既定設計，缺陷在純函數留下副作用。

> **陷阱**　read/lookup/info 找不到回 NONE（2^64−1）；跨 service 讀取是唯讀快照，只有自己的帳戶會經 G 寫回 x。

<sub>`b2-appB-read-cross-service-pure`</sub>

---

### B-14　During accumulation a service executes `ecalli 9` (`machine`, a refine-only call) with ϱ = 300 gas remaining. The team's dispatcher routes ids that are not in the invocation's table to hostCallException (below, after PR #992). What does GP 0.8.0 prescribe for this situation?
...
### B-15　The excerpt is the team's 0.7.2 `invoke` (Ω_K): it reads a 112-octet block at φ_8, runs inner machine n = φ_7 and writes the block back. Apart from the id shift (12 → 13), which GP 0.8.0 rule is missing from it?

<sub>B.6 Refine Functions — invoke — ●●● · 程式碼 · §B.6 `invoke` = 13 (Ω_K); eq. B.4 (inner PVM tuple); §B.1 inner result codes · ⚠ 0.7.2→0.8.0</sub>
...
```
<sub>PVM/host_call_refine.go (invoke, 0.7.2 numbering)</sub>

**標準答案**　Gas: the outer machine pays g = M_K + g_R up front (∞ if ϱ < g; g_R is the inner gas read from the block) and afterwards gets the inner's unspent g_R′ back, ϱ′ = ϱ − g + g_R′; the inner machine also carries a gas-charged flag for its current basic block

Ω_K（invoke = 13）：[n, o] = φ_7,8；⟨g_R, w⟩ 滿足 E_8(g_R) ⌢ E_8(w) = μ[o..+112]（8 octets gas + 13 個 8-octet 暫存器），且 N_{o..+112} 必須 ⊆ writable(μ)，否則 ⟨0, error⟩ → ☇（因為結果要寫回同一塊）；(c, i′, g_R′, f′, w′, u′) = Ψ(m[n]_p, m[n]_i, g_R, m[n]_f, w, m[n]_u)——是 Ψ 不是 Ψ_H，inner 沒有 host call，遇到 ecalli 就以 c = h̄ × h 停下；μ* 寫回 E_8(g_R′) ⌢ E_8(w′)；m*[n]_i = i′ + skip(i′) + 1 當 c ∈ {h̄} × N_R（跳過那條 ecalli，讓外層 service 代為服務後 resume），否則 i′；m*[n]_f = f′（eq. B.4 的 inner PVM tuple ⟨p, u, i, f⟩，f 是「本 basic block 的 gas 已扣」旗標，因為 0.8.0 gas 以 basic block 為單位預扣，mid-block resume 時不能再扣一次——#1046 review 也提到 A.4 的 L(i)）。gas：g = M_K + g_R（M_K = 968）；ϱ′ = ϱ − g 當 w = error ∨ n ∉ keys(m) ∨ ϱ < g，否則 ϱ − g + g_R′——也就是外層先付全部 inner gas，剩下的退回；ϱ < g 依 B.18/B.19 直接 ∞。回傳：(HOST, h)、(FAULT, x 位址)、OOG、PANIC、HALT 進 φ′_7/φ′_8（非 HOST/FAULT 時 φ′_8 不變）；n 不存在 → WHO。你們的 0.7.2 版本只扣 10 gas，inner 的 g 完全獨立、也不退還——這正是 #1046 的「invoke gas refund」項目；同一 PR 也加了 machine 的 63 台上限（GP PR #521，|m| ≥ 63 → FULL，且排在 ☇ 之前）。另外值得複查：excerpt 的 skip 用的是 input.Addition.Program.Bitmasks（外層程式的 bitmask），GP 的 skip(i′) 應以 inner 程式 m[n]_p 的 bitmask 計算。

**逐項辨析**

1. ❌ Nested host calls: an `ecalli` executed by the inner machine must be dispatched through the refine mutator F (historical_lookup, export, …) instead of stopping the inner run, so Ω_K has to invoke Ψ_H rather than Ψ and the outer service should never see the HOST result code at all  
   inner PVM 刻意沒有 host call 能力，Ω_K 明寫呼叫 Ψ，ecalli 一律以 (HOST, h) 停機交外層處理。
2. ❌ Resumption: on a HOST exit the saved instruction counter must stay on the `ecalli` itself, so that the outer service can re-execute that instruction after servicing the call, exactly as Ψ does for a page fault; advancing it by i′ + skip(i′) + 1 drops an instruction  
   m*[n]_i = i′ + skip(i′) + 1 正是要跳過那條 ecalli；停在原地會讓 resume 後無限重複同一個 ecalli。
3. ❌ Memory: the 112-octet block need only be readable, because in 0.8.0 the inner gas and the 13 registers are handed back through φ′_7…φ′_12 rather than written back into the block, so a readable-but-unwritable window is fine and the excerpt's OOB-and-panic path should be a plain WHO instead  
   GP 要求 N_{o..+112} ⊆ writable(μ)，因為 g_R′ 與 w′ 要原地寫回同一塊，不可寫就 ⟨0, error⟩ → ☇。
4. ✅ Gas: the outer machine pays g = M_K + g_R up front (∞ if ϱ < g; g_R is the inner gas read from the block) and afterwards gets the inner's unspent g_R′ back, ϱ′ = ϱ − g + g_R′; the inner machine also carries a gas-charged flag for its current basic block  
   0.7.2 只扣 10 且 inner gas 完全獨立；0.8.0 要外層預付 M_K + g_R 並退還未用完的 g_R′。

> **陷阱**　invoke 的 112 octets 必須「可寫」；inner 無 host call（ecalli → HOST）；0.8.0 外層預付 g_R 並退還 g_R′。

<sub>`b2-appB-invoke-gas-refund`</sub>

---

### B-16　The excerpt is the team's `pages` (Ω_Z) acting on inner machine n over page range [p, p+c) with mode r (φ_7…φ_10 = n, p, c, r). Compared with GP 0.8.0, which statement is correct?
...
```
<sub>PVM/host_call_refine.go (pages, 0.7.2 numbering)</sub>
...
1. ❌ The code is correct: Ω_Z zero-fills the range for every r ∈ 0…4 and only the resulting access differs (0 → inaccessible, 1/3 → R, 2/4 → W); the GP never distinguishes an allocation from a mode change, which is why App. I prices every r with one base cost and one per-page rate  
   u′_value 只在 r < 3 時被清成 0；附錄 I 也給了 free/alloc/setmode 三組不同費率而非一組。
2. ✅ Two semantic gaps: for r = 0 the GP zero-fills the range and makes it inaccessible (the code leaves it untouched), and for r ∈ {3, 4} the GP keeps the page contents and only changes the access mode to R or W (the code re-allocates zeroed pages instead)  
   r = 0 是「歸零 + 設為不可存取」，r ∈ {3, 4} 保留內容只改權限——實作恰好兩邊都做反了。
3. ❌ The only gap is the error code: when r > 2 and a page of the range is inaccessible the GP returns OOB (inner-PVM memory index not accessible) exactly as `peek` and `poke` do, rather than HUH; the page mutations themselves are right, and r = 0 rightly leaves the octets alone  
   Ω_Z 的結果只有 WHO/HUH/OK，OOB 專屬於 peek/poke 對 inner memory 的位元組範圍存取。
4. ❌ The GP validates r ∈ 0…4, p ≥ 16 and p + c < 2^32/Z_P before looking up n, so an invalid request aimed at a non-existent machine must yield HUH where the code yields WHO; the page mutations themselves are right, and an invalid r is free, no gas constant being defined for it  
   u = error（n ∉ keys(m)）是第一個 case 且回 WHO；無效 r 也不免費，M_Z,i = 80。

> **陷阱**　pages 的 r：0 釋放、1/2 配置（歸零）、3/4 只改權限（保留內容）；錯誤碼是 WHO/HUH，沒有 OOB、沒有 panic。

<sub>`b2-appB-pages-access-modes`</sub>

---

### B-17　GP 0.8.0 replaces the `sbrk` instruction with the `grow_heap` host call (index 1, present in all three invocations). A program with no RO data (so a = 32, the first RW-data page) currently has c = 8 writable RW pages (h = 40) and b = 1000; it calls grow_heap with φ_7 = 50 while ϱ = 2,000. What happens?

<sub>B.5 General Functions — grow_heap — ●●● · 版本差異 · §B.5 `grow_heap` = 1 (Ω_♊); eq. A.42 (Y conditions); M_♊,c / M_♊,p in App. I · ⚠ 0.7.2→0.8.0</sub>

**標準答案**　φ_7 is the desired end page (index + 1 of the last writable page): g = 275 + (50 − 40)·121 = 1,485 is charged, pages [40, 50) become writable and φ′_7 = 50; with φ_7 ≤ 40 or φ_7 > 1000 only 275 would be charged, φ′_7 = 40 and nothing would change

Ω_♊（grow_heap = 1）：(o, s) 由 JAM blob p 依 eq. A.42 解出（RO data 與 stack 大小）；a = (2Z_Z + Z(|o|))/Z_P 是 RW data 區的起始頁（無 RO data 時 = 2·65536/4096 = 32）；b = (2^32 − 3Z_Z − Z_I − P(s))/Z_P 是 RW 區可及的最後一頁 + 1（stack 前留一個 Z_Z guard zone）；c = |{p ∈ [a, b) : access[p] = W}| 是目前可寫頁數；h = a + c 是目前的 heap 指標（頁）。(ε′, ϱ′, φ′_7) = (▸, ϱ − M_♊,c, h) 當 φ_7 ≤ h ∨ φ_7 > b（不動、只收基本費、回傳現況）；(∞, ϱ, h) 當 ϱ < g；(▸, ϱ − g, φ_7) 否則；且 access[a..φ′_7) = [W…]；g = M_♊,c + (φ_7 − h)·M_♊,p，M_♊,c = 275、M_♊,p = 121。代入：h = 32 + 8 = 40，φ_7 = 50 → 新增 10 頁 → g = 275 + 1,210 = 1,485 ≤ 2,000 → 頁 [40, 50) 變 W、φ′_7 = 50。對照 0.7.2：sbrk 是 PVM 指令，參數是 octet 數、回傳舊 heap 指標（位址）；0.8.0（GP PR #508）把它拿掉並改成 host call，因此 host-call 編號全部 +1。你們的 #1046 實作了此項；review 註記「HeapMaxPages must reserve the major guard zone」對應上面 b 的定義——stack 區起點是 2^32 − 2Z_Z − Z_I − P(s)（eq. A.46），b 再往下保留一個 Z_Z。小心：ecosystem-notes/code-map 有把 1862 誤標成 grow_heap 的基本費，1862 是 machine（M_M,c）。

**逐項辨析**

1. ✅ φ_7 is the desired end page (index + 1 of the last writable page): g = 275 + (50 − 40)·121 = 1,485 is charged, pages [40, 50) become writable and φ′_7 = 50; with φ_7 ≤ 40 or φ_7 > 1000 only 275 would be charged, φ′_7 = 40 and nothing would change  
   φ_7 是絕對的目標頁索引（+1）：g = 275 + (50 − 40)·121 = 1,485 ≤ 2,000，故成功並回傳 50。
2. ❌ φ_7 is a page count to add: pages [40, 90) become writable, g = 275 + 50·121 = 6,325 is charged and φ′_7 = 90; a request that would pass b = 1000 is clamped to b rather than refused, and φ_7 = 0 is how a program queries the current heap pointer without paying per page  
   當成「要增加的頁數」會多配 40 頁、多收 4,840 gas；GP 對超出 b 的請求是 no-op 而非 clamp。
3. ❌ φ_7 is an octet count as with 0.7.2's sbrk instruction: the heap grows by 50 octets rounded up to one whole page, φ′_7 = the previous break address 40·Z_P = 163,840, and only the base cost 275 is charged since a single page is added; a request beyond b returns OOB  
   那是 0.7.2 sbrk 指令的語義；0.8.0 改成以頁為單位的 host call，Ω_♊ 也只有 ▸ 與 ∞ 兩種結果。
4. ❌ Memory growth is already priced by the per-basic-block gas model, so grow_heap only charges its base cost 275 regardless of how many pages are added; pages [40, 50) become writable and φ′_7 = 50, while a request above b is refused with FULL instead of being a no-op  
   附錄 I 的 M_♊,p = 121 就是每頁費用；basic-block gas 只涵蓋指令執行，不含記憶體成本。

> **陷阱**　grow_heap(φ_7 = 目標頁索引+1) → 回傳新的 heap 頁；≤ h 或 > b 是 no-op（仍收 275）；每新增一頁 121 gas。

<sub>`b2-appB-grow-heap`</sub>

---

### B-18　Service 7 (accumulating) calls `eject` (index 22) with φ_7 = 9 and φ_8 = o, where μ[o..+32] = h. Under GP 0.8.0, when does the call return OK, and what happens then?
...
### B-19　A service executes `ecalli 100` (`log`). Which statement is correct?
...
### C-1　Using the GP's general natural-number encoding E(x), what is E(128) and what is E(2^56)?

<sub>C.1 Common Terms (natural encoding) — ●●○ · 計算 · §C.1 general natural number serialization</sub>

**標準答案**　E(128) = [0x80, 0x80]; E(2^56) = [0xFF] ⌢ E_8(2^56) (9 octets)

§C.1：E(0) = [0]；若 ∃l ∈ N_8：2^{7l} ≤ x < 2^{7(l+1)}，E(x) = [2^8 − 2^{8−l} + ⌊x/2^{8l}⌋] ⌢ E_l(x mod 2^{8l})；否則（x ≥ 2^56）E(x) = [2^8 − 1] ⌢ E_8(x)。x = 128 = 2^7：l = 1，前綴 = 256 − 128 + ⌊128/256⌋ = 128 = 0x80，尾巴 E_1(128) = 0x80 → [0x80, 0x80]。x < 128：單一 byte（l = 0）。x = 2^56 不落在任何 l ≤ 7 的區間 → [0xFF] ⌢ 8 bytes LE = [0xFF, 0,0,0,0,0,0,0,0x01]。前綴的高 l 位是 1、接一個 0、其餘是 x 的高位——類似 UTF-8 但把長度資訊放在最高位。你們 EncodeUint/DecodeUint 完全對應（l = bits.Len64(x)−1 / 7）。

**逐項辨析**

1. ✅ E(128) = [0x80, 0x80]; E(2^56) = [0xFF] ⌢ E_8(2^56) (9 octets)  
   l = 1 的前綴 0x80 配 1 個尾 octet、x ≥ 2^56 的 [0xFF] ⌢ 8 octets，兩段都對。
2. ❌ E(128) = [0x80]; E(2^56) = 8 octets little-endian  
   單一 byte 只涵蓋 l = 0（x < 128），且 x ≥ 2^56 必須帶 [0xFF] 前綴共 9 octets。
3. ❌ E(128) = [0x81, 0x00]; E(2^56) = [0xFE] ⌢ 7 octets  
   l = 1 的前綴算出來是 0x80 不是 0x81；0xFE 是 l = 7 的前綴，只涵蓋到 x < 2^56。
4. ❌ E(128) = [0x00, 0x80]; E(2^56) = [0xFF, 0x01, 0, 0, 0, 0, 0, 0, 0]  
   前綴 0x00 代表 l = 0（即 E(0)），且尾巴是 little-endian，0x01 必須落在最後一個 octet。

> **陷阱**　JAM codec ≠ SCALE compact：SCALE 用最低 2 bits 表示模式；JAM 用最高位的 1 的個數。

<sub>`appC-compact-int`</sub>

---

### C-2　Which statement about JAM's codec conventions is correct?
...
### C-3　Where does the GP use fixed-width integer encodings E_l versus the general compact encoding E?
...
### C-4　The team's encoder for the availability specification (work-package spec) is still on GP 0.7.2. What must change so that it produces the GP 0.8.0 wire form E(s) of §C.2, and which on-chain rule accompanies the new field?

<sub>C.2 Block Serialization (availability specification) — ●●○ · 程式碼 · §C.2 E(s ∈ availability spec); eq. 11.5; eq. 11.31 · ⚠ 0.7.2→0.8.0</sub>
...
```
<sub>internal/types/encode.go (WorkPackageSpec.Encode) — struct fields: Hash, Length U32, ErasureRoot, ExportsRoot, ExportsCount U16</sub>

**標準答案**　Insert a U16 erasure-shard count v, encoded E_2(v), between ErasureRoot and ExportsRoot; the other widths stay as they are (E_4 length, E_2 exports count); on-chain every incoming report must carry v = |κ′|

GP 0.8.0 §C.2：E(s ∈ availability spec) ≡ E(p, E_4(l), u, E_2(v), e, E_2(n))——依序是 package hash p、bundle 長度 l（E_4）、erasure root u、erasure-shard 數 v（E_2）、segment root e、segment 數 n（E_2）。eq. 11.5 把 v ∈ N_V（validator count 型別）放在 u 與 e 之間；eq. 11.31：∀w ∈ 進來的 reports：(w_s)_v = |κ′|，因為「one chunk is distributed to each assurer, the number of chunks must equal the size of the assuring validator set」。0.8.0 的 erasure coding 以 v 為參數（§14.4.1 的 A(p, b, s, v) 用 z = 2·d(v)），所以規格本身要記錄 v；GP 也註明對兩個不同大小的 validator set 派發時要做兩次 erasure coding、產生兩份不同的 work-report。你們的 PR #1026 就是在 ErasureRoot 與 ExportsRoot 之間加 `ErasureShards U16`（producer 設為 TotalShards），這會改變每一個 on-chain WorkReport 的位元組佈局（進而影響 report hash、guarantee 簽名、ρ 內容）。

**逐項辨析**

1. ✅ Insert a U16 erasure-shard count v, encoded E_2(v), between ErasureRoot and ExportsRoot; the other widths stay as they are (E_4 length, E_2 exports count); on-chain every incoming report must carry v = |κ′|  
   §C.2 把 E_2(v) 排在 u 與 e 之間，且 eq. 11.31 要求 (w_s)_v = |κ′|——每個 assurer 恰一個 chunk。
2. ❌ Append E_2(v) after ExportsCount, because since GP 0.7.0 any newly added field of a structure must go to its end to keep decoders streaming-friendly; on-chain v must equal the number of cores C  
   「新欄位排最後」只針對變長（↕）欄位；v 是固定 2 octets、位置由 GP 明定，而且 v 不是 C。
3. ❌ Replace the fixed E_4 Length with a compact E(l) and add v as a compact natural right after it; v is whatever number of chunks the guarantor actually distributed, so it is not checked on-chain  
   長度 l 仍是 E_4 而非 compact，v 也不是 guarantor 自由決定的——eq. 11.31 在鏈上檢查它。
4. ❌ Nothing on the wire: v is derivable from the validator count V and is therefore never serialized; only the JSON test-vector schema gains an erasure_shards field for readability  
   v 必須進 wire，否則另一個 validator-set 大小的 assurer 無從驗證這份 report。

> **陷阱**　0.7.2 五欄位 (p, l, u, e, n) → 0.8.0 六欄位 (p, l, u, v, e, n)；v 是 E_2 且必須等於 |κ′|。

<sub>`appC-code-avspec-080`</sub>

---

### C-5　A work digest d = (s, c, y, g, l, u, i, x, z, e) is serialized inside a work report. Which statement about its encoding in GP 0.8.0 is correct?

<sub>C.2 Block Serialization (work digest and O(result)) — ●●○ · 概念 · §C.2 E(d ∈ work digest) and O(o); eq. 11.6–11.7</sub>

**標準答案**　E_4(s), c, y, E_8(g), then O(l) — [0] ⌢ ↕blob on success, else one octet 1…6 for ∞, ☇, BADEXPORTS, OVERSIZE, BAD, BIG — then u, i, x, z, e as compact naturals (digests are never read individually by the PVM, so space wins)

§C.2：E(d ∈ work digest) ≡ E(E_4(s), c, y, E_8(g), O(l), u, i, x, z, e)。前四個是固定寬度：service index E_4、code hash、payload hash、accumulate gas limit E_8；結果用 O(o)：o ∈ blob 時為 (0, ↕o)，否則 1 = ∞（out-of-gas）、2 = ☇（panic）、3 = BADEXPORTS、4 = OVERSIZE、5 = BAD（lookup anchor 找不到 code）、6 = BIG（code 超過 W_C）；最後五個 refine 負載欄位 u（gas used）、i（imports）、x（extrinsic 數）、z（extrinsic 總大小）、e（exports）用一般的 compact E——GP 原始碼的註解說得很白：「These are variable length, since we never access them individually, digests are never accessed directly by the PVM and space is at a premium here」。注意 operand tuple 的 g（§C.2 E(operand) = E(0, p, e, a, y, g, O(l), ↕t)）確實是 compact，這是常被混淆的地方。你們的 WorkResult.Encode：AccumulateGas 是 Gas 型別走 E_8，RefineLoad.Encode 五個欄位用 EncodeInteger（compact），WorkExecResult.Encode 的 0…6 對應完全一致。

**逐項辨析**

1. ✅ E_4(s), c, y, E_8(g), then O(l) — [0] ⌢ ↕blob on success, else one octet 1…6 for ∞, ☇, BADEXPORTS, OVERSIZE, BAD, BIG — then u, i, x, z, e as compact naturals (digests are never read individually by the PVM, so space wins)  
   五個 refine 負載欄位走一般的 compact E——GP 註解直說 digest 從不被 PVM 個別存取、空間優先。
2. ❌ O(l) uses 0 for ∞ and [1] ⌢ ↕blob for success, mirroring the ¿x optional encoding; the five load fields u, i, x, z, e are E_4 so that every digest has a predictable size for the report-size limit W_R  
   O(o) 的 0 一定是「成功 + ↕blob」，1…6 才是錯誤碼；五個負載欄位也不是 E_4。
3. ❌ Every numeric field is fixed-width — E_8 for both gas values, E_4 for the four counters and E_4 for the extrinsic size — because work reports are hashed into the state trie and fixed widths keep the trie leaves aligned  
   五個負載欄位是 compact 而非固定寬度，work report 也不是靠固定寬度去對齊 trie leaf。
4. ❌ The gas limit g is a compact natural exactly like the core index of the enclosing work report, the result carries a leading length prefix in all seven cases, and u, i, x, z, e are E_2 counters  
   work digest 的 g 是 E_8（operand tuple 的 g 才是 compact），而六種錯誤碼各只佔一個 octet。

> **陷阱**　O(result)：0 + ↕blob 是成功；1..6 依序 ∞、☇、BADEXPORTS、OVERSIZE、BAD、BIG。digest 的 g 是 E_8，operand tuple 的 g 是 compact。

<sub>`appC-work-digest-encoding`</sub>

---

### C-6　The team prefixes each accumulate input item with a byte before its body. Which explanation of the two prefixes and of what actually consumes this encoding is correct per GP 0.8.0?
...
## 附錄 D · State Merklization　<sub>8 題</sub>

### D-1　The state-key constructor C maps to 31-octet keys. Which forms are correct?
...
### D-3　In the state trie, what is the first octet of the 64-octet leaf node for a value of exactly 32 octets, and what is it for a value of 33 octets?

<sub>D.2.1 Node Encoding and Trie Identification — ●●● · 計算 · §D.2.1 functions B and L (unlabelled)</sub>

**標準答案**　0xA0 for the 32-octet value and 0xC0 for the 33-octet value — the low six bits hold the embedded value length

§D.2.1：L(k, v) = [1, 0] ⌢ bits(E_1(|v|))[2..] ⌢ bits(k) ⌢ bits(v) ⌢ [0, 0, …] 當 |v| ≤ 32；否則 [1, 1, 0, 0, 0, 0, 0, 0] ⌢ bits(k) ⌢ bits(H(v))。bits() 是 MSB-first（§3 notation：bits([160, 0]) = [1, 0, 1, 0, 0, …]）。|v| = 32：E_1(32) = [0x20] = 0010_0000，去掉前兩個 bit 得 10_0000，接在 [1, 0] 後 → 1010_0000 = 0xA0；也就是「embedded-value leaf，6-bit 長度 = 32」。|v| = 33：條件 |v| ≤ 32 不成立 → regular leaf，第一個 octet 固定 1100_0000 = 0xC0，最後 32 octets 放 Blake2b(v)。所以邊界是「≤ 32 內嵌、> 32 存 hash」。補充：branch node 第一個 octet = 左子 hash 第一個 byte 去掉最高位（bits(l)[1..]），即 l_0 & 0x7F，右子 hash 完整 32 bytes。你們的 encodeLeafNode 就是 `node[0] = 0x80 | byte(len(value))` 與 `node[0] = 0xC0`，encodeBranchNode 是 `node[0] = left[0] & 0x7F`——與 GP 完全一致。

**逐項辨析**

1. ✅ 0xA0 for the 32-octet value and 0xC0 for the 33-octet value — the low six bits hold the embedded value length  
   E_1(32) 去掉前兩個 bit 接在 [1, 0] 後即 0xA0；33 octets 越過 ≤ 32 界線改走 0xC0 的 regular leaf。
2. ❌ 0xC0 for both — 32 octets already needs the full value slot, so both are regular leaves storing H(v)  
   32 octets 剛好填滿值槽，仍算內嵌 leaf（省一次 hash），界線是 > 32 才改存 H(v)。
3. ❌ 0xA0 and 0xA1 — the 6-bit length field holds any length up to 63, so 33 still fits as an embedded value  
   規則以 |v| ≤ 32 為界而非 6 bits 能裝多少，0xA1 不存在於合法的 trie 之中。
4. ❌ 0x80 and 0xC0 — the length is not part of the first octet; it is recovered from the zero padding  
   0x80 是長度 0 的 embedded leaf（空值）；長度確實編在第一個 octet 的低六位。

> **陷阱**　10 + 6-bit 長度（≤ 32）→ 內嵌；11000000 → 存 H(v)；0 + 255 bits 左 + 256 bits 右 → branch。

<sub>`appD-leaf-first-octet`</sub>

---

### D-4　Before PR #780 the team recognised a service-info key C(255, s) by testing only stateKey[0] == 0xFF, and fuzzer traces failed with 'failed to decode expected service info from state key 0xffff0017…: EOF'. Why was byte 0 alone ambiguous, and why is the current check sound?
...
```
<sub>internal/utilities/merklization/parse_state_key_vals.go (IsServiceInfoKey) + state_key_constructor.go (ServiceWrapper.StateKeyConstruct)</sub>

**標準答案**　C(s, h) keys start with n_0, the low octet of E_4(s), so every service with s mod 256 = 255 yields storage/preimage/request keys beginning 0xFF; the new check also requires zeros in all non-service-id positions, which a Blake2b-derived key matches only with negligible probability

§D.1：C(i) = [i, 0, 0, …]；C(i, s) = [i, n_0, 0, n_1, 0, n_2, 0, n_3, 0, 0, …]（n = E_4(s)）；C(s, h) = [n_0, a_0, n_1, a_1, n_2, a_2, n_3, a_3, a_4, …, a_26]（a = H(h)）。關鍵在第三種：第 0 個 octet 是 n_0 = service id 的最低 byte，完全不是隨機值——只要 s ≡ 255 (mod 256)（255、511、…、0x????00FF），該 service 的所有 storage（h = E_4(2^32−1) ⌢ k）、preimage（E_4(2^32−2) ⌢ hash）、request（E_4(l) ⌢ hash）key 都以 0xFF 開頭。舊碼把它們當成 C(255, s) 去解 89-octet 的 account info，值太短就 EOF（issue #779，fuzzer trace 0xffff0017…）。修正後的 IsServiceInfoKey 除了 byte 0 = 0xFF，還要求位置 2、4、6 與 8…30 全為 0；C(s, h) 要滿足這點需要 a_0…a_3 與 a_4…a_26 共 27 個 Blake2b 輸出 byte 全為 0，機率 2^−216，可視為不可能。順帶一提，§D.1 末段允許實作不保存原始 storage key，只保存 hash 後的 31-byte key——這正是你們 unmatched key-vals 機制的依據。

**逐項辨析**

1. ✅ C(s, h) keys start with n_0, the low octet of E_4(s), so every service with s mod 256 = 255 yields storage/preimage/request keys beginning 0xFF; the new check also requires zeros in all non-service-id positions, which a Blake2b-derived key matches only with negligible probability  
   C(s, h) 的第 0 個 octet 是 n_0（service id 的最低 byte），s ≡ 255 (mod 256) 就會撞上 0xFF。
2. ❌ C(255, s) keys for service ids above 2^24 spill their fourth id octet into position 9, so byte 0 alone cannot separate them from the chapter keys C(9)…C(16); the fix therefore skips positions 1, 3, 5, 7 and tolerates position 9 as well  
   service id 只佔位置 1、3、5、7 四個 octet，任何 32-bit id 都放得下，不會溢出到位置 9。
3. ❌ Because C(s, h) hashes the service id together with the key, every octet of such a key — the first one included — is uniformly random, so about 1/256 of them start with 0xFF; the fix re-hashes the whole key so the 0xFF prefix becomes unreachable  
   C(s, h) 並沒有把 service id 混進 hash，第 0 個 byte 是 n_0 而非隨機，修正也沒有再 hash 一次。
4. ❌ The chapter key C(255) of the accumulation-output component θ shares its first octet with C(255, s); the fix tells them apart by demanding a non-zero service id in positions 1, 3, 5, 7 and zeros everywhere else  
   chapter key 只有 C(1)…C(16)（θ 是 C(16)），根本不存在 C(255) 這個單一元件 key。

> **陷阱**　31-byte key 的第 0 個 byte：C(i) 是 chapter 編號、C(255, s) 是 255、C(s, h) 是 service id 的低 byte——三者可能相同。

<sub>`appD-code-service-info-key`</sub>

---

### D-5　The state-key constructor C is specified as producing B_31, yet every other quantity in the state trie — child identities, the embedded value slot, H(v) — is 32 octets. What forces the key to be one octet short?

<sub>D.2.1 Node Encoding and Trie Identification — ●○○ · 設計理由 · §D.1 (C → B_31); §D.2.1 (nodes fixed at 512 bit)</sub>

**標準答案**　A node is fixed at 512 bit, and a leaf must fit a one-octet discriminator-plus-size header, the key, and a full 32-octet field for either the value or its hash: 1 + 31 + 32 = 64.

§D.2.1：「Nodes are fixed in size at 512 bit (64 bytes).」leaf 的版面是：第 1 bit 分辨 branch/leaf，第 2 bit 分辨 embedded-value leaf 與 regular leaf，該 byte 剩下的 6 bits 存 embedded value 的長度（regular leaf 則清零）；「The following 31 bytes are dedicated to the state key. The last 32 bytes are defined as the value, filling with zeroes if its length is less than 32 bytes」或是 H(v)。31 就是 64 扣掉 header octet 與整整 32 octet 的 value/hash 欄位之後剩下的空間——這也是 §D.1 一開始就說 state serialization 是「a mapping from 31-octet sequence state-keys to octet sequences of indefinite length」的原因，key 的長度是版面算出來的結果而非任何語意保留。

**逐項辨析**

1. ❌ The top octet is reserved so that the branch/leaf discriminator bit can be carried inside the key itself, which is what lets a verifier tell the two node types apart without fetching the node.  
   discriminator 位在 node 的第一個 byte；key 的每個 bit 都要拿去做 trie 導航，不能挪用。
2. ✅ A node is fixed at 512 bit, and a leaf must fit a one-octet discriminator-plus-size header, the key, and a full 32-octet field for either the value or its hash: 1 + 31 + 32 = 64.  
   1 + 31 + 32 = 64：header octet 加 key 加整整 32 octet 的 value 或 H(v)，正好填滿 512 bit。
3. ❌ Keys are Blake2b outputs with the one-octet chapter index stripped off before storage; the value field is likewise 31 octets, padded to 32 only when hashed.  
   value 欄位是完整 32 octet 並以零填充，不是 31；key 也不是把某個 byte 拿掉之後的結果。
4. ❌ 31 octets leaves one spare octet in a branch node so that a third child pointer can be appended when a sub-trie exceeds the 2^8 fan-out limit.  
   branch 只有左右兩個 child（左邊 255 bits、右邊 256 bits），沒有第三個 child 的概念。

> **陷阱**　JIP-4 chainspec 的 genesis_state 每個 key 是 62 個 hex 字元 = 31 bytes，正好對得上。

<sub>`c3-appD-key-31-octets`</sub>

---

### D-6　In T(σ) a service's storage entries, its preimages and its lookup-meta entries all go through the very same third form of C. What exactly is passed in, and what keeps the three kinds apart in the trie?
...
1. ❌ The three kinds are told apart by octet 0 of the finished 31-octet key, which is 0xFD for storage, 0xFE for a preimage and 0xFF for lookup-meta, with the remaining 30 octets holding H(k) or the hash h directly and the service index folded into the value rather than the key.  
   octet 0 是 n_0，也就是 service id 的最低位 byte——你們的 #779/#780 正是踩到這個誤解。
2. ❌ Storage uses C(s, k) with the raw key, preimages use the chapter form C(254, s) and lookup-meta uses C(253, s); no marker is needed at all because the three already live under distinct chapter indices, and the (h, l) pair of a lookup entry is recovered from the leaf's value rather than its key.  
   chapter 形式只用在 C(255, s) 的 service info；三者共用第三形式，靠輸入的 marker 分開。
3. ✅ A four-octet marker is prepended before hashing — E_4(2^32−1) ⌢ k for storage, E_4(2^32−2) ⌢ h for a preimage, and E_4(l) ⌢ h for the lookup-meta of declared length l — and C(s, ·) then interleaves the four octets of n = E_4(s) with the first four octets of a = H(·), appending a_4 … a_26.  
   三列 T(σ) 的 marker 是 E_4(2^32−1)／E_4(2^32−2)／E_4(l)，C(s, ·) 再把 E_4(s) 與 H(·) 交錯成 31 bytes。
4. ❌ All three use C(s, h) with h the raw storage key or preimage hash and no marker whatsoever; collisions cannot arise because a service storage key is required by §9 to be exactly 32 octets long, which keeps it inside a namespace disjoint from the 32-octet preimage hashes.  
   GP 明說 storage key 長度完全自由；沒有 marker 就無從保證 C 的輸入互斥。

> **陷阱**　GP 明說 storage key「not required to be known by implementations」——只要存得下 Merklisation-ready 的雜湊即可，原始 key 可以不落盤。

<sub>`c3-appD-service-subkeys`</sub>

---

### D-7　Your 0.7.2 encoder writes the C(10) entry of T(σ) as, per core, an optional pair of work-report and reporting timeslot. What does GP 0.8.0 put there instead?

<sub>D.1 Serialization — ●●● · 版本差異 · §D.1 T(σ) row C(10); eq. 11.1 (ρ spec) · ⚠ 0.7.2→0.8.0</sub>

**標準答案**　Per core an optional pair ⟨a_g, E_4(a_t)⟩ written with the ? option discriminator, where a_g is the entire guarantee G ≡ (r work-report, t timeslot, a credential of 2–3 (validator index, Ed25519 signature) pairs) — so the guarantors' signatures are now part of committed state.

GP 0.8.0（PR #494「Keep full guarantees in availability assignments state (rho)」）把 eq. 11.1 改成 ρ ∈ ⟦(g ∈ G, t ∈ N_T)?⟧_C，而 G ≡ (r ∈ R, t ∈ N_T, a ∈ ⟦(N, V̄)⟧_{2:3})（eq. 11.24；0.8.0 的 R 是 work-report 集合、W 是 work-item 集合、V̄ 是 Ed25519 簽章集合），也就是 work-report 加上 guarantor 的 credential。附錄 D 的 T(σ) 因此寫成 C(10) ↦ E([ ⟨a_g, E_4(a_t)⟩? for ⟨a_g, a_t⟩ ∈ ρ ])——外層是 optional discriminator（∅ → 0；否則 1 ⌢ …），內層 timeslot 用固定 4 byte。0.7.2 只存 work-report、signature 不進 state；升上 0.8.0 若沒改，C(10) 的 leaf 會短一大截、state root 直接對不上。注意 guarantee 自己帶一個 t（guarantee 的 timeslot），availability assignment 又帶一個 t（reported 的時間，用於 eq. 11.18 的 U 逾時清除），兩者不是同一個欄位。

**逐項辨析**

1. ❌ Per core an optional pair of work-report and timeslot exactly as before; 0.8.0 only tightened the timeslot to a fixed-length E_4 encoding and left the payload alone.  
   0.8.0 換掉的是 payload 本身：ρ 現在存整個 guarantee，連 guarantor 的 credential 都在內。
2. ✅ Per core an optional pair ⟨a_g, E_4(a_t)⟩ written with the ? option discriminator, where a_g is the entire guarantee G ≡ (r work-report, t timeslot, a credential of 2–3 (validator index, Ed25519 signature) pairs) — so the guarantors' signatures are now part of committed state.  
   #494 把 eq. 11.1 改成 ρ ∈ ⟦(g ∈ G, t ∈ N_T)?⟧_C，guarantor 簽章因此進入 committed state。
3. ❌ The availability assurances extrinsic of the current block, so that an assurer's bitfield can be proven against the state root without replaying the block.  
   extrinsic 從不進 trie，這是把 extrinsic 與 state 兩件事搞混了。
4. ❌ Only the availability specification (package hash, erasure root, segment root, bundle length) of each core's pending report, the guarantee itself being retained off-chain by the guarantors until an audit demands it; that is precisely the size reduction 0.8.0 was aiming for.  
   availability specification 是 work-report 內部的欄位而非 ρ 的內容，0.8.0 也是加大而非縮小這一格。

> **陷阱**　0.8.0 之後 ρ[c] 有兩個 timeslot：guarantee 內的 g_t，以及 assignment 的 a_t。序列化時別漏掉前者。

<sub>`c3-appD-rho-guarantee`</sub>

---

### D-8　M is defined over a dictionary keyed by bits(k), while the team's Go partitions one slice in place at each depth. A teammate proposes replacing it with 'sort the key-vals ascending, then fold them pairwise like M_B — same leaves, same root, and it vectorizes better'. Which statement is correct?
...
```
<sub>internal/utilities/merklization/merklization.go (partitionByBit, merklizeWithCache)</sub>
...
> **陷阱**　同一份 GP 裡有兩種 bit 順序：trie 導航 bits(·) 是 MSB-first，codec 的 bitstring 打包是 LSB-first。

<sub>`c3-appD-merklize-bitorder`</sub>
...
> **陷阱**　§6 的 fallback 叫 F、附錄 F 的 shuffle 也叫 𝓕——同名不同物。fallback 一個 slot 一次 hash、取前 4 bytes；Q_l 一次 hash 供八個、取 4i mod 32 的 window。

<sub>`c3-appF-seq-from-hash-vs-fallback`</sub>
...
## 附錄 G · Bandersnatch VRF　<sub>5 題</sub>

### G-1　JAM uses two Bandersnatch constructions: singly-contextualized IETF VRF signatures and ring-VRF proofs. Which statement about where each is used, its size and the output function Y is correct?

<sub>G Bandersnatch VRF (IETF VRF vs Ring VRF) — ●●○ · 概念 · §G; §3 cryptography notation; eq. 6.4, 6.14–6.18, 6.30; eq. 17.3 (audit seed)</sub>

**標準答案**　Only ticket proofs in E_T are 784-octet ring-VRF proofs (anonymous, checked against a 144-octet ring root); the seal H_S, entropy H_V and audit seeds are 96-octet IETF VRF signatures under a named key; in both, Y(·) is the first 32 octets of the VRF output and depends on the context, not the message
...
1. ✅ Only ticket proofs in E_T are 784-octet ring-VRF proofs (anonymous, checked against a 144-octet ring root); the seal H_S, entropy H_V and audit seeds are 96-octet IETF VRF signatures under a named key; in both, Y(·) is the first 32 octets of the VRF output and depends on the context, not the message  
   ring VRF 只用在 ticket proof；§G 的 Y(s) ≡ output(s)[..32]，受 context 影響而不受 message 影響。
2. ❌ Both the seal H_S and the ticket proof are 784-octet ring-VRF proofs checked against the 144-octet γ′_Z — that is what keeps the block author anonymous until the epoch ends; the entropy H_V and the audit seeds are 96-octet IETF VRF signatures under a named key; in both, Y(·) is the full 64-octet VRF output and depends on the context, not the message  
   seal 走 eq. 6.16/6.17 的 IETF 形式、作者由 H_I 公開指名；§G 的 Y 也只取前 32 octets。
3. ❌ Only ticket proofs in E_T are 784-octet ring-VRF proofs (anonymous, checked against a 144-octet ring root); the seal H_S, entropy H_V and audit seeds are 96-octet IETF VRF signatures under a named key; in both, Y(·) is the Blake2b hash of the whole signature, so it moves with the signed message — which is why tickets sign the empty message  
   Y 取的是 VRF output 本身而非簽名的雜湊，§3 註明它受 context 影響、不受 message 影響。
4. ❌ Only ticket proofs in E_T are 784-octet ring-VRF proofs, checked against a 32-octet ring root committed over the active set κ′ rather than the pending set; the seal H_S, entropy H_V and audit seeds are 96-octet IETF VRF signatures under a named key; Y(·) is the first 32 octets of the VRF output, and a key zeroed by Φ is dropped from the ring so the ring shrinks with the offender count  
   ring root ∈ B̊ ⊂ B_144 且承諾在 γ′_P 上；被 Φ 歸零的 key 以 padding point 代入，ring 大小不變。

> **陷阱**　ring VRF 只用在 ticket；seal/entropy/audit 都是 IETF；γ_Z 來自 γ′_P（pending），壞 key 用 padding point。

<sub>`appG-ietf-vs-ring`</sub>

---

### G-2　Each JAM signature is domain-separated by a context string X. Which of the following (context → primitive → use) statements is correct?

<sub>Signing contexts X (definitions appendix) and their primitives — ●●○ · 概念 · definitions appendix §Signing Contexts; eq. 6.16–6.18, 6.30, 11.14, 11.28, 17.3, 17.7, 17.16, 18.1; ch. 10 culprit/fault signature rules</sub>

**標準答案**　X_T = $jam_ticket_seal is used twice: for the ring-VRF ticket proof (context X_T ⌢ η′_2 ⌢ [e], empty message, root γ′_Z) and for the regular IETF-VRF seal (context X_T ⌢ η′_3 ⌢ [i_e], message E_U(H))

definitions 附錄「Signing Contexts」：X_A = $jam_available（Ed25519，assurances；eq. 11.14 簽 X_A ⌢ H(E(H_P, bitfield))）；X_B = $jam_beefy（BLS，eq. 18.1 簽 X_B ⌢ 最新 β_H 的 super-peak）；X_E = $jam_entropy（Bandersnatch IETF，eq. 6.18：context X_E ⌢ Y(H_S)、訊息為空）；X_F = $jam_fallback_seal（Bandersnatch，eq. 6.17）；X_G = $jam_guarantee（Ed25519，eq. 11.28 guarantee 簽 X_G ⌢ H(w)，ch. 10 的 culprit 也用它簽 X_G ⌢ report hash）；X_I = $jam_announce（Ed25519，eq. 17.7 的 audit announcement）；X_T = $jam_ticket_seal（Bandersnatch ring VRF 出票 + IETF 正常 seal）；X_U = $jam_audit（Bandersnatch，audit 選取熵，eq. 17.3 的 seed_0 與 eq. 17.12 的 seed_n）；X_valid/X_invalid = $jam_valid/$jam_invalid（Ed25519，judgments 與 faults，eq. 17.16、ch. 10）。判讀時要連 context 的組成（X ⌢ entropy ⌢ index）與原語一起看，同一個字串完全可以搭配不同的原語與不同的 entropy 版本。你們 const.go 的 JamEntropy/JamFallbackSeal/JamTicketSeal/JamValid/JamInvalid/JamAvailable/JamBeefy/JamGuarantee/JamAnnounce/JamAudit 逐一對應；#940 的 bug（用公鑰簽 judgment）就是 X_valid 這條路徑。

**逐項辨析**

1. ✅ X_T = $jam_ticket_seal is used twice: for the ring-VRF ticket proof (context X_T ⌢ η′_2 ⌢ [e], empty message, root γ′_Z) and for the regular IETF-VRF seal (context X_T ⌢ η′_3 ⌢ [i_e], message E_U(H))  
   同一個字串兩種用法，差別在 η′_2 vs η′_3、ring vs IETF、空訊息 vs E_U(H)。
2. ❌ X_E = $jam_entropy is an Ed25519 signature by the author over the unsigned header E_U(H); the Blake2b hash of that signature is what feeds the entropy accumulator η′_0 each block  
   X_E 是 Bandersnatch IETF VRF：context X_E ⌢ Y(H_S)、訊息為空，既不是 Ed25519 也不簽 header。
3. ❌ X_G = $jam_guarantee is a Bandersnatch VRF context over H(w), and its output Y(·) doubles as the guarantee identifier stored with the availability assignment in ρ  
   X_G 是 Ed25519 而不是 VRF，guarantee 也沒有任何 identifier 會被存進 ρ。
4. ❌ X_U = $jam_audit is the Ed25519 context under which validators sign the judgments that appear in the verdicts of the disputes extrinsic E_D, one signature per report hash  
   X_U 是 Bandersnatch 的 audit seed context；judgment 與 fault 用的是 X_valid / X_invalid。

> **陷阱**　口訣：Bandersnatch = T/F/E/U（票、備援 seal、熵、audit）；Ed25519 = A/G/I/valid/invalid；BLS = B。

<sub>`appG-signing-contexts`</sub>

---

### G-3　Safrole could have proved ticket validity with an ordinary IETF VRF signature plus the submitting validator's published Bandersnatch key. Why does the Gray Paper insist on a ring VRF proof instead?

<sub>G Bandersnatch VRF (IETF VRF vs Ring VRF) — ●○○ · 設計理由 · §G; §3 signing schemes; eq. 6.30 (tickets extrinsic), eq. 6.16 (ticket seal)</sub>

**標準答案**　Because the proof shows only that its author knows a secret whose public key lies in the ring committed by γ′_Z, never which one, so next epoch's slot-sealer sequence can sit in public state without revealing who will author which slot; the author is identified only when the seal appears, which is what blunts targeted denial-of-service and bribery.

§3 的簽章章節把差別講得最清楚：「Both the Bandersnatch signature and RingVRF proof strictly imply that a member utilized their secret key in combination with both the context x and the message m; the difference is that the member is identified in the former and is anonymous in the latter.」而 ring root 那段又說「A root implies a specific sequence of Bandersnatch key pairs, knowledge of one of the secrets would imply being capable of making a unique, valid—and anonymous—proof of knowledge of a unique secret within the sequence.」eq. 6.30 的 ticket proof 屬於 ⟨γ′_Z, X_T ⌢ η′_2 ⌢ i_e, []⟩ 這一族 784-octet ring proof，驗證只看 144-octet 的 ring root，因此鏈上永遠不知道某張 ticket 是誰投的；到了下一個 epoch 該 slot 的作者才用 eq. 6.16 的 96-octet IETF 簽章現身。（以下為 Sassafras/Safrole 的公認設計動機，GP 0.8.0 本文只寫到 anonymity，未展開攻擊模型）這正是 Safrole 相對 BABE 的賣點：未來的出塊者名單公開、身分不公開，攻擊者無法提前針對某台機器打 DoS 或行賄。

**逐項辨析**

1. ❌ Because a ring VRF output is unbiasable whereas a plain VRF output can be ground by its signer through resubmission, so only the ring form makes next epoch's slot-sealer sequence safe to publish; anonymity is a welcome side effect but not the reason, and grinding resistance is also why the ticket's message is empty and its context carries no entry index.  
...
## 附錄 H · Erasure Coding　<sub>5 題</sub>

### H-1　The systematic Reed-Solomon code over GF(2^16) has rate d(v):v with d(v) = max({d | d ∈ N_{v/3+2}, W_G mod 2d = 0}). What is d for v = 1023 and for v = 6, and what does this achieve?

<sub>H Erasure Coding — ●●○ · 計算 · eq. H.1</sub>

**標準答案**　d(1023) = 342 and d(6) = 3: any d of the v chunks reconstruct the data, so the system survives almost two-thirds of validators being malicious or offline; a 4,104-octet segment splits into 4104/(2·342) = 6 pieces per validator at full scale

eq. H.1：d(v) = max{d ≤ v/3 + 1（N_{v/3+2} 即 0..v/3+1）: 4104 mod 2d = 0}。v = 1023：v/3 + 1 = 342，4104 = 2·342·6 ✓ → d = 342；v = 6：v/3 + 1 = 3，4104 mod 6 = 0 ✓ → d = 3。§H：「we wish to be able to reconstruct even should almost two-thirds of the v validators be malicious or incapacitated」；𝒟 之所以要求 W_G mod 2d = 0，正是為了「encoding segments of size W_G without padding」——段落末也明說「no padding is required」。eq. H.6：任意 𝒟(v) 個 chunk（附索引）即可還原。rate 最佳（d = v/3 + 1）的 v 集合包含 6, 9, 15, …, 1023；v = 1022 時效率最差（約 1:4.5）。full 下每個 segment 產生 6 個 octet-pair per validator（「data-parallelism of order 6」）。注意 0.7.x 的 tiny test vectors 用 W_P = 1026（2 個 data shards），0.8.0 的公式給 3——遷移時要重看 tiny EC 常數（你們 code-map 3.17 指出 DataShards/TotalShards 是編譯期常數）。

**逐項辨析**

1. ✅ d(1023) = 342 and d(6) = 3: any d of the v chunks reconstruct the data, so the system survives almost two-thirds of validators being malicious or offline; a 4,104-octet segment splits into 4104/(2·342) = 6 pieces per validator at full scale  
   342 同時滿足 ≤ v/3 + 1 與 4104 mod 684 = 0，是候選集裡的最大值；4104 = 2·342·6 無需補零。
2. ❌ d(1023) = 341 and d(6) = 2: any d of the v chunks reconstruct the data, so the system survives almost two-thirds of validators being malicious or offline; since 4,104 is not a multiple of 2·341 a segment must be zero-padded before it can be coded at full scale  
   341 不是最大候選——342 已經合格；§H 也明說 no padding is required。
3. ❌ d(1023) = 512 and d(6) = 3: any d of the v chunks reconstruct the data, so the system survives half the validators being malicious or offline; a 4,104-octet segment splits into 4104/(2·512) ≈ 4 pieces per validator at full scale  
   512 已超出 N_{v/3+2} 的上界 342，4104 mod 1024 ≠ 0，門檻也不是二分之一。
4. ❌ d(1023) = 682 and d(6) = 4: at least v − d of the v chunks are needed to reconstruct, so the system survives only one-third of validators being malicious or offline; a 4,104-octet segment splits into 4104/(2·682) ≈ 3 pieces per validator at full scale  
   682 同樣超界、d(6) = 4 > 3，且 eq. H.6 是任意 𝒟(v) 個 chunk 即可還原，門檻方向說反了。

> **陷阱**　H.1 的集合是 N_{v/3+2} = {0, …, v/3+1}，所以最大候選就是 v/3 + 1。

<sub>`appH-erasure-rate`</sub>

---

### H-2　How does the chunking function 𝒞^v_k turn a data blob into v chunks, and what makes the code 'systematic'?
...
### H-3　The team's Go tree is on GP 0.7.2 and pins the erasure-coding rate as shown. GP 0.8.0 replaces the fixed rate with the function 𝒟(v). What is the minimal correct migration, and what does it do to the 6-validator tiny configuration?

<sub>H Erasure Coding — ●●○ · 程式碼 · eq. H.1 (𝒟); eq. 6.8 (𝕍); eq. 11.5 (a_v), eq. 11.31 (∀r ∈ I: (r_s)_v = |κ′|); §14 availability specifier · ⚠ 0.7.2→0.8.0</sub>

```go
// SetTinyMode()                     // SetFullMode()
...
### H-5　Appendix H says the coding rate 'is derived from' three separate considerations. Which account of them is right, and why is the code built over GF(2¹⁶) rather than over single octets?
...
<sub>1.1 Nomenclature / RFC-31 — ●○○ · 設計理由 · §1.1 & RFC-31 CoreJam</sub>

**標準答案**　From CoreJam (Polkadot Fellowship RFC-31), named after its Collect / Refine / Join / Accumulate model; only Join and Accumulate happen on-chain — Collect and Refine are off-chain/in-core — hence 'Join-Accumulate Machine', a complete protocol rather than RFC-31's scope-limited alteration

GP §1.1：「An early, unrefined, version of this protocol was first proposed in Polkadot Fellowship RFC 31, known as CoreJam. CoreJam takes its name after the collect/refine/join/accumulate model of computation at the heart of its service proposition. While the CoreJam RFC suggested an incomplete, scope-limited alteration to the Polkadot protocol, JAM refers to a complete and coherent overall blockchain protocol.」四個階段的分界要背熟：Collect（builder 收集輸入）與 Refine（in-core 計算）在鏈下；Join（guarantee/assure 上鏈）與 Accumulate（狀態整合）在鏈上——名字正取自後面那一對。§4.9.1 則說明為何不能四階段全上鏈：「everybody does everything」式的 on-chain consensus model「is unfortunately not scalable」，JAM 引入 in-core 模型正是為了繞開它。

**逐項辨析**

1. ✅ From CoreJam (Polkadot Fellowship RFC-31), named after its Collect / Refine / Join / Accumulate model; only Join and Accumulate happen on-chain — Collect and Refine are off-chain/in-core — hence 'Join-Accumulate Machine', a complete protocol rather than RFC-31's scope-limited alteration  
   名字與分工都對上 §1.1：只有 Join 與 Accumulate 上鏈，縮寫才會是 Join-Accumulate Machine。
2. ❌ From CoreJam (Polkadot Fellowship RFC-31), named after its Collect / Refine / Join / Accumulate model; Refine and Accumulate are the on-chain pair — Collect and Join are done off-chain by the package builder — hence 'Join-Accumulate Machine'  
   分工顛倒：Refine 是 in-core 的高吞吐階段，Join 才是把 work-report 帶上鏈的那一步。
3. ❌ From 'Just Another Machine', a nod to the Yellow Paper's naming; the collect / refine / join / accumulate pipeline was bolted on later by RFC-31, and all four stages run on-chain because a single validator set executes everything in consensus  
   四階段全在鏈上就是 §4.9.1 判定「is unfortunately not scalable」的 everybody-does-everything 模型。
4. ❌ From 'Joint Availability Mechanism', after the erasure-coded D3L at its centre; RFC-31 already proposed this same complete protocol under the name CoreJam, and only Collect runs on-chain, since the chain must order incoming work-packages before the other three stages run in-core  
   §1.1 明寫 RFC-31 是「incomplete, scope-limited alteration」，而 Collect 是 builder 在鏈下收集輸入。

> **陷阱**　面試「architecture」開場常問這題；順便提 RFC-31 的三種 service 想像：parachain validation、actor progression（CorePlay）、simple ordering。

<sub>`arch-corejam-name`</sub>

---

### ARCH-2　The GP names five driving factors and a principle called 'size-coherency antagonism'. Which statement is correct?

<sub>1.2–1.3 Driving Factors / Size-Coherency Antagonism — ●●○ · 設計理由 · §1.2–1.3</sub>

**標準答案**　Factors: Resilience, Generality, Performance, Coherency, Accessibility; performance and coherency are antagonistic because causality is bounded by signal speed, so larger state-spaces become less coherent — JAM answers by pipelining a highly scalable, mostly-coherent element (in-core) into a synchronous, fully-coherent element (on-chain), replacing crude partitioning with 'cache affinity'

§1.2：(1) Resilience (2) Generality (3) Performance (4) Coherency (5) Accessibility；「items 3 and 4 are antagonistic according to an information theoretic principle… we shall name it size-coherency antagonism」，而前兩項是 Web3 的隱含前提——「we make an implicit assumption of the first two items」。§1.3：state-space 越大越不 coherent（受光速、電子與軟體延遲限制，元件間平均距離與變異都上升，因果解析時間發散）；現有可擴展系統（Polkadot、Cosmos、rollup）靠粗暴分割與非同步溝通，GP 以細菌分裂比喻批評之。JAM「pipelines a highly scalable, mostly coherent element to a synchronous, fully coherent element」，以類似多核心 CPU 共享 RAM 的「cache affinity」取代分割，目標是「avoiding the persistent fragmentation of state-space」——這就是「mostly-coherent trustless supercomputer」副標的由來。GP 也明確以成本為由否定 SNARK 路線（「Unlike with SNARK-based L2-blockchain techniques… this model draws upon crypto-economic mechanisms」）。

**逐項辨析**

1. ✅ Factors: Resilience, Generality, Performance, Coherency, Accessibility; performance and coherency are antagonistic because causality is bounded by signal speed, so larger state-spaces become less coherent — JAM answers by pipelining a highly scalable, mostly-coherent element (in-core) into a synchronous, fully-coherent element (on-chain), replacing crude partitioning with 'cache affinity'  
   五項與對立的那一組（performance／coherency）都對，解法也正是 in-core → on-chain 的 pipeline 與 cache affinity。
2. ❌ Factors: Resilience, Generality, Performance, Coherency, Accessibility; the antagonism is between resilience and accessibility, since every additional validator adds a consensus round-trip and cheap access therefore forces a small set — JAM answers by pinning the validator set at 1,023 and pushing all further growth into off-chain roll-ups that settle asynchronously against the chain  
   被點名對立的是第 3、4 項而非 resilience／accessibility；0.8.0 更已讓 validator 集合大小可變。
3. ❌ Factors: Resilience, Generality, Performance, Coherency, Accessibility; performance and coherency are antagonistic because signature verification does not parallelize, so throughput must be bought with weaker composability — JAM answers by fragmenting state into 341 causally-independent shards, each small enough to stay coherent, bridged by an asynchronous message queue  
   把資訊理論論證換成「簽章無法平行化」；341 個因果獨立分片正是 GP 批評的 crude partitioning。
4. ❌ Factors: Speed, Cost, Security, Decentralization, Simplicity; the antagonism is between decentralization and performance, because hardware requirements are barriers to entry to the validator set — JAM answers by fixing validator hardware at 16 cores / 64 GB / 8 TB and using SNARKs to compress the work, so coherency survives without any pipeline at all  
   五項名稱全不對；16 核/64 GB/8 TB 只是 §20 的假設硬體，GP 也明確以成本否定 SNARK 路線。

> **陷阱**　與 SNARK rollup 的對比：§2 指 RISC-Zero 類證明比直接執行慢 6 萬倍以上、且傾向中心化。

<sub>`arch-driving-factors`</sub>

---

### ARCH-3　Why does JAM use Safrole (ticket-based, anonymous, ring-VRF) rather than a BABE-like VRF lottery?

<sub>6 Safrole rationale — ●●○ · 設計理由 · §6 intro & §19</sub>

**標準答案**　Safrole limits each 6-second slot to exactly one predetermined key-holder (near fork-free), keeps the identity of future slot authors anonymous until they seal (DoS resistance), and produces a high-quality unbiasable entropy pool; the best-chain rule additionally prefers chains with more ticket-sealed (vs fallback) ancestors

§6 intro：「The chief purpose of a block production consensus mechanism is to limit the rate at which new blocks may be authored and, ideally, preclude the possibility of forks… Safrole limits the possible author of any block within any given six-second timeslot to a single key-holder… under normal operation, the identity of the key-holder of any future timeslot will have a very high degree of anonymity. As a side effect… we can generate a high-quality pool of entropy」——三個目的：近似無分叉、DoS 抗性、高品質 entropy。對照 BABE 的 VRF 抽籤會出現多個或零個 slot leader，Safrole 用 ticket 在前一個 epoch 就決定每個 slot 恰好一個作者；注意 GP 的用字是「ideally, preclude the possibility of forks」，並非完全無分叉，所以 §19 仍需 best-chain 規則——偏好「most ancestor blocks whose author used a slot-sealer ticket, rather than a fallback key」。Safrole 是 Sassafras 的簡化版（「To follow in the footsteps of Ethereum yellow paper」——簡單到能被多個獨立實作正確實現）。

**逐項辨析**

1. ✅ Safrole limits each 6-second slot to exactly one predetermined key-holder (near fork-free), keeps the identity of future slot authors anonymous until they seal (DoS resistance), and produces a high-quality unbiasable entropy pool; the best-chain rule additionally prefers chains with more ticket-sealed (vs fallback) ancestors  
   三個目的都寫在 §6 intro：每 slot 恰一個作者、未來作者匿名、副產品是高品質 entropy，§19 再加 ticket 優先。
2. ❌ Safrole's advantage is that several validators may win the same slot so a slot is never empty: the ring VRF lets each of them prove membership of γ_P, the resulting fork is resolved by whichever block reaches Grandpa first, and tickets are drawn afresh each slot from η_0 so that no author is predictable more than one slot ahead  
...
4. ❌ Safrole is a finality gadget sitting under Grandpa: it assigns each slot one author, who must then gather 2/3+1 pre-votes before the block may extend the chain, which is why JAM has no forks at all and needs no fork-choice rule; fallback keys are used only in the genesis epoch, before any tickets have accumulated  
   Safrole 只管 block production，finality 歸 Grandpa；fallback F(k, η_2) 任何一次 ticket 不足的交界都會啟用。

> **陷阱**　anonymity 來自 ring VRF：證明「我是 γ_P 中某人」而不揭露是誰；seal 時才用普通 VRF 揭露。

<sub>`arch-why-safrole`</sub>

---

### ARCH-4　Why does JAM need BOTH availability (assurances + erasure coding) AND auditing/disputes to secure in-core computation?

<sub>4.8.1, 11, 16, 17 rationale — ●●○ · 設計理由 · §4.9.1, §16–17, ELVES paper</sub>
...
4. ❌ They are two halves of one mechanism: the three guarantors of a core sign the report and then re-execute each other's work — that mutual check is what the GP calls auditing — while assurances only acknowledge receipt of the block; because guarantors are already staked no external auditor is drawn, and a negative judgment slashes the reporter directly with no on-chain verdict needed  
   被懷疑的正是 guarantor，自審沒有安全意義；auditor 是用 Bandersnatch VRF 從整個 validator set 抽出的。

> **陷阱**　面試「design rationale」高頻題：把四階段講清楚，並說明 report 為何要先 available 才 accumulate。

<sub>`arch-availability-auditing`</sub>

---

### ARCH-5　JAM is designed so that most of a block's work can proceed while the block propagates. Which design features make that possible?

<sub>5 & 20 (pipelining) — ●●○ · 設計理由 · §5, §20 Discussion</sub>

**標準答案**　Temporal parallelism (pipelining): the header carries the PRIOR state root, so a block can be published before the new state has been Merklized — that cost lands in the next slot; plus spatial parallelism, both across the largely independent components of σ (whose dependency graph §4.2.1 deliberately keeps shallow) and across cores in-core

§5：prior state root「to facilitate the pipelining of block computation and in particular of Merklization」——這是「a departure from both Polkadot and the Yellow Paper's Ethereum, in both of which a block's header contains the posterior state's Merkle root」，Merklize 新狀態的成本因此落到下一個 slot。§4.2 的 dependency graph 刻意最小化深度，讓 β†、Safrole、disputes、assurances、guarantees 這些輕量步驟可平行，而 accumulation（最重）之後只剩 preimage 整合、α′、π′。§20 Discussion 列了三個 driver：spatial parallelism（341 cores + eq. 4.4 的 17 個大致獨立的 state 元件）、temporal parallelism（pipelining）、PVM 與硬體對齊（RISC-V → 高效 recompile）。參考硬體 16 核/64 GB/8 TB/0.5 Gbps；CPU 預算 10/16 給 auditing、2/16 block execution、1/16 Merklization——Merklization 是每個節點自己的成本，不會被 core 數分攤。

**逐項辨析**

1. ✅ Temporal parallelism (pipelining): the header carries the PRIOR state root, so a block can be published before the new state has been Merklized — that cost lands in the next slot; plus spatial parallelism, both across the largely independent components of σ (whose dependency graph §4.2.1 deliberately keeps shallow) and across cores in-core  
   兩種平行性都對：prior root 讓 Merklization 落到下一個 slot，σ 的淺依賴圖與多 core 則給空間平行。
2. ❌ Temporal parallelism comes from the header carrying the POSTERIOR state root, which lets a node accept a block without replaying it, so Merklization must finish before publication but is never repeated; spatial parallelism comes from giving each core its own fragment of σ, so two cores never touch the same state component and accumulation runs fully in parallel  
   §5 明說 posterior root 是 Polkadot 與 Yellow Paper 的做法而 JAM 刻意 departs；JAM 也不做持久性狀態切割。
3. ❌ Blocks are authored a slot early: the ticket-holder for slot n+1 is handed the state of slot n as soon as it exists, precomputes its block and merely signs it when the slot opens, so the posterior root is already known and can go in the header; the 341 cores then each replay the block, sharing the Merklization cost 341 ways  
   Safrole 只提前決定「誰」出塊，內容取決於當下 extrinsic；core 跑的是 refine 而非重放區塊。
4. ❌ Both refine and accumulate run off-chain: a core's guarantors execute the whole pipeline and publish only a state diff, leaving the on-chain step a Merkle patch; the header's state-root field is therefore left as the zero hash and corrected only once Grandpa finalizes the block, which is what bounds the pipeline to the 8-block recent-history window  
   accumulate 明訂在鏈上；先填零再由下一塊補的是 recent history β 的 state-root 欄位，不是 H_r。

> **陷阱**　Grandpa vote 帶 posterior state root（§19）讓下游仍能驗證最新狀態——彌補 header 只有 prior root。

<sub>`arch-why-prior-root-and-pipelining`</sub>

---

### ARCH-6　How do JAM services differ from Ethereum's account model, and how does external data enter the state?

<sub>4.8.2 services — ●○○ · 設計理由 · §4.9.2, §9</sub>

**標準答案**　JAM has only service accounts (code + balance + state, no secret key, no nonce); each has two entry points — refine (in-core, stateless, arbitrary input → small digest) and accumulate (on-chain, stateful); all extrinsic data enters through refine inside work-packages, authorized via coretime/authorizers rather than signed transactions

§4.9.2：「In JAM, all accounts are service accounts… Since they are not controlled by a secret key, they do not need a nonce.」「All data extrinsic to JAM is fed into the refinement code of some service… executed in-core… refinement code is executed off-chain and subject to no such constraints, enabling JAM services to scale dramatically both in the size of their inputs and in the complexity of their computation.」同節也明說 service definition「actually includes multiple code entry-points, one concerning refinement and the other concerning accumulation」，而且「there is no such concept of a *transactor*」——授權走的是 coretime 與 authorizer，不是簽章交易。CoreChains（parachains）、CorePlay（actor/continuation smart contracts，靠 PVM 可暫停/續跑）、CoreVM 等都只是 service——JAM 本身沒有 parachain 概念，升級責任下放到 service（Wiki：「Non-upgradable chain; upgradability responsibility shifted to services」）。

**逐項辨析**

1. ✅ JAM has only service accounts (code + balance + state, no secret key, no nonce); each has two entry points — refine (in-core, stateless, arbitrary input → small digest) and accumulate (on-chain, stateful); all extrinsic data enters through refine inside work-packages, authorized via coretime/authorizers rather than signed transactions  
   兩個 entry point 與「無私鑰、無 nonce」都對上 §4.9.2，外部資料一律走 work-package 裡的 refine。
2. ❌ JAM keeps Ethereum's split but renames it: service accounts hold code, balance and state, while 'authorizer accounts' hold a secret key and a nonce so they can sign work-packages; refine and accumulate are both on-chain entry points, and external data arrives as signed transactions the authorizer account pays for from its balance  
   GP 說「In JAM, all accounts are service accounts」；authorizer 授權的是程式碼而非帳戶，付費靠事先買好的 coretime。
3. ❌ JAM has only service accounts (code + balance + state, no secret key, no nonce), but a service exposes a single entry point that guarantors run in-core and the chain then re-runs on-chain to verify; external data enters as extrinsics in the block body, and coretime is charged per byte of that extrinsic much as Ethereum charges gas  
   service 有 refine 與 accumulate 兩個 entry-point；鏈上重跑正是 guarantee／assure／audit 賽局要取代的工作。
4. ❌ JAM services are the direct successors of parachains: each is registered against a fixed core, ships a validation function guarantors execute, and talks to other services over an asynchronous XCM-like queue; data enters via collators' proofs-of-validity, and the accumulate entry point exists only to move balances between services  
   service 不綁死在某個 core，CoreChains 只是眾多 service 之一，而 accumulate 遠不只搬餘額。

> **陷阱**　面試官若問「為什麼 JAM 不可升級」：協定固定、可多實作、把演進放在 service 層。

<sub>`arch-services-vs-accounts`</sub>

---

### ARCH-7　Which set of full-configuration constants is correct?

<sub>Appendix I constants — ●●○ · 概念 · Appendix I & §20</sub>

**標準答案**　C = 341 cores (|κ| = 3C = 1023 in the full configuration; V itself is no longer a protocol constant), E = 600 slots/epoch, P = 6 s, Y = 500, R = 10, H = 8, L = 14,400, D = 19,200, U = 5, K = 16, O = 8, Q = 80, I = 16, J = 8, T = 128, W_G = 4,104, W_R = 48 KiB, W_B = 13,791,360, W_C = 4,000,000, G_A = 10^7, G_R = 5·10^9, G_T = 3.5·10^9, G_I = 5·10^7, A = 8 s, F = 2

附錄 I 的常數表在 0.8.0 已**沒有 V**（validator 數改由 𝕍 ≡ {3c | c ∈ N_[2,C+1]} 決定、門檻一律取 |κ|）：C_corecount 341、epoch E 600、slot P 6 s、Y（epoch tail start）500、R（rotation）10、H（recent history）8、L（max lookup anchorage）14,400 = 24h、D（expunge）19,200 = L + 4,800、U（assurance timeout）5、K（max block tickets）16、O（auth pool）8、Q（auth queue）80、I（max package items）16、J（max report deps）8、N_V/N_O 16、S = 2^16、T（max package extrinsics）128、W_M/W_X（imports/exports）3,072、W_G segment 4,104、W_F 4,488、W_R 48·2^10、W_B 13,791,360、W_A 64,000、W_C 4,000,000、W_T memo 128、G_A 10^7、G_I 5·10^7、G_R 5·10^9、G_T 3.5·10^9、B_S/B_I/B_L = 100/10/1、A（tranche）8 s、F（audit bias）2、Z_P 2^12、Z_Z 2^16、Z_I 2^24、Z_A 2。full 配置下 |κ| = 1023 = 3·341（每個 active core 3 個 guarantor），但 #514 之後「所有配置都固定 1023」的說法已不成立。

**逐項辨析**

1. ✅ C = 341 cores (|κ| = 3C = 1023 in the full configuration; V itself is no longer a protocol constant), E = 600 slots/epoch, P = 6 s, Y = 500, R = 10, H = 8, L = 14,400, D = 19,200, U = 5, K = 16, O = 8, Q = 80, I = 16, J = 8, T = 128, W_G = 4,104, W_R = 48 KiB, W_B = 13,791,360, W_C = 4,000,000, G_A = 10^7, G_R = 5·10^9, G_T = 3.5·10^9, G_I = 5·10^7, A = 8 s, F = 2  
   整組與附錄 I 相符，關鍵在 0.8.0 已把 V 移出常數表、驗證者數改由 𝕍 決定而門檻一律取 |κ|。
2. ❌ C = 1023 cores with V = 341 validators still fixed by the protocol, E = 3,600 slots/epoch, P = 6 s, Y = 600, R = 60, H = 8, L = 14,400, D = 19,200, U = 5, K = 16, O = 8, Q = 80, I = 16, J = 8, T = 128, W_G = 4,104, W_R = 48 KiB, W_B = 13,791,360, W_C = 4,000,000, G_A = 10^7, G_R = 5·10^9, G_T = 3.5·10^9, G_I = 5·10^7, A = 8 s, F = 2  
   core 數與 validator 數對調了；E = 600（正好一小時）、Y 必須小於 E、rotation R = 10。
3. ❌ C = 341 cores with |κ| = 3C = 1023 fixed for every configuration, E = 600 slots/epoch, P = 12 s, Y = 300, R = 10, H = 24, L = 600, D = 19,200, U = 5, K = 16, O = 8, Q = 80, I = 16, J = 8, T = 128, W_G = 4,104, W_R = 48 KiB, W_B = 13,791,360, W_C = 4,000,000, G_A = 10^7, G_R = 5·10^9, G_T = 3.5·10^9, G_I = 5·10^7, A = 6 s, F = 3  
   GP 全文以 six-second timeslot 為前提；H = 8、L = 14,400、A = 8 s、F = 2，且 |κ| 已非固定。
4. ❌ C = 341 cores (|κ| = 3C = 1023 in the full configuration), E = 600 slots/epoch, P = 6 s, Y = 500, R = 10, H = 8, L = 14,400, D = 14,400 (identical to L), U = 5, K = 3, O = 80, Q = 8, I = 8, J = 16, T = 16, W_G = 4,096, W_R = 12 MB, W_B = 48 KiB, W_C = 64,000, G_A = 5·10^9, G_R = 10^7, G_T = 3.5·10^9, G_I = 5·10^7, A = 8 s, F = 2  
   多組常數被對調或寫錯：D = L + 4,800、O = 8 / Q = 80、I = 16 / J = 8、W_G = 4,104、W_C = 4,000,000。

> **陷阱**　tiny（test vectors）：V 6、C 2、E 12、Y 10、R 4、D 32、K 3→(0.8.0 由公式)、N ⌈2E/V⌉。

<sub>`arch-constants`</sub>

---

### ARCH-8　According to the GP's 'Previous Work' analysis, what are the main limitations of Polkadot 1.0 and of Ethereum-style rollups that JAM tries to overcome?

<sub>2 Previous Work — ●●○ · 設計理由 · §2</sub>

**標準答案**　Polkadot: parachains are isolated ecosystems with asynchronous, coarse-grained XCMP and access limited to ~50 auction slots; rollups: heterogeneous security/economics, SNARK proving is orders of magnitude too costly (RISC-Zero ~61,000× slower than native) and sequencers centralize — JAM keeps Polkadot's ELVES machinery but makes cores un-opinionated, permissionless and semi-coherent

§2：Polkadot「a collection of independent ecosystems with only limited opportunity for collaboration」，XCMP「asynchronous, coarse-grained and practically limited by its reliance on a high-level slowly evolving interaction language」，slot 數約 50——這是 accessibility 的限制，不是安全上限。Ethereum rollup 市場則是「heterogeneous communication… security… and economic properties」；SNARK：RISC-Zero「over 61,000 times as long as simply recompiling and executing」、成本 66,000,000×，即使 50,000× 也「several orders of magnitude greater than would be required to compete on a cost-basis with established crypto-economic techniques」；Solana 式全同步：多次停機、512 GB RAM、依賴中心化資料庫。JAM 的回答是保留（co-opt）ELVES——GP 評它為三種跨網路安全方案中最安全、經濟效率最高者——並把 core 變成 un-opinionated、permissionless、semi-coherent 的通用計算資源。

**逐項辨析**

1. ✅ Polkadot: parachains are isolated ecosystems with asynchronous, coarse-grained XCMP and access limited to ~50 auction slots; rollups: heterogeneous security/economics, SNARK proving is orders of magnitude too costly (RISC-Zero ~61,000× slower than native) and sequencers centralize — JAM keeps Polkadot's ELVES machinery but makes cores un-opinionated, permissionless and semi-coherent  
   四點都出自 §2：XCMP 非同步且粗粒度、約 50 個 slot、RISC-Zero 慢 61,000 倍，JAM 則 co-opt ELVES。
2. ❌ Polkadot: its weakness is that all parachains share one validator set, so security is capped by a single stake pool and XCMP must stay synchronous to be safe; rollups: proving costs are already within reach of crypto-economic verification and only sequencer uptime is unsolved — JAM therefore drops the ELVES audit game for SNARK-verified cores and gives each service its own validator subset  
   GP 拿單一 validator set 對比 Cosmos 的 no homogenous security；XCMP 被明寫為 asynchronous，JAM 也保留 ELVES。
3. ❌ Polkadot: parachains are isolated and XCMP is asynchronous and coarse-grained; rollups: their fragmentation is benign because Ethereum's validator set gives every roll-up identical communication, security and economic properties, and the one real obstacle is proving the EVM — JAM answers by fixing parachain validation as the single function a core may run, with coretime still auctioned in long-term slots  
   §2 說 rollup 的性質「may differ, potentially quite dramatically」；core 不預設用途，長期拍賣正是要消除的門檻。
4. ❌ Polkadot: it cannot safely host more than ~50 parachains because the ELVES audit game is the bottleneck; rollups: proof generation is already cheap enough, but Dank-sharding's erasure coding over a binary field with Merklization is what centralizes them — JAM answers by adopting KZG polynomial-commitment availability and replacing auditing with optimistic fraud proofs  
   兩邊對調：Dank-sharding 用大質數域的 polynomial commitment，binary field 加 Merklization 才是 JAM 採用的方案。

> **陷阱**　Kian 的說法：「What was once called an L2/parachain is now called a service」。

<sub>`arch-jam-vs-polkadot-eth`</sub>

---

### ARCH-9　Your implementation targets GP 0.7.2 while the current GP is 0.8.0. Which list contains ONLY genuine 0.8.0 changes?

<sub>GP 0.7.2 → 0.8.0 changes — ●●○ · 版本差異 · graypaper releases v0.8.0 (June 3 2026) · ⚠ 0.7.2→0.8.0</sub>

**標準答案**　Variable validator-set sizes (multiples of 3, 6…1023, thresholds derived from |κ|); full guarantees kept in ρ; hard caps of 16 verdicts/culprits/faults, with culprits no longer required behind a bad verdict; bless restricted to the manager service; authorizer = H(auth code hash ⌢ config); refinement context gains anchor slot + lookup-anchor posterior root; per-basic-block gas model with grow_heap replacing sbrk; tickets per validator = ⌈2E/|γ′_P|⌉

0.8.0（2026-06-03）的實際變更清單：#514 support smaller validator sets（𝕍 ≡ {3c}，門檻一律由 |κ| 導出）、#494 keep full guarantees in ρ、#525 dispute extrinsic 硬上限 16 且移除 culprits 必要性、#519 restrict bless to manager service、#522 update authorizer identification（authorizer = H(auth code hash ⌢ config)，對齊 ch. 8 與 ch. 14）、#526 expose lookup anchor posterior root and anchor slot、#508 新 gas cost model（per-basic-block，模擬 ROB/execution units）與 sbrk → grow_heap、#517 host function gas costs、#527 ticket 常數改為 ⌈2E/|γ′_P|⌉、#524 extrinsic hash、#502 processed transfer count、#497 illegal memory access 收費、#520/#521/#528。0.7.2 與 0.8.0 之間沒有任何 release，所以判準就是上面這份 PR 清單：凡不在其中的條目，都屬於更早的 release。你們 issue #1012（Update to v0.8.0）與其子 issue #1013–#1022 就是這張清單的實作追蹤。

**逐項辨析**

1. ✅ Variable validator-set sizes (multiples of 3, 6…1023, thresholds derived from |κ|); full guarantees kept in ρ; hard caps of 16 verdicts/culprits/faults, with culprits no longer required behind a bad verdict; bless restricted to the manager service; authorizer = H(auth code hash ⌢ config); refinement context gains anchor slot + lookup-anchor posterior root; per-basic-block gas model with grow_heap replacing sbrk; tickets per validator = ⌈2E/|γ′_P|⌉  
   整份清單都能對回 0.8.0 的 PR：#514、#494、#525、#519、#522、#526、#508、#527。
2. ❌ The 'Macrofication Marathon' moving every variable-length item to the end of its encoding; fixed-length validator-index serialization; W* made to depend on ρ†; accumulate absorbing on_transfer so a service has one on-chain entry point; the core index added to refine's arguments and removed from the guarantee payload; small service IDs; a version byte prefixing account serialization; 'Owned Privileges' for χ  
   Macrofication Marathon、變長欄位後移、W* 依賴 ρ† 屬 0.7.0；on_transfer 併入與 small service IDs 屬 0.7.1。
3. ❌ fetch replacing the import host call; memory-access exceptions formalized and a feature freeze declared; assurances checked with the prior validator set; Ed25519 keys in the epoch marker alongside activity statistics; the provide host call, with imports separated from exports; 'super-fetch', gratis storage and account metadata; maximum code sizes with oversize reports handled; stricter opcode and jump validity  
   fetch 取代 import 與 feature freeze 屬 0.6.0，其餘散落在 0.6.4、0.6.5、0.6.6、0.6.7。
4. ❌ 64-bit PVM registers and 64-bit addressing throughout; guarantor assignments carrying full validator keys; erasure-coding equation fixes; the info host call gaining registers 9 and 10; explicit per-invocation out-of-gas checks; the transfer gas-charge fix and the work-package size-limit fix; preimage-integration simplification; the posterior state root finalised; a metadata prefix for code blobs  
   64-bit registers 屬 0.5.0；info 的 register 9/10、transfer gas fix 等屬 0.7.2，正是你們現在的版本。

> **陷阱**　面試官依「最新 GP」出題（T&C 3.5：conformance assessed against the latest release），所以 0.8.0 差異必須熟。

<sub>`delta-summary-080`</sub>

---

### ARCH-10　Which values describe the 'tiny' test-vector configuration used by the W3F vectors and the conformance fuzzer?

<sub>Test vectors tiny configuration — ●○○ · 概念 · w3f/jamtestvectors README & JIP-4</sub>

**標準答案**　6 validators, 2 cores, epoch 12 slots, ticket submission ends at slot 10, rotation period 4, preimage expunge period 32, super-majority 5 of 6, verdict thresholds 5/0/2, ring size 6

jamtestvectors README（tiny）：num_validators 6、num_cores 2、epoch_duration 12、contest_duration（Y）10、rotation_period 4、preimage_expunge_period 32、tickets_per_validator 3（0.7.x；0.8.0 由 ⌈2E/V⌉ = 4）、max_tickets_per_extrinsic 3、num_ec_pieces_per_segment 1026；其他常數同 GP。門檻一律從 |κ| 現算：super-majority ⌊2·6/3⌋+1 = 5、wonky 門檻 ⌊6/3⌋ = 2；assurance bitfield 1 byte；Bandersnatch ring 大小 6（Zcash SRS）。對照 full 配置則是 1023 validators / 341 cores / epoch 600、expunge 19,200、super-majority 683、wonky 341。JIP-4 chainspec 的 protocol_parameters 就是用 fetch(0) 的編碼傳遞這些參數；你們 chainspec.ApplyProtocolParameters 會驗證編譯期常數必須一致。

**逐項辨析**

1. ✅ 6 validators, 2 cores, epoch 12 slots, ticket submission ends at slot 10, rotation period 4, preimage expunge period 32, super-majority 5 of 6, verdict thresholds 5/0/2, ring size 6  
   6/2/12、Y = 10、rotation 4、expunge 32、super-majority 5 of 6，全部對上 jamtestvectors 的 tiny。
2. ❌ 6 validators, 2 cores, epoch 12 slots, ticket submission ends at slot 12, rotation period 4, preimage expunge period 32, super-majority 4 of 6, verdict thresholds 4/0/2, ring size 6  
   Y 必須小於 E，tiny 是 Y = 10 < E = 12；super-majority 是 ⌊2·6/3⌋ + 1 = 5 不是 4，verdict 門檻也跟著錯。
3. ❌ 12 validators, 4 cores, epoch 60 slots, ticket submission ends at slot 50, rotation period 10, preimage expunge period 64, super-majority 9 of 12, verdict thresholds 9/0/4, ring size 12  
   雖然仍滿足 |κ| = 3C，但不是任何一份 W3F 設定的值——tiny 就是 6 個 validator、2 個 core。
4. ❌ 1023 validators, 341 cores, epoch 600 slots, ticket submission ends at slot 500, rotation period 10, preimage expunge period 19,200, super-majority 683 of 1023, verdict thresholds 683/0/341, ring size 1023  
   這其實是 full 配置的數字；tiny 改動的遠不只 gas limit，連 ring 大小與 assurance bitfield 寬度都不同。

> **陷阱**　tiny 的 Y = 10 < E = 12，所以每個 epoch 只有 slot 10、11 兩個 tail slot。

<sub>`arch-tiny-config`</sub>

---

### ARCH-11　Per the Beefy Distribution chapter, what does a validator sign, with which key, and when — and what is the signature for?

<sub>18 Beefy Distribution (ch. 18) — ●●○ · 概念 · eq. 18.1–18.2; eq. 7.7–7.8; §19–20</sub>

**標準答案**　After importing each finalized block: a BLS12-381 signature under κ′[v]'s BLS key over $jam_beefy ⌢ b, where b is the Keccak MMR super-peak of the accumulation-output belt held in the newest β_H entry; the signatures are published freely so they can be aggregated into concise finality proofs for third-party systems such as bridges
...
1. ✅ It makes a Bandersnatch VRF signature s_0 over context X_U ⌢ Y(H_V) with an empty message, then Fisher-Yates shuffles the per-core sequence q using Y(s_0) and keeps the non-empty entries among the first ten  
   eq. 17.3：a_0 = {w | w ∈ F(q, Y(s_0))[..10], w ≠ ∅}，先洗牌取前十、再濾掉 ∅。
2. ❌ It audits every report that became available in the block: q holds one entry per active core, so the initial tranche is simply all of its non-empty entries and no verifiable randomness is involved  
   全部都審等於退回 everybody-does-everything，也用不上任何可驗證亂數。
3. ❌ It audits the reports on the cores it guaranteed in the previous rotation, reusing the guarantor assignment G* of that rotation rather than drawing any fresh randomness for the tranche  
   guarantor 正是被審的一方，且 G* 公開可算，攻擊者能事先知道誰審什麼。
4. ❌ It signs the block hash with its Ed25519 key and audits those cores whose index matches the low bits of that signature, taking as many cores as the audit bias factor F = 2 permits  
   用的是 Bandersnatch VRF 而非 Ed25519 簽名；F = 2 只用在 n > 0 的後續 tranche 判準。

> **陷阱**　10 個 core 是 shuffle 後取前 10；q 的長度是 active cores = |κ|/3，不是 C = 341。

<sub>`arch-audit-initial-tranche`</sub>

---

### ARCH-22　GP §17.1 describes what happens when a negative judgment appears. Which pair of thresholds and consequences is correct?

<sub>17.1 Overview — ●●○ · 概念 · §17.1</sub>

**標準答案**　If more than 2/3 of validators still issue POSITIVE judgments, those issuing negative judgments may be punished for time-wasting; if more than 1/3 issue NEGATIVE judgments, the block containing the report is ban-listed and it and all its descendants are disregarded

§17.1：「if greater than 2/3 of the validators still issue positive judgments, then validators issuing negative judgments may receive a punishment for time-wasting. If greater than 1/3 of the validators issue negative judgments, then the block which includes the work-report is ban-listed. It and all its descendants are disregarded and may not be built on.」兩個門檻方向相反：2/3 正面是用來懲罰亂投反對票的人（讓惡意負面判決有代價），1/3 負面則丟掉整條分支。最後由 block author 把足夠的票組成 verdict 放進 disputes extrinsic（§10）——「once there are enough votes, a verdict can be constructed by a block author and placed in a disputes extrinsic」；§10 eq. 10.12 的正面票數只接受 ⌊2/3·|k|⌋ + 1（good）、0（bad）、⌊1/3·|k|⌋（wonky）三個值，而 punish-set ψ_O 同時收 culprits（保證了無效 report 的 guarantor）與 faults（判決與 verdict 相牴觸的簽署者）。另外：一個 report 的所有宣告都被正面判決匹配時即為 audited；一個區塊的所有新 available report 都 audited 時該區塊為 audited——這是 finalize 的前提之一（§19）。

**逐項辨析**

1. ✅ If more than 2/3 of validators still issue POSITIVE judgments, those issuing negative judgments may be punished for time-wasting; if more than 1/3 issue NEGATIVE judgments, the block containing the report is ban-listed and it and all its descendants are disregarded  
   兩個門檻都對上 §17.1：2/3 正面罰浪費時間者、1/3 負面 ban-list 該塊與其後代。
2. ❌ If more than 1/2 of validators issue NEGATIVE judgments, the report is dropped from its availability assignment; if fewer do, nothing happens at all — no validator is ever punished for a false negative and the block containing the report stays buildable  
   門檻是 1/3 不是 1/2；被 ban-list 的區塊「may not be built on」，反對者也確實會被罰。
3. ❌ A single NEGATIVE judgment suffices: it invalidates the block that carried the report and slashes all three guarantors of that core; no further validator need judge, no verdict reaches the disputes extrinsic, and the 2/3 positive threshold plays no part  
   verdict 需 ⌊2/3·|k|⌋ + 1 票，單獨一票既構不成 verdict，也不會登錄任何 culprit。
4. ❌ Negative judgments carry no consequence of their own; only the disputes extrinsic matters, a verdict there needs the unanimous signature of the whole active validator set, and the punish-set ψ_O fills only from culprits and never from faults  
   eq. 10.12 的票數從來不是 unanimity；ψ_O 除 culprits 外也收 faults 這種 offence。

> **陷阱**　「1/3 負面 → 整條鏈分支被丟棄」是 best-chain 的 disregard 規則；2/3 正面 → 反對者被罰。

<sub>`arch-audit-outcomes`</sub>

---

### ARCH-23　How does an auditor obtain the data it needs to re-run a work-report?
...
**σ**　state（整體狀態）　<sub>eq. 4.4</sub>

JAM 的完整狀態，拆成 17 個大致獨立的元件。

σ ≡ (α, β, θ, γ, δ, η, ι, κ, λ, ρ, τ, φ, χ, ψ, π, ω, ξ)。拆成獨立元件有兩個目的：一是讓 state-transition 的依賴圖夠淺、可以平行計算（§4.2.1）；二是讓 Merklization 的 key 空間可以用單一 byte 的 chapter 前綴分區（附錄 D 的 C(i)）。σ′ 代表 posterior（本區塊處理完之後）的狀態。

<sub>相關：σ′、C(i)、STF</sub>

**α**　authorizer pool（授權池）　<sub>eq. 8.1–8.3</sub>

每個 core 目前可用的 authorizer hash 序列（最多 O = 8 個）。

α ∈ [[H]_{:O}]_C。guarantee 上鏈時檢查 w_a ∈ α[w_c]（用 **prior** α）。每個區塊每個 core 都會從 queue 補一個進來：α′[c] = ←(F(c) ⌢ φ′[c][H_T mod Q])^O，其中 F(c) 會把本塊用掉的 authorizer 從**最左邊**移除一個（seqminusl，不是移除全部）。因為要用 posterior 的 φ′，這一步必須排在 accumulation 之後。

<sub>相關：φ、authorizer、O、Q</sub>

**β**　recent history（近期歷史）　<sub>eq. 7.1–7.8</sub>

最近 H = 8 個區塊的摘要（β_H）加上 accumulation output 的 MMR（β_B）。

β ≡ (β_H, β_B)。β_H 每筆是 (header hash, state root, accumulation-output super-peak, timeslot, 本塊回報的 package hash → segment root)。用途：擋掉重複或過期的 work-report——guarantee 的 anchor 必須對上 β† 的某一筆，package hash 不得出現在 β 的任何一筆裡。β_B 是 Keccak MMR，接收每塊的 accumulation output root。

<sub>相關：β†、β_B、anchor、H (常數)</sub>

**β†**　beta dagger（補正後的近期歷史）　<sub>eq. 7.5</sub>

把 β_H 最後一筆的 state root 用本塊的 H_R 補上之後的中間值。

因為 header 帶的是 **prior** state root，寫入 β 的當下還不知道自己的 posterior root，所以先填零 hash；下一個區塊一開始就用自己的 H_R 覆蓋回去（β† ≡ β_H except β†[|β_H|−1]_s = H_R）。之後 guarantee 的 anchor 檢查比對的是 β†，不是 β。

<sub>相關：β、H_R、pipelining</sub>

**γ**　Safrole state（Safrole 狀態）　<sub>eq. 6.3–6.5</sub>

出塊抽籤相關的四元組 (γ_P, γ_Z, γ_S, γ_A)。

γ_P 下個 epoch 的 pending validator keys（每個 epoch 開頭從 ι 重置）；γ_Z 由 γ_P 的 Bandersnatch key 組成的 ring root，用來驗 ticket 的 ring proof；γ_S 本 epoch 的 slot-sealer 序列（E 張 ticket 或 fallback 的 E 把 key）；γ_A ticket accumulator，收集下個 epoch 要用的 ticket（最多 E 筆，留最低的 E 個 id）。

<sub>相關：γ_S、γ_A、γ_Z、ι、κ</sub>

**γ_S**　slot-sealer sequence（時槽封印序列）　<sub>eq. 6.25–6.27</sub>

本 epoch 每個 slot 由誰出塊：600 張 ticket，或 fallback 的 600 把公鑰。

三種情況（eq. 6.25）：e′ = e + 1 ∧ m ≥ Y ∧ |γ_A| = E → Z(γ_A)（outside-in 排序的 ticket）；e′ = e → 不變；其餘（跳過 epoch、accumulator 未滿、比賽未結束）→ F(η′_2, κ′) 的 fallback key。出塊者用 γ_S[H_T mod E] 對應的私鑰簽 seal。

<sub>相關：γ_A、Z、F、seal、T (ticketed)</sub>

**γ_A**　ticket accumulator（票券累積器）　<sub>eq. 6.35</sub>

本 epoch 收集到的 ticket，供下個 epoch 排序使用。

γ′_A = 把新 ticket 與舊 accumulator 依 id 排序後取**最低**的 E 個（→^E）。epoch 換屆時歸零重收。提交的 ticket 必須全部留在 γ′_A 裡（否則 extrinsic 無效），且 id 不得與既有的重複。id 越小越好——這是「score」的意思。

<sub>相關：γ_S、ticket、E (常數)、Y</sub>

**δ**　service accounts（服務帳戶）　<sub>eq. 9.1–9.3</sub>

從 service index 到 service account 的字典，JAM 版的「所有帳戶」。

δ ∈ D⟨N_S → A⟩。每個 account 有 storage、preimages、requests、gratis、code hash、balance、兩個最低 gas、created/lastacc/parent。沒有 nonce（沒有交易就沒有重放問題）。中間狀態：δ† accumulation 之後、δ‡ 更新 lastacc 之後、δ′ 整合 preimage 之後。

<sub>相關：δ†、δ‡、service、a_t</sub>

**η**　entropy pool（熵累積器）　<sub>eq. 6.22–6.24</sub>

四個 32-byte 的隨機值：η_0 每塊更新，η_1..3 是前三個 epoch 結束時的快照。

η′_0 = H(η_0 ⌢ Y(H_V))——用 prior η_0 與 header VRF 的**輸出**。epoch 換屆時 (η′_1, η′_2, η′_3) = (η_0, η_1, η_2)。用途分工：提交 ticket 與 fallback、guarantor 分派用 η′_2；驗 seal 用 η′_3（就是晚一個 epoch 的同一個值）；`fetch` 給 accumulate 的是 η′_0。

<sub>相關：H_V、seal、guarantor assignment</sub>

**ι κ λ**　validator key sets（驗證者金鑰集合）　<sub>eq. 6.7, 6.14</sub>

ι 待命（staging）、κ 現行（active）、λ 上一個 epoch。

epoch 換屆時 (γ′_P, κ′, λ′) = (Φ(ι), γ_P, κ)——所以 ι 要經過兩個 epoch 邊界才會成為 active（ι → γ_P → κ）。Φ 把 ψ′_O（posterior offenders）裡的 key 換成全零。ι 由具 delegator 權限的 service 透過 `designate` 設定。每個 key 是 336 octets：Bandersnatch 32 | Ed25519 32 | BLS 144 | metadata 128。

<sub>相關：γ_P、Φ、designate、ψ_O</sub>

**ρ**　availability assignments（可用性指派）　<sub>eq. 11.1</sub>

每個 core 目前掛著、等待 availability 的 guarantee 與它進鏈的 slot。

ρ ∈ [(g ∈ 𝔾, t ∈ N_T)?]_C。0.8.0 起存的是**整個 guarantee**（含 2–3 個 guarantor 簽章），理由是要能直接向 guarantor 抓 bundle，以及在 dispute 時用這些簽章組出 culprits。中間狀態：ρ†（disputes 清掉 bad/wonky）→ ρ‡（assurances 讓它 available 或 timeout）→ ρ′（新 guarantee 進來，timestamp 記 τ′）。

<sub>相關：ρ†、ρ‡、guarantee、U</sub>

**τ**　timeslot（時槽）　<sub>eq. 6.1</sub>

最近一個區塊的 slot index；τ′ = H_T。

存 τ 是為了能算出 epoch index 與 slot phase：e ⌢ m = τ / E，e′ ⌢ m′ = τ′ / E。很多規則都取決於「prior 的 m」還是「posterior 的 m′」——例如 γ_S 的第一種情況要求 m ≥ Y（prior），ticket extrinsic 的上限則看 m′ < Y。τ ≥ τ′ 直接是無效區塊。

<sub>相關：epoch、H_T、Y</sub>

**φ**　authorizer queue（授權佇列）　<sub>eq. 8.1</sub>

每個 core 固定 Q = 80 筆的 authorizer 佇列，每個 slot 補一筆進 pool。

φ ∈ [[H]_Q]_C。只有該 core 的 assigner（χ_A[c]）能在 accumulate 裡用 `assign` 改它。補進 pool 的是 φ′[c][H_T mod Q]——注意用的是 **posterior** φ′，所以 pool 更新排在 accumulation 之後。（別跟 PVM 的暫存器 φ 搞混，那是另一個同形符號。）

<sub>相關：α、assign、χ_A</sub>

**χ**　privileges（特權服務）　<sub>eq. 9.9–9.10</sub>

五種特權：manager、delegator、registrar、每個 core 的 assigner、always-accumulate 集合。

χ ≡ (χ_M, χ_V, χ_R, χ_A, χ_Z)。χ_M 可改整個 χ 並發放 gratis storage（0.8.0 起 `bless` 只有 manager 能呼叫）；χ_V 可設 ι；χ_R 可建立 index < S = 2^16 的保留 service；χ_A[c] 可改 φ[c]；χ_Z 是每塊都會被 accumulate 的 service 與其免費 gas。

<sub>相關：bless、assign、designate、χ_Z</sub>

**ψ**　disputes state（爭議狀態）　<sub>eq. 10.1</sub>

good / bad / wonky 三個 report hash 集合，加上 offenders 的 Ed25519 key 集合。

ψ ≡ (ψ_G, ψ_B, ψ_W, ψ_O)。已經判過的 report hash 不能再被判一次（verdict 的 hash 必須與三個集合不相交）。ψ_O 只增不減，且在下次 key rotation 時讓 Φ 把對應的 validator key 清零；JAM 自己不做 slashing，只留紀錄給上層 staking 系統。

<sub>相關：verdict、culprit、fault、Φ</sub>

**π**　activity statistics（活動統計）　<sub>eq. 13.1</sub>

π_V 本 epoch、π_L 上一個 epoch 的 validator 統計，加上 per-core 與 per-service 的本塊統計。

π ≡ (π_V, π_L, π_C, π_S)。validator 每人六個計數器：b 出塊、t 帶進的 ticket 數、p preimage 數、d preimage bytes（這四個都記在**作者**身上）、g guarantee（記在有簽名的 guarantor）、a assurance（記在簽 assurance 的人）。π_C/π_S 是 per-block 的，沒活動就不出現。JAM 只產生數據，獎勵計算交給上層。

<sub>相關：G (reporters)、π_C、π_S</sub>

**ω**　ready queue（待累積佇列）　<sub>eq. 12.3</sub>

已 available 但依賴未滿足、等著被 accumulate 的 report（長度 E 的環狀緩衝）。

ω ∈ [[(ℝ, {H})]]_E，每格存 (report, 尚未滿足的 package hash 集合)。每塊從 ω[m..] ⌢ ω[..m] 依序取（m = H_T mod E，即最舊優先），能解鎖的用 Q 函數挑出來，跳過的 slot 清空，超過一個 epoch 沒解開就被覆蓋掉。0.7.x 的文獻寫成 ϑ，0.8.0 已改名為 ω。

<sub>相關：ξ、R!、R*、Q 函數</sub>

**ξ**　accumulation history（累積歷史）　<sub>eq. 12.1</sub>

最近一個 epoch 內已被 accumulate 的 work-package hash（每個 slot 一組集合）。

ξ ∈ [{H}]_E。兩個用途：判斷 report 的 prerequisite 是否已滿足（依賴的 package 在 ξ 裡就算解鎖），以及擋掉重複的 package。每塊左移一格，最後一格放本塊剛 accumulate 的 package hash。

<sub>相關：ω、prerequisite、R*</sub>

**θ**　accumulation output log（累積輸出紀錄）　<sub>eq. 7.4, 12.24</sub>

本塊各 service 透過 `yield` 產出的 (service, hash) 序列。

θ′ 由 Δ+ 的輸出 b 而來，只收「有 yield 32-byte hash」的 service。它會被 M_B（Keccak）算出一個 root，append 進 β_B 這條 MMR，其 super-peak 就是 BEEFY 簽的東西——也就是 service 對外部系統（bridge）承諾狀態的管道。

<sub>相關：yield、β_B、BEEFY</sub>

**B**　block（區塊）　<sub>eq. 4.2</sub>

B ≡ (H, E)：一個 header 加一份 extrinsic。

JAM 是 transactionless 的：extrinsic 的五個部分全部由 validator 產生，沒有使用者簽名的交易。外部資料只能經由 work-package（in-core 的 refine）與 preimage 進入狀態。

<sub>相關：H (header)、E (extrinsic)</sub>

**E**　extrinsic（區塊外部資料）　<sub>eq. 4.3</sub>

E ≡ (E_T, E_D, E_P, E_A, E_G)：tickets、disputes、preimages、assurances、guarantees。

E_T 出塊抽籤的票（m′ < Y 時最多 K = 16 張）；E_D 爭議（verdicts + culprits + faults，各最多 16）；E_P preimage（依 (service, blob) 排序且唯一）；E_A availability assurance（每個 validator 至多一筆，依 index 遞增）；E_G work-report guarantee（每個 core 至多一筆，依 core 排序）。

<sub>相關：E_T、E_G、E_A、E_D、E_P</sub>

**H**　header（區塊標頭）　<sub>eq. 5.1</sub>

十個欄位：H_P, H_R, H_X, H_T, H_E, H_W, H_O, H_I, H_V, H_S。

parent hash、**prior** state root、extrinsic hash、timeslot、epoch marker、winning-tickets marker、offenders marker、作者 index、entropy VRF 簽章、seal。E_U(H) 是「不含 seal」的序列化，也就是 seal 簽的訊息。

<sub>相關：H_R、H_S、H_V、E_U(H)</sub>

**H_R**　prior state root（前態狀態根）　<sub>§5</sub>

header 帶的是**父區塊**的 posterior state root，不是自己的。

GP 明說理由是「to facilitate the pipelining of block computation and in particular of Merklization」——出塊者不必先把新狀態 Merklize 完才能發布區塊。代價是 β 最新一筆的 state root 要留白、由下一塊補（β†），以及 Grandpa 投票必須另外帶上 posterior state root。

<sub>相關：β†、Grandpa、pipelining</sub>

**H_S / H_V**　seal / entropy VRF signature（封印與熵簽章）　<sub>eq. 6.16–6.18</sub>

兩個 Bandersnatch VRF 簽章：一個證明出塊權，一個產生熵。

ticket 模式：Y(H_S) 必須等於該 slot 的 ticket id，context 是 X_T ⌢ η′_3 ++ i_e，訊息是 E_U(H)，並標記 T = 1。fallback 模式：γ_S[H_T mod E] 必須等於作者的 Bandersnatch key，context 改 X_F ⌢ η′_3，T = 0。兩種模式都要有 H_V：context X_E ⌢ Y(H_S)、訊息為空——訊息先被固定住，所以熵無法被出塊者操弄。

<sub>相關：γ_S、η、T (ticketed)、Bandersnatch</sub>

**H_X**　extrinsic hash（外部資料雜湊）　<sub>eq. 5.4–5.7</sub>

對五個 extrinsic 元件各自的承諾再取一次 hash。

0.8.0 (#524) 改成：preimage 部分承諾 (E_4(service), H(data)) 對，guarantee 部分承諾 (H(report), E_4(slot), credential)。目的是「allow for the possibility of reports and preimages to individually have their inclusion proven」——不必附上整個 blob 就能證明某筆在區塊裡。

<sub>相關：E_P、E_G、Merkle proof</sub>

**H_E / H_W / H_O**　epoch / winning-tickets / offenders markers（三個標記）　<sub>eq. 6.28–6.29, 10.19</sub>

讓只同步 header 的節點也能追上 validator 與 ticket 的變化。

H_E 只在 e′ > e 時出現，內容是 (η_0, η_1) 加上 γ′_P 每個 validator 的 (Bandersnatch, Ed25519) key。H_W 只在同一 epoch 內首次跨過 Y、且 accumulator 飽和時出現，內容是 Z(γ_A)。H_O 是本塊新增的 offender key（culprits 在前、faults 在後），可以是空序列。H_E 與 H_W 互斥。

<sub>相關：γ_P、γ_A、Z、offenders</sub>

### 流程與角色

****　in-core vs on-chain（核內／鏈上）　<sub>§4.9.1</sub>

in-core 只有被指派的少數 validator 執行；on-chain 是所有人都跑。

in-core（refine）由該 core 的 guarantor 執行，靠 guaranteeing → assuring → auditing →（必要時）judging 這套 crypto-economic 遊戲取信，因此算力可隨網路規模成長（GP 估計約單機的 300 倍）；on-chain（accumulate）是「everybody does everything」，受單機能力限制。兩者由同一批 validator 執行，安全性相當，差別在「可擴展 vs 同步」。

<sub>相關：refine、accumulate、ELVES</sub>

****　refine（精煉）　<sub>§4.9.2, eq. B.5</sub>

service 的 in-core 進入點：無狀態、吃大量輸入、吐出小的 digest。

Ψ_R 在 core 上執行，沒有一般的鏈上狀態可讀（只有 historical_lookup 能依 lookup-anchor 時點查 preimage），但可以建立 inner PVM、export segment 到 D³L。每個 work-item 一次 refine，gas 上限 G_R = 5×10^9。輸出（work result）與統計資料組成 work-digest。

<sub>相關：accumulate、work-item、Ψ_R、historical_lookup</sub>

****　accumulate（累積）　<sub>§12, eq. B.9</sub>

service 的 on-chain 進入點：有狀態、可轉帳、可建立 service。

Ψ_A 以 PC = 5 為進入點，參數只有 (timeslot, service id, input 數)，實際的 operand 與 transfer 用 `fetch`(14/15) 讀。每個 report 的 accumulate gas 上限 G_A = 10^7、每塊總量 G_T = 3.5×10^9。0.7.1 起 on_transfer 併入 accumulate——轉帳以 input 的形式在下一輪送達收款方。

<sub>相關：refine、Δ+、deferred transfer、yield</sub>

****　guarantor（保證人）　<sub>§11.3, §15</sub>

被指派到某個 core、負責執行 refine 並簽署 work-report 的 validator。

每個 active core 固定 3 個 guarantor，由 η′_2 對 [⌊i/3⌋] 做 Fisher–Yates shuffle、再依 rotation index ⌊(t mod E)/R⌋ 旋轉決定（R = 10 slots 換一次）。兩個簽章就足以把 report 送給出塊者。GP 建議「平均每個 slot 簽不超過兩份 report」，以免被出塊者當成 spam 忽略。

<sub>相關：M (assignments)、rotation、guarantee、R (常數)</sub>

****　assurer（擔保人）　<sub>§11.2, §16</sub>

宣告自己已持有某 core 的 erasure-coded shard 的 validator。

每個 slot 發一份 assurance：bitfield 每個 core 一 bit，anchor 必須是 H_P，用 prior κ 的 Ed25519 key 對 X_A ⌢ H(E(H_P, bitfield)) 簽名。要打勾必須同時握有兩類 shard：report bundle 的（審計用，審完可丟）與所有 exported segment 的（保存 28 天）。超過 2/3·|κ| 打勾，report 才算 available。

<sub>相關：assurance、erasure coding、D³L、R (available)</sub>

****　auditor（審計者）　<sub>§17</sub>

每個 validator 都要對隨機抽到的 report 重跑一次並公開判決。

初始 tranche：用 X_U ⌢ Y(H_V) 的 VRF 洗牌後取 10 個 core。之後每 8 秒（A）一個 tranche，遇到負面判決或 no-show（有人宣告卻沒交判決）就擴大，期望每個 no-show 多帶 F = 2 個審計者。審計要先把 bundle 從 1/3 的 validator 手上的 chunk 重建出來，再重跑 computereport 逐欄位比對。

<sub>相關：ELVES、no-show、tranche、A、F</sub>

****　ELVES（ELVES 機制）　<sub>§17 & eprint 2024/961</sub>

JAM 審計機制的理論基礎，源自 Polkadot 的 backing/approval/inclusion。

論文用語與 JAM 的對照：backing = guaranteeing、approval = auditing、inclusion = accumulation。核心想法是「非冗餘分割 + 提案與審計賽局」——不需要所有人重算，只要作弊被抓到的機率夠高、代價夠大即可。安全性假設 ≥ 2/3+1 的 validator 誠實且在線。

<sub>相關：auditor、guarantor、dispute</sub>

**R**　newly available reports（新可用報告）　<sub>eq. 11.17</sub>

本區塊因 assurance 達標而變成 available 的 report 序列。

R ≡ [ρ†[c] 的 report | Σ_a a_f[c] > 2/3·|κ|]。它接著被拆成 R!（沒有依賴、立刻累積）與 R^Q（有依賴、進 ω 排隊），合起來經 Q 函數得到本塊真正要 accumulate 的序列 R*。0.7.x 文獻寫作 W。注意別跟常數 R（rotation period = 10）或 rotation 函數 R(c, n) 搞混。

<sub>相關：ω、ξ、R*、assurance</sub>

**G**　reporters（報告者集合）　<sub>eq. 11.28</sub>

本塊 guarantee credential 裡出現過簽章的 validator 的 Ed25519 key 集合。

唯一用途是統計：π 的 g 計數器對每個 κ′[v] ∈ G 的 validator +1。0.7.x 文獻把它寫成 R，容易與「新可用報告 R」混淆——0.8.0 改成 G。

<sub>相關：π、guarantee、R (available)</sub>

### 資料結構

**P**　work-package（工作包）　<sub>eq. 14.2</sub>

⟨j, h, u, f, c, w⟩：授權 token、授權服務、授權 code hash、config、refinement context、work-items。

authorizer = H(u ⌢ f)。一個 package 最多 I = 16 個 work-item，整包（含 extrinsic 與 import）不得超過 W_B = 13,791,360 octets。package 由 builder 組裝、交給 guarantor；它本身不上鏈，上鏈的是它的 work-report。

<sub>相關：work-item、authorizer、work-report、W_B</sub>

****　work-item（工作項）　<sub>eq. 14.3</sub>

package 內的一個工作單位：指定 service、code hash、payload、兩個 gas 上限、imports、extrinsics、export 數。

每個 item 對應一次 refine 與（之後）一筆 work-digest。imports 以 (segment root 或 package hash, index) 指定要從 D³L 匯入的 segment；extrinsics 以 (hash, length) 指定隨包附帶的資料。宣告的 export 數若與實際 export 不符，結果會被換成 BADEXPORTS。

<sub>相關：work-package、work-digest、segment、refine</sub>

**ℝ**　work-report（工作報告）　<sub>eq. 11.2</sub>

(s, c, c, a, t, l, d, g)：avspec、refinement context、core、authorizer、auth trace、segment-root lookup、digests、auth gas。

這是 in-core 計算「上鏈」的形式：不含原始資料，只含承諾與結果。變動大小（auth trace + 所有成功輸出）受 W_R = 48 KiB 限制；digest 數 1..16；|segment-root lookup| + |prerequisites| ≤ J = 8。report 的 hash H(E(w)) 是 dispute 與 ρ 清除時的識別碼。

<sub>相關：work-digest、avspec、refinement context、W_R</sub>

**𝔻**　work-digest（工作摘要）　<sub>eq. 11.6</sub>

一個 work-item 的 refine 結果加上資源使用量。

欄位：service、code hash（必須與鏈上 δ[s]_c 相符）、payload hash、accumulate gas 上限、結果（blob 或錯誤）、實際用掉的 refine gas、import 數、extrinsic 數與大小、export 數。accumulate 時它會被轉成 operand tuple 餵給 service。

<sub>相關：work-report、operand tuple、𝔼</sub>

**𝔼**　work error set（工作錯誤集合）　<sub>eq. 11.7</sub>

{∞ OOG, ☇ panic, BADEXPORTS, OVERSIZE, BAD, BIG}。

∞ gas 用盡、☇ 非預期終止、BADEXPORTS 宣告的 export 數不符、OVERSIZE 輸出會撐破 report 的大小上限、BAD 在 lookup-anchor 的狀態下找不到 service code、BIG code 超過 W_C = 4,000,000。錯誤仍然會被 accumulate（operand 的結果就是錯誤值），只是沒有輸出 blob。注意 0.8.0 的 𝕁 是 segment 的集合，不是錯誤集合。

<sub>相關：work-digest、𝕁、W_C</sub>

**𝕐**　availability specification（可用性規格）　<sub>eq. 11.5</sub>

(p, l, u, v, e, n)：package hash、bundle 長度、erasure root、shard 數、segment root、segment 數。

u 是對所有 erasure chunk 的 Merkle root（審計與重建的依據）；v 必須等於 |κ′|，因為每個 assurer 拿一個 chunk；e 是對 exported segment 做的 constant-depth Merkle root，供之後的 package import 時驗證。容易記反的是 v（shards）與 n（segments）。

<sub>相關：erasure coding、segment、assurer</sub>

**ℂ**　refinement context（精煉語境）　<sub>eq. 11.4</sub>

(a, n, s, b, l, t, r, p)：anchor 四件組、lookup-anchor 三件組、prerequisites。

anchor（hash、slot、posterior state root、accumulation-output super-peak）必須對上 β† 的某一筆，也就是最多 8 塊以內；lookup-anchor（hash、slot、posterior state root）必須在最近 L = 14,400 slots 的 ancestor 裡找得到——這是少數無法只靠鏈上狀態驗證、必須保留 header 鏈的檢查。prerequisites 是本 report 依賴的 package hash。

<sub>相關：anchor、lookup anchor、β†、ancestors</sub>

****　authorizer / token / trace（授權者／憑證／軌跡）　<sub>§8.1</sub>

authorizer 是判斷「這個 package 能不能用這個 core」的邏輯；token 是隨包附上的憑證；trace 是授權成功的輸出。

authorizer 的身分是 H(auth code hash ⌢ config)。它在 in-core 執行（Ψ_I，gas 上限 G_I = 5×10^7），鏈上只檢查 authorizer hash 有沒有在該 core 的 pool 裡。這個設計把「誰買了 coretime」與「誰提交工作」拆開，因此同時支援 Ethereum 式與 Polkadot 式的使用模式。

<sub>相關：α、coretime、Ψ_I</sub>

**𝕁**　segment（分段）　<sub>eq. 14.1, §14.4</sub>

D³L 的固定大小資料單位，W_G = 4,104 octets。

refine 用 `export` 產生 segment、用 import 取回別的 package 之前 export 的 segment。每個 package 最多 3,072 個 import 與 3,072 個 export。segment 在 Audit DA 的額外足跡是 W_F = W_G + 32·⌈log₂ W_X⌉ = 4,488（多出來的是 justification 路徑）。segment 保存 28 天。

<sub>相關：D³L、export、W_G、W_F</sub>

****　D³L / DA（資料可用性層）　<sub>§16, §17.2</sub>

兩套資料可用性：audit DA（bundle，短期）與 import DA（segment，28 天）。

audit DA 讓審計者能重建 work-package bundle 重跑計算，審計完成即可丟棄；import DA（也就是 D³L，Distributed Data Lake）保存 exported segment 28 天，讓後續的 work-package 能 import 前人算出的資料，而不必重新塞進可用性系統——這是 JAM 相對於單純 rollup DA 的特色。

<sub>相關：segment、assurer、erasure coding</sub>

**𝔾**　guarantee（保證）　<sub>eq. 11.24–11.28</sub>

(work-report, slot, 2–3 個 (validator index, Ed25519 簽章))。

簽的是 X_G ⌢ H(E(report))。簽署者必須在該 slot 被指派到該 core：t 落在本 rotation 就用 M，落在前一個 rotation 就用 M*；視窗是 R·(⌊τ′/R⌋ − 1) ≤ t ≤ τ′。core 必須是 active（index < |κ′|/3），即使前一個 rotation 時它是 active 也一樣。credential 依 validator index 排序且唯一。

<sub>相關：guarantor、M、rotation、reporters</sub>

****　assurance（可用性擔保）　<sub>eq. 11.11–11.16</sub>

(anchor, bitfield, validator index, Ed25519 簽章)。

anchor 必須等於 H_P；bitfield 長度 C bits（full 是 43 bytes，LSB-first 打包）；只能對 ρ†[c] ≠ ∅ 的 core 打勾；整份 extrinsic 依 validator index 嚴格遞增。計票是**逐塊**的：狀態裡沒有跨塊累計器，所以必須某一塊單獨湊到 > 2/3·|κ|。

<sub>相關：assurer、R (available)、ρ‡</sub>

****　verdict / culprit / fault（判決／共犯／過失）　<sub>eq. 10.2–10.13</sub>

verdict 是一組判決票；culprit 是保證過壞 report 的人；fault 是投了與 verdict 相反票的人。

每個 verdict 恰好帶 ⌊2|k|/3⌋+1 張票（tiny 5、full 683），正票數只能是這個數（good）、0（bad）或 ⌊|k|/3⌋（wonky），其餘任何組合都讓區塊無效。verdict 的 epoch index 決定用 κ 還是 λ 驗簽。0.8.0 加了每份 extrinsic 最多 16 verdicts / 16 culprits / 16 faults，並取消了「bad verdict 必須附 ≥ 2 個 culprit」。

<sub>相關：ψ、ρ†、offenders</sub>

**𝕏**　deferred transfer（延遲轉帳）　<sub>eq. 12.14</sub>

(sender, dest, amount, 128-byte memo, gas)。

`transfer` 呼叫當下就從發送方扣款（並檢查扣完仍高於自身 threshold，否則 CASH），但接收方要到 Δ+ 的**下一輪**才會以 input 形式收到並入帳。gas 欄位是預付給接收方處理這筆轉帳的量，必須 ≥ 接收方的 a_m（否則 LOW）。轉給已被刪除的 service 會被丟棄。

<sub>相關：accumulate、transfer、Δ+</sub>

**𝕌**　operand tuple（運算元組）　<sub>eq. 12.13</sub>

accumulate 看到的每個 work-digest：(package hash, segment root, authorizer, payload hash, gas limit, auth trace, result)。

accumulate 不會拿到整個 work-report，而是這種抽出重點的元組；它們與收到的 deferred transfer 一起，透過 `fetch`(14/15) 讀入。同一個 service 在同一輪的所有 digest 會合併成一次 PVM 呼叫，以攤銷啟動成本。

<sub>相關：accumulate、work-digest、fetch</sub>

### PVM 與 host call

**PVM**　Polkadot Virtual Machine（Polkadot 虛擬機）　<sub>§4.7, 附錄 A</sub>

以 RISC-V RV64EM 為基礎的暫存器機：13 個 64-bit 暫存器、4 KiB 分頁、32-bit 定址。

選 RISC-V 的理由：可高效重編譯到 x86-64/ARM、有 LLVM 工具鏈、指令語意確定（沒有浮點等非決定性行為）、易於計量，且能自然支援暫停／續跑（CorePlay 需要）。相較 EVM 少了密碼學指令與環境互動指令——那些都走 host call。

<sub>相關：φ、host call、Ψ、gas</sub>

**φ**　registers（暫存器）　<sub>eq. A.47</sub>

13 個 64-bit 暫存器 φ_0..φ_12。

標準初始化：φ_0 = 2^32 − 2^16（return address；dynamic jump 到這裡就是 halt）、φ_1 = stack pointer、φ_7 = 參數位址、φ_8 = 參數長度，其餘為 0。host call 也用 φ_7 起算的暫存器傳參數與回傳值。（0.7.x 的文獻用 ω 表示暫存器，0.8.0 已改為 φ——ω 現在是 ready queue。）

<sub>相關：PVM、host call、standard init</sub>

**ϱ**　gas counter（gas 計數器）　<sub>§A.5, eq. A.54</sub>

以 basic block 為單位、事先扣除的計量單位。

0.8.0 起：進入一個 basic block（或跳回它的開頭）時，先把整個 block 的成本扣掉；不夠就以 ∞（out-of-gas）中止且**不扣款**。成本 ϱ^Δ = max(cycles − 3, 1)，cycles 來自一個模擬的亂序 CPU（32 條目 reorder buffer、每 cycle 4 decode / 5 start、ALU/LOAD/STORE/MUL/DIV 單元，記憶體延遲 𝔪 = 25）。0.7.x 是「每條指令 1 gas」。

<sub>相關：basic block、∞、gas model</sub>

**μ**　memory（記憶體）　<sub>eq. 4.24, A.9–A.10</sub>

2^32 個 octet，分成 Z_P = 4,096 octet 的頁，每頁可為 R、W 或不可存取。

低於 2^16 的位址一律直接 panic（null pointer 保護），其他不可存取的位址則產生 page fault，回報**最低**那個不可存取的頁位址（可以補頁後續跑）。標準初始化把 RO data 放在 Z_Z = 2^16、RW/heap 在 2Z_Z + Z(|o|)、stack 與參數放在高位址，每段之間留一個 64 KiB 的 guard zone。

<sub>相關：Z_P、Z_Z、page fault、standard init</sub>

**ε**　exit reason（結束原因）　<sub>eq. A.1</sub>

∎ halt、☇ panic、∞ out-of-gas、page fault × 位址、host call × id。

halt 與 panic 是最終狀態（回傳的 instruction counter 為 0）；其餘三種都可以「續跑」：補記憶體、補 gas、或執行 host call 後把 PC 前進 1 + skip。這個設計讓 refine 能建立 inner PVM 並代理它的 host call（CorePlay 的 pause/resume 也是靠它）。

<sub>相關：PVM、host call、page fault</sub>

**ϖ**　basic blocks（基本區塊）　<sub>§A.3</sub>

以 terminator 指令切分的程式片段，是 gas 計費與跳躍目標的單位。

terminator 包含 trap、fallthrough、jump、jump_ind、load_imm_jump(_ind) 與所有 branch_*。任何靜態跳躍、動態跳躍與 branch 的目標（包含 branch 不成立時的下一條）都必須是 basic block 的開頭，否則 panic。0.8.0 的 deblob 還要求程式最後一個 block 必須以 terminator 收尾。

<sub>相關：ϱ、djump、deblob</sub>

**Ψ**　PVM invocation functions（PVM 呼叫函數）　<sub>eq. A.1, A.38, B.1/B.5/B.9</sub>

Ψ 單純執行、Ψ_H 代理 host call、Ψ_M 帶參數與回傳、Ψ_I/Ψ_R/Ψ_A 是三種協定用途。

Ψ_I is-authorized（無狀態，只有 gas/grow_heap/fetch）；Ψ_R refine（多了 historical_lookup、export 與 inner PVM 那一組）；Ψ_A accumulate（多了 read/write/lookup/info 與所有會改狀態的 host call）。三者都以 Ψ_M 包裝，差別在 entry point 與 host-call 分派函數。

<sub>相關：host call、refine、accumulate</sub>

****　host call（宿主呼叫）　<sub>附錄 B</sub>

PVM 透過 `ecalli` 呼叫外界的介面，0.8.0 編號 0–27。

gas 0、grow_heap 1、fetch 2、lookup 3、read 4、write 5、info 6、historical_lookup 7、export 8、machine 9、peek 10、poke 11、pages 12、invoke 13、expunge 14、bless 15、assign 16、designate 17、checkpoint 18、new 19、upgrade 20、transfer 21、eject 22、query 23、solicit 24、forget 25、yield 26、provide 27；JIP-1 另定義 log = 100。未知編號扣 M_∅ = 1000 gas 並回傳 WHAT。0.8.0 因為插入 grow_heap，整組編號比 0.7.2 位移了一格。

<sub>相關：Ψ、WHAT、gas cost</sub>

****　host-call result codes（宿主呼叫回傳碼）　<sub>§B.1</sub>

OK = 0；錯誤碼從 2^64−1 往下數。

NONE 2^64−1（項目不存在）、WHAT −2（名稱未知）、OOB −3（inner PVM 記憶體不可存取）、WHO −4（index 未知）、FULL −5（空間不足或已配置）、CORE −6、CASH −7（餘額不足）、LOW −8（gas 太低）、HUH −9（操作不合法或權限不足）。inner PVM 另有 HALT 0 / PANIC 1 / FAULT 2 / HOST 3 / OOG 4。常見混淆：WHO 是「找不到」，HUH 是「不能這樣做」。

<sub>相關：host call、WHO、HUH</sub>

****　fetch（fetch 宿主呼叫）　<sub>§B.5 (index 2)</sub>

三種 invocation 共用的萬用讀取介面，用 φ_10 選資料。

selector 0 是協定常數的編碼（JIP-4 chainspec 的 protocol_parameters 用同一格式）、1 是 η′_0、2 auth trace、3–6 extrinsic 與 import segment、7–13 package/config/token/context/item 摘要/payload、14–15 accumulate 的 inputs。共通模式是「把資料的 [offset, offset+length) 寫進記憶體、回傳資料的完整長度」，不可用時回 NONE。

<sub>相關：host call、accumulate、refine</sub>

****　checkpoint / yield（checkpoint 與 yield）　<sub>§B.7 (18, 26)</sub>

checkpoint 存檔（panic/OOG 時採用），yield 提交 32-byte 的 accumulation 輸出。

accumulate 的 context 是一對 (x, y)：x 是正常維度、y 是例外維度，只有 `checkpoint` 會把 x 複製進 y。若程式以 ☇ 或 ∞ 結束，collapse 函數採用 y——也就是最後一次 checkpoint 的狀態；正常結束才用 x。yield 寫下的 hash 會進 θ、再進 β_B 的 MMR，最後被 BEEFY 簽名。

<sub>相關：accumulate、θ、BEEFY</sub>

### 密碼學

**H**　Blake2b（Blake2b 雜湊）　<sub>§3.8</sub>

GP 的預設雜湊函數，輸出 32 octets。

state trie、extrinsic hash、report hash、entropy、state key 等絕大多數地方都用 H。例外是 accumulation-output belt 與 BEEFY 相關的 H_K（Keccak）——那是為了讓 EVM 這類「legacy systems」能便宜地驗證。

<sub>相關：H_K、state trie、BEEFY</sub>

**H_K**　Keccak（Keccak 雜湊）　<sub>§7, §18</sub>

只用在 accumulation output 的 Merkle root、MMR 與 BEEFY 承諾。

GP 原文：「Throughout, the Keccak hash function is used to maximize compatibility with legacy systems」。實務意義：橋接合約可以用 EVM 便宜的 keccak256 precompile 驗證 JAM 的 accumulation output 證明。

<sub>相關：H、β_B、BEEFY</sub>

****　Bandersnatch VRF（Bandersnatch VRF）　<sub>附錄 G</sub>

建構在 BLS12-381 純量域上的曲線，JAM 用它做 seal、entropy 與 ring proof。

兩種用法：(1) 一般（IETF 式）VRF——seal H_S、entropy H_V、審計選擇 s_n 都是；(2) ring VRF——提交 ticket 時證明「我是 γ_Z 這個 ring 裡的某個人」而不揭露身分。VRF 的輸出 Y(·) 只取決於 context 與訊息，這是 ticket id 與 entropy 的來源。

<sub>相關：ticket、seal、γ_Z、ring root</sub>

****　signing contexts（簽章語境字串）　<sub>附錄 I</sub>

每種簽章都有專屬前綴，避免跨用途重放。

$jam_ticket_seal（X_T）、$jam_fallback_seal（X_F）、$jam_entropy（X_E）、$jam_available（X_A）、$jam_guarantee（X_G）、$jam_valid / $jam_invalid（判決）、$jam_audit（X_U）、$jam_announce、$jam_beefy。面試常考「這個簽章的 context 與訊息各是什麼」——例如 seal 的訊息是 E_U(H)，而 H_V 的訊息是空序列。

<sub>相關：seal、assurance、guarantee、BEEFY</sub>

****　erasure coding（抹除碼）　<sub>附錄 H</sub>

GF(2^16) 上的 systematic Reed–Solomon，任意 d(v) 個 chunk 即可還原。

d(v) = max{d ≤ v/3 + 1 : W_G mod 2d = 0}；v = 1023 → 342、v = 6 → 3。也就是「即使將近三分之二的 validator 惡意或離線也能重建」。systematic 的好處是前 d 個 chunk 就是原始資料，直接串接即可。每個 validator 每個 segment 拿到的是 12 octets（full 配置）。

<sub>相關：assurer、D³L、avspec</sub>

****　Fisher–Yates shuffle（洗牌函數）　<sub>附錄 F</sub>

以 32-byte 熵驅動的確定性洗牌，用於 guarantor 分派與審計選擇。

用 Q_l(h)[i] = decode_4(H(h ⌢ E_4(⌊i/8⌋))[4(i mod 8) …]) 產生亂數序列——一次雜湊供應 8 個 32-bit 數。取 r[i] mod 剩餘長度、輸出該元素、再把最後一個元素搬進空位。確定性是關鍵：所有節點必須算出同一個分派。

<sub>相關：guarantor、M (assignments)、auditor</sub>

### 編碼與 Merklization

**E**　codec（編碼函數）　<sub>附錄 C</sub>

JAM 自己的序列化：緊湊自然數 + 固定寬度整數 + 長度前綴序列。

E(0) = [0]；小於 128 的數是單一 byte；再大就用「前綴的高位放幾個 1 表示後面還有幾個 byte」的形式；≥ 2^56 用 0xFF + 8 bytes LE。↕x 表示需要長度前綴的變長項；x? 是 0 或 1 ⌢ x；dictionary 依 key 排序；bit sequence 以 LSB-first 打包。注意這與 SCALE 的 compact 不同（SCALE 用最低兩個 bit 當模式）。

<sub>相關：E_l、state serialization、SCALE</sub>

**E_l**　fixed-length encoding（固定長度編碼）　<sub>附錄 C, D</sub>

E_2/E_4/E_8：little-endian 的定寬整數。

狀態序列化裡「所有非 discriminator 的數值都用定寬」（附錄 D 的註）。區塊／extrinsic 則逐結構定義：timeslot E_4、**所有 validator index 都是 E_2**、service index E_4、gas E_8、長度前綴才用緊湊 E。

<sub>相關：E、state serialization</sub>

**C(i)**　state key constructor（狀態鍵建構子）　<sub>§D.1</sub>

把狀態元件或 service 資料對應到 31-octet 的 trie key。

三種形式：C(i) = [i, 0, …]（第 i 章的單一元件，1 α、2 φ、3 β、4 γ、5 ψ、6 η、7 ι、8 κ、9 λ、10 ρ、11 τ、12 χ、13 π、14 ω、15 ξ、16 θ）；C(i, s) 把 service id 的 4 個 byte 與 0 交錯（C(255, s) 是 account info）；C(s, h) 把 service id 與 H(h) 的前 4 byte 交錯、再接 hash 的第 4..26 byte。storage/preimage/request 分別用 E_4(2^32−1)、E_4(2^32−2)、E_4(length) 當前綴避免碰撞。

<sub>相關：state trie、δ、E_4</sub>

****　state trie（狀態樹）　<sub>§D.2</sub>

binary Patricia Merkle trie，節點固定 512 bits。

第一個 bit 區分 branch 與 leaf。branch = 0 + 左子 hash 的後 255 bits + 右子完整 256 bits。leaf 有兩種：值 ≤ 32 octets 直接內嵌（前兩 bit 10 + 6-bit 長度 + 31-byte key + 補零的值），否則存 H(value)（前 8 bits 11000000）。空子樹是零 hash。設計目標是「optimized for modern compute hardware」——固定大小、少分支。

<sub>相關：C(i)、H、H_R</sub>

****　Merkle functions（Merkle 函數群）　<sub>附錄 E</sub>

N 節點函數、M_B 一般根、M 定深根、MMR append 與 super-peak。

N 在 ⌈n/2⌉ 處切開，雜湊 $node ⌢ 左 ⌢ 右（well-balanced）。M_B 直接把原始 blob 當 leaf（刻意不逐項雜湊），用於 extrinsic hash 與每塊的 accumulation output root。M 是定深版：先把每個 leaf 雜湊成 $leaf ⌢ v、再補零 hash 到 2 的冪——segment root 與 paged proof 用它。MMR 的 append 像二進位加法進位，super-peak 用 $peak 前綴由左往右摺疊。

<sub>相關：MMR、segment root、β_B</sub>

### 常數

**C = 341**　core count（核心數）　<sub>附錄 I</sub>

總 core 數固定 341，但只有前 |κ′|/3 個是 active。

每個 active core 需要 3 個 guarantor，所以 full 配置 |κ| = 1023 = 3·341 時全部 341 個 core 都 active。0.8.0 允許較小的 validator 集合（6 到 1023 的 3 的倍數），此時 active core 數等比例縮小，超出範圍的 core 永遠是 ∅。

<sub>相關：𝕍、guarantor、active core</sub>

**E = 600 / P = 6**　epoch length / slot period（epoch 長度／時槽長度）　<sub>§4.8</sub>

每個 slot 6 秒、每個 epoch 600 slots（正好一小時）。

timeslot index 是自 JAM Common Era（2025-01-01 12:00 UTC，Unix 1,735,732,800）起算的 6 秒週期數，型別 N_2^32（可用到西元 2840 年）。中午 UTC 的選擇是為了讓「距起點 24 小時整數倍」的時刻在所有主要時區都是同一天。

<sub>相關：τ、Y、JAM Common Era</sub>

**Y = 500**　epoch tail start（票券截止點）　<sub>§6.7</sub>

epoch 的第 500 個 slot 之後不再接受 ticket。

m′ < Y 時每塊最多 K = 16 張 ticket，之後 E_T 必須是空的。這段「尾巴」讓下個 epoch 的 slot-sealer 序列有時間定案：只有在 prior 區塊的 m ≥ Y 且 accumulator 飽和時，下個 epoch 才會使用 ticket，否則走 fallback。

<sub>相關：γ_S、γ_A、K、fallback</sub>

**R = 10**　rotation period（輪替週期）　<sub>§11.3</sub>

guarantor 與 core 的對應每 10 個 slot 換一次。

rotation index = ⌊(t mod E)/R⌋，每個 epoch 有 60 次輪替。guarantee 的 slot 可以落在本 rotation 或前一個 rotation（視窗 R·(⌊τ′/R⌋−1) ≤ t ≤ τ′）；若前一個 rotation 屬於前一個 epoch，驗證要改用 λ′ 與 η′_3。輪替是為了限制惡意 guarantor 能持續盯上同一個 core 的時間。

<sub>相關：M、M*、guarantee</sub>

**H = 8 / L = 14,400**　recent history / lookup anchorage（近期歷史長度／查找錨點上限）　<sub>§7, eq. 11.37</sub>

anchor 最多回溯 8 個區塊；lookup anchor 最多回溯 14,400 slots（24 小時）。

兩者的驗證方式不同：anchor 對照鏈上的 β†（狀態內），lookup anchor 必須對照保存的 header 鏈（ancestors A）——這是少數無法只靠狀態驗證的規則，也是 conformance fuzzer 把 Ancestry 列為 M1 必要功能的原因。D = L + 4,800 = 19,200 是 preimage 可被真正刪除前的等待期。

<sub>相關：β†、ancestors、D</sub>

**U = 5**　assurance timeout（擔保逾時）　<sub>eq. 11.18</sub>

report 進鏈後 5 個 slot 內沒湊到 2/3 擔保就被清掉。

條件是 H_T ≥ t + U，t 是 report **進鏈**的 slot（τ′），不是 guarantee 自己帶的 slot。0.8.0 另外規定 |κ| ≠ |κ′|（validator 集合大小改變）時所有 pending 一併清空——因為 shard 數與 assurer 集合都對不上了。被清掉的 report 不會被 accumulate，就這樣消失。

<sub>相關：ρ‡、assurance、R (available)</sub>

**G_A / G_R / G_T / G_I**　gas limits（gas 上限）　<sub>附錄 I</sub>

每份 report 的 accumulate 10^7、每個 package 的 refine 5×10^9、每塊 accumulate 總量 3.5×10^9、is-authorized 5×10^7。

這組數字反映 in-core 與 on-chain 的不對稱：refine 的預算是 accumulate 的 500 倍，因為只有 3 個 validator 執行；accumulate 是所有人都跑，所以整塊只有 G_T。Δ+ 的實際預算 g = max(G_T, G_A·C + Σ χ_Z 的 gas)。

<sub>相關：refine、accumulate、Δ+</sub>

**W_G / W_B / W_R / W_C**　size limits（大小上限）　<sub>附錄 I</sub>

segment 4,104；package bundle 13,791,360；report 變動內容 48 KiB；service code 4,000,000。

W_B = W_M·W_F + 4,096 + 64 + 64，其中 W_F = W_G + 32·⌈log₂ W_X⌉ = 4,488 是一個 import segment 在 Audit DA 的足跡。W_R 限制的是 auth trace 加上所有成功輸出的總和——這是為了「fair use of a block's extrinsic space」。W_A = 64,000 是 authorizer code 的上限。

<sub>相關：segment、work-package、work-report</sub>

**B_S / B_I / B_L**　deposit constants（押金常數）　<sub>eq. 9.8</sub>

100 / 10 / 1：service 的基本、每項、每 octet 押金。

threshold balance a_t = max(0, B_S + B_I·a_i + B_L·a_o − a_f)。a_i = 2·|requests| + |storage|（一筆 preimage request 算兩項），a_o 每筆 request 算 81 + 宣告長度、每筆 storage 算 34 + key + value。餘額低於 a_t 時 write/solicit/transfer 會回 FULL 或 CASH——這是防止狀態膨脹的機制。

<sub>相關：a_t、service、FULL</sub>

**𝕍**　validator count set（合法驗證者數）　<sub>eq. 6.8</sub>

𝕍 ≡ {3c | c ∈ N_[2, C+1]}：6 到 1023 之間 3 的倍數。

0.8.0 (#514) 把 V 從協定常數改成這個集合——validator 數可以變，所有門檻改成對 |κ| 取值：super-majority ⌊2|κ|/3⌋+1、每人票數 n = ⌈2E/|γ′_P|⌉、erasure shard 數 = |κ′|、active core 數 = |κ′|/3。這是 0.7.2 → 0.8.0 影響面最廣的變更之一。

<sub>相關：κ、C、super-majority</sub>

****　tiny vs full（tiny 與 full 配置）　<sub>w3f/jamtestvectors</sub>

測試向量用的縮小參數：6 個 validator、2 個 core、epoch 12 slots。

tiny：V 6、C 2、E 12、Y 10、R 4、D 32、每人 ticket 數 ⌈2·12/6⌉ = 4（0.7.x 是常數 3）、super-majority 5、verdict 門檻 5/0/2、Bandersnatch ring 大小 6、erasure d(6) = 3。full：V 1023、C 341、E 600、Y 500、R 10、D 19,200、ticket 2、super-majority 683、verdict 683/0/341、d(1023) = 342。其餘常數與 GP 相同。

<sub>相關：𝕍、test vectors、conformance</sub>

### 鏈下協定

****　Safrole（Safrole 出塊機制）　<sub>§6</sub>

Sassafras 的簡化版：用 ring VRF 匿名決定每個 slot 唯一的出塊者。

每個 slot 恰好一個預先決定的持票人，因此幾乎不會分叉；未來 slot 的持有者身分匿名（抗針對性 DoS）；副產品是高品質的熵池。相對於 BABE 的抽籤（可能零個或多個 leader），Safrole 在前一個 epoch 就把整個序列定案。抽籤失敗時退回 fallback key 序列，best-chain 規則會偏好 ticket 出塊較多的鏈。

<sub>相關：γ_S、ticket、fallback、best chain</sub>

****　Grandpa / finality（Grandpa 與最終性）　<sub>§19</sub>

決定哪條鏈被最終確定；投票內容包含 posterior state root。

best block 必須：以最新 finalized block 為祖先、已被審計（isaudited）、未 finalized 的祖先中沒有 equivocation（同一 slot 兩個有效區塊）。符合條件者中選 ticket-sealed 祖先最多的那條。投票帶上 posterior state root，是為了彌補 header 只有 prior root——同一個 header 但不同 posterior root 的票會被視為不同區塊。

<sub>相關：best chain、H_R、BEEFY、isaudited</sub>

****　BEEFY（BEEFY 橋接簽章）　<sub>§18</sub>

對每個 finalized 區塊的 accumulation-output MMR super-peak 做 BLS 簽章。

C_v ≡ BLS_{κ′[v]_l}(X_B ⌢ last(β_H) 的 super-peak)，X_B = $jam_beefy。BLS 可聚合，因此能給外部系統（bridge）一份簡短的最終性證明。這條路徑全程用 Keccak，方便 EVM 驗證。

<sub>相關：θ、β_B、H_K</sub>

****　coretime（核心時間）　<sub>§4.9.2</sub>

JAM 取代 gas 購買模型的資源：預先購買、指派給 authorizer。

GP 明說 coretime 的購買機制不在本文範圍，預期由某個 system parachain（在 parachains service 裡）處理。它與 authorizer 搭配，把「誰付錢買了算力」與「誰提交具體工作」拆開——這也是為什麼 JAM 不需要交易簽章與 nonce。

<sub>相關：authorizer、α、service</sub>

****　service（服務）　<sub>§4.9.2, §9</sub>

JAM 唯一的帳戶型態：程式碼 + 餘額 + 狀態，permissionless 建立。

取代 Ethereum 的 contract/EOA 二分法。兩個進入點：refine（in-core）與 accumulate（on-chain）。CoreChains（把 Polkadot parachain 搬上 JAM）、CorePlay（用 PVM 的暫停／續跑做同步感的 actor 合約）都只是 service——JAM 本身沒有 parachain 概念，升級責任下放到 service 層。

<sub>相關：δ、refine、accumulate、CoreChains</sub>

### 生態系與規則

****　JAM Prize / M1（JAM Prize 與 M1）　<sub>jam.web3.foundation</sub>

W3F 的實作獎金；M1（IMPORTER）要求通過狀態轉換一致性測試並能匯入區塊。

規則 12：提交後可能被要求面試，用來確認「團隊成員確實是程式碼的作者」，且對 Gray Paper 與自己的實作都有明確的專業能力——「INABILITY TO PROVE THIS EXPERTISE MAY RESULT IN A REDUCED PRIZE OR FULL DISQUALIFICATION」。milestone 交付範本另外載明面試會被錄影，並須由 Fellowship 以鏈上 remark 批准。T&C 3.5 規定一致性以**評估當下最新的 Gray Paper 版本**為準——這就是 0.8.0 差異要熟的原因。

<sub>相關：conformance fuzzer、Gray Paper、M1 interview</sub>

****　conformance fuzzer（一致性 fuzzer）　<sub>davxy/jam-conformance</sub>

透過 Unix socket 餵區塊、逐塊比對 state root 的測試工具。

同步的 request/response 協定，訊息用 JAM codec 編碼並加上 32-bit little-endian 長度前綴；流程是 PeerInfo 交握 → Initialize（附 ancestry）→ ImportBlock（回 StateRoot 或 Error）→ 不一致時 GetState 抓整份狀態產生報告。M1 必須支援 Ancestry 與 Forking 兩個 feature。W3F 的門檻俗稱「一百萬步」。

<sub>相關：M1、test vectors、ancestors</sub>

****　JIP（JAM 改進提案）　<sub>polkadot-fellows/JIPs</sub>

Gray Paper 之外的實作標準：JIP-1..5+。

JIP-1 debug 用的 `log` host call（index 100，對共識無影響）；JIP-2 節點 RPC（JSON-RPC over WebSocket，通常 19800）；JIP-3 telemetry（JAM Tart）；JIP-4 chainspec（genesis_state 用 62 字元的十六進位 31-byte key、protocol_parameters 用 `fetch` selector 0 的編碼）；JIP-5 由 32-byte seed 推導驗證者金鑰（blake2b("jam_val_key_ed25519" ⌢ seed) 等）。這些不在 GP 裡，但 M1 的工具鏈會用到。

<sub>相關：chainspec、log、conformance fuzzer</sub>

****　0.7.2 → 0.8.0（版本差異）　<sub>graypaper v0.8.0 (2026-06-03)</sub>

面試最容易踩的雷：實作在 0.7.2，但一致性以最新版為準。
...
<sub>相關：𝕍、ρ、grow_heap、refinement context</sub>
