...
# ─────────────────────────────────────────────────────────── 3 · L2 · rationale
{
 ...
 "lens": "設計",
  ...
  "optionsZh": [
   ...
   "只有 ω′ 受影響——那兩個情形都會留下剛被 accumulate 的 report，所以 ready 佇列會無界成長，直到一個 epoch 長度的位移把陳舊條目丟掉為止。Q 本身仍然安全，因為一旦依賴集合被剝除，每次遞迴收到的都是嚴格更短的序列",
   "什麼都不會壞。Q 仍然終止，因為它的遞迴受 epoch 長度 E 所限；而 §11.4 的脈絡檢查會拒絕任何 package 雜湊已在 ξ 中的進來 report，所以待在 ω 裡的條目也不可能被 accumulate 第二次；丟棄整筆條目純粹是讓 ω 保持精簡的最佳化",
   "反而是剝除依賴那件事變得多餘：一旦條目永不被移除，一份 report 就只可能被同一佇列中的另一份 report 阻擋，所以每一個 E 本來會剝除的依賴，都屬於 E 本來也會刪除的某個條目。因此 Q 仍然終止、ω′ 仍然會排空，只是比較慢——一次一個 epoch 長度的位移，而不是一次一輪 accumulation"
  ],
...
# ─────────────────────────────────────────────────────────── 4 · L2 · concept
{
 ...
 "lens": "時機",
  ...
  "optionsZh": [
   ...
   "Q 只會擋它一陣子：在 E = 600 個 slot 之後該依賴被視為過期，i ≥ τ′ − τ 的情形不再對那一槽套用 E，於是 Q 把 w 的依賴集合當成空的、以它手上有的運算元把它 accumulate 掉——所以這份工作至多延遲一個 epoch，但絕不會被無聲遺失",
   "Q 永遠不會送出它，因為它的依賴集合永遠不會被清空；它每個區塊只是被 i ≥ τ′ − τ 的情形 ω′↺[m−i] = E(ω↺[m−i], ξ′[E−1]) 往前帶，並在至多一個 epoch 之後、當 slot 索引繞回、i = 0 的情形以新區塊的 R^Q 覆寫那一槽時消失"
  ],
...
# ─────────────────────────────────────────────────────────── 7 · L2 · concept
{
 ...
 "lens": "時機",
  ...
  "optionsZh": [
   ...
   "只有一次，在最後的 δ′ = I(δ‡, E_P)：`provide` host call 在 accumulation 期間就直接寫進該 service 的 a_p 與 a_l 了，所以 Δ* 沒有東西可整合。Y(d, s, i) 除非 s ∈ K(d) 且該 request 為 [] 否則為 ⊥，所以指向同輪被 eject 之 service 的 blob 會被丟棄而不追究——但這只可能咬到 extrinsic，永遠咬不到 `provide`",
   "兩次——一次在 Δ* 內部、一次在最後——但兩次的第二個引數都是 E_P，因為 `provide` 提供的 blob 在 Δ* 執行前就已經被併進 E_P 了。Y(d, s, i) 另外要求該 blob 小於 W_C = 4,000,000 個 octet、且該 service 持有它的儲存押金，而任一條件失敗會讓區塊無效而不是丟棄該 blob",
   "一次在每個正在 accumulate 之 service 的 Δ1 內部——所以每個 service 在 Δ* 合併帳戶字典之前先摺進自己的 provision——另一次在最後的 δ′ = I(δ‡, E_P)。Y(d, s, i) 接受形狀為 [] 或 [x, y] 的 request 槽，也就是「已 solicit」或「曾可用而後被 forget」，這正是讓同一份 preimage 能在一個 service 的生命週期中被提供兩次的機制"
  ],
