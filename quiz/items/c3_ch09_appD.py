...
# ---------------------------------------------------------------- ch. 9 ----
{
 ...
 "difficulty": 1, "kind": "rationale", "tags": ["preimage", "timeslot"],
  "stemZh": "§9.2 把歷史查詢函數 Λ 的 slot 引數限制在 (H_t − D … H_t) 這個窗口內，而常數附錄把 D 定為 19,200 個 slot。GP 為這個特定數字給出的理由是什麼？",
  "optionsZh": [
   "19,200 個 slot 恰好是 32 個 epoch，也正是 accumulation 歷史 ξ 保留 work-package 雜湊的時間長度，所以在某份 work-package 還可能被重新 accumulate 之前，它用到的 preimage 不得被丟棄；D 因此被定為 32·E",
   "這段期間必須比兩個 epoch 的稽核窗口（1,200 個 slot）長十六倍，好讓 erasure-code 過的 bundle 在爭議期間仍能從 1,023 位 validator 的碎片重建；D 是從那個保存期導出的，與任何 anchor 年齡無關",
   "Ω_H 在任何仍可能發生稽核的時刻都必須回傳相同的答案；而 lookup anchor 本身可能比近期歷史再舊上 L = 14,400 個 slot，所以這段期間就是那個 anchor 年齡再加上 4,800 個 slot（八小時）的安全餘裕",
   "每 octet 的押金 B_L 會在這段期間內線性退還，而 19,200 個 slot 在每槽六秒之下正是 32 小時——這是選用性狀態的標準押金退還時程；B_L 會在那些 slot 內等額分期退回 a_b"
  ],
...
},
{
 ...
 "difficulty": 2, "kind": "concept", "tags": ["preimage", "service"],
  ...
  "optionsZh": [
   ...
   "[] → 該 request 條目被丟棄；[x] → [x, t]；[x, y] → 只要 y < t 就立刻清除該條目與 a_p[h]，不必等 D 個 slot；[x, y, w] → 直接丟棄而不是改寫；其餘每一種情況都回傳 HUH",
   "[] → 丟棄；[x] → 連同 a_p[h] 一起丟棄；[x, y] → 連同 a_p[h] 一起丟棄；[x, y, w] → 連同 a_p[h] 一起丟棄；每一種形狀都立刻塌陷，而那個 D 個 slot 的延遲只管 `eject`"
  ],
