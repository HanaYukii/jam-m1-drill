...
# ---------------------------------------------------------------------------------------------
{
 ...
 "difficulty": 2, "kind": "concept", "tags": ["authorizer", "notation", "entropy"],
...
},
{
 ...
 "difficulty": 2, "kind": "concept", "tags": ["ticket", "recent history", "authorizer"],
  ...
  "stem": "Three state-transition rules use the arrow operators of GP §3.7.2: β_H′ ≡ ←(β_H† ⧺ e)^H (eq. 7.8), α′[c] ≡ ←(F(c) ⧺ φ′[c][H_T]^⟲)^O (eq. 8.2) and γ′_A ≡ →(n ∪ γ_A sorted ascending by ticket id)^E (eq. 6.35). How do you read the arrows — which end of the sequence does each keep — and what does that mean for each of the three rules?",
 "options": [
...
},
{
 ...
 "difficulty": 3, "kind": "code", "tags": ["Merklization", "state root"],
...
},
{
 ...
 "difficulty": 3, "kind": "concept", "tags": ["Bandersnatch", "notation", "seal"],
  ...
  "stem": "GP §3.8.2 writes a Bandersnatch VRF signature as Ṽ_k^m⟨x⟩ ⊂ B_96 with VRF output Y(·) ∈ H, and eq. 6.16 requires H_S ∈ Ṽ_{H_A}^{E_U(H)}⟨X_T ⌢ η′_3 ⧺ i_e⟩. Read that: what are the subscript, the superscript and the angle-bracketed term, and what does Y(H_S) depend on?",
 "options": [
...
},
{
 ...
 "difficulty": 2, "kind": "concept", "tags": ["serialization", "notation"],
  ...
  "stem": "Per GP §3.8.1, what are H and H_K, what happens when a non-blob is passed to a hash function, and what do the subscripts on E and E^{-1} mean?",
 "options": [
...
},
{
 ...
 "difficulty": 2, "kind": "concept", "tags": ["notation"],
  ...
  "stem": "Let d = {1 ↦ a, 2 ↦ b} and e = {2 ↦ c, 3 ↦ a} be dictionaries in ⟨N → B⟩. Applying GP §3.5 (eq. 3.7–3.11), what are d ∪ e and V(d ∪ e), and what does writing d[3] in a rule assert?",
 "options": [
...
},
{
 ...
 "difficulty": 3, "kind": "concept", "tags": ["dependency", "extrinsic"],
  ...
  "stem": "Reading the 0.8.0 dependency graph (eq. 4.5–4.20) literally, where does each of the five extrinsic components enter the transition — and in particular, does anything from the extrinsic feed the accumulation step directly?",
 "options": [
...
},
{
 ...
 "difficulty": 2, "kind": "rationale", "tags": ["core"],
