...
ITEMS = [
{
 ...
 "difficulty": 2, "kind": "concept", "tags": ["recent history"],
  ...
  "optionsZh": [
   "每筆 β_H 條目為：(header 雜湊 h, state root s, accumulation-output log 的 super-peak b, slot t, 從被回報的 work-package 雜湊映到 segment root 的字典 p)；β_B 是 accumulation-output belt，一個由每塊 accumulation-output root 構成的 MMR",
   "每筆 β_H 條目為：(header 雜湊, posterior state root, extrinsic 雜湊, slot)；β_B 是最近 H = 8 個區塊上蒐集到的 BEEFY 簽章集合，這正是橋接方不必跟隨 header 鏈也能證明 finality 的憑藉",
   "每筆 β_H 條目為：(header 雜湊, state root, slot, 該區塊中每份被擔保 work-report 的雜湊清單)；β_B 是最近 H = 8 個區塊 header 的序列，保留它是為了讓 lookup-anchor 能直接從狀態解出",
