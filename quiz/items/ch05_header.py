...
},
{
 ...
 "difficulty": 1, "kind": "rationale", "tags": ["state root", "header"],
  ...
  "optionsZh": [
   ...
   "為了讓分叉時兩個競爭區塊帶有相同的 state root，使 best-chain 規則能單純比較 slot 而不必比較狀態"
  ],
...
},
{
 ...
 "difficulty": 1, "kind": "concept", "tags": ["timeslot", "header"],
  "stemZh": "一個區塊的 slot H_T 必須滿足什麼條件，現在才算有效？",
  "optionsZh": [
   "P(H)_t < H_T ∧ H_T · P ≤ 𝕋——嚴格大於父區塊的 slot，且換算成秒之後不得晚於目前的牆鐘時間 𝕋",
   "P(H)_t ≤ H_T ∧ H_T · P ≤ 𝕋——允許與父區塊同一個 slot，因為同槽的兩個區塊會由 best-chain 規則決定取捨",
   "P(H)_t < H_T ∧ H_T · P ≥ 𝕋——slot 必須落在未來，讓其他 validator 有時間在該槽到來前收到區塊",
...
},
{
 ...
 "difficulty": 2, "kind": "concept", "tags": ["preimage", "lookup-anchor"],
  "stemZh": "GP 只要求實作保存過去 L = 14,400 個 slot（24 小時）內出塊的祖先 header。是哪一項鏈上檢查需要這個祖先集合 A？",
  "optionsZh": [
   "驗證某個 guarantee 的 lookup-anchor 區塊（雜湊、slot 與執行後的 state root）確實出現在這條鏈上——這是狀態 σ 自己無法佐證的",
