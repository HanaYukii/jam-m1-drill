...
ITEMS = [
{
 ...
 "tags": ["seal", "serialization"],
  ...
  "optionsZh": [
   ...
   "只雜湊父區塊的 state root 與 slot，因為這兩個欄位已經唯一決定了父區塊；雜湊整個 header 會讓這個承諾依賴於子區塊無法獨立重建的資料"
  ],
...
},
{
 ...
 "tags": ["header"],
  ...
  "optionsZh": [
   "它能檢查結構是否完整、slot 是否大於父區塊的、以及該 slot 是否未落在未來；它無法驗證 seal，因為要知道誰有資格出那一槽，需要 sealer 序列，而它住在狀態裡",
   "除了 extrinsic 雜湊之外它什麼都能檢查，因為只有那個欄位依賴 header 之外的資料；seal 是自足的，因為出塊者的公鑰本身就是 header 的欄位之一、不需要任何外部查找",
   "它能檢查 seal 與熵簽章，因為兩者光憑出塊者索引就能驗證；它無法檢查的是 slot，因為要與父區塊比較就必須保存過父 header",
   "沒有狀態它什麼都檢查不了，因為連 slot 的界限都是對著 posterior 的 validator 集合表述的；這正是為什麼 GP 要求實作在驗證任何 header 之前先保存 24 小時的祖先"
  ],
...
},
{
 ...
 "tags": ["header", "gas"],
  ...
  "optionsZh": [
   ...
   "執行後的狀態由 seal 承諾，因為它簽署了出塊者算出的 state root；receipt 被 header 裡的 accumulation output log 取代；而 gas 被省略是因為 JAM 是以 slot 而非以 gas 計量 core time",
