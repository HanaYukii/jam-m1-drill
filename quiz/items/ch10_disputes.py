...
},
{
 ...
 "explanation": "eq. 10.12：V(a, j) = ⊤ 當 t = ⌊2|k|/3⌋+1；⊥ 當 t = 0；∅ 當 t = ⌊|k|/3⌋。tiny |k| = 6：⌊4⌋+1 = 5、0、⌊2⌋ = 2；full |k| = 1023：683、0、341。三個門檻都是**等式**而不是區間，落在它們之外的 t 讓 V 無定義 → 整個區塊無效（你們的 code 回 `bad_vote_split`），而不是忽略該 verdict。註腳：這三個門檻恰是三種動作的決策門檻，且安全假設要求 ≥ 2/3+1 的 validator 是 live 的（ELVES 論文）。",
